#!/usr/bin/env bash
# Canonical test runner for ouro-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.ouro/...)
#
# Usage:
...
# (OURO_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.ouro/hermes-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.ouro/hermes-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${OURO_PYTHON:-}" ] && [ -x "$OURO_PYTHON" ] \
    && "$OURO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: OURO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `hermes` binary rather
  # than the devShell hook.
  PYTHON="$OURO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via OURO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and OURO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.ouro/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.ouro"
...
#
#   * OURO_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * OURO_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     OURO_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# OURO_RUN_SLOW_PET_TESTS / OURO_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no OURO_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in OURO_TEST_IMAGE OURO_TEST_WORKERS OURO_TEST_PATHS \
  OURO_TEST_FILE_TIMEOUT OURO_TEST_FILE_RETRIES OURO_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${OURO_RUN_SLOW_PET_TESTS:+OURO_RUN_SLOW_PET_TESTS="$OURO_RUN_SLOW_PET_TESTS"} \
  ${OURO_E2E_BROWSER:+OURO_E2E_BROWSER="$OURO_E2E_BROWSER"} \
