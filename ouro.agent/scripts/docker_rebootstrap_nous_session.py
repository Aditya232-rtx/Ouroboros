...
``stage2-hook.sh`` seeds ``auth.json`` from ``OURO_AUTH_JSON_BOOTSTRAP`` only
...
``OURO_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
# from OURO_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
...
REBOOTSTRAP_ENV = "OURO_AUTH_JSON_REBOOTSTRAP"
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a OURO_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    OURO_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             OURO_AUTH_JSON_BOOTSTRAP path handle it)
