...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:OURO_HOME) { $env:OURO_HOME } else { "$env:LOCALAPPDATA\ouro" }),
    [string]$InstallDir = $(if ($env:OURO_HOME) { "$env:OURO_HOME\ouro-agent" } else { "$env:LOCALAPPDATA\ouro\ouro-agent" }),
    ...
    # builds apps/desktop into a launchable Ouro.exe.
    #
    # Why opt-in:
    #   * Ouro-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Ouro.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Ouro.exe
    ...
    #     `ouro desktop` already builds on demand.
...
# OURO_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[ouro] $Message")
}
...
if ($PSBoundParameters.ContainsKey('HermesHome')) {
    ...
    $HermesHome = ConvertTo-LongPath $(
        if ($env:OURO_HOME) { $env:OURO_HOME } else { "$env:LOCALAPPDATA\ouro" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:OURO_HOME) { "$env:OURO_HOME\ouro-agent" } else { "$env:LOCALAPPDATA\ouro\ouro-agent" }
    )
}
# Pre-rebrand installs placed data under %LOCALAPPDATA%\hermes. If the caller
# did not pin an explicit home/install dir, honour an existing pre-rebrand
# install so a rebrand upgrade keeps the user's config, sessions, and .env
# instead of orphaning them in a fresh empty %LOCALAPPDATA%\ouro.
if (-not $PSBoundParameters.ContainsKey('HermesHome') -and -not $PSBoundParameters.ContainsKey('InstallDir') -and -not $env:OURO_HOME) {
    $preRebrandHome = "$env:LOCALAPPDATA\hermes"
    if (Test-Path (Join-Path $preRebrandHome "ouro-agent")) {
        $HermesHome = ConvertTo-LongPath $preRebrandHome
        $InstallDir = ConvertTo-LongPath (Join-Path $preRebrandHome "ouro-agent")
    }
}
...
function Write-Banner {
    ...
    Write-Host "|             * Ouro Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Uv {
    # Ouro owns its own uv at $HermesHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `ouro update` stay in sync.
...
# Put the Ouro-managed Node dir at the FRONT of the persisted User PATH.
#
...
# Upgrade the Ouro-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Ouro-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("ouro-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and OURO_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Ouro-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Ouro setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Ouro uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\ouro\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Ouro needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Ouro copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\ouro\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``OURO_GIT_BASH_PATH`` (User scope) so Ouro can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Ouro-managed PortableGit install instead..."
    }
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Ouro features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Ouro needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``OURO_GIT_BASH_PATH`` (User env scope) so Ouro can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("OURO_GIT_BASH_PATH", $candidate, "User")
            $env:OURO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set OURO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Ouro may not find Git Bash."
    Write-Info "If needed, set OURO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Ouro requires Node >=26)"
    }
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        Write-Success "Node.js $version found (Ouro-managed)"
    ...
    Write-Info "Installing Ouro-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\ouro-node-extract"
            ...
            if ($extractedDir) {
                ...
                if (Test-Path "$HermesHome\node") {
                    try {
                        ...
                        Write-Warn "Ouro-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\ouro-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    $stashName = "ouro-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``ouro update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Ouro behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running ouro processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\ouro-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\ouro-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `ouro update` (see the notes at the shared clone-path
    ...
    # next `ouro update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Ouro-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running ouro process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running ouro processes before recreating venv..."
            ...
            & taskkill /F /T /IM ouro.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM ouro.exe is NOT enough: the gateway/agent that a
            ...
            # Ouro children re-exec through .ouro-runtime, so killing only
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Ouro " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `ouro update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Ouro (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install ouro-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    if (-not $NoVenv) {
        ...
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Ouro processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Ouro processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Ouro and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        ...
        # `hermes` command while ouro-agent.exe / ouro-acp.exe exist
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `ouro dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `ouro dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `ouro dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up ouro command..."
    
    if ($NoVenv) {
        $hermesBin = "$InstallDir"
    } else {
        # Expose ONLY the ouro launchers on PATH -- never the whole
        ...
        # (#83797): unrelated projects start resolving python to Ouro'
        # runtime interpreter. A dedicated bin dir with copies of the
        # launcher exes keeps `ouro` globally available without
        ...
        foreach ($launcher in @("ouro.exe", "ouro-acp.exe")) {
    ...
    if ((-not $NoVenv) -and $currentPath -like "*$legacyBin*") {
        ...
        $currentPath = $cleaned
        Write-Info "Removed legacy venv\Scripts from user PATH (kept ouro via $hermesBin)"
    }
    ...
    # Set OURO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\ouro instead
    # of the Unix default ~/.ouro
    $currentHermesHome = [Environment]::GetEnvironmentVariable("OURO_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("OURO_HOME", $HermesHome, "User")
        Write-Success "Set OURO_HOME=$HermesHome"
    }
    $env:OURO_HOME = $HermesHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "ouro command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.ouro-bootstrap-complete which tells the Ouro
    ...
    $markerPath = Join-Path $InstallDir ".ouro-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the OURO_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\hermes)
    ...
    # (the default PS5 behaviour), and Ouro's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Ouro-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'ouro setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\ouro-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\ouro-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\ouro-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `ouro tools`
...
function Install-BrowserUseCli {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Ouro' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'ouro tools')"
        }
    } catch {
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `ouro computer-use install` runs. Bounded
        ...
        # Ouro' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'ouro tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: ouro computer-use install"
            }
        } else {
...
# the final `electron` -> `Ouro` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Ouro.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Ouro.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Ouro-managed
    ...
    # The --dir mode produces an unpacked Ouro.exe in
    ...
    # are NOT trying to work around). The Ouro icon + product name are
    # stamped onto Ouro.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\ouro-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Ouro.exe",
        "$desktopDir\release\win-arm64-unpacked\Ouro.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Ouro.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Ouro icon + identity are stamped onto Ouro.exe by the
    ...
    #    Ouro.exe. We deliberately do NOT point them at `ouro desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Ouro.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Ouro.lnk')
        )
        ...
        # with the Ouro icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.ouro/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'ouro setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\ouro.exe"
    if (-not (Test-Path $hermesCmd)) {
        # Pre-rebrand venvs shipped `hermes.exe`.
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'ouro whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: ouro gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: ouro gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: ouro gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\ouro-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   ouro              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   ouro setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   ouro config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   ouro config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   ouro gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   ouro update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Ouro repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Ouro-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Ouro to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
