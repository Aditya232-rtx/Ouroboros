#!/usr/bin/env python3
"""Drive the Ouro TUI under OURO_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.ouro/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with OURO_TUI_RESUME set so it
bypasses the hermes_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  OURO_PERF_LOG     (default ~/.ouro/perf.log)
  OURO_PERF_NODE    (default node from $PATH)
  OURO_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests OURO_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("OURO_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("OURO_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — OURO_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["OURO_DEV_PERF"] = "1"
    env["OURO_DEV_PERF_MS"] = str(args.threshold_ms)
    env["OURO_DEV_PERF_LOG"] = str(log)
    env["OURO_TUI_RESUME"] = sid
    ...
    node = os.environ.get("OURO_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="OURO_DEV_PERF_MS (0 = capture all)")
