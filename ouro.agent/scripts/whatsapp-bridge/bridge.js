 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.ouro/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.ouro', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (OURO_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.OURO_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.ouro', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.OURO_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.ouro', 'document_cache');
const AUDIO_CACHE_DIR = process.env.OURO_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.ouro', 'audio_cache');
...
// `ouro update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Ouro*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Ouro itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Ouro and trigger
      ...
      // Ignore Ouro' own reply messages in self-chat mode to avoid loops.
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `ouro_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `ouro_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
