...
batch (tracker: [#77056](https://github.com/Aditya232-rtx/ouro/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Ouro home, same model, same tasks, same reps.
...
1. Create a dedicated Ouro home with credentials for the models under test:
   ...
   OURO_HOME=$ABEVAL_HOME ouro plugins enable observability/nemo_relay
   ```
