...
measured in the production session DB) through `ouro chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    OURO_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Ouro home with credentials for the
...
(tracker: Aditya232-rtx/ouro#77056).
"""
import json
import os
import re
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "OURO_HOME": str(HOME),
                "OURO_NEMO_RELAY_ATOF_ENABLED": "1",
                "OURO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "OURO_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "OURO_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
