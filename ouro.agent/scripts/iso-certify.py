...
What it does
------------
1. Spawns a SCRATCH dashboard (``ouro dashboard``) bound to loopback on a
   free port, with an ISOLATED ``OURO_HOME`` (temp dir, minimal seeded state).
...
2. Arms the synthetic GIL-heavy turn seam (``OURO_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch OURO_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"OURO_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"OURO_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch OURO_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch OURO_HOME on exit")
