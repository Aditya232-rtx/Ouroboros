#!/bin/bash
# repro.sh -- reproduce desktop-update paths against a sandboxed OURO_HOME.
#
# Nothing here touches your real ~/.ouro or checkout. Each mode builds (or
# reuses) a disposable install under /tmp and drives the REAL code path --
# the actual installer, the actual orchestrator, the actual `ouro update`.
#
#   repro.sh shim          shim UI only: success event after 6s
#   repro.sh shim-fail     shim UI only: error event after 6s
#   repro.sh fresh         fresh install into a sandbox OURO_HOME
...
SANDBOX="${OURO_UPDATE_REPRO_HOME:-/tmp/hermes-update-repro}"
...
ensure_sandbox_install() {
  ...
  OURO_HOME="$SANDBOX" bash "$SANDBOX_ROOT/scripts/install.sh" --non-interactive --skip-setup --hermes-home "$SANDBOX"
}

case "$MODE" in
  shim)
    OURO_SELFTEST_HOLD_SECONDS="${OURO_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  shim-fail)
    OURO_SELFTEST_FAIL=1 OURO_SELFTEST_HOLD_SECONDS="${OURO_SELFTEST_HOLD_SECONDS:-6}" \
      bash "$SCRIPT_DIR/posix.sh" --self-test-ui
    ;;
  fresh)
    rm -rf "$SANDBOX"
  ...
  behind)
    ...
    OURO_HOME="$SANDBOX" bash "$SCRIPT_DIR/posix.sh" \
      --install-root "$SANDBOX_ROOT" --branch main --desktop-pid 0 || true
    say "result file:"
    cat "$SANDBOX/.ouro-update-result.json" 2>/dev/null || echo "(none written)"
    echo
  ...
  gate)
    ...
    expect "appimage (not under unpacked)"      skew     "$(decide --relaunch-target /opt/Ouro/hermes)"
    ...
    if python3 -c "import json,sys; d=json.load(open('$QHOME/.ouro-update-result.json')); sys.exit(0 if d['branch']=='evil\"branch\\\\n\$(x)' and d['ok']==False else 1)"; then
  ...
  launch)
    ...
    expect_msg() { # name python-expr
      if python3 -c "import json,sys; d=json.load(open('$L/.ouro-update-result.json')); sys.exit(0 if ($2) else 1)"; then
        printf 'ok   %s\n' "$1"
      else
        printf 'FAIL %s -> %s\n' "$1" "$(cat "$L/.ouro-update-result.json" 2>/dev/null)"; fails=$((fails+1))
      fi
    }
    stub_install() { # creates a fake install whose ouro update succeeds
    ...
    if [ "$(uname)" != "Darwin" ]; then
      ...
      expect_msg "instant-exit relaunch downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Ouro' in d['message']"
    else
      ...
      expect_msg "missing bundle downgrades to manual" "d['ok']==True and d['manual']==True and 'Reopen Ouro' in d['message']"
    fi
    ...
    stub_install
    bash "$SCRIPT_DIR/posix.sh" --no-ui --desktop-pid 0 --install-root "$L/hermes-agent" \
      --relaunch-target /opt/Ouro/hermes >/dev/null 2>&1 || true
    if [ "$(uname)" != "Darwin" ]; then
