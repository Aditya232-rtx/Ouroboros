...
# (%OURO_HOME%\hermes-setup.exe). That binary has no self-update path --
...
#
# This script lives in the repo checkout, so EVERY `ouro update` refreshes
...
#     -InstallRoot <path>   repo checkout (OURO_HOME\ouro-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Ouro.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .ouro-update-in-progress in place (tests)
#
...
# .ouro-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim OURO_HOME\.ouro-update-in-progress with OUR pid as
...
# `ouro update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Ouro.exe pid.
try {
...
$MarkerPath = Join-Path $HermesHome ".ouro-update-in-progress"
$LogDir = Join-Path $HermesHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $HermesHome ".ouro-update-result.json"
$script:Ui = $null
...
function Show-ProgressWindow {
    ...
    try {
        ...
        $sub.Text = "Ouro will open once done."
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `ouro debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"ouro debug share`" in a terminal to send a report."
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Ouro.exe before writing the replacement,
    ...
    # DO NOT spawn Ouro.exe as our child: Electron/Chromium calls
...
function Invoke-HermesStep([string]$Exe, [string[]]$HermesArgs, [string]$Tag) {
    ...
    # afterwards, where `ouro debug share` picks it up.
    ...
    # ouro update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
...
# the page can be QA'd in any browser; OURO_SELFTEST_FAIL=1 exercises the
# error state, OURO_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:OURO_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:OURO_SELFTEST_HOLD_SECONDS }
    Start-Sleep -Seconds $hold
    if ($env:OURO_SELFTEST_FAIL) {
...
try {
    ...
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Ouro window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Ouro fully and try again."
    ...
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Ouro install open (venv\Scripts\hermes.exe locked after 20s). Nothing was changed. Close other Ouro windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # from a user leaving a second Ouro window open. It is the inherited
    ...
    # Start Menu shortcut points at a Ouro.exe that no longer exists.
    #
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Ouro installer or `ouro doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    ...
    Write-HandoffLog "ouro update exit code: $($res.Code)"

    if ($res.Code -ne 0 -and $res.Code -ne 2) {
        # One retry for the update-boundary class (fresh code on disk, stale
        # code in memory). Exit 2 ("close all Ouro windows") is not retryable.
    ...
    # `ouro update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "ouro update reported a desktop build failure (non-fatal there, fatal here); retrying build"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `ouro desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `ouro debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Ouro is back",
    ...
    if ($finalCode -ne 0) {
        ...
        if (-not $cameBack -and $RelaunchExe) {
            ...
            $finalMsg = "Update complete. Reopen Ouro to finish (it could not restart itself)."
