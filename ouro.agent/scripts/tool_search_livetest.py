#!/usr/bin/env python3
"""Live test harness for Ouro's Tool Search feature.
...
# Force-isolate the test environment BEFORE any ouro imports.
ORIGINAL_HOME = os.environ.get("OURO_HOME")
ORIGINAL_AUTH = Path.home() / ".ouro" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.ouro/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.ouro/.env`` so
    ...
    real_env_file = Path.home() / ".ouro" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".ouro"))
...
def reset_module_state():
    """Drop cached modules so the new OURO_HOME takes effect."""
...
def main():
    ...
    # Restore original OURO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["OURO_HOME"] = ORIGINAL_HOME
    else:
