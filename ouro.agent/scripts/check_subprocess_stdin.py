...
When Ouro runs in TUI mode, the gateway child process communicates with
...
# ``get_hermes_home() / "plugins"`` (user) and ``./.ouro/plugins/`` (project,
# gated behind ``OURO_ENABLE_PROJECT_PLUGINS``) — see
...
def main() -> int:
    ...
    # ./.ouro/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_hermes_home() / "plugins"]
    if os.environ.get("OURO_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".ouro" / "plugins")
