...
#
# Usage:
#   curl -fsSL https://ouro.dev/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Ouro terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Aditya232-rtx/ouro.git"
REPO_URL_HTTPS="https://github.com/Aditya232-rtx/ouro.git"
OURO_HOME="${OURO_HOME:-$HOME/.ouro}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${OURO_INSTALL_DIR:-}" ]; then
...
#   code at /usr/local/lib/hermes-agent, command at /usr/local/bin/ouro,
#   data still at /root/.ouro (OURO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --ouro-home)
            ...
            shift 2
            ;;
        --hermes-home)   # legacy alias for --ouro-home
            OURO_HOME="$2"
            shift 2
            ;;
        ...
        -h|--help)
            ...
            echo "                   write \$OURO_HOME/.no-bundled-skills so future"
            echo "                   'ouro update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Ouro.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.ouro/hermes-agent"
            echo "                   default (root, Linux): /usr/local/lib/hermes-agent"
            echo "  --ouro-home PATH   Data directory (default: ~/.ouro, or \$OURO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Ouro installs the code under"
            echo "  /usr/local/lib/hermes-agent and links the command into"
            echo "  /usr/local/bin/ouro (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$OURO_HOME"
            echo "  (default /root/.ouro).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$OURO_HOME/hermes-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `ouro update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Ouro-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Ouro","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install ouro command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `ouro` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $OURO_HOME/hermes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $OURO_HOME/hermes-agent
...
#                              $OURO_HOME/hermes-agent — then preserve it)
#
# Always no-op when the user set --dir or $OURO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in OURO_HOME.
    if is_termux; then
        INSTALL_DIR="$OURO_HOME/hermes-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$OURO_HOME/hermes-agent/.git" ]; then
            INSTALL_DIR="$OURO_HOME/hermes-agent"
        ...
        # /usr/local/bin/ouro wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/ouro"
        log_info "  Data:    $OURO_HOME (unchanged)"
...
# Point a Ouro-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $OURO_HOME/node/bin — which is NOT on
...
# Ouro's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Ouro-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$OURO_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$OURO_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$OURO_HOME/node/etc/npmrc"
}

get_ouro_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/ouro" ]; then
        echo "$link_dir/ouro"
    else
        echo "ouro"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://ouro.dev/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Ouro owns its own uv at $OURO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `ouro update` stay in sync.
    local _managed_uv="$OURO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $OURO_HOME/bin ..."
    mkdir -p "$OURO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/ouro-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/ouro-uv-installer.$$.sh")"
    ...
    # directly into $OURO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$OURO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Ouro-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Ouro-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Ouro-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Ouro-managed Node from a previous run over a too-old system one.
    if [ -x "$OURO_HOME/node/bin/node" ] && [ -x "$OURO_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$OURO_HOME/node/bin/node" --version)"; then
        export PATH="$OURO_HOME/node/bin:$PATH"
        log_success "Node.js $("$OURO_HOME/node/bin/node" --version) found (Ouro-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Ouro-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Ouro requires Node >=26) — installing Ouro-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.ouro/node/..."
    ...
    # Place into ~/.ouro/node/ and symlink binaries into the same bin dir
    # the ouro command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$OURO_HOME/node"
    mkdir -p "$OURO_HOME"
    mv "$extracted_dir" "$OURO_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$OURO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$OURO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$OURO_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$OURO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.ouro/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Ouro install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Ouro itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `ouro update` path
                # (#4735).
            ...
            # cannot succeed — mirror ``ouro update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Ouro behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up ouro command..."

    if [ "$USE_VENV" = true ]; then
        OURO_BIN="$INSTALL_DIR/venv/bin/python"
        OURO_ENTRYPOINT="$INSTALL_DIR/ouro"
    else
        OURO_BIN="$(which ouro 2>/dev/null || echo "")"
        if [ -z "$OURO_BIN" ]; then
            log_warn "ouro not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$OURO_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$OURO_ENTRYPOINT" ]; }; then
        log_warn "Ouro launcher prerequisites not found"
    ...
    # Create a user-facing shim for the ouro command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $OURO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$OURO_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/ouro" "$command_link_dir/hermes"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/ouro" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$OURO_BIN" "$OURO_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/ouro" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/ouro"
    log_success "Installed ouro launcher → $command_link_display_dir/ouro"

    # Also expose `ouro-agent`. The `ouro-agent` console script declared in
    ...
    rm -f "$command_link_dir/ouro-agent" "$command_link_dir/hermes-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/ouro-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$OURO_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/ouro-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$OURO_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/ouro-agent"
    log_success "Installed ouro-agent launcher → $command_link_display_dir/ouro-agent"

    # Also expose `ouro-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `ouro-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Ouro as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/ouro-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/ouro-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$OURO_BIN" "$OURO_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/ouro-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/ouro-acp"
    log_success "Installed ouro-acp launcher → $command_link_display_dir/ouro-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "ouro command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v ouro' \
                >/dev/null 2>&1; then
        ...
        log_info "ouro not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Ouro — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Ouro — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Ouro — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so ouro works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.ouro directory structure (config at top level, code in subdir)
    mkdir -p "$OURO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.ouro/.env (top level, easy to find)
    if [ ! -f "$OURO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$OURO_HOME/.env"
            log_success "Created ~/.ouro/.env from template"
        else
            touch "$OURO_HOME/.env"
            log_success "Created ~/.ouro/.env"
        fi
    else
        log_info "~/.ouro/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$OURO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.ouro/config.yaml (top level, easy to find)
    if [ ! -f "$OURO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$OURO_HOME/config.yaml"
            log_success "Created ~/.ouro/config.yaml from template"
        fi
    else
        log_info "~/.ouro/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$OURO_HOME/SOUL.md" ]; then
        cat > "$OURO_HOME/SOUL.md" << 'SOUL_EOF'
You are Ouro, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.ouro/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.ouro/"

    # Seed bundled skills into ~/.ouro/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `ouro update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'ouro update'." \
            > "$OURO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $OURO_HOME/.no-bundled-skills"
        log_info "  Future 'ouro update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.ouro/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.ouro/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$OURO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$OURO_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.ouro/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Ouro browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Ouro will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Ouro browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `ouro update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `ouro tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$OURO_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'ouro tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `ouro tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `ouro computer-use install` on their own).
    ...
    # Same upstream installer `ouro computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Ouro install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Ouro'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'ouro tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: ouro computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'ouro setup' after install."
        return 0
    fi
    ...
    # Run ouro setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Ouro to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$OURO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'ouro whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                OURO_CMD="$(get_ouro_command_path)"
                $OURO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'ouro whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'ouro gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $OURO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $OURO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: ouro gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: ouro gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $OURO_CMD gateway > "$OURO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.ouro/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: ouro gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: ouro gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.ouro-bootstrap-complete, which tells the Ouro
    ...
    local marker_path="$INSTALL_DIR/.ouro-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $OURO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $OURO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $OURO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}ouro${NC}                Start chatting"
    echo -e "   ${GREEN}ouro setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}ouro config${NC}       View/edit configuration"
    echo -e "   ${GREEN}ouro config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}ouro gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}ouro update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'ouro' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'ouro' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'ouro' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$OURO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$OURO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$OURO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `ouro update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$OURO_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Ouro` rename
...
# _purge_electron_build_cache() used by `ouro desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Ouro-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `ouro desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Ouro" ]; then
            app="$desktop_dir/release/linux-unpacked/Ouro"
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Ouro.app" \
            "$desktop_dir/release/mac/Ouro.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `ouro desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading OURO_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if OURO_HOME="$OURO_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
...
PYEOF
    ...
    # checkout stays clean for the next `ouro update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Ouro-managed Node
            # provisioned during prerequisites/node-deps (at $OURO_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $OURO_HOME.
    # $OURO_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'ouro update' on this host install.
