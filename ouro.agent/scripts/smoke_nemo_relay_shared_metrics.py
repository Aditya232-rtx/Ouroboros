"""Run a real Ouro CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_hermes_executable(hermes_repo: Path) -> Path:
    ...
    raise SystemExit(
        "Ouro executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Ouro source checkout containing .venv/bin/hermes",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated OURO_HOME and captured output",
    )
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Ouro development environment requires jsonschema"
        ) from exc
...
def main() -> int:
    ...
    ouro = _resolve_hermes_executable(hermes_repo)
    ...
    if result.returncode != 0:
        raise AssertionError(
            f"Ouro exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Ouro model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Ouro did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Ouro did not print the mock model response")
    ...
    print("Ouro -> NeMo Relay shared-metrics smoke test passed")
