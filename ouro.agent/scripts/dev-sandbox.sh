...
$OURO_HOME/ouro-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  OURO_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .ouro-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `ouro setup` & the browser tools for speed.
...
GIT_ROOT="${OURO_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${OURO_DEV_SANDBOX_DIR:-.ouro-sandbox}"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/ouro.dev"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/ouro.dev/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/ouro.dev/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://ouro.dev/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/ouro.git update-ref refs/heads/main "$next_main"; then
...
# checkout to $OURO_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/ouro.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q ouro-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  git -C "$SNAPSHOT_REPO" config user.name 'Ouro sandbox'
