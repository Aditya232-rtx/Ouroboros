...
$OURO_OSINT_CACHE/icij/ (default: ~/.cache/hermes-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("OURO_OSINT_CACHE")
    if base:
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "ouro-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $OURO_OSINT_CACHE/icij or ~/.cache/hermes-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
