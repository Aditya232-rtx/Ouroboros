...
### 1. Start order: editor first, Ouro second

Ouro probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Ouro session.
...
restart the editor, restart the Ouro session.
...
server fails to bind (Output Log) or Ouro' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.ouro/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Ouro

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Ouro' connection
lives in `~/.ouro/config.yaml` via `ouro mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Ouro — just don't
mistake it for the Ouro setup step.
...
explicitly warns against overlapping calls. Ouro executes same-turn tool
...
### 9. Timeouts: Ouro gives up before Unreal does

Ouro' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Ouro runs elsewhere (SSH backend,
