#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `ouro kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed ouro CLI."""
    try:
        out = subprocess.run(
        ...
        pass
    # Fallback: textual parse of `ouro kanban list`
    out = subprocess.run(
