...
Maps each role archetype to the Ouro skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public ouro-agent
...
## Ouro skills relevant to video production

### Visual / rendering skills (`ouro-agent/skills/creative/`)
...
### Design / pre-production skills (`ouro-agent/skills/creative/`)
...
### Audio / media skills (`ouro-agent/skills/creative/` + `skills/media/`)
...
These are **not** Ouro skills but external CLIs / APIs that profiles invoke.
...
## Built-in Ouro tools for media review

These are native Ouro tools — not invoked via terminal but through their own
...
key is present in `${OURO_HOME:-~/.ouro}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Ouro profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${OURO_HOME:-~/.ouro}/.env` → environment variable.
...
When a new Ouro-public video skill ships:
