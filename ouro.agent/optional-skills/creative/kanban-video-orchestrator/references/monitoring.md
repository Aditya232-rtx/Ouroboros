...
ouro kanban watch --tenant <project-slug>

# Snapshot of the board
ouro kanban list --tenant <project-slug>
ouro kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
ouro kanban stats --tenant <project-slug>

# Visual dashboard (browser)
ouro dashboard

# Inspect a specific task (includes comments + events)
ouro kanban show <task-id>

# Follow a single task's event stream
ouro kanban tail <task-id>
```

Verify available subcommands with `ouro kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `ouro kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `ouro kanban block <id>` then `ouro kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `ouro kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `ouro kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `ouro profile list` (profile exists?), `ouro status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(ouro kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
ouro kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
ouro kanban block <task-id>
ouro kanban archive <task-id>

# Diagnose what happened
ouro kanban show <task-id>      # task body, comments, recent events
ouro kanban tail <task-id>      # follow the live event stream
ouro kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `ouro kanban create` for the director
...
while true; do
    clear
    ouro kanban list --tenant <slug>
    echo "---"
    ouro kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `ouro kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `ouro profile list` and start
