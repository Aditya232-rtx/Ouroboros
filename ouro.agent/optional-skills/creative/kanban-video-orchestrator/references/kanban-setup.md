...
the actual `setup.sh` that creates the project workspace, configures Ouro
...
2. **Create profiles** — `ouro profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.ouro/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `ouro kanban create` assigned to the director
...
```bash
ouro profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.ouro/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.ouro/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Ouro
...
Each profile gets a `SOUL.md` at `~/.ouro/profiles/<name>/SOUL.md` that
...
```bash
ouro kanban create "Direct production of <video title>" \
    --assignee director \
...
the Ouro `.env` (`${OURO_HOME:-$HOME/.ouro}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${OURO_HOME:-$HOME/.ouro}/.env"
...
check_key ELEVENLABS_API_KEY ouro ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY ouro OPENROUTER_API_KEY || exit 1
# ...
```
