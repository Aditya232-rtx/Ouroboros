...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("OURO_HOME", os.path.expanduser("~/.ouro")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("OURO_HOME", Path.home() / ".ouro")) / "skills" / "red-teaming" / "godmode"
...
OURO_HOME = Path(os.getenv("OURO_HOME", Path.home() / ".ouro"))
CONFIG_PATH = OURO_HOME / "config.yaml"
PREFILL_PATH = OURO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Ouro models — already uncensored, just needs clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _get_current_model() -> tuple:
    """Read current model and provider from Ouro config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.ouro/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Ouro for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Ouro for changes to take effect.")
