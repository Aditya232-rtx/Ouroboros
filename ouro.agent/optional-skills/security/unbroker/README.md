...
The design is **Ouro-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
ouro skills install official/security/unbroker
```

Then start a new Ouro session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Ouro home directory (`0600`, optionally
