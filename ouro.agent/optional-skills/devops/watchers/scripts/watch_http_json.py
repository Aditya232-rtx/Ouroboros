...
Usage (via cron with --no-agent):
    ...
    ouro cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$OURO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Ouro-Watcher/1.0"})
