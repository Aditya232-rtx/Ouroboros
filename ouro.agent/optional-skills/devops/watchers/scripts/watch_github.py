...
Usage (via cron with --no-agent):
    ...
    ouro cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$OURO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo Aditya232-rtx/ouro --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Ouro .env file
(``${OURO_HOME:-~/.ouro}/.env``) to avoid the 60 req/hr
