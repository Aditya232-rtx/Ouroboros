...
is typically only in `~/.ouro/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Ouro,
...
`hermes-custom-providers` skill, pitfall 2). Ouro hits this because it forwards
