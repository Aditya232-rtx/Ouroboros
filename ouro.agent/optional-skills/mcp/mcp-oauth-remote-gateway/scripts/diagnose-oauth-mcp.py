...
Usage:
  ...
  <server_name> matches the files in $OURO_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Ouro' own resolver (profile-safe); fall back to env then ~/.ouro.
    try:
        from hermes_constants import get_hermes_home
        return str(get_hermes_home())
    except Exception:
        return os.environ.get("OURO_HOME") or os.path.expanduser("~/.ouro")
