"""
Ouro MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Ouro-specific extra)

Usage:
    ouro mcp serve
    ouro mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using OURO_HOME."""
    try:
        from hermes_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("OURO_HOME", Path.home() / ".ouro")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("OURO_HOME", Path.home() / ".ouro")
...
class EventBridge:
    ...
    This is the Ouro equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            ...
            db_file = Path(os.environ.get("OURO_HOME", Path.home() / ".ouro")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Ouro MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "hermes",
        instructions=(
            "Ouro messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Ouro MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
