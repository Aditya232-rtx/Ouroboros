# Política de Seguridad de Ouro

Este documento describe el modelo de confianza de Ouro, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/Aditya232-rtx/ouro/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **Ouro no opera un programa de
...
- Detalles del entorno (`ouro version`, SHA del commit, SO, versión de Python).
...
Ouro es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Ouro,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Ouro al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Ouro
...
Ouro admite dos posturas de aislamiento a nivel de SO. Abordan
...
Ouro admite esto de dos maneras:

- **La propia imagen Docker de Ouro y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Ouro
...
Ouro filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Ouro
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Ouro
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Ouro no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Ouro o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Ouro ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Ouro
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Ouro. Los errores en la ruta de
  instalación de habilidades o plugins de Ouro que impidan al
...
- Ouro incluye guardias de cadena de suministro para lanzamientos de servidores
