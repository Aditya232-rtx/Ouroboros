...
_OURO_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (OURO_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `ouro tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (OURO_KANBAN_TASK env "
    ...
    # Coding posture (base Ouro — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    # ==========================================================================
    # Full Ouro toolsets (CLI + messaging platforms)
    #
    ...
    # the `ouro send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "hermes-cli": {
        ...
        "tools": _OURO_CORE_TOOLS,
        "includes": []
    },

    "hermes-cron": {
        # Mirrors hermes-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `ouro tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hermes-cli; gated by `ouro tools`",
        "tools": _OURO_CORE_TOOLS,
        "includes": []
    },
    ...
    "hermes-email": {
        "description": "Email bot toolset - interact with Ouro via email (IMAP/SMTP)",
        "tools": _OURO_CORE_TOOLS,
        "includes": []
    },
    ...
    "hermes-sms": {
        "description": "SMS bot toolset - interact with Ouro via SMS (Twilio)",
        "tools": _OURO_CORE_TOOLS,
        "includes": []
    },
    ...
    "hermes-webhook": {
        ...
        "tools": _OURO_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    Platform bundles are defined as ``_OURO_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``ouro-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        ...
        # Gives them _OURO_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("hermes-"):
            ...
            try:
                ...
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_OURO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
