...
storage discipline as the other indexes). On a populated database, run
    ...
    ouro sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.ouro/config.yaml` to disable. Override
the .so location with `OURO_FTS5_CJK_SO`.
