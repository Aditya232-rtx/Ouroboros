"""ACP session manager — maps ACP sessions to Ouro AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.ouro/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Ouro itself is running in WSL.

    Windows ACP clients can launch ``ouro acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Ouro from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["ouro-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Ouro agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Ouro AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Ouro runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.ouro/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``OURO_HOME`` dynamically rather than relying on
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Ouro daemon's process cwd.
