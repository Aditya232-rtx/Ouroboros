"""ACP agent server — exposes Ouro via the Agent Client Protocol."""
...
try:
    from hermes_cli import __version__ as OURO_VERSION
except Exception:
    OURO_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Ouro running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Ouro cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Ouro/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Ouro AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Ouro maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Ouro inventory is also used by ``ouro model``, the TUI,
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Ouro estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Ouro changes it.

        When the internal Ouro head rotated (e.g. compression-driven session
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["ouro-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="ouro-agent", version=OURO_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Ouro' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Ouro setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Ouro
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Ouro on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Ouro' local kawaii waiting/status
        ...
        # os.environ["OURO_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue OURO_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind OURO_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Ouro workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.ouro/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global OURO_CRON_SESSION (#37968).
            ...
            previous_session_id = os.environ.get("OURO_SESSION_ID")
            ...
            try:
                ...
                # Restore OURO_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("OURO_SESSION_ID", None)
                else:
                    os.environ["OURO_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Ouro DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (OURO_SESSION_KEY in
        ...
        # Ouro' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Ouro install tree into the session's
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Ouro has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
