"""ACP permission bridging for Ouro dangerous-command approvals."""
...
# Maps ACP permission option ids to Ouro approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match Ouro approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping Ouro semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_hermes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Ouro approval strings."""
...
def make_approval_callback(
    ...
    """
    Return a Ouro-compatible approval callback that bridges to ACP.
