"""CLI entry point for the ouro-agent ACP adapter.

Loads environment variables from ``~/.ouro/.env``, configures logging
...
try:
    ...
    # shadowing Ouro's own modules — ``ouro acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from OURO_HOME (default ``~/.ouro``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="ouro-acp",
        description="Run Ouro as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Ouro version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Ouro provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.ouro/node/ "
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting ouro-agent ACP adapter")
    ...
    if os.environ.get("OURO_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
