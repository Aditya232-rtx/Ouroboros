"""ACP auth helpers — detect and advertise Ouro authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Ouro runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Ouro can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Ouro.
    ...
    not have Ouro provider credentials configured yet, so Ouro always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Ouro using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Ouro provider",
            description=(
                "Open Ouro' interactive model/provider setup in a terminal. "
                "Use this when Ouro has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
