...
订阅代理是一个本地 HTTP 服务器，让外部应用——OpenViking、Karakeep、Open WebUI，以及任何支持 OpenAI 兼容聊天补全（chat completions）的应用——能够将你的 Ouro 托管提供商订阅用作其 LLM 端点。代理会自动附加正确的凭据（并在需要时自动刷新），因此应用无需静态 API 密钥。
...
| 使用场景 | "将 Ouro 用作聊天后端" | "从其他应用使用我的 Portal 订阅" |
...
这会打开浏览器进行 Nous Portal OAuth 流程。Ouro 将刷新令牌存储在 `~/.ouro/auth.json` 中——与所有 Ouro 提供商登录信息存放在同一位置。
...
```
Starting Ouro proxy for Nous Portal
...
Model:      Ouro-4-70B    # 或 Ouro-4.3-36B、Ouro-4-405B
```
...
```
Ouro proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

如果显示 `not logged in`，请运行 `ouro portal`。如果显示 `credentials need attention`，说明你的刷新令牌已被撤销（较少见——通常发生在你从 Portal Web UI 退出登录时）——重新运行 `ouro portal` 即可。
...
INFERENCE_TEXT_MODEL=Ouro-4-70B
```
...
```bash
ouro proxy start --host 0.0.0.0 --port 8645
```
