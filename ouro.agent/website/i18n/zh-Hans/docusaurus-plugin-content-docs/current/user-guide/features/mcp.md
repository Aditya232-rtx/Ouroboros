...
sidebar_position: 4
title: "MCP（模型上下文协议）"
description: "通过 MCP 将 Ouro 连接到外部工具服务器，并精确控制 Ouro 加载哪些 MCP 工具"
---

# MCP（模型上下文协议）

MCP 让 Ouro 连接到外部工具服务器，使 agent 能够使用 Ouro 本身之外的工具——GitHub、数据库、文件系统、浏览器栈、内部 API 等等。

如果你曾经希望 Ouro 使用某个已经存在于其他地方的工具，MCP 通常是最简洁的方式。

## MCP 能给你带来什么

- 无需先编写原生 Ouro 工具，即可访问外部工具生态系统
- 在同一配置中同时支持本地 stdio 服务器和远程 HTTP MCP 服务器
- 启动时自动发现并注册工具
- 在服务器支持的情况下，提供针对 MCP 资源和 prompt（提示词）的实用工具封装
- 按服务器过滤，只向 Ouro 暴露你真正需要的 MCP 工具
...
2. 在 `~/.ouro/config.yaml` 中添加一个 MCP 服务器：
...
Ouro 会发现 MCP 服务器的工具，并像使用其他工具一样使用它们。
...
- MCP 服务器托管在其他地方
- 你的组织暴露了内部 MCP 端点
- 你不希望 Ouro 为该集成在本地启动子进程

## 基本配置参考

Ouro 从 `~/.ouro/config.yaml` 的 `mcp_servers` 下读取 MCP 配置。
...
| `enabled` | bool | 若为 `false`，Ouro 完全跳过该服务器 |
...
对于知名 MCP 服务器，`ouro mcp add` 接受 `--preset` 标志，自动填写传输层细节，无需手动查找命令和参数。预设只提供默认值——你在同一命令行传入的其他内容（环境变量、头信息、过滤规则）仍然优先生效。
...
你可以使用任意本地名称（`ouro mcp add my-codex --preset codex` 完全可以）；预设只提供 `command`/`args` 默认值。

## Ouro 注册 MCP 工具的方式

Ouro 为 MCP 工具添加前缀，避免与内置名称冲突：
...
实际使用中，你通常不需要手动调用带前缀的名称——Ouro 在正常推理过程中会自动识别并选择该工具。

## MCP 实用工具

在服务器支持的情况下，Ouro 还会围绕 MCP 资源和 prompt 注册实用工具：
...
这些实用工具现在具备能力感知：
- 只有当 MCP 会话实际支持资源操作时，Ouro 才注册资源实用工具
- 只有当 MCP 会话实际支持 prompt 操作时，Ouro 才注册 prompt 实用工具

因此，一个只暴露可调用工具而没有资源/prompt 的服务器，不会获得这些额外的封装。

## 按服务器过滤

你可以控制每个 MCP 服务器向 Ouro 贡献哪些工具，从而精细管理工具命名空间。
...
若 `enabled: false`，Ouro 完全跳过该服务器，甚至不尝试连接。
...
如果你的配置过滤掉了所有可调用工具，并禁用或省略了所有支持的实用工具，Ouro 不会为该服务器创建空的运行时 MCP 工具集。
...
Ouro 在启动时发现 MCP 服务器，并将其工具注册到普通工具注册表中。

### 动态工具发现

MCP 服务器可以在运行时通过发送 `notifications/tools/list_changed` 通知，告知 Ouro 其可用工具发生了变化。Ouro 收到该通知后，会自动重新获取服务器的工具列表并更新注册表——无需手动执行 `/reload-mcp`。
...
对于 stdio 服务器，Ouro 不会盲目传递你的完整 shell 环境。
...
因为 Ouro 现在只在以下两个条件同时满足时才注册这些封装：
1. 你的配置允许它们
2. 服务器会话实际支持该能力
...
当 `supports_parallel_tool_calls` 为 `true` 时，Ouro 可能在单次工具调用批次中同时执行该服务器的多个工具，就像对内置只读工具（`web_search`、`read_file` 等）的处理方式一样。
...
MCP 服务器可以通过 `sampling/createMessage` 协议向 Ouro 请求 LLM 推理。这允许 MCP 服务器代表自己请求 Ouro 生成文本——适用于需要 LLM 能力但没有自己模型访问权限的服务器。
...
除了连接**到** MCP 服务器，Ouro 也可以**作为** MCP 服务器运行。这让其他支持 MCP 的 agent（Claude Code、Cursor、Codex 或任何 MCP 客户端）能够使用 Ouro 的消息能力——列出会话、读取消息历史，以及跨所有已连接平台发送消息。

### 适用场景

- 你希望 Claude Code、Cursor 或其他编程 agent 通过 Ouro 发送和读取 Telegram/Discord/Slack 消息
- 你需要一个单一的 MCP 服务器，同时桥接 Ouro 所有已连接的消息平台
- 你已经有一个运行中的 Ouro gateway，并已连接各平台
...
将 Ouro 添加到你的 MCP 客户端配置中。例如，在 Claude Code 的 `~/.claude/claude_desktop_config.json` 中：
...
MCP 服务器暴露 10 个工具，与 OpenClaw 的 channel bridge 接口一致，并额外提供一个 Ouro 专属的 channel 浏览器：
...
MCP 服务器包含一个实时事件桥，轮询 Ouro 的会话数据库以获取新消息。这让 MCP 客户端能够近实时感知新来的会话：
...
```bash
ouro mcp serve              # 普通模式
ouro mcp serve --verbose    # 在 stderr 输出调试日志
```

### 工作原理

MCP 服务器直接从 Ouro 的会话存储（`~/.ouro/sessions/sessions.json` 和 SQLite 数据库）读取会话数据。后台线程轮询数据库以获取新消息，并维护一个内存事件队列。发送消息时，使用与 Ouro agent 本身相同的 `send_message` 基础设施。

读取操作（列出会话、读取历史、轮询事件）**不需要** gateway 运行。发送操作**需要** gateway 运行，因为平台适配器需要活跃连接。

### 当前限制

- 内嵌的 `ouro mcp serve` 目前只暴露 **stdio-only** MCP 服务器。如果你需要 HTTP MCP 服务器，请运行单独的适配器——或者，更常见的做法是使用 Ouro 的 MCP **客户端**侧，它已经同时支持 stdio 和 HTTP（`mcp_servers.yaml` / `config.yaml` 中的 `url` + `headers`；参见上方的 [HTTP 服务器](#http-servers)）。
