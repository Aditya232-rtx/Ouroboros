---
title: "Ouro Skill 编写——在仓库中编写 SKILL"
sidebar_label: "Ouro Skill 编写"
...
| 标签 | `skills`, `authoring`, `ouro-agent`, `conventions`, `skill-md` |
...
# 编写 Ouro-Agent Skills（仓库内）

## 概述

SKILL.md 可以存放在两个位置：

1. **用户本地：** `~/.ouro/skills/<maybe-category>/<name>/SKILL.md` — 个人使用，不共享。通过 `skill_manage(action='create')` 创建。
...
- 你正在提交一个应随 ouro-agent 一起发布的可复用工作流
...
- Ouro-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
...
`metadata.hermes.related_skills` 在加载时会合并两个目录树（仓库内 `skills/` 和 `~/.ouro/skills/`）。你**可以**从仓库内 skill 引用用户本地 skill，但对于全新克隆仓库的其他用户，该引用无法解析。仓库内 skill 优先只引用仓库内 skill。如果某个频繁被引用的 skill 仅存在于 `~/.ouro/skills/`，请考虑将其提升到仓库中。
...
1. **对仓库内 skill 使用 `skill_manage(action='create')`。** 它会写入 `~/.ouro/skills/`，而非仓库目录树。仓库内创建请使用 `write_file`。
...
- [ ] 文件位于 `skills/<category>/<name>/SKILL.md`（不在 `~/.ouro/skills/` 中）
