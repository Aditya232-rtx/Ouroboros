...
description: "使用 Nix 安装和部署 Ouro——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Ouro 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`ouro setup`、`ouro gateway install`、编辑配置）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期有所不同：配置存放在 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，CLI 配置命令被屏蔽。管理 ouro 的方式与管理其他 NixOS 服务相同。
:::
...
nix run github:Aditya232-rtx/ouro -- chat
...
执行 `nix profile install` 后，`hermes`、`ouro-agent` 和 `ouro-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`ouro setup` 引导你完成提供商选择，`ouro gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.ouro/`。
...
设置 `addToSystemPackages = true` 有两个作用：将 `hermes` CLI 添加到系统 PATH，**并**在系统范围内设置 `OURO_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `hermes` 会创建独立的 `~/.ouro/` 目录。
:::
...
- 路由是透明的：`ouro chat`、`ouro sessions list`、`ouro version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
- 如果容器未运行，CLI 会短暂重试（交互式使用时显示 5 秒 spinner，脚本中静默等待 10 秒），然后以明确的错误退出——不会静默回退
- 对于在 ouro 代码库上工作的开发者，设置 `OURO_DEV=1` 可绕过容器路由，直接运行本地检出版本

设置 `container.hostUsers` 可创建 `~/.ouro` 到服务状态目录的符号链接，使主机 CLI 和容器共享会话、配置和记忆：
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo ouro chat`。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Ouro 默认使用 OpenRouter。
:::
...
这会完全绕过 `settings`——不合并，不生成。每次激活时，该文件会原样复制到 `$OURO_HOME/config.yaml`。
...
| 给 Agent 设置个性 | `${services.hermes-agent.stateDir}/.ouro/SOUL.md` | 直接管理该文件 |
...
| 覆盖 ouro 包 | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$OURO_HOME/.env` 中。Ouro 在每次启动时读取此文件，因此更改在 `systemctl restart ouro-agent` 后生效——无需重建容器。
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Ouro 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：Ouro 从 `$OURO_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.hermes-agent.stateDir}/.ouro/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
:::tip
`env` 值中的环境变量在运行时从 `$OURO_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Ouro 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
Token 存储在 `$OURO_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。

<details>
<summary><strong>无头服务器上的初始 OAuth 授权</strong></summary>

首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Ouro 将授权 URL 打印到 stdout/日志，而不是打开浏览器。
...
当 ouro 通过 NixOS 模块运行时，以下 CLI 命令会被**屏蔽**，并显示指向 `configuration.nix` 的描述性错误：

| 被屏蔽的命令 | 原因 |
|---|---|
| `ouro setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `ouro config edit` | 配置由 `settings` 生成 |
| `ouro config set <key> <value>` | 配置由 `settings` 生成 |
| `ouro gateway install` | systemd 服务由 NixOS 管理 |
| `ouro gateway uninstall` | systemd 服务由 NixOS 管理 |

这可以防止 Nix 声明的内容与磁盘上实际内容之间产生漂移。检测使用两个信号：

1. **`OURO_MANAGED=true`** 环境变量——由 systemd 服务设置，对 gateway 进程可见
2. **`.managed` 标记文件**，位于 `OURO_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it ouro-agent ouro config set ...` 也会被屏蔽）
...
启用容器模式后，ouro 在持久化 Ubuntu 容器内运行，Nix 构建的二进制文件以只读方式从主机绑定挂载：

```
主机                                    容器
────                                    ─────────
/nix/store/...-hermes-agent-0.1.0  ──►  /nix/store/... (ro)
~/.ouro -> /var/lib/hermes/.ouro       （符号链接桥接，按 hostUsers）
...
|---|---|---|---|---|
| `systemctl restart ouro-agent` | 否 | 保留 | 保留 | 保留 |
...
仅当容器的**身份哈希**发生变化时才会重建容器。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 和入口点脚本。环境变量、settings、文档或 ouro 包本身的变更**不会**触发重建。
...
`preStart` 脚本在 `${stateDir}/.gc-root` 创建一个指向当前 ouro 包的 GC root。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC root 损坏，重启服务会重新创建它。

---

## 插件

NixOS 模块支持声明式插件安装——无需命令式的 `ouro plugins install`。
...
插件在激活时以符号链接方式安装到 `$OURO_HOME/plugins/`。Ouro 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
该包的 `site-packages` 会添加到 ouro wrapper 的 PYTHONPATH 中。`importlib.metadata` 在会话启动时发现入口点。

### 可选依赖组（`extraDependencyGroups`）

对于已在 ouro-agent 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
:::note
构建时冲突检查可防止插件包覆盖核心 ouro 依赖。如果插件提供了封闭 venv 中已有的包，`nixos-rebuild` 会以明确的错误失败。
:::
...
nix build .#checks.x86_64-linux.managed-guard       # OURO_MANAGED 屏蔽变更操作
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `ouro-agent` 二进制文件存在且 `ouro version` 可运行 |
| `entry-points-sync` | `pyproject.toml` 中 `[project.scripts]` 的每个条目在 Nix 包中都有对应的封装二进制文件 |
| `cli-commands` | `ouro --help` 暴露 `gateway` 和 `config` 子命令 |
| `managed-guard` | `OURO_MANAGED=true ouro config set ...` 打印 NixOS 错误 |
| `bundled-skills` | skills 目录存在，包含 SKILL.md 文件，wrapper 中设置了 `OURO_BUNDLED_SKILLS` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 ouro-agent 服务 |
| `package` | `package` | `ouro-agent` | 使用的 ouro-agent 包 |
| `user` | `str` | `"hermes"` | 系统用户 |
| `group` | `str` | `"hermes"` | 系统组 |
| `createUser` | `bool` | `true` | 自动创建用户/组 |
| `stateDir` | `str` | `"/var/lib/hermes"` | 状态目录（`OURO_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `hermes` CLI 添加到系统 PATH 并在系统范围内设置 `OURO_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$OURO_HOME/.env` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `ouro gateway` 的额外参数 |
| `extraPackages` | `listOf package` | `[]` | Agent 可用的额外包。添加到 ouro 用户的每用户 profile，终端命令、skills 和 cron 任务均可见 |
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$OURO_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.ouro` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `hermes` 组 |
...
```
/var/lib/hermes/                     # stateDir（归 hermes:ouro 所有，权限 0750）
├── .ouro/                         # OURO_HOME
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Ouro 二进制文件 + 所有 Nix 依赖 |
...
| `ouro version` 显示旧版本 | 容器未重启 | `systemctl restart ouro-agent` |
| `/var/lib/hermes` 权限拒绝 | 状态目录为 `0750 hermes:hermes` | 使用 `docker exec` 或 `sudo -u hermes` |
| `nix-collect-garbage` 删除了 ouro | GC root 缺失 | 重启服务（preStart 会重新创建 GC root） |
| `no container with name or ID "ouro-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user hermes` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart ouro-agent` |