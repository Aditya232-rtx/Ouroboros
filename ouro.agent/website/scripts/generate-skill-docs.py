...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Aditya232-rtx/ouro/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `ouro skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Ouro"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Ouro ships with a large built-in skill library copied into `~/.ouro/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Ouro also syncs bundled skills on `ouro update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.ouro/skills/` tree, it is still shipped with Ouro; restore it with `ouro skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with ouro-agent — install via ouro skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with ouro-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "ouro skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "ouro skills install official/blockchain/solana",
        "ouro skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "ouro skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
