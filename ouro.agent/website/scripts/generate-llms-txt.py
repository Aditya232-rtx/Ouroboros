#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Ouro docs site.
...
Both publish at:
  https://ouro.dev/docs/llms.txt
  https://ouro.dev/docs/llms-full.txt
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Ouro as a Python Library", None),
        ("guides/use-mcp-with-hermes", "Use MCP with Ouro", None),
        ("guides/use-voice-mode-with-hermes", "Use Voice Mode with Ouro", None),
        ("guides/use-soul-with-hermes", "Use SOUL.md with Ouro", None),
        ("guides/build-a-hermes-plugin", "Build a Ouro Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Ouro"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "ouro-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/Aditya232-rtx/ouro")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Ouro — Full Documentation\n",
        (
            "This file is the entire Ouro documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Ouro, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://ouro.dev/docs\n",
        "Short index: https://ouro.dev/docs/llms.txt\n",
        "\n---\n\n",
    ]
