...
description: "Run Ouro directly on an Android phone with Termux"
---

# Ouro on Android with Termux
...
Ouro can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Ouro from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Ouro now ships a Termux-aware installer path:
...
Or set keys directly in `~/.ouro/.env`.
...
### `ouro doctor` says ripgrep or Node is missing
