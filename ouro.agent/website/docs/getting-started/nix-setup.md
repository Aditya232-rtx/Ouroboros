...
description: "Install and deploy Ouro with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Ouro ships a Nix flake & a NixOS module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`ouro setup`, `ouro gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage ouro the same way you manage any other NixOS service.
:::
...
```bash
# Run the desktop app
nix run github:Aditya232-rtx/ouro#desktop

# Or install persistently
nix profile install github:Aditya232-rtx/ouro#desktop

# run the tui
nix run github:Aditya232-rtx/ouro -- setup
nix run github:Aditya232-rtx/ouro -- --tui

# or install it in your profile
nix profile install github:Aditya232-rtx/ouro
ouro setup
ouro --tui
```

After `nix profile install`, `hermes`, `ouro-agent`, and `ouro-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `ouro setup` walks you through provider selection, `ouro gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.ouro/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries ouro-agent might need. if you want a smaller variant, check the other flake outputs. 
...
```bash
git clone https://github.com/Aditya232-rtx/ouro.git
cd ouro-agent
nix develop
ouro setup
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    ouro-agent.url = "github:Aditya232-rtx/ouro";
  };

  outputs = { nixpkgs, ouro-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        ouro-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o ouro /dev/stdin /var/lib/hermes/env
```
...
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `OURO_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.ouro/` directory.
:::
...
- The routing is transparent: `ouro chat`, `ouro sessions list`, `ouro version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the ouro codebase, set `OURO_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.ouro` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo ouro chat` manually.
:::
...
systemctl status ouro-agent

# Watch logs (Ctrl+C to stop)
journalctl -u ouro-agent -f

# If addToSystemPackages is true, test the CLI
ouro version
ouro config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Ouro defaults to OpenRouter.
:::
...
This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$OURO_HOME/config.yaml` on each activation.
...
| Give the agent a personality | `${services.hermes-agent.stateDir}/.ouro/SOUL.md` | manage the file directly |
...
| Override the ouro package | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$OURO_HOME/.env` at activation time (`nixos-rebuild switch`). Ouro reads this file on every startup, so changes take effect with a `systemctl restart ouro-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Ouro looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Ouro loads its primary `SOUL.md` from `$OURO_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.ouro/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$OURO_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Set `auth = "oauth"` for servers using OAuth 2.1. Ouro implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
Tokens are stored in `$OURO_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Ouro prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u hermes` (native):

```bash
# Container mode
docker exec -it ouro-agent \
  ouro mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u ouro OURO_HOME=/var/lib/hermes/.ouro \
  ouro mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
ouro mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.ouro/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/hermes/.ouro/mcp-tokens/
...
When ouro runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `ouro setup` | Config is declarative — edit `settings` in your Nix config |
| `ouro config edit` | Config is generated from `settings` |
| `ouro config set <key> <value>` | Config is generated from `settings` |
| `ouro gateway install` | The systemd service is managed by NixOS |
| `ouro gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`OURO_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `OURO_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it ouro-agent ouro config set ...` is also blocked)
...
When container mode is enabled, ouro runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-hermes-agent-0.1.0  ──►  /nix/store/... (ro)
~/.ouro -> /var/lib/hermes/.ouro       (symlink bridge, per hostUsers)
...
|---|---|---|---|---|
| `systemctl restart ouro-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the ouro package itself do **not** trigger recreation.
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current ouro package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `ouro plugins install` needed.
...
Plugins are symlinked into `$OURO_HOME/plugins/` at activation time. Ouro discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the ouro wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in ouro-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.
...
```nix
{
  inputs.hermes-agent.url = "github:Aditya232-rtx/ouro";
  outputs = { ouro-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ ouro-agent.overlays.default ];
    # Then:
...
:::note
A build-time collision check prevents plugin packages from shadowing core ouro dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # OURO_MANAGED blocks mutation
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `ouro-agent` binaries exist and `ouro version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `ouro --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `OURO_MANAGED=true ouro config set ...` prints the NixOS error |
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `OURO_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the ouro-agent service |
| `package` | `package` | `ouro-agent` | The ouro-agent package to use |
| `user` | `str` | `"hermes"` | System user |
| `group` | `str` | `"hermes"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`OURO_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `OURO_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$OURO_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `ouro gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the ouro user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$OURO_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.ouro` symlink to the service stateDir and are auto-added to the `hermes` group |
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Ouro binary + all Nix deps |
...
cd /etc/nixos && nix flake update ouro-agent
...
```bash
systemctl status ouro-agent
docker ps -a --filter name=ouro-agent
docker inspect ouro-agent --format='{{.State.Status}}'
docker exec -it ouro-agent bash
docker exec ouro-agent readlink /data/current-package
docker exec ouro-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop ouro-agent
docker rm -f ouro-agent
sudo rm /var/lib/hermes/.container-identity
sudo systemctl start ouro-agent
```
...
```bash
# Native mode
sudo -u ouro cat /var/lib/hermes/.ouro/.env

# Container mode
docker exec ouro-agent cat /data/.ouro/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec ouro-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u ouro-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `ouro version` shows old version | Container not restarted | `systemctl restart ouro-agent` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 hermes:hermes` | Use `docker exec` or `sudo -u hermes` |
| `nix-collect-garbage` removed ouro | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "ouro-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
...
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart ouro-agent` |
