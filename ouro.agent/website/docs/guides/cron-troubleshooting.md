...
description: "Diagnose and fix common Ouro cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`ouro gateway` for foreground, or `ouro gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `ouro cron tick`.
...
```bash
date
ouro cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.ouro/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.ouro/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.ouro/.env` |
...
| `local` | Write access to `~/.ouro/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `ouro cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `ouro skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `ouro skills list`.
...
2. `~/.ouro/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `ouro cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Ouro config directory). Verify:
```bash
ls ~/.ouro/scripts/your-script.py   # Must exist
ouro cron edit <job_id> --script ~/.ouro/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `ouro skills install <skill-name>`.
...
Jobs are stored in `~/.ouro/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.ouro/cron/jobs.json
chmod 600 ~/.ouro/cron/jobs.json   # Your user should own it
```
...
```bash
ouro cron list                    # Show all jobs, states, next_run times
ouro cron run <job_id>            # Schedule for next tick (for testing)
ouro cron edit <job_id>           # Fix configuration issues
ouro logs                         # View recent Ouro logs
ouro skills list                  # Verify installed skills
```
...
1. Run the job with `ouro cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.ouro/logs/agent.log` for scheduler messages and `~/.ouro/logs/errors.log` for warnings
3. Open an issue at [github.com/Aditya232-rtx/ouro](https://github.com/Aditya232-rtx/ouro) with:
