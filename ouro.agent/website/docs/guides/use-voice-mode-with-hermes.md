---
sidebar_position: 8
title: "Use Voice Mode with Ouro"
description: "A practical guide to setting up and using Ouro voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Ouro
...
- you want Ouro sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Ouro.
...
## Step 1: make sure normal Ouro works first
...
Ouro supports both local and cloud speech stacks.
...
### If you use `ouro setup`

If you choose NeuTTS in the setup wizard, Ouro checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Ouro transcribes and responds
...
- asking Ouro to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Ouro loop.

## Tuning CLI behavior

### Silence threshold

If Ouro starts/stops too aggressively, tune:
...
Ouro stays a normal chat bot, but can speak replies.
...
- you want Ouro to function like a portable research or ops assistant
...
Ouro joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Ouro detects speech boundaries
- transcripts are posted in the associated text channel
- Ouro responds in text and audio
...
1. get text Ouro working
2. run `ouro setup tts` to enable voice support
