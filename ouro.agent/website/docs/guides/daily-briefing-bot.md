...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `ouro setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Ouro spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.ouro/cron/output/` and you can read them anytime.
:::
...
Ouro will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Ouro
```
...
Before creating cron jobs, ensure Ouro has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Ouro what you want:
...
Ouro will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Ouro to delegate each topic to a sub-agent:
...
Ouro will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
