...
```bash
ouro teams-pipeline token-health
ouro teams-pipeline token-health --force-refresh
```
...
```bash
ouro teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
ouro teams-pipeline maintain-subscriptions
ouro teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Ouro cron (recommended if you already run the Ouro gateway)

Ouro ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.ouro/scripts/`. First create the script:

```bash
mkdir -p ~/.ouro/scripts
cat > ~/.ouro/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec ouro teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.ouro/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
ouro cron create "0 */12 * * *" \
...
```ini
[Unit]
Description=Ouro Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run Ouro Teams pipeline subscription maintenance every 12 hours
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.ouro/.env` at the top of a wrapper script that crontab calls.
...
```bash
ouro teams-pipeline subscriptions   # should show expirationDateTime advanced
ouro teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
ouro teams-pipeline list
ouro teams-pipeline list --status failed
ouro teams-pipeline show <job-id>
```

### Replay a stored job

```bash
ouro teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
ouro teams-pipeline fetch --meeting-id <meeting-id>
ouro teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `ouro teams-pipeline maintain-subscriptions --dry-run`
- inspect `ouro teams-pipeline list --status failed`
...
- run `ouro teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `ouro teams-pipeline run`
...
- [ ] `ouro teams-pipeline validate` returns an OK snapshot
- [ ] `ouro teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Ouro cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
