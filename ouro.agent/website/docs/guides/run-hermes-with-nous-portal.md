...
3. Sets `model.provider: nous` in `~/.ouro/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Ouro is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
ouro setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
ouro auth add nous --type oauth
# Then re-run `ouro setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Ouro release notes" and summarize the top 3 hits.
```

You should see Ouro call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`ouro setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
ouro config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Ouro-4 for agent work

Ouro-4-70B and Ouro-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Ouro itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Ouro-side opinion.
...
These rows appear in `ouro tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Ouro runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Ouro will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Ouro profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.ouro/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `ouro portal info` shows "not logged in" after `ouro setup --portal`
...
```bash
ouro config set model.provider nous
```
...
Re-verify with `ouro portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Ouro doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/Aditya232-rtx/ouro/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`ouro portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Ouro profiles where you're using the wrong one (check `ouro profile list`)

### Want to revoke and start clean

```bash
ouro auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Ouro tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Ouro configurations
