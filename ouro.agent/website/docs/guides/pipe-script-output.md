...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `ouro send`."
---

# Pipe Script Output to Messaging Platforms

`ouro send` is a small, scriptable CLI that pushes a message to any
messaging platform Ouro is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | ouro send --to slack:#builds`)
...
ouro send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
ouro send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
ouro send --list

# Filter by platform
ouro send --list telegram
```
...
Any platform Ouro ships adapters for works as a target:
...
`ouro send` resolves the message body in this order:

1. **Positional argument** — `ouro send --to telegram "hi"`
2. **`--file PATH`** — `ouro send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | ouro send --to telegram`

When stdin is a TTY (no pipe), Ouro does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  ouro send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `ouro send` reuses your Ouro config, the same script works on
any host where Ouro is installed — no need to export bot tokens into
...
`ouro send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  ouro send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | ouro send \
...
```bash
./train.py --epochs 200 && \
  ouro send --to telegram "training done" || \
  ouro send --to telegram "training failed (exit $?)"
```
...
ouro send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(ouro send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `ouro send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `ouro send` calls
the platform's REST endpoint directly using credentials from
`~/.ouro/.env` and `~/.ouro/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `ouro gateway start` and retry.
...
The listing is built from `~/.ouro/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`ouro gateway
...
| Approach | Multi-platform | Reuses Ouro creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `ouro send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`ouro send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `ouro send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `ouro send` shares with cron delivery.
