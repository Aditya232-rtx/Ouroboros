...
Copy-paste blueprints for common automation patterns. Each blueprint uses Ouro's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`ouro webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `ouro webhook subscribe`) |
...
```bash
ouro cron create "0 2 * * *" \
  "You are a project manager triaging the Aditya232-rtx/ouro GitHub repo.

1. Run: gh issue list --repo Aditya232-rtx/ouro --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
ouro webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
ouro cron create "0 9 * * 1" \
  "Scan the Aditya232-rtx/ouro repo for documentation drift.

1. Run: gh pr list --repo Aditya232-rtx/ouro --state merged --json number,title,files,mergedAt --limit 30
...
```bash
ouro cron create "0 6 * * *" \
  "Run a dependency security audit on the ouro-agent project.

1. cd ~/.ouro/hermes-agent && source .venv/bin/activate
...
```bash
ouro webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
ouro webhook subscribe alert-triage \
...
```python title="~/.ouro/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Ouro-Monitor/1.0"})
...
ouro cron create "every 30m" \
  ...
  --script ~/.ouro/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
ouro cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Ouro development." \
...
```bash
ouro webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
ouro webhook subscribe stripe-payments \
...
```bash
ouro cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the ouro-agent codebase.
