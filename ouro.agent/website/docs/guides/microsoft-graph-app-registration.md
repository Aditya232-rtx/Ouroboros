...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.ouro/.env` at the end.
...
4. Fill in:
   - **Name:** `Ouro Teams Meeting Pipeline` (or any name you'll recognize).
...
Put the three values you collected into `~/.ouro/.env`:
...
Ouro ships a Graph auth smoke-test. From your Ouro install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.ouro/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `ouro gateway restart`.
