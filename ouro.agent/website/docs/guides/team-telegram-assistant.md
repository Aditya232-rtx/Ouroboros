...
This tutorial walks you through setting up a Telegram bot powered by Ouro that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Ouro installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.ouro/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Ouro itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Ouro Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Ouro. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u ouro-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `ouro gateway install` to update the plist.
:::
...
```bash
ouro gateway stop && ouro gateway start
```
...
ouro pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.ouro/.env`:
...
Control how much detail the bot shows when using tools. In `~/.ouro/config.yaml`:
...
Customize how the bot communicates by editing `~/.ouro/SOUL.md`:

For a full guide, see [Use SOUL.md with Ouro](/guides/use-soul-with-hermes).
...
```bash
# From the CLI
ouro cron list          # View all scheduled jobs
ouro cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u ouro-gateway` (Linux) or `~/.ouro/logs/gateway.log` (macOS) |
| Cron job output | `~/.ouro/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.ouro/cron/jobs.json` |
| Pairing data | `~/.ouro/pairing/` |
| Session history | `~/.ouro/sessions/` |
