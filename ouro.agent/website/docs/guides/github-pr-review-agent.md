...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Ouro  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Ouro instantly when PRs are opened or updated.
:::
...
- **Ouro installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  ouro gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.ouro/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Ouro can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Aditya232-rtx/ouro --state open --limit 3
```
...
Still in the chat, ask Ouro to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Aditya232-rtx/ouro
```
...
A skill gives Ouro consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.ouro/skills/code-review
```

Create `~/.ouro/skills/code-review/SKILL.md`:
...
This is what makes the reviewer actually useful. Start a session and teach Ouro your team's standards:
...
2. Teach Ouro your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
ouro gateway status    # Is the gateway running?
ouro cron list         # Is the job enabled?
```
