...
description: "Frequently asked questions and solutions to common issues with Ouro"
---
...
### What LLM providers work with Ouro?

Ouro works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `ouro auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `ouro model` or by editing `~/.ouro/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Ouro in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Ouro inside WSL2
...
- let Ouro use the resulting MCP browser tools

This is more reliable than trying to force Ouro core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Ouro](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Ouro does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.ouro/`.

### Can I use it offline / with local models?

Yes. Run `ouro model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Ouro minimum; set this to match your server's actual context window
```
...
Ouro persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Ouro — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Ouro auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `OURO_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Ouro itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Ouro instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Ouro programmatically:
...
**Cause:** Ouro requires Python 3.11 or newer.
...
**Cause:** Ouro builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Ouro runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Ouro auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.ouro/config.yaml`:
...
#### The agent says "Ouro policy" or "Ouro guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Ouro runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Ouro enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Ouro does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Ouro tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
3. Retry in a fresh session with another configured model or provider. A refusal that changes with the model is model/provider behavior, not a Ouro execution control.
4. If an explicit tool error appears, use its exact text when reporting the problem.

See [Security](/user-guide/security) for Ouro' documented execution controls and [Providers](/integrations/providers) for provider configuration.
...
**Solution:** Exit your session and use `ouro model` from your terminal to add new providers:

```bash
# Exit the Ouro chat session first (Ctrl+C or /quit)
...
After adding a new provider via `ouro model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `ouro model` (from terminal) |
| Enter/change API keys | `ouro model` (from terminal) |
...
# Or set directly
ouro config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.ouro/.env` for conflicting entries.
:::
...
# Set a valid model
ouro config set OURO_MODEL anthropic/claude-opus-4.7

# Or specify per-session
ouro chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `ouro chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Ouro detected the wrong context length for your model.
...
ouro chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Ouro may have the wrong context length for your model. Check what it detected:
...
**Cause:** Ouro detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Ouro never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `ouro chat`
...
cat ~/.ouro/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `ouro gateway setup`
- Check gateway logs: `cat ~/.ouro/logs/gateway.log | tail -50`
...
Configure in `~/.ouro/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.ouro/hermes-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `ouro gateway start` fails
...
tmux new -s ouro 'ouro gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup ouro gateway run > ~/.ouro/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'ouro gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `ouro gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
ouro gateway install    # Re-snapshots your current PATH
ouro gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `ouro chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `ouro chat -t "terminal"`
...
If the baseline looks high before you've typed anything, that's the fixed prompt budget — the system prompt plus tool schemas sent on every call. Run [`ouro prompt-size`](/reference/cli-commands#hermes-prompt-size) to measure it, then trim: disable toolsets you don't use (`ouro tools`) and uninstall or disable skills you don't need (`ouro skills`).
...
cd ~/.ouro/hermes-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.ouro/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
ouro config show | grep -A 12 mcp_servers

# Restart Ouro or reload MCP after config changes
ouro chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Ouro](/guides/use-mcp-with-hermes)
...
:::warning
If an MCP server crashes mid-request, Ouro will report a timeout. Check the server's own logs (not just Ouro logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `ouro profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Ouro home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `ouro update`?

`ouro update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `ouro update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.ouro/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Ouro can route subagents to a different model automatically. Set this in `~/.ouro/config.yaml`:
...
Now when you tell Ouro "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Ouro do this?

**Current limitation:** Ouro profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Ouro reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `ouro skills config` settings don't seem to take effect.

**Solution:** Use `ouro skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`ouro gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Ouro creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Ouro to another machine
...
1. Install Ouro on the new machine:
   ```bash
   curl -fsSL https://ouro.dev/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   ouro backup
   ```
   This creates a zip of your entire `~/.ouro/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   ouro import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `ouro setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
ouro profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
ouro profile import ./work-backup.tar.gz work
```
...
### `ouro backup` vs `ouro profile export`

| Feature | `ouro backup` | `ouro profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.ouro` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='ouro-agent' ~/.ouro/ newmachine:~/.ouro/
```

:::tip
`ouro backup` produces a consistent snapshot even while Ouro is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Ouro installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Ouro-specific issue — it's a shell config permissions problem.
...
ouro config show | head -20

# Re-run model selection
ouro model

# Or test with a known-good model
ouro chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/Aditya232-rtx/ouro/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Ouro version (`ouro --version`), and the full error message
