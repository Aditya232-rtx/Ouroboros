...
This page covers all commands related to [Ouro profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `ouro profile` without a subcommand shows help.
...
ouro profile use default
```

## `ouro profile create`

```bash
ouro profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `ouro profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `ouro update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.ouro`), use `ouro skills opt-out` / `ouro skills opt-in`. |
...
ouro profile create work2 --clone-from work

# Clone everything from a specific profile
ouro profile create work2-backup --clone-from work --clone-all
```

## `ouro profile describe`

```bash
ouro profile describe [<name>] [options]
```
...
ouro profile describe researcher

# Set it explicitly
ouro profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
ouro profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
ouro profile describe --all --auto
```

## `ouro profile delete`

```bash
ouro profile delete <name> [options]
```
...
```bash
ouro profile delete mybot
ouro profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.ouro`) cannot be deleted — use `ouro uninstall` to remove everything.
:::

## `ouro profile show`

```bash
ouro profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Ouro home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ ouro profile show work
Profile: work
Path:    ~/.ouro/profiles/work
...
```bash
ouro profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Ouro installation.
...
```bash
ouro profile alias work
# Creates/updates ~/.local/bin/work

ouro profile alias work --name mywork
# Creates ~/.local/bin/mywork

ouro profile alias work --remove
...
```bash
ouro profile rename <old-name> <new-name>
```
...
```bash
ouro profile rename mybot assistant
# ~/.ouro/profiles/mybot → ~/.ouro/profiles/assistant
...
```bash
ouro profile export <name> [options]
```
...
```bash
ouro profile export work
# Creates work.tar.gz in the current directory

ouro profile export work -o ./work-2026-03-29.tar.gz
```
...
```bash
ouro profile import <archive> [options]
```
...
```bash
ouro profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

ouro profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `ouro -p telemetry`). |
...
ouro profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
ouro profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
ouro profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
ouro profile install ./telemetry/
```

### `ouro profile update`

```bash
ouro profile update <name> [--force-config] [--yes]
```
...
```bash
ouro profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Ouro version, author, env var requirements, the source URL/path, and
...
`ouro profile list` also shows the distribution name and version in a
`Distribution` column, and `ouro profile show <name>` / `delete <name>`
...
ouro profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
ouro profile install https://github.com/your-org/internal-assistant.git
```
...
Ouro version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Ouro can clone from.
3. Tell recipients to run `ouro profile install <your-repo-url>`.
...
```bash
ouro -p <name> <command> [options]
ouro --profile <name> <command> [options]
```

Global flag to run any Ouro command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
ouro -p work chat -q "Check the server status"
ouro --profile dev gateway start
ouro -p personal skills list
ouro -p work config edit
```

## `ouro completion`

```bash
ouro completion <shell>
```
...
- `ouro profile <TAB>` — subcommands (list, use, create, etc.)
- `ouro profile use <TAB>` — profile names
- `ouro -p <TAB>` — profile names
