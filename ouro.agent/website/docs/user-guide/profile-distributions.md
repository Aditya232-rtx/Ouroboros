...
A **profile distribution** packages a complete Ouro agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Ouro has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `ouro profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `ouro profile update` pulls new versions | Re-send the file |
...
| Made with | `ouro profile install` / `update` | `/export` and `/import`, or `ouro profile export` / `import` |
...
Before distributions, sharing a Ouro agent meant sending someone:
...
```bash
ouro profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `ouro profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Ouro in 2026, that's already true.
...
:::caution
**Ouro does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `ouro profile install` or `ouro profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
ouro profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.ouro/profiles/research-bot/SOUL.md
...
Create `~/.ouro/profiles/research-bot/distribution.yaml`:
...
Create `~/.ouro/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `ouro profile update research-bot` will pull the latest.
...
```bash
ouro profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.ouro/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
ouro profile install github.com/you/research-bot

# Full HTTPS
ouro profile install https://github.com/you/research-bot.git

# SSH
ouro profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
ouro profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
ouro profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
ouro profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
ouro profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
ouro profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Ouro distribution.
...
```bash
cp ~/.ouro/profiles/research-bot/.env.EXAMPLE ~/.ouro/profiles/research-bot/.env
...
```bash
ouro profile info research-bot
```
...
`ouro profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
ouro profile update research-bot
```
...
```bash
ouro profile delete research-bot
```
...
Path:    ~/.ouro/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `ouro profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
ouro profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `ouro profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.ouro/profiles/polymarket-trader
...
# Anyone
ouro profile install github.com/you/hermes-polymarket-trader --alias
```
...
You built Ouro-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
ouro profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
ouro profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `ouro profile info <name>` and hold off on updates until you're ready.
:::
...
ouro profile info research-bot | grep Version
...
```yaml
# ~/.ouro/profiles/research-bot/local/my-overrides.yaml
...
ouro profile delete research-bot --yes
ouro profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
ouro profile update research-bot --force-config --yes
```
...
ouro profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.ouro/profiles/forked-research-bot/
...
ouro profile install ~/.ouro/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
ouro profile delete research-bot-test --yes
ouro profile install ~/.ouro/profiles/research-bot --name research-bot-test
```
...
```bash
ouro profile export research-bot
ouro profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
ouro profile import ./research-bot.tar.gz
ouro profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `ouro -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.ouro`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.ouro`) is exported through an allow-list — only known Ouro artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.ouro/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `ouro -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `ouro profile` — not a parallel command tree.
...
- [`ouro profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Ouro](../guides/use-soul-with-hermes.md) — authoring personalities
