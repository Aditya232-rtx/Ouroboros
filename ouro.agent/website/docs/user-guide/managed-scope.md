...
`~/.ouro/config.yaml`, `~/.ouro/.env`, and even the shell environment — for
...
```text
/etc/hermes/
├── config.yaml     # managed config layer (wins over ~/.ouro/config.yaml)
└── .env            # managed env layer (wins over ~/.ouro/.env + shell)
```
...
The location can be relocated with the `OURO_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `OURO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Ouro.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export OURO_MANAGED_DIR=/opt/org/hermes-policy
```

:::warning
A user who can set `OURO_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `ouro doctor` reports the *resolved* managed directory so
...
| 2 | `~/.ouro/config.yaml` (user) | `~/.ouro/.env` (user) |
...
```bash
ouro config        # shows a header naming the managed source + the pinned keys
ouro doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Ouro refuses and names the source:

```bash
$ ouro config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hermes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `ouro config set` / setup will not write
...
Changes take effect on the next Ouro start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`ouro doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Ouro as `root`), managed scope is advisory.
