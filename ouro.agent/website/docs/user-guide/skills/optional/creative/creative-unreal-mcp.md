...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/creative/unreal-mcp` |
...
`~/.ouro/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Ouro — that
...
2. Start the Ouro session. Tools register as `mcp_unreal_engine_*`. If
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Ouro runs batched calls
...
- **Start order matters.** Editor + server up first, then the Ouro
...
- **Timeouts on long operations.** Ouro' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.ouro/config.yaml` for
