...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Ouro |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Ouro terminal calls (desktop app flow only)
...
### Service Account (recommended for Ouro)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${OURO_HOME:-~/.ouro}/.env` (the skill will prompt for this on first load).
...
## Ouro Execution Pattern (desktop app flow)

Ouro terminal commands are non-interactive by default and can lose auth context between calls.
