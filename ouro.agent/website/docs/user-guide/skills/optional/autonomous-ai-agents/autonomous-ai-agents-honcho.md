---
title: "Honcho — Configure and troubleshoot Honcho memory for Ouro"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Ouro"
---
...
Configure and troubleshoot Honcho memory for Ouro.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ouro skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Ouro

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Ouro profile its own peer identity while sharing a unified view of the user.
...
```bash
ouro honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Ouro profile's AI peer
...
Honcho models conversations as interactions between **peers**. Ouro creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Ouro instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Ouro run |
| `global` | Single session across all directories |

Manual override: `ouro honcho map my-project-name`
...
Each Ouro profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
ouro honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Ouro when Honcho memory is active.
...
Config file: `$OURO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `ouro honcho setup`. Ensure `memory.provider: honcho` is in `~/.ouro/config.yaml`.

### Memory not persisting across sessions
Check `ouro honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `ouro profile create <name> --clone`. For existing profiles: `ouro honcho sync`.
...
| Command | Description |
|---------|-------------|
| `ouro honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `ouro honcho status` | Show resolved config, connection test, peer info for active profile |
| `ouro honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `ouro honcho disable` | Disable Honcho for the active profile |
| `ouro honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `ouro honcho peers` | Show peer identities across all profiles |
| `ouro honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `ouro honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `ouro honcho sessions` | List known directory-to-session-name mappings |
| `ouro honcho map <name>` | Map current working directory to a Honcho session name |
| `ouro honcho identity` | Seed AI peer identity or show both peer representations |
| `ouro honcho sync` | Create host blocks for all Ouro profiles that don't have one yet |
| `ouro honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Ouro + Honcho |
| `ouro memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `ouro memory status` | Show active memory provider and config |
| `ouro memory off` | Disable external memory provider |
