...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/mcp/fastmcp` |
...
| Related skills | [`ouro-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ouro-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Ouro or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Ouro. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.ouro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.ouro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Ouro integration, either:

- configure the server in `~/.ouro/config.yaml` using the `native-mcp` skill, or
...
### Ouro cannot see the deployed server

The server-building part may be correct while the Ouro config is not. Load the `native-mcp` skill and configure the server in `~/.ouro/config.yaml`, then restart Ouro.
