---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Ouro"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Ouro"
---
...
Import an OpenClaw setup (memories, skills) into Ouro.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ouro skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Ouro (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Ouro`, `Memory`, `Persona`, `Import` |
| Related skills | [`ouro-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ouro-agent) |
...
# OpenClaw -> Ouro Migration

Use this skill when a user wants to move their OpenClaw setup into Ouro with minimal manual cleanup.
...
```bash
ouro claw migrate              # Full interactive migration
...
ouro claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `ouro setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Ouro home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Ouro memory entries
- merge OpenClaw command approval patterns into Ouro `command_allowlist`
- migrate Ouro-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Ouro working-directory configuration
- copy OpenClaw skills into `~/.ouro/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Ouro workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.ouro/tts/`
- archive non-secret docs that do not have a direct Ouro destination
...
- `~/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.ouro/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.ouro/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Ouro-compatible secrets, currently:
...
Ouro CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Ouro. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Ouro workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Ouro skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.ouro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Ouro unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Ouro targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Ouro destination. Unsupported auth blobs must still be reported as skipped.
...
- Ouro persona state imported
- Ouro memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.ouro/skills/openclaw-imports/`
