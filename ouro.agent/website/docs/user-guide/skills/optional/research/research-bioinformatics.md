...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/research/bioinformatics` |
...
3. Read the specific skill:
   ...
   ```
4. Follow the fetched skill as reference material. These are NOT Ouro-format skills — treat them as expert domain guides. They contain correct parameters, proper tool flags, and validated pipelines.
...
- The fetched skills are NOT in Ouro SKILL.md format. They use their own structure (bioSkills: code pattern cookbooks; ClawBio: README + Python scripts). Read them as expert reference material.
