---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Ouro"
...
Set up Actual Computer (actual.inc) inference in Ouro.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ouro skills install official/devops/actual-setup` |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Ouro inference
...
- User has an `ac_` key and wants Ouro routed through their Actual cluster.
...
- Ouro has **first-class `actual` provider support** (provider id `actual`,
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.ouro/.env`.
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Ouro at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
...
1. **reasoning_effort trap (handled by Ouro since the first-class provider).**
...
2. **Context-window overflow on small local models.** Ouro' default toolset
   ...
   servers emit a bare `data: [DONE]` — Ouro reports
