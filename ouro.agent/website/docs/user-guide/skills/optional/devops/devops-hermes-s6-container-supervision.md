---
title: "Ouro S6 Container Supervision — Modify or debug s6 services in the Ouro Docker image"
sidebar_label: "Ouro S6 Container Supervision"
description: "Modify or debug s6 services in the Ouro Docker image"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Ouro S6 Container Supervision

Modify or debug s6 services in the Ouro Docker image.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `ouro skills install official/devops/hermes-s6-container-supervision` |
...
# Ouro s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Ouro Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the Ouro and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `OURO_ARGS` for a service `run` script to consume.
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/hermes/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to ouro via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.
...
2. Drop to ouro via `s6-setuidgid hermes` at the top of run (unless you specifically need root).
...
`docker exec` defaults to root. Either pass `--user hermes` or rely on the stage2 chown sweep next reboot. Don't write files under `$OURO_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `ouro -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).
...
- `hermes-agent-dev`: General ouro-agent codebase navigation
- `hermes-tool-quirks`: Specific Ouro-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with ouro built-in tools.
