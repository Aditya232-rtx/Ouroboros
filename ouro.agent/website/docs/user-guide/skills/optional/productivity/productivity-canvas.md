...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Ouro"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${OURO_HOME:-~/.ouro}/.env`:
...
```bash
CANVAS="python $OURO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
