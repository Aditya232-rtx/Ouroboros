...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${OURO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${OURO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${OURO_SKILL_DIR}/scripts/publish.sh"
...
```bash
DRIVE="${OURO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
