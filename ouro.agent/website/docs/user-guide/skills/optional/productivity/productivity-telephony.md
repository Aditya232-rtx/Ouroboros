...
| | |
|---|---|
| Source | Optional — install with `ouro skills install official/productivity/telephony` |
...
This optional skill gives Ouro practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${OURO_HOME:-~/.ouro}/.env`
...
It does **not** turn Ouro into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Ouro memory
...
### 1) "I want Ouro to own a real phone number"
Use **Twilio**.
...
- pairs well with Ouro `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.ouro/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
ouro skills search telephony
ouro skills install official/productivity/telephony
```
...
Then save credentials into Ouro:
...
3. Buy it and save it into `${OURO_HOME:-~/.ouro}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Ouro calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Ouro's existing `text_to_speech` support.

Use this when:
- you want the call to use Ouro's configured TTS voice rather than Twilio `<Say>`
...
Recommended Ouro TTS -> Twilio Play workflow:

1. Generate the audio with Ouro `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Ouro to create the message audio with `text_to_speech`
...
Important note:
- Ouro TTS is great for prerecorded outbound messages
...
- Ouro STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Ouro memory.
...
- Do not store arbitrary third-party phone numbers in Ouro memory.
...
3. persist that number to `${OURO_HOME:-~/.ouro}/.env`
