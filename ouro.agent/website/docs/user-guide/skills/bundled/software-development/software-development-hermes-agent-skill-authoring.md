---
title: "Ouro Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Ouro Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Ouro Skill Authoring
...
| Tags | `skills`, `authoring`, `ouro-agent`, `conventions`, `skill-md` |
...
# Authoring Ouro-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.ouro/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the ouro-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with ouro-agent
...
- Don't use for: personal skills in `~/.ouro/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `ouro skills install official/<category>/<skill>`.
...
- Credit the **human first**, then "Ouro" as secondary collaborator: `Ben Barclay (benbarclay), Ouro`.
- Never `author: Ouro` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Ouro`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.ouro/skills/`.
...
### Reference Ouro tools, not raw shell
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.ouro/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Ouro` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Ouro tools
