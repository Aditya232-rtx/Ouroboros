...
- A long-running process (ouro gateway, tui_gateway) misbehaves and you can't restart it
...
The ouro test runner and pytest both support this:
...
For long-lived processes: Ouro gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Ouro you have two practical options:
...
## Debugging Ouro-specific Processes
...
### `tui_gateway` subprocess (spawned by `ouro --tui`)
...
debugpy.wait_for_client()
```
Start `ouro --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Ouro subagents, debug one process at a time.
