...
| List/filter by metadata (age, source, cost, tokens, workspace) | `ouro sessions list` / `stats` via terminal |
| Rename | `ouro sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `ouro sessions archive <filters>` |
| Delete (destructive) | `ouro sessions delete` / `ouro sessions prune <filters>` |
| Export before deleting anything valuable | `ouro sessions export --session-id <id> --format md` |
...
`ouro sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `ouro sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `ouro sessions list`
