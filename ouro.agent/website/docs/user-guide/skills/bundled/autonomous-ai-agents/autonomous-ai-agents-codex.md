...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`ouro-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ouro-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Ouro terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Ouro itself, `model.provider: openai-codex` uses Ouro-managed Codex
OAuth from `~/.ouro/auth.json` after `ouro auth add openai-codex`. For the
...
When invoking the Codex CLI from a Ouro gateway/service context (for example,
