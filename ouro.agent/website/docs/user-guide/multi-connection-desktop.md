...
# Connecting Desktop to Many Ouro Instances

Register every Ouro backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Ouro Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Ouro gateway…"**) deep-links straight to
...
device, remote gateways on your network, and Ouro Cloud instances. All of
them are stored here."* Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Ouro runtime managed by this app." | automatic |
| **Remote gateway** | "A Ouro gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Ouro install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Ouro Cloud** | "A hosted instance discovered through your Ouro Cloud account." | portal sign-in |
...
- **Test** probes the connection's own HTTP *and* WebSocket legs, so a pass
  ...
  host pinged.
- Cloud entries come from the Ouro Cloud sign-in/discovery flow
...
4. Fill the fields:
   ...
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `ouro serve` backend,
...
[Bot Mode](https://github.com/NousResearch/Ouro-Bot-Mode)) render:
...
connection is registered) dispatches `ouro update` to every eligible
...
- **Ouro Cloud** instances are skipped with a *"Managed by Ouro Cloud"*
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `ouro serve` is running on the remote host, the
...
- **"Update Ouro Desktop to chat with agents on other connections"** — the
