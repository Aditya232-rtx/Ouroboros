...
sidebar_position: 3
title: "Desktop App"
description: "The native Ouro desktop app — a polished experience for chatting with Ouro, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Ouro desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Ouro core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `hermes` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Ouro has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`hermes`) and **[TUI](./tui.md)** (`ouro --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`ouro dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Ouro Desktop](../getting-started/installation.md).

If you already have Ouro installed, simply run
...
- **The same conversation history** as every other Ouro surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Ouro instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Ouro Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend).

#### Repository discovery

Ouro Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Ouro sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `ouro desktop --cwd <path>` (or the `OURO_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Ouro to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `ouro model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Ouro management surface so you don't have to drop to a terminal:
...
- **Profiles** — switch between [Ouro profiles](./profiles.md) (isolated config/skills/sessions).
...
roster where every [Ouro profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Ouro cron). Create new agents from the roster —
...
each other's Bot Chats directly (`ouro -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Ouro, and more.
...
You can do the same from the terminal — `ouro uninstall --gui` for the GUI alone, or `ouro uninstall` / `ouro uninstall --full` for the agent too.

:::note
Running `ouro uninstall --gui` from a **source checkout** (a `ouro desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `ouro desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `ouro desktop`

To launch via the CLI, simply run `ouro desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `ouro update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `OURO_DESKTOP_CWD`)           |
| `--hermes-root PATH` | Override the Ouro source root the app uses (sets `OURO_DESKTOP_OURO_ROOT`)          |
...
The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Ouro runtime into `OURO_HOME` (`~/.ouro`, or `%LOCALAPPDATA%\hermes` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `OURO_DESKTOP_OURO_ROOT`, then a completed managed install, then a probed `hermes` on `PATH` (unless `--ignore-existing` / `OURO_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `OURO_DESKTOP_HERMES` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `ouro serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `ouro --tui`. The desktop app is **self-contained**: it runs its own `ouro serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Ouro backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

**Settings → Gateway → Connection mode** offers the alternatives to the local gateway:

- **Remote gateway** — enter the URL of a `ouro serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Ouro Cloud** — sign in once to Ouro Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Alongside the per-profile connection mode above, **Settings → Connections** manages a named registry of every agent source the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Ouro Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Ouro gateway…**) or via **⌘K → Connections**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Ouro Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the Ouro Cloud sign-in/discovery flow above, not from a hand-typed URL.
...
Side-by-side routing is live: each registered source dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** in the Connections panel dispatches `ouro update` to every eligible source at once — Ouro Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `ouro serve` process
"Remote backend" means a **`ouro serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `ouro serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.ouro/.env` (the secrets file, mode 0600):
...
ouro serve --host 0.0.0.0 --port 9119
```

Keep that `ouro serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `ouro serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `OURO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.ouro/.env` so the credentials are in the environment at boot.
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `OURO_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `OURO_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `OURO_DASHBOARD_BASIC_AUTH_USERNAME` / `OURO_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.ouro/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `OURO_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Ouro Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Ouro Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).
...
`$OURO_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Boot logs land in `OURO_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.ouro/hermes-agent/.ouro-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.ouro/hermes-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.ouro.agent
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.ouro/hermes-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
OURO_DESKTOP_OURO_ROOT=/path/to/clone npm run dev
OURO_HOME=/tmp/throwaway npm run dev
...
certificate once and tell Ouro to use it:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Ouro Local Signing`, Identity Type: *Self-Signed Root*,
   Certificate Type: **Code Signing**.
3. `ouro config set desktop.macos_signing_identity "Ouro Local Signing"`
...
`tccutil reset All com.ouro.agent` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `ouro --tui` and the dashboard chat tab
