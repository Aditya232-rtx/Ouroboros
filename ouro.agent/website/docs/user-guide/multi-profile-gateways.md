...
If you only run one Ouro agent, you don't need this page — see
...
[Connecting Desktop to Many Ouro Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Ouro agents that should all
...
(`ouro-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
ouro profile create coder
ouro profile create personal-bot
ouro profile create research
...
```bash
ouro config set gateway.multiplex_profiles true
ouro gateway restart
```

Equivalently, in the default profile's `~/.ouro/config.yaml`:
...
You do **not** run `ouro gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `ouro gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.ouro/profiles/coder/.env`; the default listener key is rejected.
...
home). `ouro status` reports the multiplexer and the profiles it serves;
`ouro status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/ouro-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: ouro-gateways {start|stop|restart|status|list}"
}
...
```bash
ouro-gateways start      # start every configured profile
ouro-gateways stop       # stop every configured profile
ouro-gateways restart    # restart all
ouro-gateways status     # status across all
ouro-gateways list       # delegates to `ouro gateway list`
```

:::tip
The `default` profile is targeted with `ouro gateway <action>` (no `-p`),
not `ouro -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `ouro -p coder gateway <action>` — useful if a
...
| Linux    | `~/.config/systemd/user/ouro-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.hermes.gateway.plist` /
`ouro-gateway.service`.
...
```bash
# Default profile
tail -f ~/.ouro/logs/gateway.log
tail -f ~/.ouro/logs/gateway.error.log

# Named profile
tail -f ~/.ouro/profiles/<name>/logs/gateway.log
tail -f ~/.ouro/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.ouro/logs/gateway.log ~/.ouro/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
ouro logs -f                  # follow default profile
ouro -p coder logs -f         # follow one profile
ouro logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
ouro profile list             # profiles + model + gateway state
ouro-gateways status          # full status across every profile
launchctl list | grep ouro    # macOS — PIDs and labels
systemctl --user list-units 'ouro-gateway-*'   # Linux — units
```
...
The default profile uses `~/.ouro/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
ouro config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.ouro/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=ouro --why="gateways running" \
  sleep infinity &
...
`ouro-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.ouro/.env ~/.ouro/profiles/*/.env
```

## Updating the code

`ouro update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `ouro gateway start` after a previous `ouro gateway stop`. The
...
cat ~/.ouro/profiles/<profile>/gateway.pid
...
# Linux
systemctl --user restart ouro-gateway-<profile>.service
```

### Health check

```bash
ouro doctor                  # default profile
ouro -p <profile> doctor     # one profile
```
