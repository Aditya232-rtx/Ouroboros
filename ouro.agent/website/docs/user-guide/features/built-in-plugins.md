...
description: "Plugins shipped with Ouro that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Ouro ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.ouro/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Ouro Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.ouro/plugins/<name>/`
3. **Project** — `./.ouro/plugins/<name>/` (requires `OURO_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `ouro memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `ouro plugins list` and the interactive `ouro plugins` UI), but none load until you explicitly enable them:

```bash
ouro plugins enable disk-cleanup
```

Or via `~/.ouro/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Ouro. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
ouro plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `ouro plugins enable <name>`.
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real Ouro session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `ouro memory` and `ouro plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OURO_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under OURO_HOME | always | Never |
...
**State** — everything lives at `$OURO_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `OURO_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `ouro plugins enable disk-cleanup` (or check the box in `ouro plugins`).

**Disabling again:** `ouro plugins disable disk-cleanup`.
...
**Enabling:** `ouro plugins enable security-guidance` (or check the box in `ouro plugins`).

**Disabling again:** `ouro plugins disable security-guidance`.
...
Traces Ouro turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Ouro' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `ouro logs`.
...
```bash
ouro tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Ouro and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
ouro plugins enable observability/langfuse
```

Then put the credentials in `~/.ouro/.env`:

```bash
OURO_LANGFUSE_PUBLIC_KEY=pk-lf-...
OURO_LANGFUSE_SECRET_KEY=sk-lf-...
OURO_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Ouro turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `OURO_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Ouro session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `ouro chat` session lives under one Langfuse session.

**Verify:**

```bash
ouro plugins list                 # observability/langfuse should show "enabled"
ouro chat -q "hello"              # check the Langfuse UI for a "Ouro turn" trace
```
...
| `OURO_LANGFUSE_RELEASE` | — | Release/version tag |
| `OURO_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `OURO_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `OURO_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Ouro-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Ouro-prefixed wins when both are set.
...
**Disabling:** `ouro plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### observability/nemo_relay

Relays Ouro execution boundaries — sessions, turns, LLM calls, and tool invocations — to an [NVIDIA NeMo Relay](https://docs.nvidia.com/nemo/relay/about-nemo-relay/overview) endpoint. Ouro core owns the Relay session/turn/LLM/tool scopes; the plugin configures exporters (ATOF JSONL, ATIF trajectories, OpenTelemetry) and adds observer marks for approvals and delegated subagents. Full exporter setup lives in the plugin's `README.md` under `plugins/observability/nemo_relay/`.

**Enabling:**

```bash
ouro plugins enable observability/nemo_relay
```
...
- Post-meeting artifacts (transcript, status) saved under `~/.ouro/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
ouro plugins enable google_meet
ouro meet setup   # preflight: playwright, chromium, auth file
ouro meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `ouro plugins disable google_meet`. Any saved transcripts stay in `~/.ouro/workspace/meetings/` until you remove them.

### hermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Ouro session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Ouro feature changes.

**How it works:**

- Scans your entire `~/.ouro/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$OURO_HOME/plugins/hermes-achievements/state.json`
...
| Secret | Hidden until Ouro detects the first related signal in your history |
...
**State files** — live under `$OURO_HOME/plugins/hermes-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Ouro updates. |
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `ouro dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.ouro/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$OURO_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Ouro plugin — see [Build a Ouro Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.ouro/plugins/<name>/`
- Manifest source is reported as `bundled` in `ouro plugins list`
