...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Ouro — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Ouro supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Ouro](../../guides/use-voice-mode-with-hermes.md).
...
1. **Ouro installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `ouro model` or set your preferred provider credentials in `~/.ouro/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.ouro/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.ouro/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `ouro setup --portal` wires both up at once.
:::
...
cd ~/.ouro/hermes-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.ouro/hermes-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.ouro/hermes-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.ouro/hermes-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`ouro chat`) and the **TUI** (`ouro --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.ouro/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
ouro                # Start the interactive CLI
```
...
:::tip
The record key is configurable via `voice.record_key` in `~/.ouro/config.yaml` (default: `ctrl+b`).
:::
...
```bash
ouro gateway        # Start the gateway (connects to configured platforms)
ouro gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.ouro/.env`:
```bash
...
```bash
ouro gateway        # Start with existing configuration
```
...
If you are running Ouro inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.ouro/.env`
...
- Review gateway logs: `tail -f ~/.ouro/logs/gateway.log`
