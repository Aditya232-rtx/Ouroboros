...
Type `@` followed by a reference to inject content directly into your message. Ouro expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Ouro env: `$OURO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$OURO_HOME/skills/.hub/`
