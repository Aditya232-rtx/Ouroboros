...
Ouro ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
ouro memory setup      # interactive picker + configuration
ouro memory status     # check what's active
ouro memory off        # disable external provider
```

You can also select the active memory provider via `ouro plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.ouro/config.yaml`:
...
When a memory provider is active, Ouro automatically:
...
**Setup Wizard:**
```bash
ouro memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `ouro honcho setup` command still works (it now redirects to `ouro memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$OURO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$OURO_HOME/honcho.json` > `~/.ouro/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Aditya232-rtx/ouro/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `ouro honcho`
If you previously used `ouro honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Ouro profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Ouro profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Ouro profile. Host key `hermes` → default; `hermes.<profile>` for others. |
...
```bash
ouro profile create coder --clone
```
...
Scans every Ouro profile, creates host blocks for any profile without one, inherits settings from the default `hermes` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `ouro memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/Aditya232-rtx/ouro/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
# Then configure Ouro
ouro memory setup    # select "openviking"
# Or manually:
ouro config set memory.provider openviking
```

`ouro memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.ouro/.env`, and for named profiles use
`~/.ouro/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is Ouro' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
ouro memory setup    # select "mem0" → "Platform"
# Or manually:
ouro config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.ouro/.env
```

**Setup (OSS):**
```bash
ouro memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
ouro memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
ouro memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
ouro memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
ouro memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.ouro/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.ouro/.env
```
...
**Config:** `$OURO_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.ouro/.env`.
...
**Switching modes:** Re-run `ouro memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
ouro memory setup    # select "hindsight"
# Or manually:
ouro config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.ouro/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p ouro ui start`

**Config:** `$OURO_HOME/hindsight/config.json`
...
| `retain_context` | `conversation between Ouro and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Aditya232-rtx/ouro/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
ouro memory setup    # select "holographic"
# Or manually:
ouro config set memory.provider holographic
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$OURO_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
ouro memory setup    # select "retaindb"
# Or manually:
ouro config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.ouro/.env
```
...
# Then configure Ouro
ouro memory setup    # select "byterover"
# Or manually:
ouro config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$OURO_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
ouro memory setup    # select "supermemory"
# Or manually:
ouro config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.ouro/.env
```
...
Before running `ouro memory setup`, set `base_url` in
`$OURO_HOME/supermemory.json`:
...
Then run `ouro memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$OURO_HOME/supermemory.json`
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Ouro profile
...
ouro config set memory.provider memori
ouro memory setup
```
...
- **Local storage providers** (Holographic, ByteRover) use `$OURO_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$OURO_HOME/` so each profile has its own credentials
