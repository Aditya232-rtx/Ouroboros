...
Top-level model calls run in the background automatically. Ouro returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
When a top-level agent provides a `tasks` array, Ouro returns one background handle, runs the subagents in parallel, and posts one consolidated result after every child finishes. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
...
When a background delegation finishes, Ouro stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Ouro restarts after completion but before delivery, the pending
...
Ouro cannot prove whether its external side effects happened. Pending and
...
```bash
tail -f ~/.ouro/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. Ouro returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchronous execution when they cannot deliver a detached result later.
...
- A Ouro process restart does **not** resume a running child. Its attempt becomes `unknown` because Ouro cannot prove which side effects happened.
