...
title: "Plugins"
description: "Extend Ouro with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Ouro has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Ouro
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Ouro Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.ouro/plugins/` with a `plugin.yaml` and Python code:

```
~/.ouro/plugins/my-plugin/
...
Start Ouro — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.ouro/plugins/hello-world/plugin.yaml`**
...
**`~/.ouro/plugins/hello-world/__init__.py`**

```python
"""Minimal Ouro plugin — registers a tool and a hook."""
...
Drop both files into `~/.ouro/plugins/hello-world/`, restart Ouro, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Ouro does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.ouro/plugins/` are disabled by default. Enable them only for trusted repositories by setting `OURO_ENABLE_PROJECT_PLUGINS=true` before starting Ouro.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `ouro <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `ouro plugins install` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Ouro — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.ouro/plugins/` | Personal plugins |
| Project | `.ouro/plugins/` | Project-specific plugins (requires `OURO_ENABLE_PROJECT_PLUGINS=true`) |
...
Within each source, Ouro also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.ouro/plugins/model-providers/<name>/` and `~/.ouro/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `ouro plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.ouro/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
ouro plugins                    # interactive toggle (space to check/uncheck)
ouro plugins enable <name>      # add to allow-list
ouro plugins disable <name>     # remove from allow-list + add to disabled
```

After `ouro plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
ouro plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Ouro checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `ouro plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`ouro plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Ouro' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.ouro/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.ouro/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Ouro tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Ouro runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Ouro never
materializes the prompt on another surface.

Ouro still owns hardline blocks, sudo-stdin protection, user deny rules,
...
Ouro has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.ouro/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Ouro also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Ouro Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Ouro Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Ouro Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `ouro <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `OURO_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Ouro auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `ouro skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.ouro/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `ouro plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
ouro plugins                               # unified interactive UI
ouro plugins list                          # table: enabled / disabled / not enabled
ouro plugins search <term>                 # search the community plugin index
ouro plugins install <name>                # install by index name (resolved to repo @ pinned ref)
ouro plugins install user/repo             # install from Git, then prompt Enable? [y/N]
ouro plugins install user/repo --enable    # install AND enable (no prompt)
ouro plugins install user/repo --no-enable # install but leave disabled (no prompt)
ouro plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
ouro plugins remove my-plugin              # uninstall
ouro plugins enable my-plugin              # add to allow-list
ouro plugins disable my-plugin             # remove from allow-list + add to disabled
ouro plugins capabilities [my-plugin]      # declared vs granted capabilities
```
...
When a plugin declares capabilities, `ouro plugins install` (and
`ouro plugins enable`) shows the list with one-line risk descriptions and
...
granted, `ouro plugins update` surfaces the additions and asks again. New
...
`ouro plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
ouro plugins capabilities             # all plugins with declared/granted capabilities
ouro plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Ouro has not reviewed the plugin's code. Only install
...
`ouro plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
ouro plugins search telegram               # fuzzy search
ouro plugins search                        # browse the whole index
ouro plugins search --capability platform  # filter by declared capability
ouro plugins search media --json           # machine-readable output
ouro plugins search --refresh              # bypass the 24h local cache
```
...
```bash
ouro plugins install hermes-media-studio
```
...
overridable via `ouro config set plugins.index_url <url>`). Fetches are
cached under `~/.ouro/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Ouro — so search works fully offline.
...
```bash
ouro plugins pack show ./hermes-pack.yaml     # dry-run review
ouro plugins pack install ./hermes-pack.yaml  # review → confirm → install
ouro plugins pack export > hermes-pack.yaml   # snapshot the current install
ouro plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `ouro plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `ouro plugins install`.
...
provenance (installed via `ouro plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`ouro skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `ouro plugins install` and `ouro plugins update` runs a static
...
On `ouro plugins update`, a dangerous verdict on the updated tree
...
Running `ouro plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `ouro plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Ouro reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Ouro rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Ouro can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Ouro drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Ouro checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Ouro' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
