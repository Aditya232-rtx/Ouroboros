...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `ouro tools`.
...
Ouro generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `ouro tools` and persists in `config.yaml`.
...
`max_parallel_requests` defaults to `4`. Ouro clamps it to at least one and
...
as a tool the chat model may call, and Ouro cannot force the call — the
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Ouro materializes them to the local cache so delivery still works after expiry
