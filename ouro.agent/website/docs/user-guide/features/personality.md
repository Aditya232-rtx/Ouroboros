...
description: "Customize Ouro's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Ouro's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `OURO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Ouro is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Ouro now seeds a default `SOUL.md` automatically in:

```text
~/.ouro/SOUL.md
```

More precisely, it uses the current instance's `OURO_HOME`, so if you run Ouro with a custom home directory, it will use:
...
- Ouro creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Ouro loads `SOUL.md` only from `OURO_HOME`
- Ouro does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Ouro falls back to a built-in default identity
...
If Ouro loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `OURO_HOME`, the personality belongs to the Ouro instance itself.

That also makes it easier to teach users:
- "Edit `~/.ouro/SOUL.md` to change Ouro' default personality."
...
- how Ouro should handle uncertainty, disagreement, or ambiguity
...
## What Ouro injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Ouro falls back to a built-in default identity ("You are Ouro, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Ouro ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Ouro, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Ouro its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.ouro/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.ouro/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Ouro speaks
- `display.skin` and `/skin` affect how Ouro looks in the terminal
