...
Ouro runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Ouro
...
1. Ouro captures a baseline of current diagnostics for the file.
...
…). Ouro auto-detects the binary on PATH or in
`<OURO_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Ouro at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<OURO_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`ouro lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Ouro installs both packages together when you
run `ouro lsp install typescript` or auto-install fires on first
use.

## CLI

```
ouro lsp status          # service state + per-server install status
ouro lsp list            # registry, optionally --installed-only
ouro lsp install <id>    # eagerly install one server
ouro lsp install-all     # try every server with a known recipe
ouro lsp restart         # tear down running clients
ouro lsp which <id>      # print resolved binary path
```

`ouro lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <OURO_HOME>/lsp/bin
...
When `install_strategy: auto`, Ouro installs binaries into
`<OURO_HOME>/lsp/bin/`. NPM packages land in
`<OURO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Ouro-owned and is
...
**`ouro lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<OURO_HOME>/lsp/bin/`. Run
`ouro lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `ouro lsp status`**
...
When `ouro lsp status` shows a `Backend warnings` section, install
...
Check `~/.ouro/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `ouro lsp restart` to clear the set;
