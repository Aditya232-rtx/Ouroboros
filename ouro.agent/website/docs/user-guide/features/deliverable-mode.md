...
When Ouro runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.ouro/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.ouro/config.yaml` (switchable per session
via `/personality`).
...
If you use Ouro' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.ouro/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Ouro's deliverable
