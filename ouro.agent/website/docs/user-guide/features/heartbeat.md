...
Inspired by Prime-Agent's `/heartbeat`. The Ouro adaptation keeps the strict message-flow invariants: the heartbeat is injected only between turns (never mid-run), as a plain user-role message.

## Heartbeat vs cron: which one do I want?

They look similar but serve different jobs:

| | `/heartbeat` | [`ouro cron`](./cron) |
|---|---|---|
...
Ouro: [Heartbeat — recurring instruction, fires every 15m]
