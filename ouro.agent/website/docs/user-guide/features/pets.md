...
Ouro can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<OURO_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Ouro renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `ouro pets list [query] [--limit N]` |
| List installed pets | `ouro pets list --installed` |
| Install a pet | `ouro pets install <slug> [--select] [--force]` |
| Set the active pet | `ouro pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `ouro pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `ouro pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `ouro pets off` |
| Remove an installed pet | `ouro pets remove <slug>` |
| Diagnose setup | `ouro pets doctor` |

`ouro pets show` flags:
...
Beyond installing pre-made pets from the gallery, Ouro can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `ouro pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `ouro tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `OURO_PET_IMAGE_PROVIDER` env var (e.g. `OURO_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Ouro is
...
Run `ouro pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Ouro uses its own
  profile-scoped `<OURO_HOME>/pets/` instead — install through `ouro pets`.

## See also

- The [`ouro-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-ouro-agent.md)
