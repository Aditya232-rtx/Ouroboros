...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Ouro' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Ouro")

The wake word turns Ouro into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Ouro listens in the background for a spoken
trigger phrase. Say it, and Ouro starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Ouro backend (for example a
...
Ouro supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with Ouro, so
...
```bash
cd ~/.ouro/hermes-agent && uv pip install -e ".[wake]"
```
...
desktop ear button — also writes `wake_word.enabled` to `~/.ouro/config.yaml`,
...
default) to let Ouro choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Ouro installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Ouro surfaces, and `surface` picks which one
...
including when Ouro surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Ouro does not silently fail over to another open surface.
...
"Hey Ouro" works out of the box — the bundled openWakeWord model
...
command (`ouro -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.ouro/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
Create a "Hey Ouro" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey hermes"
  porcupine:
    keyword: ~/.ouro/wakewords/hey_hermes.ppn
```

Set your access key in `~/.ouro/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `ouro tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `ouro-agent[wake]`).
...
but the phrase never fires. Ouro detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Ouro
backend (it may appear as your terminal, `python`, or Ouro), then toggle the
...
```bash
ouro config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
ouro config set wake_word.input_device null
```
