...
Ouro can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Ouro integration. It
...
**Fresh installs already have the driver.** The Ouro installer
...
- **`ouro tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
ouro computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `ouro computer-use
status` to verify the install.

Already have cua-driver? Ouro reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `ouro update`, and the first
`computer_use` call of a session, Ouro checks the local version and
...
selected with `OURO_CUA_DRIVER_CMD` stays
under your control, so Ouro reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Ouro autodetection is a
planned cua-driver follow-up, so currently point Ouro at that directory or
...
interface. The built-in toolset provides Ouro actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `ouro computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Ouro host identity. |
...
```
ouro -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.ouro/config.yaml`.

## Permission modes and logged-in browser profiles

Ouro maps its existing approval UX onto cua-driver's immutable runtime
...
| Ouro session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Ouro approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Ouro risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Ouro then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.ouro/cua-manifest.yaml
```
...
for the format). Ouro launches a private runtime with
...
resetting or closing the Ouro session, cancellation cleanup, or process exit
closes that transport session. Ouro also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Ouro
...
embedded service under the Ouro host identity.
...
## `ouro computer-use doctor` — your first triage stop

`ouro computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ ouro computer-use doctor
...
Ouro run declares a public cua-driver **session name** (something like
...
Ouro keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Ouro-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Ouro
autodetection is a planned cua-driver follow-up, so currently point Ouro at
...
These are **platform deep dives, not duplicates of the Ouro skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Ouro
...
Ouro applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.ouro/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Ouro applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Ouro agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Ouro agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Ouro warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Ouro from inside the RDP / console
...
```
OURO_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
OURO_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Ouro disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Ouro sets
...
When it's on, `ouro computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Ouro at a binary you built from source instead
of the published release. Ouro resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`OURO_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Ouro testing (see notes).
...
### Option B — point Ouro straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`OURO_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
OURO_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
OURO_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Ouro is using your build

- `ouro computer-use status` prints the resolved binary path and
  version.
- `ouro computer-use doctor` confirms the binary is reachable and
...
- **Ouro spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Ouro YOLO, Ouro instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Ouro
  change needed either way.
- **Local builds skip the version check.** Ouro warns when the
...
**First action when anything's off: run `ouro computer-use doctor`.**
...
Run `ouro computer-use install` to fetch the cua-driver binary, or
run `ouro tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `ouro computer-use doctor` will
...
- **Ouro-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Ouro `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Ouro autodetection is a
  planned follow-up; currently point Ouro at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Ouro among them)
