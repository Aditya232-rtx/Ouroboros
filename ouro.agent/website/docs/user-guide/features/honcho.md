...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Ouro's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Ouro instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
ouro memory setup    # select "honcho" from the provider list
```
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Ouro syncs them back at session init.
...
The `ouro honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `ouro memory setup honcho` (or run `ouro memory setup` and pick it from the list); the `ouro honcho` subcommand then appears on the next invocation.

```bash
ouro memory setup honcho    # Configure Honcho directly (works before activation)
ouro honcho status          # Connection status, config, and key settings
ouro honcho setup           # Redirects to `ouro memory setup` (post-activation alias)
ouro honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
ouro honcho peer            # Show or update peer names + dialectic reasoning level
ouro honcho mode            # Show or set recall mode (hybrid/context/tools)
ouro honcho tokens          # Show or set token budget for context and dialectic
ouro honcho identity        # Seed or show the AI peer's Honcho identity
ouro honcho sync            # Sync Honcho config to all existing profiles
ouro honcho peers           # Show peer identities across all profiles
ouro honcho sessions        # List known Honcho session mappings
ouro honcho map             # Map current directory to a Honcho session name
ouro honcho enable          # Enable Honcho for the active profile
ouro honcho disable         # Disable Honcho for the active profile
ouro honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `ouro honcho`

If you previously used the standalone `ouro honcho setup`:
...
No re-login or re-setup needed. Run `ouro memory setup` and select "honcho" — the wizard detects your existing config.
