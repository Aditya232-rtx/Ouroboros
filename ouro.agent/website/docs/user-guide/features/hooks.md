...
Ouro has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.ouro/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.ouro/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.ouro/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.ouro/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".ouro" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.ouro/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.ouro/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".ouro" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.ouro/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.ouro/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.ouro/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Ouro does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.ouro/BOOT.md` with natural-language startup instructions.
...
Create `~/.ouro/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `ouro cron list` and check if any scheduled jobs failed overnight.
...
**`~/.ouro/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.ouro/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.ouro/hooks/boot-md/handler.py`**

```python
"""Run ~/.ouro/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".ouro" / "BOOT.md"
...
```bash
ouro logs --follow --level INFO | grep boot-md
```
...
Delete `~/.ouro/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Ouro shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.ouro/hooks/`
...
- Runtime event-name validity comes from `hermes_cli.plugins.VALID_HOOKS`. `ouro hooks list` lists configured shell/outbound hooks, not every available event; `ouro hooks test <event>` reports the valid set only when an invalid event is supplied.
...
| `session_id` | `str` | Current Ouro session id |
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Ouro's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

The clean user-message `content` remains unchanged. For replay and prompt-cache stability, Ouro may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "Ouro, Teknium, Nous Research, kanban"
...
Declare shell-script hooks in your `~/.ouro/config.yaml` and Ouro will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.ouro/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.ouro/agent-hooks/` (by convention) | `~/.ouro/plugins/<name>/` | `~/.ouro/hooks/<name>/` |
...
Each time the event fires, Ouro spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Ouro-canonical

// Modify a pre_tool_call — rewrite tool args before dispatch:
{"action": "modify", "args": {"new_string": "fixed content"}}         // Ouro-canonical
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/.ouro/agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `ouro hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.ouro/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.ouro/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.ouro/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.ouro/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.ouro/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.ouro/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Ouro event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.ouro/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.ouro/agent-hooks/log-orchestration.sh
log=~/.ouro/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Ouro sees it, then persists the decision to `~/.ouro/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `ouro --accept-hooks chat`)
2. `OURO_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.ouro/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `ouro hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.ouro/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/hermes/.ouro/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `ouro hooks list`.

### The `ouro hooks` CLI

| Command | What it does |
|---------|--------------|
| `ouro hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `ouro hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `ouro hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `ouro hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.ouro/agent-hooks/` so the path is easy to audit.
- Re-run `ouro hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake Ouro when the world changes; outbound webhooks tell the world when Ouro does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and Ouro POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* Ouro instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.ouro/config.yaml`:
...
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      ...
      secret_env: OURO_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.ouro/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `ouro hooks list`).
...
| `X-Ouro-Event` | The hook event name |
| `X-Ouro-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-Ouro-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-Ouro-Delivery` header) — remember recently seen ids and skip duplicates. Ouro retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `OURO_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`ouro hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
