...
All skills live in **`~/.ouro/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Ouro at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `ouro update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.ouro` profile):

```bash
curl -fsSL https://ouro.dev/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
ouro profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
ouro skills opt-out            # stop future seeding — nothing on disk is touched
ouro skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
ouro skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `ouro update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `ouro skills opt-in`) to re-enable.

:::note Safe by default
`ouro skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Ouro installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Ouro to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.ouro/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
ouro chat --toolsets skills -q "What skills do you have?"
ouro chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Ouro-tool framing, no invented
commands).
...
/home/user/.ouro/cache/chart-q4-2025.png
...
When a missing value is encountered, Ouro asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `ouro setup` or `~/.ouro/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `ouro config migrate` prompts for unconfigured settings, and `ouro config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.ouro/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Ouro scans the
...
If you maintain skills outside of Ouro — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Ouro to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.ouro/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.ouro/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Ouro process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Ouro ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.ouro/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.ouro/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `ouro bundles list`.
...
ouro bundles create research

# Overwrite an existing bundle
ouro bundles create backend-dev --skill ... --force

# Delete a bundle
ouro bundles delete backend-dev

# Re-scan ~/.ouro/skill-bundles/ and report changes
ouro bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.ouro/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.ouro/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.ouro/pending/skills/` and are
...
```bash
ouro skills browse                              # Browse all hub skills (official first)
ouro skills browse --source official            # Browse only official optional skills
ouro skills search kubernetes                   # Search all sources
ouro skills search react --source skills-sh     # Search the skills.sh directory
ouro skills search https://mintlify.com/docs --source well-known
ouro skills inspect openai/skills/k8s           # Preview before installing
ouro skills install openai/skills/k8s           # Install with security scan
ouro skills install official/security/1password
ouro skills install skills-sh/vercel-labs/json-render/json-render-react --force
ouro skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
ouro skills list --source hub                   # List hub-installed skills
ouro skills check                               # Check installed hub skills for upstream updates
ouro skills update                              # Reinstall hub skills with upstream changes when needed
ouro skills audit                               # Re-scan all hub skills for security
ouro skills uninstall k8s                       # Remove a hub skill
ouro skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
ouro skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
ouro skills publish skills/my-skill --to github --repo owner/repo
ouro skills snapshot export setup.json          # Export skill config
ouro skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Ouro. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `ouro skills search <query> --source skills-sh`. Ouro resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Ouro currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Ouro repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Ouro can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
ouro skills search https://mintlify.com/docs --source well-known
ouro skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
ouro skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Ouro can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
ouro skills install openai/skills/k8s
ouro skills tap add myorg/skills-repo
```
...
[Skills Hub](https://ouro.dev/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Ouro-side changes required.
...
- Ouro source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Ouro can search and convert agent entries from LobeHub's public catalog into installable Ouro skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Ouro source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Ouro integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Ouro' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Ouro source id: `browse-sh`
- Trust level: `community`

```bash
ouro skills search airbnb --source browse-sh
ouro skills inspect browse-sh/airbnb.com/search-listings-ddgioa
ouro skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Ouro also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Ouro source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Ouro does not enumerate or copy unrelated files from the host.

```bash
ouro skills install https://sharethis.chat/SKILL.md
ouro skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
ouro skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `ouro skills update` re-fetches from the same URL automatically when you want to refresh.
...
`ouro skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Ouro | Always trusted |
...
```bash
ouro skills check          # Report which installed hub skills changed upstream
ouro skills update         # Reinstall only the skills with updates available
ouro skills update react   # Update one specific installed hub skill
ouro skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `ouro skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Ouro users add with `ouro skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Ouro discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Ouro user can subscribe and install:

```bash
ouro skills tap add my-org/hermes-skills
ouro skills search deploy
ouro skills install my-org/hermes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.ouro/skills/.hub/taps.json`:
...
The `ouro skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `ouro skills tap list` shows the effective path per tap.
...
```bash
ouro skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.ouro/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`ouro skills reset`)

Ouro ships with a set of bundled skills in `skills/` inside the repo. On install and on every `ouro update`, a sync pass copies those into `~/.ouro/skills/` and records a manifest at `~/.ouro/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Ouro recomputes the hash of your local copy and compares it to the origin hash:
...
ouro skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `OURO_HOME`, so `ouro -p coder skills reset <name>` only affects that profile.
:::
