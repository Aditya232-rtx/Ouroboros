...
Ouro is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Ouro checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.ouro/config.yaml`:
...
| `timeout` | `300` | Seconds Ouro waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `OURO_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `ouro --yolo` or `ouro chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `OURO_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `OURO_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Ouro shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Ouro refuses to run them **regardless** of:
...
Configure the timeout in `~/.ouro/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.ouro/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.ouro/.env` | Overwrite sensitive file via redirection |
...
The `OURO_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.ouro/config.yaml`:
...
:::tip
Use `ouro config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`ouro approvals suggest`)
...
```bash
ouro approvals suggest            # dry run — prints a numbered proposal
ouro approvals suggest --apply 1,3  # merge picks into command_allowlist
ouro approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.ouro/state.db`) for
...
Before `write_file` or `patch` touches disk, Ouro checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `OURO_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Ouro credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under OURO_HOME (active profile and global root) |
| Project secret files | `.env`, `.env.local`, `.env.production`, `.envrc` anywhere on disk |

Sensitive paths inside the safe root are still blocked — pointing `OURO_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside OURO_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### OURO_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/Aditya232-rtx/ouro) (`OURO_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.ouro/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.ouro/cron/jobs.json`, profile skills, or other Ouro state outside that prefix

To allow both a workspace and Ouro home:

```bash
export OURO_WRITE_SAFE_ROOT=/path/to/project:/home/you/.ouro
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [OURO_WRITE_SAFE_ROOT](../reference/environment-variables.md#hermes_write_safe_root).

### Cron and other Ouro state

Do not ask the agent to `patch` `~/.ouro/cron/jobs.json` directly. Use the `cronjob` tool, [`ouro cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Ouro control files when write safety blocks direct edits.
...
When running the messaging gateway, Ouro controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.ouro/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.ouro/.env to allow open access,
...
For more flexible authorization, Ouro includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `ouro pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.ouro/config.yaml`:
...
ouro pairing approve telegram ABC12DEF

# Revoke a user's access
ouro pairing revoke telegram 123456789

# Clear all pending codes
ouro pairing clear-pending
```
...
```bash
docker exec -u ouro ouro-agent ouro pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/Aditya232-rtx/ouro/issues/10270
:::

**Storage:** Pairing data is stored in `~/.ouro/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Ouro applies strict security hardening to every container.
...
Container resources are configurable in `~/.ouro/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.ouro/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `OURO_HOME`. Skills declare these in frontmatter:
...
When loaded, Ouro checks if these files exist in the active profile's `OURO_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Ouro infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Ouro infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Ouro integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Ouro under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.ouro/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.ouro/logs/` for unauthorized access attempts
10. **Keep updated** — run `ouro update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.ouro/.env`:
...
Ouro ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `ouro doctor` for the full remediation.
- **`ouro doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Ouro installs these **lazily** on first use rather than eagerly under `ouro-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `ouro tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `ouro tools`.
