...
Ouro automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.ouro/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Ouro stores session history so it can resume conversations, but it does not
...
Ouro explicitly injects for that turn.
...
For example, if a user sends an image and asks Ouro to make a meme from it,
Ouro may inspect that image once with vision and run an image-processing
...
`ouro sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `ouro sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`hermes` or `ouro chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.ouro/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `ouro -c` again in the *same* terminal, Ouro resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
ouro -c "my project"   # → resumes "my project #3"
```
...
ouro --resume 20250305_091523_a1b2c3d4
ouro -r 20250305_091523_a1b2c3d4

# Resume by title
ouro --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
ouro --resume latest

# Or with the chat subcommand
ouro chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `ouro sessions list`.
...
ouro --resume latest --in ./my-project

# Works with the TUI too
ouro --tui --resume latest --in ./my-project
```
...
```bash
ouro --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`ouro sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
ouro sessions list --workspace my-project
ouro sessions list --workspace ~/code/hermes-agent
```

### Conversation Recap on Resume

When you resume a session, Ouro displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Ouro session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.ouro/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `ouro -r "<title>"` from the shell) and pick up where the platform left off.
...
Ouro automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `ouro sessions list` or `ouro sessions browse`.
...
```bash
ouro sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Ouro creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`ouro -c "my project"`), it automatically picks the most recent session in the lineage.
...
Ouro provides a full set of session management commands via `ouro sessions`:
...
# Filter by platform
ouro sessions list --source telegram

# Show more sessions
ouro sessions list --limit 50
```
...
`ouro sessions export` is one surface for every export format, selected with `--format`:
...
ouro sessions export backup.jsonl

# Export sessions from a specific platform
ouro sessions export telegram-history.jsonl --source telegram

# Export a single session
ouro sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
ouro sessions export backup.jsonl --redact
```
...
ouro sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
ouro sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
ouro sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
ouro sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
ouro sessions export --format trace

# One session to a local trace file
ouro sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
ouro sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.ouro/session-exports`).

```bash
# Export one session to Markdown
ouro sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
ouro sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
ouro sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
ouro sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
ouro sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
ouro sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
ouro sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
ouro sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
ouro sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
ouro sessions pin 20250305_091523_a1b2c3d4
ouro sessions pin 20250305 20250306

# Remove the pin
ouro sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
ouro sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
ouro sessions pinned --json > pinned-sessions.json
```
...
ouro sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
ouro sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
ouro sessions prune --newer-than 5h

# Explicit window with absolute timestamps
ouro sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
ouro sessions prune --source telegram
ouro sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
ouro sessions prune --newer-than 5h --title "smoke test"   # title substring
ouro sessions prune --older-than 30 --max-messages 3        # tiny sessions
ouro sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
ouro sessions prune --model gpt-5 --older-than 1w           # by model (substring)
ouro sessions prune --provider openrouter --older-than 60   # by billing provider
ouro sessions prune --branch feature/old-experiment         # by git branch
ouro sessions prune --user 12345678 --chat-type group       # by messaging origin
ouro sessions prune --max-tokens 500 --older-than 7         # by token usage
ouro sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
ouro sessions prune --newer-than 5h --dry-run

# Skip confirmation
ouro sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `ouro sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `ouro sessions prune` keeps the 90-day cutoff. Every
...
`ouro sessions archive` takes the same filters as `prune` but soft-hides
...
ouro sessions archive --newer-than 5h

# Archive by title substring, preview first
ouro sessions archive --title "dry run" --dry-run
ouro sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `ouro sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`ouro sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`ouro insights`](/reference/cli-commands#hermes-insights).
...
`ouro sessions repair-routing` finds message-bearing session rows with no
...
ouro sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
ouro sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.ouro/state.db ~/.ouro/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Ouro and
continue it here. Ouro reads Claude Code's session logs
...
ouro sessions import

# Limit to one tool, or point at a specific file
ouro sessions import --from claude
ouro sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
`ouro sessions import` creates a new Ouro session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `ouro --resume <id>` command.
...
When Ouro cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Ouro uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Ouro in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.ouro/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.ouro/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.ouro/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.ouro/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.ouro/sessions/sessions.json` is a **legacy mirror** of it,
...
`ouro sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.ouro/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `ouro sessions list`, the cause is
`state.db` not receiving them — run `ouro sessions repair` and watch for a
...
`*.jsonl` files in `~/.ouro/sessions/`. They are no longer written or
read by Ouro. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Ouro process in the same `OURO_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.ouro/config.yaml`:
...
ouro sessions delete <session_id>
