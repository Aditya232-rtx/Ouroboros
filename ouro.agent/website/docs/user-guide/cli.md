...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Ouro terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Ouro's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `ouro setup --portal` — and you're ready to `ouro chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Ouro also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `ouro --tui` — see the [TUI](tui.md) guide.
:::
...
ouro chat --model "anthropic/claude-sonnet-4"

# With a specific provider
ouro chat --provider nous        # Use Nous Portal
ouro chat --provider openrouter  # Force OpenRouter

# With specific toolsets
ouro chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
ouro -s hermes-agent-dev,github-auth
ouro chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
ouro --continue             # Resume the most recent CLI session (-c)
ouro --resume <session_id>  # Resume a specific session by ID (-r)
ouro --resume latest        # Resume the most recent session (same as -c)
ouro --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
ouro chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
ouro -w                         # Interactive mode in worktree
ouro -w -z "Fix issue #123"     # Single query in worktree
```

### Plugin management

The `ouro plugins` commands manage native Ouro plugins and portable Agent
...
ouro plugins disable <plugin-name>
ouro plugins update <plugin-name>
ouro plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Ouro currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Ouro CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Ouro CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `OURO_YOLO_MODE` is on (either `ouro --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. Ouro refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`ouro -c` or `ouro --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Ouro to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.ouro/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Ouro supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Ouro replies |
...
```bash
ouro -s hermes-agent-dev,github-auth
ouro chat -s github-pr-workflow -s github-auth
```

Ouro loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.ouro/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.ouro/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Ouro..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Ouro also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Ouro recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Ouro — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Ouro is working, Ouro prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Ouro to the background — just like any terminal process. The shell prints a confirmation:

```
Ouro has been suspended. Run `fg` to bring Ouro back.
```
...
Resume this session with:
  ouro --resume 20260225_143052_a1b2c3
...
```bash
ouro --continue                          # Resume the most recent CLI session
ouro -c                                  # Short form
ouro -c "my project"                     # Resume a named session (latest in lineage)
ouro --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
ouro --resume "refactoring auth"         # Resume by title
ouro --resume latest                     # Resume the most recent session (same as -c)
ouro --resume latest --in ./my-project   # Latest session for ./my-project's workspace
ouro -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `ouro sessions rename <id> <title>` from the command line. Use `ouro sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Ouro's SQLite state database under `~/.ouro/state.db`. The database keeps:
...
Ouro immediately confirms the task and gives you back the prompt:
