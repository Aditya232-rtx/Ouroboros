...
Connect Ouro to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Ouro — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Ouro runs it in a
...
ouro photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Ouro` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.ouro/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.ouro/.env`
...
(device token, dashboard project id) lives in `~/.ouro/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Ouro
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Ouro replies with a pairing code. Approve it with:

```bash
ouro pairing approve photon <CODE>
```

Use `ouro pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.ouro/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.ouro/.env`):
...
By default Ouro responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Ouro` and
`@Ouro agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Ouro will reply.
...
assigned iMessage line Ouro uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run ouro photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `ouro photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `ouro photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `ouro photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Ouro sends images, voice
  ...
  media.
- **Native polls are supported.** Ouro sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Ouro sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `ouro send`, the dashboard) reuse the sidecar the
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Ouro is set
...
| `PHOTON_MENTION_PATTERNS` | Ouro wake words  | JSON list / comma / newline regex patterns for group mentions |
