...
description: Control your smart home with Ouro via Home Assistant integration.
...
Ouro integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Ouro"
5. Copy the token
...
Ouro registers four tools for smart home control:
...
Configure which events the agent sees in `~/.ouro/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Ouro".
...
The adapter reads credentials from `~/.ouro/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Ouro profile
...
running Ouro — `curl -H "Authorization: Bearer <token>" <url>/api/` should
