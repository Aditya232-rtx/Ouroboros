...
Connect Ouro to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Ouro supports two WeCom integration modes:
...
> Run `ouro gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Ouro decrypts the message, queues it for the agent
...
(Use `ouro gateway start` only after `ouro gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Ouro and the token the
...
in `~/.ouro/.env` values around `=` will also break signature checks. After
fixing, restart `ouro gateway run`.
...
Check `ouro gateway run` logs for the bound host/port. If the adapter bound to
