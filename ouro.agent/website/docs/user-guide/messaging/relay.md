...
sidebar_position: 30
title: "Ouro Relay"
description: "Connect Ouro to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Ouro Relay (Connector)
...
Ouro Relay is not a chat platform itself — it is a **connector system** that
...
secret. `ouro gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.ouro/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.ouro/.env`:
...
`ouro gateway enroll` refuses to run in managed/hosted installs — there the
...
token. Re-login with `ouro auth add nous` (or `ouro setup`) and retry.
...
startup. Restart it (`ouro gateway restart`).
