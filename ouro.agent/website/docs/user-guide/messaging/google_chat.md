...
sidebar_position: 12
title: "Google Chat"
description: "Set up Ouro as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Ouro to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Ouro
...
> Run `ouro gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Ouro can read (e.g.,
`~/.ouro/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a ouro restart)
...
Also grant `Pub/Sub Viewer` on the same subscription — Ouro calls
...
- **App name**: whatever you want users to see ("Ouro" is reasonable).
...
`ADDED_TO_SPACE` event that Ouro uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Ouro

Add the Google Chat section to `~/.ouro/.env`:
...
Send "hola" in the test DM. The bot posts a "Ouro is thinking…" marker, then
...
`~/.ouro/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Ouro is thinking…").
...
Thread support: when a user replies inside a thread, Ouro detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Ouro session.
...
3. Download the JSON. Move it onto the host that runs Ouro.
4. Register the client with Ouro (run under the profile you want it scoped to):
...
ouro -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Ouro home (e.g.
`~/.ouro/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.ouro/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.ouro/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Ouro isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `ouro gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Ouro will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.ouro/google_chat_user_tokens/<sanitized_email>.json` (filesystem
