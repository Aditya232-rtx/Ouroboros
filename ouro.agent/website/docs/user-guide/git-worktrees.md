...
title: "Git Worktrees"
description: "Run multiple Ouro agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Ouro is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Ouro so each session has a clean, isolated working directory.

## Why Use Worktrees with Ouro?

Ouro treats the **current working directory** as the project root:

- CLI: the directory where you run `hermes` or `ouro chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.ouro/config.yaml`
...
Ouro creates `.worktrees/my-experiment/` inside the repo (branch
...
Now you can `cd` into the new worktree and run Ouro there:

```bash
cd ../repo-feature

# Start Ouro in the worktree
hermes
```
...
- Ouro checkpoint data under `~/.ouro/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Ouro experiment**
...
- **Avoid running Ouro from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `ouro -w` (Automatic Worktree Mode)

Ouro has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
```bash
ouro -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `ouro -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Ouro session its own clean checkout.
