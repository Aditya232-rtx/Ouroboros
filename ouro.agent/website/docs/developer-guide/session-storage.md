...
Ouro uses a SQLite database (`~/.ouro/state.db`) to persist session
...
Multiple ouro processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.ouro/state.db
...
Default path: `~/.ouro/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.ouro/` by default, or the value of `OURO_HOME` environment variable.
