...
description: "Extend the native Ouro Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Ouro Desktop](/user-guide/desktop) app is contribution-driven: every
...
`$OURO_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Ouro. This page is the **native
desktop app** (`ouro desktop`) SDK — the `@hermes/plugin-sdk` module and
`$OURO_HOME/desktop-plugins/`. The **web dashboard** (`ouro dashboard`) has
its own, unrelated plugin system on `window.__OURO_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Ouro Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$OURO_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$OURO_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
...
Create `$OURO_HOME/desktop-plugins/hello/plugin.js` (that's `~/.ouro/...`
by default, or `~/.ouro/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.ouro/desktop-plugins/hello/plugin.js
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Ouro' }),
      jsx('div', {
...
for requests; `targetProfile` is the backend Ouro profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Ouro
...
a package into `~/.ouro/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.ouro/plugins` is not reachable as a filesystem —
...
`dashboard/` subfolder of a regular Ouro plugin and declare it in a
`manifest.json`:
...
ouro-agent codebase directly (`hermes_state`, `hermes_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.ouro/`)
...
`$OURO_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `ouro logs gui -f`.
...
`~/.ouro/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.ouro/logs/errors.log` for
