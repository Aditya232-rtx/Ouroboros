...
sidebar_position: 1
title: "Architecture"
description: "Ouro internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Ouro internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── hermes_constants.py       # OURO_HOME, profile-aware paths
...
│   ├── skills_config.py      # ouro skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # ouro tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # ouro gateway start/stop
│
...
Three discovery sources: `~/.ouro/plugins/` (user), `.ouro/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `ouro plugins` or `config.yaml`.
...
Exposes Ouro as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`ouro -p <name>`) gets its own OURO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
