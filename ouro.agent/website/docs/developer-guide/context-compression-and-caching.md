...
Ouro uses a dual compression system and Anthropic prompt caching to
...
Configure via `ouro plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Ouro has two separate compression layers that operate independently:
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Ouro raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$OURO_HOME`
...
```bash
ouro config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
ouro config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Ouro' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Ouro
  records the resulting compaction events (compression counters, session
  events). Set `hermes` to let Ouro' compression threshold initiate
  app-server compaction, or `off` to disable Ouro-initiated automatic
  compaction entirely (codex may still compact natively).

Ouro' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Ouro' summary compressor.
...
encrypted `compaction` output item. Ouro captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Ouro
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Ouro can avoid; user-facing docs for `/model`, fallback providers, and
