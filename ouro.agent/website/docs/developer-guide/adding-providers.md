...
description: "How to add a new inference provider to Ouro — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Ouro can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `ouro model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `ouro setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$OURO_HOME/plugins/model-providers/` both get picked up.
...
7. `ouro model` menu includes the provider
8. `ouro setup` wizard delegates to `main.py` automatically
...
User plugins at `$OURO_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `ouro model` menu entries with bespoke auth flows
...
- What env vars should Ouro check, and in what priority order?
...
Be careful with API-key precedence. Ouro already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `ouro model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `ouro model` and `ouro setup` automatically.
:::
...
When you add a native provider, double-check that Ouro is only sending fields that provider actually understands.
...
### 7. Updating `ouro model` but not `ouro setup`
