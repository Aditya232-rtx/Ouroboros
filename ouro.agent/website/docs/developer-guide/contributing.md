...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Ouro — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Ouro! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Ouro core? Start with [Build a Ouro Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Ouro itself? Start with [Adding Tools](./adding-tools.md)
...
The installer creates the Ouro venv, wires the `hermes` command, stamps the
install method for `ouro update`, and clones the full git project into
`$OURO_HOME/hermes-agent` (usually `~/.ouro/hermes-agent`). That keeps your
...
You can also run a fully isolated Ouro instance (throwaway OURO_HOME, separate Electron
...
uv venv ~/.ouro/venvs/hermes-dev --python 3.11
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.ouro/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.ouro` — use `get_hermes_home()` from `hermes_constants` for code paths and `display_hermes_home()` for user-facing messages. See [AGENTS.md](https://github.com/Aditya232-rtx/ouro/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Ouro can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Ouro to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/Aditya232-rtx/ouro/blob/main/LICENSE).
