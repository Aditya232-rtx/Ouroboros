...
| Variable | Meaning |
|----------|---------|
| `OURO_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `OURO_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `OURO_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Ouro itself — they're private to these helpers. The variables Ouro *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `ouro --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_hermes_root)" || { echo "htui: not in a Ouro checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$OURO_MAIN_CHECKOUT/.venv/bin/python" -m hermes_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `OURO_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_hermes_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `OURO_TUI_DIR` are mutually exclusive
`OURO_TUI_DIR` points Ouro at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `ouro --tui --dev` exits with an error. Run `unset OURO_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_hermes_root)" || { echo "hgui: not in a Ouro checkout" >&2; return 1; }
  deps="${OURO_GUI_DEPS_CHECKOUT:-$OURO_MAIN_CHECKOUT}"
  ...
  ( cd "$desktop"
    ...
    OURO_DESKTOP_OURO_ROOT="$root" \
    OURO_DESKTOP_PYTHON="$OURO_MAIN_CHECKOUT/.venv/bin/python" \
    OURO_DESKTOP_IGNORE_EXISTING=1 \
    OURO_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `OURO_DESKTOP_OURO_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `hermes`. |
| `OURO_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `OURO_DESKTOP_IGNORE_EXISTING` | Ignores any `hermes` on `PATH` so it can't shadow the worktree. |
| `OURO_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Ouro checkout.
_hermes_root() {
  local root
...
- [TUI](../user-guide/tui.md) — `ouro --tui --dev` and the `OURO_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/Aditya232-rtx/ouro/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `OURO_*` variable Ouro reads
