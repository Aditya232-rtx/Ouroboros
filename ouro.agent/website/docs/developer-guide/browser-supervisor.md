...
description: "How Ouro detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Ouro' browser tooling:
...
One `asyncio.Task` running in a background daemon thread per Ouro `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `ouro-acp`, `hermes-api-server`, and core toolsets (gated on CDP reachability)
