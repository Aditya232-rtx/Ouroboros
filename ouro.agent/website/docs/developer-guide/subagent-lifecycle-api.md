...
Plugins can launch and supervise fresh Ouro child sessions without importing
...
active agent turn fails closed with `No active Ouro parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Ouro can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Ouro's existing unsafe-tool block remains enforced.
