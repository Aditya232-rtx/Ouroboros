...
description: "How Ouro stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `ouro cron` subcommands |
...
Jobs are stored in `~/.ouro/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `ouro cron` commands are run or during active CLI sessions.
...
```python
# ~/.ouro/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.ouro/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `ouro cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `ouro cron` CLI provides direct job management:

```bash
ouro cron list                    # Show all jobs
ouro cron create                  # Interactive job creation (alias: add)
ouro cron edit <job_id>           # Edit job configuration
ouro cron pause <job_id>          # Pause a running job
ouro cron resume <job_id>         # Resume a paused job
ouro cron run <job_id>            # Trigger immediate execution
ouro cron remove <job_id>         # Delete a job
```
