...
description: "How the iron-proxy egress firewall integrates with Ouro — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`ouro egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
hermes_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `ouro egress {install,setup,start,stop,
...
hermes_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       ...
                                       `ouro proxy` subparser, which uses
                                       dest='proxy_command').
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _OURO_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on OURO_RUN_E2E=1).
...
ouro egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.ouro/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.ouro/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

ouro egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.ouro/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `ouro egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `ouro egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `ouro egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, OURO_EGRESS_PROXY) and the per-mapping `OURO_PROXY_TOKEN_<NAME>` vars.
...
OURO_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `ouro egress`
OURO_HOME=/tmp/hermes-egress-test python3 -m hermes_cli.main egress --help
OURO_HOME=/tmp/hermes-egress-test python3 -m hermes_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://ouro.dev/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://ouro.dev/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`ouro secrets bitwarden`](https://ouro.dev/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`ouro egress`](https://ouro.dev/docs/reference/cli-commands#hermes-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://ouro.dev/docs/reference/environment-variables#egress-proxy-sandbox-injected)
