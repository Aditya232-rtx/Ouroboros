...
description: "How to build a memory provider plugin for Ouro"
---

# Building a Memory Provider Plugin

Memory provider plugins give Ouro persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `ouro plugins`.
:::

## Installation Layouts

Ouro discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Ouro. Closed to new providers — see [CONTRIBUTING](https://github.com/Aditya232-rtx/ouro/blob/main/CONTRIBUTING.md). |
| User | `$OURO_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.ouro/plugins/<name>/` | Opt-in via `OURO_ENABLE_PROJECT_PLUGINS=1`. |
...
Ouro, in `$OURO_HOME/plugins/<name>/` when installed by a user, or in
`./.ouro/plugins/<name>/` for a project-local one:
...
package layout. No copy under `$OURO_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Ouro reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `ouro <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active OURO_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `ouro memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `ouro memory setup`:
...
Every field in `get_config_schema()` is prompted during `ouro memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$OURO_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
the `messages` parameter; Ouro will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.ouro`:
...
data_dir = Path("~/.ouro/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `ouro my-provider status`, `ouro my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `ouro <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `ouro --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: ouro my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the ouro my-provider argparse tree.
