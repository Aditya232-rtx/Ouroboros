...
This guide covers adding a new messaging platform to the Ouro gateway. A platform adapter connects Ouro to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.ouro/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Ouro code. Your plugin is a directory with two files:

```
~/.ouro/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `ouro config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
With `provides_tools` declared, Ouro imports only `tools.py` during plugin
...
`ouro tools enable my_platform --platform cli`, or by listing the toolset
...
| `ouro config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `ouro status` | Shows plugin platforms with `(plugin)` tag |
| `ouro gateway setup` | Plugin platforms appear in setup menu |
| `ouro tools` / `ouro skills` | Plugin platforms in per-platform config |
...
direct `ouro send --to ...` and cron `deliver=platform:...` by declaring send
...
def register(ctx):
    ctx.register_platform(
        ...
        # May be a regular function or async def. Ouro awaits any awaitable
...
Most users set up a platform by dropping env vars into `~/.ouro/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `ouro gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `ouro cron run` separate from `ouro gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `ouro config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Ouro.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Ouro core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`toolsets.py`** — Add `"hermes-newplat"` toolset definition with `_OURO_CORE_TOOLS`
2. **`toolsets.py`** — Add `"hermes-newplat"` to the `"ouro-gateway"` includes list
