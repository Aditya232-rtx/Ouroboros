...
The ACP adapter wraps Ouro' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
ouro acp / ouro-acp / python -m acp_adapter
...
- `allow_once` -> Ouro `once`
- `allow_always` -> Ouro `always`
- reject options -> Ouro `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Ouro tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["ouro-acp"])
...
Instead it reuses Ouro' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Ouro provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run ACP clients can open Ouro' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.ouro/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `ouro-acp` toolset definition
- `hermes_cli/main.py` — `ouro acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `ouro-acp` script
