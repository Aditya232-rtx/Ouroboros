"""Centralized logging setup for Ouro.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.ouro/logs/`` (profile-aware via ``get_hermes_home()``).
...
# essentially always in Ouro (TUI, gateway, ``hy_memory`` server, MCP
...
# Used by _ComponentFilter and exposed for ``ouro logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Ouro logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Ouro home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the ouro group. However, both ``_open()`` (initial creation)
...
# the module header): when several Ouro processes log to the same file, an
