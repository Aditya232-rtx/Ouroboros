...
## 5. Ouro Configuration

### Config location

`$OURO_HOME/config.yaml` (defaults to `~/.ouro/config.yaml` when `OURO_HOME` is unset)
...
Restart the Ouro session for changes to take effect. The MCP connection is
