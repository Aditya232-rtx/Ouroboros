...
OURO_HOME_DIR="${OURO_HOME:-$HOME/.ouro}"
OURO_CFG="${OURO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Ouro config has twozero_td MCP entry ──
if [[ ! -f "$OURO_CFG" ]]; then
    echo -e " ${FAIL} Ouro config not found at ${OURO_CFG}"
    manual_steps+=("Create ${OURO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$OURO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Ouro config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Ouro config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${OURO_CFG} manually"); }
    manual_steps+=("Restart Ouro session to pick up config change")
fi
