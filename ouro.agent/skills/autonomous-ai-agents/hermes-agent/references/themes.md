# Themes / Skins — Author a Ouro Color Theme

Author a Ouro **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.ouro/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Ouro itself.
...
- Write access to the Ouro home dir — `~/.ouro` by default, or `$OURO_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `ouro config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   ouro config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `ouro config set display.skin default` (or they can `/skin default`).
...
```
ouro skin set <key> <hex>      # e.g. ouro skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `ouro skin set background "#08201f"` changes
only the background; `ouro skin use <name>` / `ouro skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.ouro`** when a profile is active — resolve the real home
  from `$OURO_HOME` first, falling back to `~/.ouro`.
...
- **Never hand-edit `config.yaml` to activate.** Use `ouro config set
...
- **You apply it, not the user.** `ouro config set display.skin <name>` is
...
- Run `ouro config get display.skin` and confirm it reports `<name>`.
