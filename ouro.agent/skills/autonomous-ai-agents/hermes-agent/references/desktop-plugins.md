...
Write plugins for the Ouro desktop app: statusbar items, layout panes,
...
system (`~/.ouro/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$OURO_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$OURO_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Ouro desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$OURO_HOME/desktop-plugins/` (usually
  `~/.ouro/desktop-plugins/`).

## How to Run

1. Create `$OURO_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.ouro/...` by default, or `~/.ouro/profiles/<profile>/...` under a
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Ouro (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.ouro/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
