...
Edit with `ouro config edit` or `ouro config set section.key value`.
Full reference: https://ouro.dev/docs/user-guide/configuration
...
`ouro config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `ouro tools` (interactive) or `ouro tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_OURO_CORE_TOOLS` is the default bundle most platforms inherit).
