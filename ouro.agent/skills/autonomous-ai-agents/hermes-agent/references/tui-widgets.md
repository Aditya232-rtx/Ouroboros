...
Author widget apps for the Ouro TUI (`ouro --tui`): glanceable ambient
...
- The TUI must be in use (`ouro --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.ouro/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.ouro/logs/tui_gateway_crash.log` if a widget never appears.
