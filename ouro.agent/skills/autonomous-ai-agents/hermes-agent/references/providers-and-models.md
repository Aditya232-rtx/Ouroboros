...
Set via `ouro model` (picker) or `ouro setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://ouro.dev/docs/integrations/providers
...
| nous | OAuth device code | `ouro auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `ouro auth add openai-codex` |
| qwen-oauth | OAuth | `ouro auth add qwen-oauth` |
| minimax-oauth | OAuth | `ouro auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`ouro auth`).
Fallback chain when the primary fails: `ouro fallback add|remove|list`.
...
#   ouro config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
