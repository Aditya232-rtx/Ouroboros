...
For occasional contributors and PR authors. Full developer docs: https://ouro.dev/docs/developer-guide/
...
Config: `~/.ouro/config.yaml` (settings), `~/.ouro/.env` (API keys) — both under `$OURO_HOME` when it is set.
...
`_OURO_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_hermes_home()` for paths,
never hardcode `~/.ouro`. For custom/local-only tools, write a plugin in
`~/.ouro/plugins/` instead of editing core — see the developer docs.
...
- Tests auto-redirect `OURO_HOME` to temp dirs — never touch real `~/.ouro/`.
