...
## Layer 1 — Is this thing a Ouro plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Ouro process**. Its LLM calls go through Ouro's already-configured provider. | Already uses Portal if user's Ouro is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Ouro. That LLM call is already authenticated
via Ouro's provider config. The plugin's own server auth (e.g.
...
What Ouro has that *feels* like OAuth — `ouro login --provider nous`
opening a browser, user signs in, token lands in `~/.ouro/auth.json` — is a
**Ouro-specific browser flow**. Under the hood it produces a credential
Ouro uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Ouro's existing Portal credential out of `~/.ouro/auth.json`.
...
rides on the credential Ouro already holds.

Where this could live in Ouro:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Ouro itself.
...
1. First decide: Ouro plugin (runs inside Ouro) or separate app? If plugin,
   it already uses Portal via Ouro's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Ouro agent itself") is fully supported. Ouro treats agent
...
pitch in the Tool Gateway announcement is specifically about Ouro's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Ouro is configured to use Portal as its provider. It is
