#!/usr/bin/env bash
# GitHub environment detection helper for Ouro skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _hermes_env="${OURO_HOME:-$HOME/.ouro}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env" 2>/dev/null; then
