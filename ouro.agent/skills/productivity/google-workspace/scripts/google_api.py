#!/usr/bin/env python3
"""Google Workspace API CLI for Ouro.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Ouro-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("OURO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Ouro")
