#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Ouro.
...
TOKEN_PATH = OURO_HOME / "google_token.json"
CLIENT_SECRET_PATH = OURO_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = OURO_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Ouro profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Ouro Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Ouro Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Ouro home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Ouro")
