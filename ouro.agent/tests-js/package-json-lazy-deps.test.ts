 ...
 *
 * The root ``package.json`` is installed by ``ouro update`` on every user,
 ...
 *   workspace graph entirely. ``ouro update`` and ``ouro doctor --fix``
 ...
 *   ``ouro tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``ouro update`` for
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      '`ouro tools` → Browser Automation → Camofox.'
  )
})
