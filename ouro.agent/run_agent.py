...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``ouro -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = os.environ.get("OURO_SESSION_SOURCE", "")
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"Ouro/{_OURO_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Ouro-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.ouro/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[ouro-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `ouro --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``ouro chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``OURO_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``OURO_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("OURO_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``OURO_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("OURO_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        ouro-agent #21444 for the symptom history.  The upstream backend
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Ouro against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Ouro can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("OURO_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _redact_message_content(content):
        ...
        Respects ``OURO_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.ouro/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Ouro-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects OURO_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Ouro' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Ouro model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Ouro-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``OURO_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("OURO_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.ouro/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``OURO_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("OURO_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Ouro process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Ouro process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `ouro doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.ouro/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.ouro/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`ouro doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (OURO_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (OURO_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset OURO_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("OURO_DEV_CREDITS"))
        ...
        if _dev:
            # OURO_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `ouro logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Ouro YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``OURO_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``ouro auth add xai-oauth``) whose tokens belong to
        ...
        *,
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("OURO_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.ouro/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``ouro setup``) updates
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # OURO_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        try:
            # Serialize the full load -> run -> flush region across Ouro
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Ouro process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Ouro process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Ouro process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Ouro process kept this session busy too "
