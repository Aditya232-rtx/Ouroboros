...
``{title, url, description, position}`` rows every other Ouro web
provider produces.
...
prefers Ouro-managed xAI Grok OAuth (via ``ouro auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.ouro/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``ouro tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `ouro auth` to sign in with "
