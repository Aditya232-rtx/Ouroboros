...
This plugin ships bundled with Ouro but is **opt-in** — it only loads when
...
ouro tools  # → Langfuse Observability
...
Set these in `~/.ouro/.env` (or via `ouro tools`):
...
ouro chat -q "hello"              # then check Langfuse for a "Ouro turn" trace
```

Generation observations include the Ouro system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `OURO_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
OURO_LANGFUSE_ENV=production       # environment tag
OURO_LANGFUSE_RELEASE=v1.0.0       # release tag
OURO_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
OURO_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
OURO_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
OURO_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`OURO_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
ouro plugins disable observability/langfuse
```
