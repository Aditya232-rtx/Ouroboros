"""nemo_relay — optional Ouro plugin for NeMo Relay observability."""
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Ouro profile; keeping the existing process-global configuration "
...
class _Runtime:
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Ouro core Relay session is unavailable")
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Ouro installs the managed execution
    ...
    with _LOCK:
        ...
        try:
            ...
            if resolved_host is None:
                raise RuntimeError("Ouro core Relay runtime is unavailable")
            runtime = _Runtime(
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("OURO_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("OURO_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("OURO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("OURO_NEMO_RELAY_ATOF_FILENAME") or "hermes-atof.jsonl",
        atof_mode=_env("OURO_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("OURO_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("OURO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("OURO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "hermes-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("OURO_NEMO_RELAY_ATIF_AGENT_NAME") or "Ouro",
        atif_agent_version=_env("OURO_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("OURO_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
        atif_export_timeout_s=_env_float(
            "OURO_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", 30.0
        ),
    )
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Ouro cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Ouro-owned "
...
def _atif_subagent_export_mode() -> str:
    mode = _env("OURO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
