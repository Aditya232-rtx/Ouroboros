...
Optional Ouro observability plugin that configures exporters and maps
Ouro-specific observer hooks to NeMo Relay marks and ATIF state. Ouro core
...
not replace Ouro's planner, tools, memory, model provider routing, or
CLI UX. Ouro core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Ouro can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Ouro core.
- Add Ouro session, turn, approval, and subagent mark events.
...
The `OURO_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `OURO_HOME` that the
agent run will use:

```bash
env OURO_HOME=/tmp/hermes-nemo-relay-test \
  ouro plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`OURO_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`OURO_HOME` is the Ouro profile/config home used by both
`ouro plugins enable ...` and the later `ouro chat ...` run. If unset,
Ouro uses the user's default home, usually `~/.ouro`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export OURO_HOME=/tmp/hermes-nemo-relay-test
ouro plugins enable observability/nemo_relay
ouro chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv sync
uv run ouro plugins enable observability/nemo_relay
uv run ouro chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Ouro installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control Ouro shared client metrics.
...
The plugin can configure exporters directly from `OURO_NEMO_RELAY_*`
...
```bash
export OURO_NEMO_RELAY_ATOF_ENABLED=1
export OURO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export OURO_NEMO_RELAY_ATIF_ENABLED=1
export OURO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `OURO_NEMO_RELAY_ATOF_FILENAME`
- `OURO_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `OURO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `OURO_NEMO_RELAY_ATIF_AGENT_NAME`
- `OURO_NEMO_RELAY_ATIF_AGENT_VERSION`
- `OURO_NEMO_RELAY_ATIF_MODEL_NAME`
- `OURO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Ouro at it:

```bash
export OURO_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `OURO_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`OURO_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`OURO_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Ouro does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Ouro keeps the direct env-var fallbacks active for that run.

Ouro core routes provider and tool execution through NeMo Relay managed APIs
...
Ouro uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Ouro-owned
...
with this Ouro-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Ouro rejects
...
Ouro activates these plugins before registering its managed LLM and tool
...
For the full generic Ouro middleware contract, see
...
with Ouro and a local Ollama model served through the OpenAI-compatible API.

```bash
export OURO_HOME=/tmp/hermes-nemo-relay-docs/hermes-home
mkdir -p "$OURO_HOME"

cat > "$OURO_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Ouro session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export OURO_NEMO_RELAY_ATOF_ENABLED=1
export OURO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/subagent/atof
export OURO_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export OURO_NEMO_RELAY_ATOF_MODE=overwrite
export OURO_NEMO_RELAY_ATIF_ENABLED=1
export OURO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/subagent/atif
export OURO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export OURO_NEMO_RELAY_ATIF_AGENT_NAME='Ouro E2E'
export OURO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export OURO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Ouro E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Ouro dispatches the read-only tools as one batch, and NeMo Relay
...
export OURO_NEMO_RELAY_ATOF_ENABLED=1
export OURO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/parallel/atof
export OURO_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export OURO_NEMO_RELAY_ATOF_MODE=overwrite
export OURO_NEMO_RELAY_ATIF_ENABLED=1
export OURO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/parallel/atif
export OURO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export OURO_NEMO_RELAY_ATIF_AGENT_NAME='Ouro E2E'
export OURO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Ouro sessions map to `agent` scopes.
- Ouro core managed provider calls map to `llm` scope start/end events.
- Ouro core managed tool calls map to `tool` scope start/end events.
...
Ouro core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Ouro-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Ouro calls the
...
```bash
export OURO_NEMO_RELAY_PLUGINS_TOML=/tmp/hermes-middleware-test/plugins.toml
```
...
Ouro provider call
  -> nemo_relay.llm.execute(...)
    -> Ouro provider adapter callback(...)

Ouro tool call
  -> nemo_relay.tools.execute(...)
    -> Ouro authorization and dispatch callback(...)
```
...
middleware for a local Ouro run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export OURO_HOME=/tmp/hermes-middleware-test/hermes-home
mkdir -p "$OURO_HOME" /tmp/hermes-middleware-test/nemo-relay
...
agent_name = "Ouro Middleware E2E"
agent_version = "local"
...
export OURO_NEMO_RELAY_PLUGINS_TOML=/tmp/hermes-middleware-test/nemo-relay/plugins.toml
