...
own pages inject it via ``window.__OURO_SESSION_TOKEN__`` so logged-in
...
This means ``ouro dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def get_board(
    ...
    through to the active board (``OURO_KANBAN_BOARD`` env → on-disk
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                ...
                # backend can run under a different OURO_HOME than the
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``ouro kanban
...
def list_diagnostics(
    ...
    AND for ``ouro kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``ouro kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``ouro kanban specify <task_id>``.
    ...
    # OURO_KANBAN_BOARD env var — this endpoint runs in FastAPI's
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``ouro kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.ouro/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Ouro profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.ouro/profiles/*`` on disk and every
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of Ouro
...
def list_kanban_projects():
    """List first-class Ouro projects for board scoping.
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``ouro profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``ouro kanban decompose <task_id>``.
    ...
    # OURO_KANBAN_BOARD env var would let concurrent requests for
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.ouro/config.yaml.
