...
Or via $OURO_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Ouro can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``ouro memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'ouro memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $OURO_HOME/hindsight/config.json  (profile-scoped)
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Ouro config."""
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Ouro .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Ouro profile name (from agent_identity)
      {workspace} — Ouro workspace name (from agent_workspace)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Ouro and the User"
    ...
    def save_config(self, values, hermes_home):
        """Write config to $OURO_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Ouro starter template (best-effort)."""
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Ouro and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Ouro and the User")
        ...
        # Ouro emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Ouro as a non-root user, or switch "
                    "to cloud / local_external mode via 'ouro memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Ouro get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See Aditya232-rtx/ouro#5820.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Ouro created/used that client on the shared async loop,
