...
Config in $OURO_HOME/config.yaml (profile-scoped):
  plugins:
    hermes-memory-store:
      db_path: $OURO_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Expand $OURO_HOME in user-supplied paths so config values like
        # "$OURO_HOME/memory_store.db" or "~/.ouro/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$OURO_HOME", _hermes_home)
            db_path = db_path.replace("${OURO_HOME}", _hermes_home)
