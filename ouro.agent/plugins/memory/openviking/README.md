...
- OpenViking server running and reachable from Ouro

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Ouro can identify older servers that expose the legacy status-only health
...
connection values into Ouro, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.ouro/.env`; for a named profile use
...
OpenViking's server config is separate from Ouro:
...
Ouro-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `hermes` | Ouro peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Ouro lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Ouro sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Ouro built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Ouro action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Ouro
