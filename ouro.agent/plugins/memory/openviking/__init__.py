...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Ouro peer ID in OpenViking (default: hermes)
...
_AGENT_PROMPT_LABEL = "Ouro peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "OURO_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Ouro slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Ouro to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Ouro refused the connection."
        ) from exc
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("OURO_HOME", "")).expanduser() if os.environ.get("OURO_HOME") else Path.home() / ".ouro"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Ouro did not start "
    ...
    try:
        ...
        # The Ouro Desktop backend can include the Ouro venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Ouro venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Ouro venv,
        # aborting `ouro update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Ouro will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Ouro session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Ouro only", "write values only to Ouro .env"),
    ...
    _print_openviking_ready("Connection saved to Ouro .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Ouro peer ID in OpenViking, sent as the actor peer and "
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if not hermes_home:
            try:
                ...
                hermes_home = str(Path.home() / ".ouro")
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Ouro will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.ouro/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts ouro (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Ouro will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Ouro' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Ouro canonical messages into OpenViking batch payloads."""
