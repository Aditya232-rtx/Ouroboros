...
Configuration
-------------
Secret (lives in $OURO_HOME/.env or the environment):
...
Behavioral settings (live in $OURO_HOME/mem0.json, set via `ouro memory
setup`):
...
def _load_config() -> dict:
    """Load config from env vars, with $OURO_HOME/mem0.json overrides.
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $OURO_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $OURO_HOME/mem0.json) —
