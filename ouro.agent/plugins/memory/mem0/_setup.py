...
def _setup_platform(hermes_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.ouro/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.ouro/.env (or unset it) or Ouro will keep "
...
def post_setup(hermes_home: str, config: dict) -> None:
    """Entry point called by ouro memory setup framework.
