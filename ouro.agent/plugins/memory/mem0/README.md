...
Behavioral settings live in `$OURO_HOME/mem0.json` (set them via `ouro memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.ouro/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Ouro session and call `mem0_search` — it connects to your server.
...
```bash
ouro memory setup mem0 --mode oss \
...
```bash
ouro memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$OURO_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.ouro/mem0_qdrant"}}
  }
}
```
...
```bash
ouro memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$OURO_HOME/mem0.json`).
