...
Handles: ouro honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all Ouro profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'ouro honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'ouro honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `ouro update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Ouro profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $OURO_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Ouro persistent cross-session memory.")
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'ouro honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'ouro honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'ouro honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'ouro honcho setup' again once you have a key.\n")
                return
    ...
    # These keys only affect the Ouro GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Ouro gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: ouro config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    ouro honcho status     -- show full config")
    print("    ouro honcho mode       -- change recall/observation mode")
    print("    ouro honcho tokens     -- tune context and dialectic budgets")
    print("    ouro honcho peer       -- update peer names")
    print("    ouro honcho map <name> -- map this directory to a session name\n")
...
def _active_profile_name() -> str:
    """Return the active Ouro profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: ouro honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'ouro honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'ouro honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: ouro honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Ouro' identity in Honcho. Seed with 'ouro honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: ouro honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: ouro honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Ouro asks Honcho's AI peer a question")
        ...
        print("\n  Set with: ouro honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'ouro honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'ouro honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    ouro honcho identity --show        — show both peer representations")
        print("    ouro honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Ouro + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Ouro\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Ouro persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  ouro honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'ouro honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'ouro honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  ouro honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print("  (Ouro calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    ouro honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'ouro honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Ouro, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'ouro honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    ouro honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  ouro honcho identity <path/to/SOUL.md>")
    ...
    print("    Ouro:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Ouro:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Ouro:   Honcho observes every message and updates representations")
    ...
    print("    Ouro:   per-session by default — each run gets its own session")
    print("              Map a custom name:  ouro honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. ouro honcho setup              — configure API key (required)")
        print("  2. ouro honcho migrate            — re-run this walkthrough")
    else:
        print("  1. ouro honcho status             — verify Honcho connection")
        print("  2. ouro                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. ouro honcho identity --show    — verify AI peer representation")
        print("  4. ouro honcho tokens             — tune context and dialectic budgets")
        print("  5. ouro honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'ouro memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``ouro honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``ouro honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to ouro memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Ouro Honcho",
    )
