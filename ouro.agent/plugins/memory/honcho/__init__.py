...
Config: Uses the existing Honcho config chain:
  1. $OURO_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $OURO_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Ouro CLI/gateway startup responsive when Honcho is down,
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every Ouro run creates a fresh
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return (
            ...
            "'ouro honcho setup' to re-authenticate will restore it."
        )
