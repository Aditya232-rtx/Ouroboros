...
```bash
ouro memory setup honcho   # configure Honcho directly (works on a fresh install)
ouro memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
ouro config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.ouro/.env
```

> `ouro honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `ouro memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `ouro model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$OURO_HOME/honcho.json` | Profile-local (isolated Ouro instances) |
| 2 | `~/.ouro/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Ouro profile: `hermes` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `ouro honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Ouro session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`ouro-agent`) |
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-ouro-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening ouro in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Ouro session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Ouro profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Ouro profile: `hermes` (default) or `hermes_<profile>` (e.g. `ouro -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `OURO_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `ouro-agent`) |
...
|---------|-------------|
| `ouro memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `ouro honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `ouro memory setup`) |
| `ouro honcho status` | Show resolved config for active profile |
| `ouro honcho enable` / `disable` | Toggle Honcho for active profile |
| `ouro honcho mode <mode>` | Change recall or observation mode |
| `ouro honcho peer --user <name>` | Update user peer name |
| `ouro honcho peer --ai <name>` | Update AI peer name |
| `ouro honcho tokens --context <N>` | Set context token budget |
| `ouro honcho tokens --dialectic <N>` | Set dialectic max chars |
| `ouro honcho map <name>` | Map current directory to a session name |
| `ouro honcho sync` | Create host blocks for all Ouro profiles |
