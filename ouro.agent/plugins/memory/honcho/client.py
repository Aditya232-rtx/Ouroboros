...
Resolution order for config file:
  1. $OURO_HOME/honcho.json  (instance-local, enables isolated Ouro instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Ouro profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Ouro profile.

    Resolution order:
      1. OURO_HONCHO_HOST env var (explicit override)
    ...
    """
    explicit = os.environ.get("OURO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named Ouro
    # profile share that same host.  Keep named profiles isolated; only the
    # default Ouro profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $OURO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.ouro/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $OURO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Ouro profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # baseUrl / base_url keys below are the Ouro-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Ouro session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Ouro session title (from /title command; non-per-session)
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'ouro honcho setup' account switches produce a NEW cache identity
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'ouro honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `ouro honcho setup` to configure)."
            )
