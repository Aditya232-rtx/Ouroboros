...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with ouro-agent)
2. User-installed providers: ``$OURO_HOME/plugins/<name>/``
3. Project-local providers: ``./.ouro/plugins/<name>/``, opt-in via
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$OURO_HOME/plugins/`` or None if unavailable."""
    try:
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.ouro/plugins/`` or None if unavailable or not opted in.

    Gated on ``OURO_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    try:
        ...
        if not _env_enabled("OURO_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".ouro" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($OURO_HOME/plugins/<name>/)
    # 3. Project-local providers (./.ouro/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``ouro <provider>`` subcommands — working, but a second-class citizen next
...
def load_memory_provider(
    ...
    *,
    ...
    (``$OURO_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``ouro memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every ouro startup.
        """
