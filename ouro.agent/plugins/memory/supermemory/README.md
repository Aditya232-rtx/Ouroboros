...
Before running `ouro memory setup`, add the local endpoint to
...
Then run `ouro memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$OURO_HOME/supermemory.json`
...
`https://api.supermemory.ai`. Ouro resolves it once and uses the same endpoint
...
it groups Ouro-written memories into a dedicated "Ouro" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Ouro profile:
