...
three virtual model IDs so the ``ouro tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `ouro tools` → Image "
                    "Generation → OpenAI to configure, or `ouro setup` "
                    "to add the key."
                ),
