...
# image_gen.model from ``ouro tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``ouro tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `ouro tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/Aditya232-rtx/ouro",
            "X-Title": "Ouro",
        }
