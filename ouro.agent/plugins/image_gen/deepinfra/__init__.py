...
models show up in ``ouro tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from ouro the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `ouro tools` → Image "
                    "Generation → DeepInfra to configure, or `ouro setup` "
                    "to add the key."
                ),
