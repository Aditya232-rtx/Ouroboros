"""
Microsoft Teams platform adapter for Ouro.
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Ouro owns dotenv loading; third-party import side effects must not.
    """
    try:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Ouro"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; Ouro/1.0)"},
            )
            response.raise_for_status()
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Ouro\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.ouro/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Ouro
