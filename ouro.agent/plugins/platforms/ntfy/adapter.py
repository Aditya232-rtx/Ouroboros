"""ntfy platform adapter (Ouro plugin).
...
``plugins/platforms/ntfy/``. The Ouro plugin loader scans the
...
_ECHO_TAG = "ouro-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``ouro cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Ouro dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `ouro gateway status` without
