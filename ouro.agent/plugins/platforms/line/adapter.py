"""
LINE Messaging API platform adapter for Ouro.
...
to Ouro. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``OURO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hermes_home = Path.home().joinpath(".ouro").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``ouro status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``ouro status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ouro setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.ouro/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.ouro/.env")
        return
