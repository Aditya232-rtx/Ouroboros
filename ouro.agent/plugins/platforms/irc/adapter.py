"""
IRC Platform Adapter for Ouro.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Ouro agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Ouro")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Ouro shutting down")
...
def check_requirements() -> bool:
    ...
    # ouro setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `ouro gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Ouro to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #ouro — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.ouro/.env")
    print_info("Restart the gateway for changes to take effect: ouro gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Ouro (cron)")
