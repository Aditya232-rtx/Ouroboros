...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Ouro command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Ouro accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Ouro ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Ouro
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("OURO_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("OURO_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Ouro" become "Agent".
        """
        if not body:
            return ""
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Ouro delivers cron job results and notifications.")
