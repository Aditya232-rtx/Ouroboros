...
# ``model_tools`` import time, so ``ouro status``, ``ouro chat``, etc.
...
#     fresh "Ouro is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a ouro MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${OURO_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Ouro is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("OURO_HOME", str(_Path.home() / ".ouro"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${OURO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a ouro MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Ouro is thinking…' marker message.
        ...
        "Ouro is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Ouro is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Ouro is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Ouro is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``ouro setup``.
    ...
    prompt for env vars, persist them to ``~/.ouro/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.ouro/.env")
    print_info("Restart the gateway: ouro gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``ouro cron`` running as a
    separate process from ``ouro gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Ouro plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ouro setup` to install Google Chat support.",
        ...
        # the "Ouro is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Ouro is thinking…' marker message "
