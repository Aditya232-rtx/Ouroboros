...
- Per-user tokens (keyed by sender email):
    ``${OURO_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${OURO_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${OURO_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${OURO_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${OURO_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's OURO_HOME helper so the token follows the user's
# profile (e.g. tests can override via OURO_HOME=/tmp/...).
try:
...
def _hermes_home() -> Path:
    """Resolve OURO_HOME at call time (NOT module import).

    Tests and ``OURO_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.ouro/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `ouro setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `ouro setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into OURO_HOME."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Ouro (native attachment delivery)"
    )
