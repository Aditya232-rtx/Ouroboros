# A2A — Agent-to-Agent protocol for Ouro
...
peer (another Ouro, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
ouro gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Ouro serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.ouro/a2a_audit.jsonl`.
- Conversations persist to `~/.ouro/a2a_conversations/` — they survive context
