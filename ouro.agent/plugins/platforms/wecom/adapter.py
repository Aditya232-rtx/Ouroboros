...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("OURO_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("OURO_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
...
def qr_scan_for_bot_info(
    *,
    ...
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "Ouro/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "Ouro/1.0"})
...
def interactive_setup() -> None:
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("Approve with: ouro pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("Direct messages disabled.")
        else:
            print_info("Skipped — configure later with 'ouro gateway setup'")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ouro setup` to install WeCom support.",
