...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Ouro media cache directory.
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Ouro-managed Node/npm so Windows installs are not broken by a
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `ouro gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `ouro whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `ouro whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `ouro update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".ouro-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Ouro-managed portable Node before falling back to PATH.
                ...
                try:
                    ...
                    if install_result.returncode != 0:
                        ...
                        self._set_fatal_error(
                            ...
                            f"WhatsApp bridge npm install failed. Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `ouro gateway`.",
                            retryable=False,
                        )
                        return False
                    ...
                    self._set_fatal_error(
                        "whatsapp_npm_install_failed",
                        f"WhatsApp bridge npm install failed ({e}). Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `ouro gateway`.",
                        retryable=False,
                    )
                    return False
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `ouro update`, so without this check it
            ...
            # hardcodes ~/.ouro/{image,audio,document}_cache, which diverges
            # under OURO_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["OURO_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["OURO_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["OURO_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: ouro whatsapp")
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``ouro setup`` even
