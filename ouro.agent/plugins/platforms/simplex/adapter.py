"""SimpleX Chat platform adapter (Ouro plugin).
...
This adapter ships as a Ouro platform plugin under
``plugins/platforms/simplex/``. The Ouro plugin loader scans the
...
discoverable and ``ouro setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Ouro — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("OURO_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``ouro setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.ouro/.env`` via
    ``hermes_cli.config``.
    """
    print()
