"""
``ouro photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Ouro
...
# Writable sidecar runtime dir (mirrors to OURO_HOME on immutable
...
# wiring imports this module for `ouro --help`).
...
# when called later (gateway start, ouro status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `ouro photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Ouro')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``ouro photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Ouro" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.ouro/.env,
    ...
    print("  Start the gateway:  ouro gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `ouro photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`ouro photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.ouro/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `ouro photon telemetry on` / `ouro photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.ouro/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  ouro gateway restart")
    return 0
...
#
# `ouro gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``ouro photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `ouro gateway setup` wizard."""
