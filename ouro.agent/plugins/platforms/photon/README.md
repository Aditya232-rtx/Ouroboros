...
This plugin connects Ouro to iMessage (and other Spectrum
...
TypeScript-only, Ouro runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Ouro channel,
...
Run `ouro photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.ouro/.env` (the same place every other
...
Management metadata lives in `~/.ouro/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `ouro photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Ouro
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Ouro coalesces
...
- **Native polls are supported.** Ouro posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `ouro send`) cannot spawn the sidecar;
...
6. Verify end-to-end: `ouro photon status`, a DM and a group roundtrip,
