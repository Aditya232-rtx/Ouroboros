...
Credential storage mirrors every other Ouro channel:

    * runtime SDK creds  -> ``~/.ouro/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.ouro/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a ouro dependency
...
# registers Ouro as its own client_id.
...
# Default name of the project Ouro provisions for the operator.
DEFAULT_PROJECT_NAME = "Ouro"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of ouro-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.ouro/auth.json`` honouring the active Ouro profile."""
    try:
        from hermes_constants import get_hermes_home
        return Path(get_hermes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.ouro")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.ouro/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.ouro/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.ouro/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `ouro photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `ouro photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `ouro photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `ouro photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `ouro photon setup`)"
