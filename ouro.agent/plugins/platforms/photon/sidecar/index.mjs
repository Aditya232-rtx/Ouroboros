...
//
// Inbound  (gRPC -> Ouro): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Ouro -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Ouro-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `ouro photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Ouro can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Ouro'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Ouro allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Ouro, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Ouro supplied them so a known-good
