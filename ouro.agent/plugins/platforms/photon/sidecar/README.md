...
Small Node helper that bridges Ouro to Photon's Spectrum SDK
(`spectrum-ts`).  Ouro is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Ouro-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Ouro can receive inbound messages
...
The Ouro plugin's `ouro photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Ouro is Python, so replies go through this sidecar
