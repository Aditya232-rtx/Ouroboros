...
under an immutable ``/opt/hermes`` (read-only for the ouro user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$OURO_HOME/photon/sidecar`` (the durable data volume,
