...
# identify Ouro traffic — matching other Ouro outbound surfaces
# that already set ``Ouro/<version>`` for platform-partner attribution.
try:
    from hermes_cli import __version__ as _OURO_VERSION
except Exception:
    _OURO_VERSION = "unknown"
_OURO_SLACK_USER_AGENT_PREFIX = f"Ouro/{_OURO_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
    ...
    Features:
      ...
      - Slash commands (/ouro, legacy /hermes)
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `ouro slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `ouro gateway setup` "
                "or in the active profile's ~/.ouro/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `ouro gateway setup` "
                "or add it to your active profile's ~/.ouro/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `ouro gateway setup` "
                "or in the active profile's ~/.ouro/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `ouro gateway setup` "
                "or add it to your active profile's ~/.ouro/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_OURO_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Ouro has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Ouro can "
            ...
            # /model work directly without /ouro prefix). A single regex
            ...
            # manifest (see `ouro slack manifest`). In Socket Mode, Slack
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/(?:ouro|hermes)$")
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'ouro slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Ouro handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Ouro session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Ouro owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer Ouro agents in Socket Mode on some
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The catch-all ``/ouro <subcommand> [args]`` form (and the legacy
        ``/hermes`` equivalent for older workspace manifests) is preserved
        for backward compatibility and for users who want a single entry
        point for free-form questions (``/ouro what's the weather`` —
        non-slash text is treated as a regular message).
        """
        ...
        if slash_name in {"ouro", "hermes", ""}:
            # Legacy /ouro (and /hermes) <subcommand> [args] routing +
            # free-form questions. Empty slash_name falls into this branch
            # for backward compat with any caller that didn't populate
            # command["command"].
        ...
        # questions via "/ouro <question>" must produce public replies so
...
def interactive_setup() -> None:
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under OURO_HOME, and print
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Ouro",
                bot_description="Your Ouro agent on Slack",
            )
            ...
            print_info(
                "   Re-run `ouro slack manifest --write` anytime to refresh after "
                "Ouro adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `ouro update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://ouro.dev/docs/user-guide/messaging/slack/")
    print()
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ouro setup` to install Slack support.",
