...
In single-bot mode (what Ouro currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("OURO_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "OURO_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "OURO_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("OURO_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("OURO_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "OURO_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Ouro gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Ouro does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Ouro webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Ouro' SessionSource fields.
    ...
    async def _connect_websocket(self) -> None:
        ...
        self._ws_client = FeishuWSClient(
            ...
            # See https://github.com/Aditya232-rtx/ouro/issues/50656
...
# Called by `ouro gateway setup` via _setup_feishu() in hermes_cli/gateway.py.
# =============================================================================
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `ouro setup` to install Feishu support."}
...
def interactive_setup() -> None:
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `ouro pairing approve`.")
    elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ouro setup` to install Feishu support.",
