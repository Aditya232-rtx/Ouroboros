...
Auto-tracks and cleans up ephemeral files created during Ouro
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$OURO_HOME` and `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `OURO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `OURO_HOME` or `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$OURO_HOME/disk-cleanup/` is itself excluded
- `$OURO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
