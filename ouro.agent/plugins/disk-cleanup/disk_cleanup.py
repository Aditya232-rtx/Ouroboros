"""disk_cleanup — ephemeral file cleanup for Ouro.
...
Rules:
  ...
  - empty dirs    → always delete (under OURO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly OURO_HOME and /tmp/hermes-*
Never touches: ~/.ouro/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("OURO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".ouro").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$OURO_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$OURO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under OURO_HOME or ``/tmp/hermes-*``.
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "ouro-agent", "backups", "profiles", ".worktrees",
...
# Paths under $OURO_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so OURO_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside OURO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under OURO_HOME, but never recurse into known
    # durable state trees.  Some installs place the Ouro checkout, venv,
    # and desktop build under OURO_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under OURO_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
