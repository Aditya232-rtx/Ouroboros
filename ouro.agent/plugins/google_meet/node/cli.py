"""`ouro meet node ...` subcommand tree.

Wired into the existing ``ouro meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``ouro meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             ouro meet node approve <name> ws://<host>:{args.port} {token}")
        try:
