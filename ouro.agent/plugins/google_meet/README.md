...
Let the ouro agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where ouro runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `ouro meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `ouro meet` CLI |
...
| `cli.py` | `ouro meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$OURO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `ouro meet node {run,list,approve,remove,status,ping}` |
...
ouro meet install                                      # pip + Chromium
ouro meet setup                                        # preflight
ouro meet auth                                         # optional
ouro meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
ouro meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.ouro/.env
ouro meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
ouro meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
ouro meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.ouro/.env
ouro meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, ouro will **not** switch your system audio input automatically — the
...
ouro plugins enable google_meet
ouro meet node run --display-name my-mac --host 0.0.0.0 --port 18789
