...
ouro meet install                 # pip deps + Chromium (transcribe only)
ouro meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
ouro meet auth                    # optional; skips guest-lobby wait
ouro meet setup                   # preflight checks
```

`ouro meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or OURO_MEET_REALTIME_KEY in ~/.ouro/.env
```
...
pip install playwright websockets && python -m playwright install chromium
ouro plugins enable google_meet
ouro meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
ouro meet node approve my-mac ws://<mac-ip>:18789 <token>
ouro meet node ping my-mac                   # confirm reachable
```

Run `ouro meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Ouro agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `ouro meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `OURO_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
Local:
```
$OURO_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$OURO_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `ouro meet node approve`.
