...
A standards-compliant OpenID Connect Relying Party for the ``ouro dashboard``
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.ouro/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    OURO_DASHBOARD_OIDC_ISSUER
    OURO_DASHBOARD_OIDC_CLIENT_ID
    OURO_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    OURO_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Ouro does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``OURO_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "OURO_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "OURO_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("OURO_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.ouro/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "OURO_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(OURO_DASHBOARD_OIDC_ISSUER + OURO_DASHBOARD_OIDC_CLIENT_ID) "
