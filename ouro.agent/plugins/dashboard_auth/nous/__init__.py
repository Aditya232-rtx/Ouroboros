...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      OURO_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      OURO_DASHBOARD_PORTAL_URL       — defaults to
...
Skip reasons:
  ...
  ("Set OURO_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via OURO_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between OURO_DASHBOARD_PORTAL_URL / OURO_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``OURO_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("OURO_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``OURO_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("OURO_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``OURO_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set OURO_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"OURO_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
