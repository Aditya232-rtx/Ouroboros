...
gateway (which Ouro uses to bill Browser Use sessions to a Nous
...
Auth env vars (one of)::
    ...
    # OR a managed Nous gateway entry (configured via 'ouro setup')
"""
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Ouro only needs a task-
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the ouro tools picker: the "Browser Use" row now
