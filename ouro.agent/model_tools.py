...
def _is_dispatcher_owned_worker() -> bool:
    """False when OURO_KANBAN_* is present but this execution does not own it
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("OURO_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by OURO_KANBAN_TASK and
        ...
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("hermes-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (hermes-*) include _OURO_CORE_TOOLS, and
    ...
    # window). Core Ouro tools (toolsets._OURO_CORE_TOOLS) are NEVER
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to ouro-agent's coercion layer.
