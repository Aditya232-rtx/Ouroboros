# nix/hermes-agent.nix — Overridable Ouro package
#
...
let
  ...
  # lib.nix) — the wrapper exposes them via OURO_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from OURO_BUNDLED_PLUGINS.
  ...
  # at by OURO_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  # exposed via OURO_OPTIONAL_MCPS.
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in ouro sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set OURO_BUNDLED_SKILLS $out/share/hermes-agent/skills \
          --set OURO_OPTIONAL_SKILLS $out/share/hermes-agent/optional-skills \
          --set OURO_BUNDLED_PLUGINS $out/share/hermes-agent/plugins \
          --set OURO_BUNDLED_LOCALES $out/share/hermes-agent/locales \
          --set OURO_OPTIONAL_MCPS $out/share/hermes-agent/optional-mcps \
          --set OURO_WEB_DIST $out/share/hermes-agent/web_dist \
          --set OURO_TUI_DIR $out/ui-tui \
          --set-default OURO_BIN $out/bin/hermes \
          --set OURO_PYTHON ${hermesVenv}/bin/python3 \
          --set OURO_NODE ${lib.getExe hermesNpmLib.nodejs}${
  ...
  passthru =
    ...
    in
    {
      ...
      # ("existing ouro on PATH") then picks up the fully wrapped
      ...
      devShellHook = ''
        export OURO_PYTHON=${devPython}/bin/python3
      '';
