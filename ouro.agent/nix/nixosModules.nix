# nix/nixosModules.nix — NixOS module for ouro-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: ouro runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $OURO_HOME/.env
# and read by ouro at startup — no container recreation needed for env changes.
#
# Tool resolution: the ouro wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    ouro-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-ouro-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "hermes-config-attrs";
      description = "Ouro YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    # OURO_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "hermes-documents" { } (
      ...
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'OURO_DOC_EOF'\n${value}\nOURO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "ouro-agent";
    ...
    # ouro user + sudo on first boot (writable layer persists), then
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      OURO_UID="''${OURO_UID:?OURO_UID must be set}"
      OURO_GID="''${OURO_GID:?OURO_GID must be set}"

      # ── Group: ensure a group with GID=$OURO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$OURO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="hermes"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$OURO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$OURO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$OURO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$OURO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$OURO_UID" -g "$OURO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$OURO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$OURO_UID:$OURO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure OURO_HOME is owned by the target user.
      ...
      if [ -n "''${OURO_HOME:-}" ] && [ -d "$OURO_HOME" ]; then
        find "$OURO_HOME" \! -user "$OURO_UID" -exec chown "$OURO_UID:$OURO_GID" {} +
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$OURO_UID" --regid="$OURO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $OURO_HOME/.env.
  ...
  in {
    options.services.hermes-agent = with lib; {
      enable = mkEnableOption "Ouro gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = ouro-agent;
        description = "The ouro-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .ouro/ subdir (OURO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Ouro config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $OURO_HOME/.env at activation time.
          Ouro reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $OURO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $OURO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `ouro gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the ouro user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the ouro plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Ouro discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as ouro (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in ouro-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the ouro CLI to environment.systemPackages and export
          OURO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.ouro symlink to the service
            stateDir. These users are automatically added to the ouro group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the ouro CLI to system PATH and export OURO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.ouro/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.OURO_HOME = "${cfg.stateDir}/.ouro";
      })
      ...
      # Wire extraPackages into the ouro user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            ...
            Set addToSystemPackages = true or ensure ouro is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.ouro        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ouro/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ouro/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ouro/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ouro/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.ouro/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."hermes-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          mkdir -p ${cfg.stateDir}/.ouro
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.ouro ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.ouro ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.ouro -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.ouro/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.ouro/$_subdir"
            chmod 2770 "${cfg.stateDir}/.ouro/$_subdir"
            find "${cfg.stateDir}/.ouro/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.ouro/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.ouro/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ouro/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.ouro/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.ouro/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ouro/.managed
          chmod 0644 ${cfg.stateDir}/.ouro/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.ouro/.container-mode <<'OURO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/hermes
    OURO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ouro/.container-mode
            chmod 0644 ${cfg.stateDir}/.ouro/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.ouro/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.ouro";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.ouro" ]; then
                  rm -f "${symlinkPath}"
                  echo "ouro-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.ouro -> stateDir/.ouro for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.ouro";
                target = "${cfg.stateDir}/.ouro";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "ouro-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.ouro/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.ouro/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.ouro/auth.json
              fi
            ''}
          ''}
          ...
          # Ouro reads $OURO_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.ouro/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'OURO_NIX_ENV_EOF'
    ${envFileContent}
    OURO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.ouro/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.ouro/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.ouro/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hermes-agent = {
          ...
          serviceConfig = {
            ...
            # $OURO_HOME/.env by the activation script. load_hermes_dotenv()
            ...
            # so interactive users in the ouro group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.hermes-agent = {
          description = "Ouro Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              OURO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              OURO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env OURO_UID="$OURO_UID" \
                --env OURO_GID="$OURO_GID" \
                --env OURO_HOME=${containerDataDir}/.ouro \
                --env OURO_MANAGED=true \
