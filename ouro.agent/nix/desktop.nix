# nix/desktop.nix — Ouro Desktop (Electron) app build + wrapper
#
...
# `OURO_DESKTOP_HERMES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Ouro CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # OURO_DESKTOP_HERMES to the absolute path of the nix-built `hermes`
    # binary so the desktop's resolver step 4 ("existing Ouro CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/hermes-desktop \
      --add-flags "$out/share/hermes-desktop" \
      --set OURO_DESKTOP_HERMES "${lib.getExe hermesAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Ouro";
