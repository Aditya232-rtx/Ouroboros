...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ouro-agent = self'.packages.default;
      hermesVenv = ouro-agent.hermesVenv;
    ...
    in {
      ...
      checks = {
        ...
        build-package = pkgs.runCommand "hermes-build-package" { } ''
          echo "PASS: package built at ${ouro-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${ouro-agent}/bin/hermes || (echo "FAIL: ouro binary missing"; exit 1)
          test -x ${ouro-agent}/bin/hermes-agent || (echo "FAIL: ouro-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${ouro-agent}/bin/hermes version 2>&1 | grep -qi "hermes" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in ouro ouro-agent ouro-acp; do
            test -x ${ouro-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          echo "=== Checking ouro --help ==="
          ${ouro-agent}/bin/hermes --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${ouro-agent}/bin/hermes --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          test -d ${ouro-agent}/share/hermes-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${ouro-agent}/share/hermes-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "OURO_BUNDLED_SKILLS" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: OURO_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${ouro-agent}/share/hermes-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${ouro-agent}/share/hermes-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "OURO_OPTIONAL_SKILLS" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, OURO_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          test -d ${ouro-agent}/share/hermes-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${ouro-agent}/share/hermes-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "OURO_BUNDLED_PLUGINS" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: OURO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          test -d ${ouro-agent}/share/hermes-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${ouro-agent}/share/hermes-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${ouro-agent}/share/hermes-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "OURO_BUNDLED_LOCALES" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: OURO_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + OURO_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && OURO_BUNDLED_LOCALES=${ouro-agent}/share/hermes-agent/locales \
            ${hermesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with OURO_BUNDLED_LOCALES set"; exit 1)
        ...
        # OURO_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "hermes-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${ouro-agent}/share/hermes-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${ouro-agent}/share/hermes-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "OURO_OPTIONAL_MCPS" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: OURO_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${ouro-agent}/bin/hermes mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: ouro mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          test -d ${ouro-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${ouro-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "OURO_TUI_DIR" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: OURO_TUI_DIR set in wrapper"
        ...
        # Verify OURO_NODE is set in wrapper and points to Node 26+
        # (Ouro pins its toolchain to Node 26 everywhere)
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          set -e
          echo "=== Checking OURO_NODE in wrapper ==="
          grep -q "OURO_NODE" ${ouro-agent}/bin/hermes || \
            (echo "FAIL: OURO_NODE not set in wrapper"; exit 1)
          echo "PASS: OURO_NODE present in wrapper"

          OURO_NODE=$(sed -n "s/^export OURO_NODE='\(.*\)'/\1/p" ${ouro-agent}/bin/hermes)
          test -x "$OURO_NODE" || (echo "FAIL: OURO_NODE=$OURO_NODE not executable"; exit 1)
          echo "PASS: OURO_NODE executable at $OURO_NODE"

          NODE_MAJOR=$("$OURO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Ouro requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All OURO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify OURO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(OURO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking OURO_MANAGED guards ==="
          check_blocked "config set" ${ouro-agent}/bin/hermes config set model foo
          check_blocked "config edit" ${ouro-agent}/bin/hermes config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = ouro-agent.override {
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${ouro-agent}/bin/hermes; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = ouro-agent.override {
