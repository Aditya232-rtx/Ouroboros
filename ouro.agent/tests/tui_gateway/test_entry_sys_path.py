...
Ouro's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Ouro root.  harden_import_path must relocate the
    Ouro root to the front so ``from utils import ...`` resolves to Ouro."""
    import sys
