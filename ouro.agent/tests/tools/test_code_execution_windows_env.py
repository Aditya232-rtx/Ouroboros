...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``OURO_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # OURO_* handling (#27303): only the operational allowlist passes;
        # every other OURO_* is dropped (the broad prefix was removed).
        "OURO_HOME": "/home/alice/.ouro",        # allowlisted → kept
        "OURO_PROFILE": "default",                 # allowlisted → kept
        "OURO_INTERACTIVE": "1",                   # not allowlisted → dropped
        "OURO_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "OURO_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
