...
Verifies:
  - Tools are gated on OURO_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `ouro chat` sessions (no OURO_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("OURO_KANBAN_TASK", raising=False)
    home = tmp_path / ".ouro"
    home.mkdir()
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: OURO_HOME isolated, OURO_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".ouro"
    home.mkdir()
    monkeypatch.setenv("OURO_HOME", str(home))
    monkeypatch.setenv("OURO_PROFILE", "test-worker")
    monkeypatch.delenv("OURO_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("OURO_KANBAN_TASK", tid)
    return tid
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("OURO_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated OURO_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to OURO_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``OURO_PROFILE`` so a worker can't forge a comment
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from OURO_PROFILE in the fixture, not the
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("OURO_HOME", str(home))
    monkeypatch.setenv("OURO_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has OURO_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no OURO_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # OURO_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no OURO_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via OURO_KANBAN_BOARD env var,
...
# restarting Ouro. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Ouro home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    OURO_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".ouro"
    home.mkdir()
    monkeypatch.setenv("OURO_HOME", str(home))
    # Make sure neither OURO_KANBAN_DB nor OURO_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("OURO_KANBAN_DB", raising=False)
    monkeypatch.delenv("OURO_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("OURO_KANBAN_TASK", raising=False)
    monkeypatch.setenv("OURO_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: OURO_SESSION_PLATFORM + OURO_SESSION_CHAT_ID set.
# - TUI sessions: OURO_SESSION_KEY (or OURO_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("OURO_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("OURO_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("OURO_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("OURO_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("OURO_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("OURO_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports OURO_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("OURO_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("OURO_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("OURO_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("OURO_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("OURO_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("OURO_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("OURO_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("OURO_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("OURO_SESSION_KEY", raising=False)
    monkeypatch.delenv("OURO_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.ouro; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".ouro"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("OURO_HOME", str(home))
    monkeypatch.setenv("OURO_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("OURO_SESSION_CHAT_ID", "channel-1")
...
def default_url_guard(monkeypatch):
    ...
    Clears OURO_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("OURO_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
