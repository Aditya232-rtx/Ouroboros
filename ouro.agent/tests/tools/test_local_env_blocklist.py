...
Verifies that Ouro-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Ouro secrets.

See: https://github.com/Aditya232-rtx/ouro/issues/1002
See: https://github.com/Aditya232-rtx/ouro/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _OURO_PROVIDER_ENV_BLOCKLIST,
    _OURO_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.ouro/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Ouro inference secret
        ...
        models`` run inside a Ouro terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _OURO_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Ouro-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "OURO_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _OURO_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_OURO_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_OURO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_OURO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_OURO_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Ouro venv path instead of the project's own ``.venv`` —
    silently clobbering the Ouro environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Ouro venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.ouro/hermes-agent/venv",
        })
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "ouro-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Ouro-owned entry stripping (#74817).

    The Desktop Electron app injects the Ouro repo root and the Ouro
    ...
    entries Ouro itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Ouro-owned entries are removed; everything else survives
        ...
        cannot silently pass), duplicate Ouro entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Ouro ordering with an
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Ouro-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Ouro venv
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Ouro-owned entries.
        ...
        Ouro-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Ouro venv site-packages entries are stripped
        by the same Ouro-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_hermes_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        hermes_win = f"C:\\\\Users\\\\u\\\\.ouro\\\\ouro-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
        ...
        # the Ouro-owned check cannot match a backslash path here.
        assert hermes_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_hermes_owned_paths_stripped(self):
        """On Windows, a Ouro venv site-packages entry written with
        backslashes is stripped by the same Ouro-owned check, while a
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Ouro venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Ouro runtime producer contract.
        """
        import tools.environments.local as local

        repo_root = tmp_path / "ouro-agent"
    ...
    def test_builders_strip_hermes_venv_pythonpath(self, builder):
        ...
        Ouro venv site-packages is stripped, user entries survive.
        """
    ...
    def test_scrub_child_env_strips_hermes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Ouro venv
    ...
    def test_execute_code_composition_strips_inherited_hermes_entries(self, same_env):
        ...
        Seeds a contaminated inherited PYTHONPATH (Ouro repo root + Ouro
        ...
        * inherited Ouro venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        assert norm_venv not in norm_parts, \
            "inherited Ouro venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_HOME", str(configured_home))
        ...
        assert launcher_entry == configured_home / "ouro-agent"
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with OURO_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home OURO_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "ouro-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("OURO_HOME", str(configured_home))
        lexical_root = configured_home / "ouro-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) OURO_HOME.
        aliases = local._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        (e.g. D:\\hermes\\ouro-agent -> C:\\...\\ouro-agent) while the
        ...
        try:
            _make_directory_link(configured_home / "ouro-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "ouro-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "ouro-agent"
        ...
        for lookalike in (configured_home / "ouro-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        try:
            _make_directory_link(configured_root / "ouro-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "ouro-agent"
        aliases = local._build_hermes_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local._same_path(a, lexical_root) for a in aliases)
        assert not any(local._same_path(a, configured_home / "ouro-agent") for a in aliases)
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Ouro runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Ouro venv and crashes
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_OURO_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Ouro (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _OURO_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _OURO_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Ouro-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _OURO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Ouro, and blocklisting
        ...
        leaked_block = general_chain & _OURO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _OURO_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _OURO_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        from tools.environments import local as local_mod
        saved = local_mod._OURO_BIN_DIR
        local_mod._OURO_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._OURO_BIN_DIR = saved
...
class TestHermesBinDirOnPath:
    """The ouro install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``hermes`` via the terminal tool must work
    even when the gateway was launched without the ouro install dir on
    ...
    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._OURO_BIN_DIR = local_mod._SENTINEL
    ...
    def test_make_run_env_injects_hermes_bin_dir(self):
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._OURO_BIN_DIR = "/opt/hermes/bin"
        with patch.dict(
            os.environ,
...
class TestHermesInternalDynamicSecrets:
    """Dynamically-named Ouro secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_OURO_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _OURO_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _OURO_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _OURO_PROVIDER_ENV_BLOCKLIST
