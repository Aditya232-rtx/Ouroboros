...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"OURO_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_CUA_DRIVER_CMD", str(driver))
...
class TestLazyMcpInstall:
    ...
    def test_start_reports_incompatible_existing_driver_before_mcp_setup(self):
        ...
        with patch.object(
            ...
            with pytest.raises(RuntimeError, match="ouro computer-use install"):
                cua_backend.CuaDriverBackend().start()
...
class TestContractAutoRepair:
    ...
    installed driver (a state Ouro' own version-floor bump created),
    ...
    def test_explicit_override_is_never_repaired(self, monkeypatch):
        ...
        monkeypatch.setattr(cua_backend, "_contract_repair_attempted", False)
        monkeypatch.setenv("OURO_CUA_DRIVER_CMD", "/opt/custom/cua-driver")
        with patch.object(
                 cua_backend,
                 "cua_driver_runtime_contract_status",
                 return_value=self._incompatible(),
             ...
             patch("tools.lazy_deps.ensure"):
            with pytest.raises(RuntimeError, match="OURO_CUA_DRIVER_CMD"):
                cua_backend.CuaDriverBackend().start()
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Ouro-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (Aditya232-rtx/ouro#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (Aditya232-rtx/ouro#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (Aditya232-rtx/ouro#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Ouro patch.
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so OURO_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (Aditya232-rtx/ouro#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (Aditya232-rtx/ouro#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (Aditya232-rtx/ouro#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Ouro never declared a cua-driver
