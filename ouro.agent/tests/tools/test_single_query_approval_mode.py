...
Background (#86878): ``ouro chat -q "..."`` runs one turn and exits. cli.py
exports ``OURO_INTERACTIVE=1`` (needed for interactive sudo password
...
class TestSingleQueryContextDetection:
    def test_env_var_marks_single_query(self, monkeypatch):
        monkeypatch.setenv("OURO_SINGLE_QUERY_SESSION", "1")
    ...
    def test_env_var_unset_is_not_single_query(self, monkeypatch):
        monkeypatch.delenv("OURO_SINGLE_QUERY_SESSION", raising=False)
    ...
    def test_env_var_false_is_not_single_query(self, monkeypatch):
        monkeypatch.setenv("OURO_SINGLE_QUERY_SESSION", "0")
        assert approval_module._is_single_query_approval_context() is False
...
class TestSingleQueryDenyMode:
    """When OURO_SINGLE_QUERY_SESSION is set and single_query_mode=deny,
...
class TestSingleQueryApproveMode:
    """When OURO_SINGLE_QUERY_SESSION is set and single_query_mode=approve,
...
class TestSingleQueryModeInteractions:
    ...
    def test_yolo_overrides_single_query_deny(self, monkeypatch):
        ...
        commands."""
        monkeypatch.setenv("OURO_SINGLE_QUERY_SESSION", "1")
        monkeypatch.setenv("OURO_YOLO_MODE", "1")
        monkeypatch.setenv("OURO_INTERACTIVE", "1")
        monkeypatch.delenv("OURO_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting OURO_YOLO_MODE). Patch the
