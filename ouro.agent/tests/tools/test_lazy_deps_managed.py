...
A package-manager install (NixOS, and anything else shipping Ouro from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.ouro/config.yaml (a local
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets OURO_MANAGED *and* a writable target.

    Dockerfile sets OURO_LAZY_INSTALL_TARGET and the NixOS container module
    passes OURO_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("hermes_cli.config.get_managed_system", lambda: "nixos")
