...
TTS provider tools must resolve API keys from ``~/.ouro/.env`` (via
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.ouro/.env`` carries the
...
class TestRegressionGuard:
    ...
    ``hermes_cli.config.load_env`` to simulate ``~/.ouro/.env`` carrying the
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.ouro/.env carrying the key (load_env returns the dict
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.ouro/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
