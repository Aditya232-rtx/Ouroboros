...
class TestCronDenyMode:
    """When OURO_CRON_SESSION is set and cron_mode=deny, dangerous commands are blocked."""
...
class TestCronApproveMode:
    """When OURO_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        # prompt injection from runtime-setting OURO_YOLO_MODE). When the
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds OURO_SESSION_PLATFORM via contextvars for
