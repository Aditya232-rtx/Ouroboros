...
at startup, by THREE separate code paths:
  ...
  3. hermes_cli/config.py:set_config_value
                       -> bridges via the canonical ``TERMINAL_CONFIG_ENV_MAP``
                          (one-shot when the user runs ``ouro config set …``)
...
dicts) keeps the test independent of the user's ~/.ouro/config.yaml and
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``ouro config set foo bar``.
...
def test_save_config_set_supports_critical_bridged_keys():
    """``ouro config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`ouro config set terminal.X` doesn't sync these load-bearing "
...
def test_docker_extra_args_is_bridged_everywhere():
    ...
    present in DEFAULT_CONFIG, TERMINAL_CONFIG_ENV_MAP (so ``ouro config
    ...
    producing the "Ouro partially reads the Docker config" symptom.  Guard
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Ouro exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Ouro sweeps stale
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``ouro config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``ouro config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
