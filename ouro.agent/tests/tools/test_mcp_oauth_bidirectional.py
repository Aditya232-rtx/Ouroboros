...
async def test_hermes_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _OURO_PROVIDER_CLS, reset_manager_for_tests

    assert _OURO_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
async def test_hermes_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _OURO_PROVIDER_CLS is not None
