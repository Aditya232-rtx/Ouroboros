...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OURO_HOME": str(hermes_home)}):
        ...
        assert mounts[0]["container_path"] == "/root/.ouro/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.ouro/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"OURO_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.ouro")

        assert mounts[0]["container_path"] == "/home/user/.ouro/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.ouro/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.ouro/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape OURO_HOME."""
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of OURO_HOME must be allowed."""
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside OURO_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
        monkeypatch.setenv("OURO_HOME", str(hermes_home))
        ...
        # The resolved path escapes OURO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape OURO_HOME."""
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
        monkeypatch.setenv("OURO_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside OURO_HOME via config must still mount."""
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
        monkeypatch.setenv("OURO_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.ouro/cache/documents" in paths
        assert "/root/.ouro/cache/audio" in paths
        assert "/root/.ouro/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.ouro/cache/documents" in container_paths
        assert "/root/.ouro/cache/images" in container_paths
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.ouro/attachments" in container_paths
        assert "/root/.ouro/images" in container_paths
        assert "/root/.ouro/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``OURO_HOME/images``, not
        ...
        assert "/root/.ouro/images" in by_container
        assert by_container["/root/.ouro/images"] == str(hermes_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.ouro/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.ouro/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        assert mapped == "/root/.ouro/cache/images/x.png"
...
class TestToAgentVisiblePathPerBackend:
    ...
    Ouro cache — not just docker — and skips the ones where the host path
    ...
    def test_docker_maps_to_root_hermes(self, tmp_path, monkeypatch):
        ...
        assert to_agent_visible_cache_path(staged) == "/root/.ouro/attachments/drop.zip"

    def test_ssh_maps_to_tilde_hermes(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.ouro/attachments/drop.zip"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — OURO_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping OURO_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"OURO_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.ouro/google_token.json" in paths
        assert "/root/.ouro/.env" not in paths
