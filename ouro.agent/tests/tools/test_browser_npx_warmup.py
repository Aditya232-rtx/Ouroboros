...
warm_agent_browser_npx_cache() is the fire-and-forget helper `ouro update` /
`ouro doctor --fix` call to pre-fetch agent-browser via npx so the first real
...
`ouro update` — not only when a browser tool is actually used), must pass
...
def test_captures_stdout_and_stderr_instead_of_inheriting_parent_fds():
    """The npx registry fetch runs on every `ouro update` — its stdout/
...
def test_uses_credential_scrubbed_environment():
    ...
    os.environ with every provider/gateway credential Ouro holds."""
...
def test_merges_extended_path_so_managed_only_npx_can_find_sibling_node():
    """If npx was resolved via the Ouro-managed/extended search (not the
