...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning ouro PID to write to <session>.owner_pid
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which ouro process owns a daemon so that
    concurrent ouro processes don't reap each other's active browser
...
class TestLeakedDaemonWithLiveOwner:
    ...
    long-lived ouro process, pinning ~5 CPU cores and driving load to 100+.
...
class TestPeriodicOrphanReap:
    ...
    boot — which is exactly what happens in a ouro process that stays up
    for days.
    """
