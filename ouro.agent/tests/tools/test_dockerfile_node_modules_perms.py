...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV OURO_TUI_DIR=/opt/hermes/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hermes:ouro /opt/hermes/ui-tui" not in text
    assert "chown -R hermes:ouro /opt/hermes/node_modules" not in text
