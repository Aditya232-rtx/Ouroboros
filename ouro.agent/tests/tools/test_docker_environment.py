...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across ouro processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._cwd_marker = "__OURO_CWD_test123__"
...
def test_init_env_args_uses_hermes_dotenv_for_allowlisted_env(monkeypatch):
    ...
    # Use a var that is NOT in _OURO_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
...
def test_init_env_args_uses_hermes_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.ouro/.env.
    """
...
def test_egress_node_options_overrides_conflicting_ca_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: ([], {"_OURO_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca"}, []),
    )
    calls = _mock_subprocess_run(monkeypatch)
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Ouro image, or ``gosu``/``su``
...
def test_run_command_tags_hermes_agent_label(monkeypatch):
    """Every container ouro-agent starts must carry the ouro-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=ouro-agent=1`` call. Regression test
    ...
    assert "ouro-agent=1" in labels, (
        f"ouro-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Ouro process spawning a new container
...
def test_extra_args_proxy_override_refuses_under_egress(monkeypatch):
    """docker_extra_args are appended after Ouro args, so egress enforcement
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Ouro process uses it. Without this
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    Ouro startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.ouro/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.ouro/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="ouro-agent:latest")
