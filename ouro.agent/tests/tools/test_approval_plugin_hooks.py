...
def isolated_session(monkeypatch, tmp_path):
    ...
    OURO_HOME so the real user's command_allowlist doesn't leak in."""
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: OURO_INTERACTIVE is set, the
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: OURO_GATEWAY_SESSION is set and a
