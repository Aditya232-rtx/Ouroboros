...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad OURO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: OURO_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("OURO_GATEWAY_SESSION", "1")
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "OURO_HOME": "/h", "OURO_PROFILE": "p",
        "OURO_CONFIG": "/c.yaml", "OURO_ENV": "/e",
        "OURO_DELEGATED_CHILD_CONTEXT": "1",
        # other OURO_* → dropped (broad prefix removed)
        "OURO_BASE_URL": "https://x", "OURO_INTERACTIVE": "1",
        "OURO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "OURO_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "OURO_BASE_URL", "OURO_INTERACTIVE", "OURO_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped OURO_* vars."""
    import logging
