...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``ouro cron``
    runs separately from ``ouro gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``OURO_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``OURO_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``OURO_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
