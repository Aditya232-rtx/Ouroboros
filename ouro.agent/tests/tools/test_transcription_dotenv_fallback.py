...
``~/.ouro/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.ouro/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.ouro/.env`` carrying the key while ``os.environ`` does not.
