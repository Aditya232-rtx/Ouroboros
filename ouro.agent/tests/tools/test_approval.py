...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("OURO_SESSION_KEY", session_key)
        monkeypatch.setenv("OURO_EXEC_ASK", "1")
        monkeypatch.delenv("OURO_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"OURO_SESSION_KEY": "bob"}, clear=False):
...
class TestHermesConfigWriteProtection:
    ...
    ~/.ouro/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.ouro/config.yaml",
            "echo '  mode: off' >> ~/.ouro/config.yaml",
            "echo x | tee ~/.ouro/config.yaml",
            "echo x | tee $OURO_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.ouro/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Ouro absolute config.yaml is
        # handled by the project patterns, not the Ouro-home rule.
        for cmd in (
            "cat ~/.ouro/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.ouro/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Ouro-home prefixes must fold to ~/ and
    ~/.ouro/ in dangerous-command detection.
    ...
    SSH, and Ouro config/env files returned "safe" without an approval prompt.
    ...
    HOME/OURO_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("OURO_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.ouro/hermes-agent && source venv/bin/activate && python -m hermes_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart ouro-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Ouro security config via IFS.
            "sed${IFS}-i ~/.ouro/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Ouro service
    label achieves the same effect as `ouro gateway stop|restart` and
    ...
    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Ouro labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("OURO_GATEWAY_SESSION", "OURO_CRON_SESSION",
                      "OURO_YOLO_MODE",
                      "OURO_SESSION_KEY", "OURO_INTERACTIVE")
        }
        os.environ.pop("OURO_YOLO_MODE", None)
        os.environ.pop("OURO_INTERACTIVE", None)
        # OURO_CRON_SESSION takes priority over OURO_GATEWAY_SESSION in
        ...
        os.environ.pop("OURO_CRON_SESSION", None)
        os.environ["OURO_GATEWAY_SESSION"] = "1"
        os.environ["OURO_SESSION_KEY"] = self.SESSION_KEY
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("hermes_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"OURO_INTERACTIVE": "1"}, clear=False):
