...
class TestGating:
    ...
    def test_a_profile_sees_a_credential_held_at_the_global_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_HOME", str(root / "profiles" / "work"))
...
class TestPollTransport:
    ...
    def test_off_messaging_the_clip_stays_a_file_and_no_tag_is_offered(self, tmp_path, monkeypatch, platform):
        ...
        monkeypatch.setenv("OURO_SESSION_PLATFORM", platform)
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
    ...
    def test_a_cli_session_is_recognised_by_its_source(self, tmp_path, monkeypatch):
        # The CLI, TUI, and desktop leave OURO_SESSION_PLATFORM empty and
        # identify themselves on OURO_SESSION_SOURCE instead, so keying only
        # on the platform would miss them.
        monkeypatch.delenv("OURO_SESSION_PLATFORM", raising=False)
        monkeypatch.setenv("OURO_SESSION_SOURCE", "tui")
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
