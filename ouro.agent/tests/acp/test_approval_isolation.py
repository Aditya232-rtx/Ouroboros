...
- 96vc: ACP didn't set OURO_EXEC_ASK, so `check_all_command_guards`
...
1. Setting OURO_EXEC_ASK inside _run_agent (wraps the agent call).
...
def _isolate_approval_state(monkeypatch):
    ...
    developer's real ``~/.ouro/config.yaml`` command allowlist into
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set OURO_INTERACTIVE so
    ...
    (OURO_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When OURO_INTERACTIVE is set and an approval callback is
        ...
        monkeypatch.delenv("OURO_YOLO_MODE", raising=False)
        ...
        # Without OURO_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without OURO_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With OURO_INTERACTIVE: callback IS called, approval flows through it
        monkeypatch.setenv("OURO_INTERACTIVE", "1")
        called_with.clear()
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert called_with, (
            "with OURO_INTERACTIVE the approval path should consult the "
