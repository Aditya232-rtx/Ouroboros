...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_OURO_BEHAVIORAL_VARS`` now blanks ``OURO_VOICE`` /
    ``OURO_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("OURO_VOICE_TTS") == "1"
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``OURO_VOICE_TTS=1`` set.
    Before ``OURO_VOICE``/``OURO_VOICE_TTS`` were added to
    ``_OURO_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "OURO_VOICE_TTS" not in os.environ
    assert "OURO_VOICE" not in os.environ
