...
Under a gateway (Telegram, Discord, Slack, ...) Ouro passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Ouro via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Ouro profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `ouro -p partner` pinned to a different person's peer).
