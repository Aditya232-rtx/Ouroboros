...
def test_client_id_defaults_to_hermes_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "ouro-agent"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=ouro-agent so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["hermes"]["oauth"]["clientId"] == "ouro-agent"


def test_config_path_rides_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, config_path="~/.ouro/honcho.json")
    q = parse_qs(urlparse(url).query)
    assert q["config_path"][0] == "~/.ouro/honcho.json"
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    under_home = Path.home() / ".ouro" / "profiles" / "work" / "honcho.json"
    assert oauth_flow._display_config_path(under_home) == "~/.ouro/profiles/work/honcho.json"
