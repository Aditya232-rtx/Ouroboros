...
class TestNamedProfileHintIntegration:
    ...
    ``OURO_HOME`` and no resolver mocks.
    """
    ...
    def test_real_hermes_home_under_profiles_renders_correct_paths(
        ...
        assert "Active Ouro profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """OURO_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".ouro"
        ...
        assert "Active Ouro profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "OURO_AGENT_HELP_GUIDANCE", "HELP")
...
class TestTelegramRichMessagesHint:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under OURO_HOME with ``gateway.platforms.telegram.extra.rich_messages``
