...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Ouro passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("OURO_HOME", "/users/alice/.ouro/profiles/backend-worker")
        monkeypatch.setenv("OURO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "OURO_KANBAN_DB",
            "/users/alice/.ouro/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.ouro/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Ouro secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "OURO_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "OURO_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
