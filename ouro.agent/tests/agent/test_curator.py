...
def curator_env(tmp_path, monkeypatch):
    """Isolated OURO_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".ouro"
    ...
    # save_state() resolves the state path from OURO_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.ouro once monkeypatch
    ...
    # thread here, while OURO_HOME is still pinned to this test's tmp home
...
# `ouro model` → auxiliary picker, the dashboard Models tab, and the full
