"""Regression tests for the codex_app_server → Ouro UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_hermes_tools_mcp_server_emits_bare_tool_name(self):
        """The hermes-tools MCP server wraps Ouro' own tools for codex;
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "ouro agent docs"
        assert calls[0].args[3] == {"query": "ouro agent docs"}
