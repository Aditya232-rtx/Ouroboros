...
def hermes_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.ouro/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".ouro"
    home.mkdir()
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different OURO_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".ouro"
    ...
    monkeypatch.setenv("OURO_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("OURO_HOME", str(home_b))
