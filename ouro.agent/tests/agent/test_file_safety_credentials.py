"""Tests for OURO_HOME credential-file read blocking in file_safety.

Regression for https://github.com/Aditya232-rtx/ouro/issues/17656 —
``read_file`` was previously only sandboxed against ``OURO_HOME`` itself,
...
for the credential stores while leaving arbitrary ``OURO_HOME`` files
...
def test_arbitrary_hermes_home_file_not_blocked(fake_home):
    """Non-credential files inside OURO_HOME stay readable."""
...
def test_identically_named_hermes_files_outside_home_not_blocked(
    ...
    """Ouro-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside OURO_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside OURO_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside OURO_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, OURO_HOME = <root>/profiles/<name>, but
