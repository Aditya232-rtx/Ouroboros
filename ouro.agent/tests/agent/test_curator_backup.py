...
def backup_env(monkeypatch, tmp_path):
    """Isolate OURO_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".ouro"
    home.mkdir()
    (home / "skills").mkdir()
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under OURO_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level OURO_DIR picks up the tmp HOME."""
    import hermes_constants
    importlib.reload(hermes_constants)
