...
class TestBuildContextFilesPrompt:
    ...
    def test_hermes_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".ouro.md").write_text("Ouro-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Ouro-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "hermes_home"))
...
# =========================================================================
# .ouro.md helper functions
# =========================================================================


class TestFindHermesMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".ouro.md").write_text("rules")
        assert _find_hermes_md(tmp_path) == tmp_path / ".ouro.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".ouro.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_hermes_md(sub) == tmp_path / ".ouro.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .ouro.md planted in /tmp, /home, or /
        ...
        parent.mkdir()
        (parent / ".ouro.md").write_text("planted by another user")
        cwd = parent / "work"
        cwd.mkdir()
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """OURO_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("OURO_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
