...
thread that did not bind the OURO_HOME ContextVar.
...
so an unbound build thread fell back to ``~/.ouro`` (default) and leaked
default's full skills index + "Active Ouro profile: default" into a bot's
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient OURO_HOME, on a
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ouro ROOT (get_default_hermes_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: OURO_HOME env points at the profile dir itself.
    monkeypatch.setenv("OURO_HOME", str(bot_home))
...
def test_profile_name_correct_when_ambient_is_another_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OURO_HOME", str(other_home))
