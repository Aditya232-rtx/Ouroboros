"""Tests for the cross-Ouro-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Ouro profile's
...
accidentally edited skills under both ~/.ouro/profiles/hermes-security/skills/
AND ~/.ouro/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Ouro root with two profiles, monkeypatch the
...
class TestResolveActiveProfileName:
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If OURO_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
