...
thinking phase.  Ouro's default cloud-stream stale detector
(``OURO_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``OURO_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_non_reasoning_model_keeps_default(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("OURO_API_CALL_STALE_TIMEOUT", raising=False)
