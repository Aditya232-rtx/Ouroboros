...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Ouro Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.ouro.
    assert "/root/.ouro/attachments/archive.zip" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.ouro/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.ouro/auth.json` and read the
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.ouro/auth.json and @file:.ouro/.anthropic_oauth.json "
        "and @file:.ouro/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Ouro/composer-images/a.png",
