...
def isolated_home(monkeypatch):
    """Temp OURO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="hermes_test_31179_")
    hermes_home = os.path.join(test_home, ".ouro")
...
def _fresh_modules():
    """Drop cached ouro modules so each test reloads against current env."""
    _drop_reload_targets()
