...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate OURO_HOME under tmp_path."""
    monkeypatch.setenv("OURO_HOME", str(tmp_path / ".ouro"))
    (tmp_path / ".ouro").mkdir()

    # Force the constants/image cache helpers to re-read OURO_HOME.
    import sys
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under OURO_HOME — gateway cleanup
