...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"ouro-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "ouro-agent-dev", "declared": True},
        },
    )
...
# `ouro curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `ouro curator pin <umbrella>` in the rename block so
