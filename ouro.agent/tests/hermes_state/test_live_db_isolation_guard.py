...
developer's REAL ~/.ouro/state.db, and a pytest-spawned process flipped
...
# Must match the root the guard itself computes.  Hardcoding ``~/.ouro``
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.ouro/state.db must fail hard."""
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_hermes_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic OURO_HOME must succeed."""
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            hermes_state, "DEFAULT_DB_PATH", hermes_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestSubprocessChildCovered:
    def test_child_without_hermes_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no OURO_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("OURO_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_hermes_home_succeeds(self, tmp_path):
        """Same child, hermetic OURO_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["OURO_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from hermes_state import SessionDB\n"
            "db = SessionDB()\n"
