...
entirely on the process environment: ``OURO_HOME`` says *where* to write and
...
# hardcoded ~/.ouro — that root is %LOCALAPPDATA%\ouro on Windows) and
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``OURO_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("OURO_HOME", "OURO_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no OURO_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``OURO_HOME`` is gone, and the env-only guard
