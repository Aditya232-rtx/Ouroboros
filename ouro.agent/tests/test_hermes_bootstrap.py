...
The bootstrap module is imported at the top of every Ouro entry point
(hermes, ouro-agent, ouro-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Ouro entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Ouro CLI
...
class TestEntryPointsImportBootstrap:
    """Every Ouro entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke Ouro as a process.  Each one must
    # import hermes_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "hermes_cli/main.py",   # ouro CLI (console_script)
        "run_agent.py",          # ouro-agent (console_script)
        "acp_adapter/entry.py",  # ouro-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``ouro update`` (git-reset landed new code but
        ...
        """
        # Resolve relative to the ouro-agent repo root.  Tests live
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # ouro start even when hermes_bootstrap hasn't been
...
class TestHardenImportPath:
    ...
    directory from shadowing Ouro's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("OURO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["OURO_PYTHON_SRC_ROOT"] = env
            elif "OURO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["OURO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hermes")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("OURO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["OURO_PYTHON_SRC_ROOT"] = original_env
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["OURO_PYTHON_SRC_ROOT"] = "/env/hermes"
