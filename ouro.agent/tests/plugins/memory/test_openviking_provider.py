...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Ouro's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Ouro
    venv instead of its own, and on Windows locks Ouro venv DLLs so the
    venv cannot be rebuilt during `ouro update`.
    """
    popen_calls = []
    ...
    monkeypatch.setenv("OURO_PROFILE", "test-profile")
    ...
    assert child_env.get("OURO_PROFILE") == "test-profile"
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Ouro will retry on a later access "
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Ouro and
