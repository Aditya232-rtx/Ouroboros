...
_CATALOG = {
    "templates": [
        ...
        {"id": "ouro-gateway-bot", "name": "Gateway Bot", "integrations": ["hermes"],
         "manifest_file": "templates/ouro-gateway-bot.json"},
    ]
}
...
def test_fetch_hermes_templates_filters_to_hermes(monkeypatch):
    ...
    assert ids == ["conversation", "ouro-gateway-bot"]  # coding-agent excluded
...
def test_fetch_manifest_resolves_relative_url(monkeypatch):
    ...
    assert seen["url"] == "https://raw.example/data/templates/ouro-gateway-bot.json"
...
def test_run_template_step_applies_selected(monkeypatch):
    monkeypatch.setattr(tpl, "fetch_hermes_templates", lambda url=None: [
        {"id": "ouro-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"},
    ])
    ...
    assert result == "ouro-gateway-bot"
    assert applied["bank"] == "hermes"


def test_run_template_step_blank_selection_skips(monkeypatch):
    entries = [{"id": "ouro-gateway-bot", "name": "Gateway Bot", "manifest_file": "templates/x.json"}]
