...
Run from the PR worktree:
    ...
    cd ~/.ouro/hermes-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.ouro/hermes-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.ouro/hermes-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a ouro-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a ouro-agent checkout"
)
...
# Isolated OURO_HOME for the config write.
