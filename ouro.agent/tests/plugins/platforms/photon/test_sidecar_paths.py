...
baked and current, and mirror the sidecar to the writable ``OURO_HOME``
...
def test_adapter_import_does_not_resolve_sidecar_dir(monkeypatch) -> None:
    ...
    files to OURO_HOME; the adapter and CLI resolve lazily on first use so
    a bare import (plugin discovery, `ouro --help`, test collection) has
