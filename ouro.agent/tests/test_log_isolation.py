"""The test suite must never write into the operator's real Ouro logs.
...
the whole pytest session's logging to `<OURO_HOME>/logs/agent.log`.

If OURO_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets OURO_HOME at module scope for that reason - this
...
class TestLogIsolation:
    def test_hermes_home_is_sandboxed_before_imports(self):
        ...
        # fixture has sandboxed OURO_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import OURO_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set OURO_HOME before test modules import"
        assert Path(home).resolve() != _real_hermes_home().resolve(), (
            f"OURO_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        ...
        assert offenders == [], (
            "the test session is writing into the operator's real Ouro logs:\n  "
            + "\n  ".join(offenders)
        )
