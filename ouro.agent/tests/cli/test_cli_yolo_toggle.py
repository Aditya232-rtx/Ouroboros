...
``os.environ["OURO_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``ouro --yolo`` (process-start env), ``OURO_YOLO_MODE=1``,
and ``ouro config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # Ouro-driven test runs may inherit OURO_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``ouro --yolo`` sets ``OURO_YOLO_MODE`` before tool imports, so
