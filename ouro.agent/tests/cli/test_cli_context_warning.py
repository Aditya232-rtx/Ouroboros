...
def _isolate(tmp_path, monkeypatch):
    """Isolate OURO_HOME so tests don't touch real config."""
    home = tmp_path / ".ouro"
    home.mkdir()
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Ouro' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Ouro' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Ouro' minimum context length."""
