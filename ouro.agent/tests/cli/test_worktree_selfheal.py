...
Regression for the Aug 2026 `ouro -w` timeout incident: 39 accumulated packs
...
(lock pid = the live ouro process), poisoning every retry.
...
class TestCleanupFailedWorktreeAdd:
    def _simulate_timed_out_add(self, repo):
        ...
        # Live-pid lock, exactly what `ouro -w` writes before the checkout.
        _git(repo, "worktree", "lock", str(wt), "--reason", "ouro pid=999999")
