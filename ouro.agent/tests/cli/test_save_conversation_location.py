...
file to be discoverable via ``ouro sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.ouro/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    ...
    # File MUST be under ~/.ouro/sessions/saved/
    ...
    assert "ouro --resume 20260101_120000_abc123" in out, out
