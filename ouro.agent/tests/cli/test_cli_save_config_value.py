...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # point OURO_HOME at the temp dir (the _hermes_home import-time
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in OURO_HOME/config.yaml
    ...
    install whose OURO_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read OURO_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh OURO_HOME with NO config.yaml (managed/desktop first launch).
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
        monkeypatch.setenv("OURO_HOME", str(hermes_home))
