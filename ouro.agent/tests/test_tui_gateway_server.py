...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's OURO_HOME."""
    ...
    monkeypatch.setenv("OURO_HOME", str(tmp_path / "default"))
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["OURO_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("OURO_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("OURO_VOICE", "1")
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("OURO_VOICE", "1")
    monkeypatch.setenv("OURO_VOICE_TTS", "1")
    ...
    assert os.environ["OURO_VOICE"] == "0"
    assert os.environ["OURO_VOICE_TTS"] == "0"
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("OURO_VOICE", "1")
    # setenv (not delenv) — the handler writes OURO_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("OURO_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("OURO_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("OURO_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "hermes_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown OURO_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("OURO_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("OURO_MODEL", raising=False)
    monkeypatch.delenv("OURO_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set OURO_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("OURO_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("OURO_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `ouro --tui -m <model>` sets OURO_MODEL/OURO_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("OURO_MODEL", "one-shot/model")
    monkeypatch.setenv("OURO_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("OURO_MODEL", "seed/model")
    monkeypatch.setenv("OURO_INFERENCE_MODEL", "seed/model")
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("OURO_MODEL", "nous/hermes-test")
    monkeypatch.setenv("OURO_TUI_PROVIDER", "nous")
    monkeypatch.delenv("OURO_INFERENCE_PROVIDER", raising=False)
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("OURO_MODEL", "nous/hermes-test")
    monkeypatch.delenv("OURO_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("OURO_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("OURO_MODEL", "sonnet")
    monkeypatch.delenv("OURO_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("OURO_INFERENCE_PROVIDER", raising=False)
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.delenv("OURO_INFERENCE_MODEL", raising=False)
    monkeypatch.delenv("OURO_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("OURO_DESKTOP", raising=False)
    monkeypatch.delenv("OURO_DESKTOP_TERMINAL", raising=False)
    monkeypatch.setattr(
        server,
        "_load_cfg",
        lambda: {
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_hermes_home", tmp_path)
    # Point the canonical resolver (load_config → env OURO_HOME) at the
    # temp home too, so the smart default is asserted against THIS config
    # rather than whatever the developer's real ~/.ouro happens to hold.
    monkeypatch.setenv("OURO_HOME", str(tmp_path))
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # that path resolves OURO_HOME from the environment, not
    # server._hermes_home.
    monkeypatch.setenv("OURO_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves OURO_HOME from the environment.
    monkeypatch.setenv("OURO_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    # tools.approval resolver (load_config → env OURO_HOME).
    monkeypatch.setenv("OURO_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("OURO_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("OURO_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("OURO_EXEC_ASK", raising=False)
    monkeypatch.delenv("OURO_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("OURO_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["OURO_GATEWAY_SESSION"] == "1"
    assert server.os.environ["OURO_EXEC_ASK"] == "1"
    assert server.os.environ["OURO_INTERACTIVE"] == "1"
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing OURO_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("OURO_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "hermes_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["OURO_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "OURO_TUI_PROVIDER" not in os.environ
        assert "OURO_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write OURO_MODEL / OURO_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("OURO_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("OURO_MODEL", raising=False)
    monkeypatch.delenv("OURO_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["OURO_TUI_PROVIDER"] == "openai-codex"
        assert "OURO_MODEL" not in os.environ
        assert "OURO_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("OURO_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("OURO_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["OURO_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["OURO_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        # attachments dir.
        assert not (workspace / ".ouro" / "desktop-attachments").exists()
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # fixture pins OURO_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_branch_installs_parent_profile_secret_scope(monkeypatch, tmp_path):
    ...
    session.branch already binds the parent's OURO_HOME and state.db, but the
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `ouro model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `ouro model` and the classic CLI /model picker.
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``ouro --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_reload_env_rpc_calls_hermes_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.ouro/.env
...
def test_session_save_writes_under_hermes_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Ouro profile
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Ouro.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert payload["system_prompt"] == "You are Ouro."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # env var), always targeting OURO_HOME/config.yaml — point it at tmp_path.
    monkeypatch.setenv("OURO_HOME", str(tmp_path))
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("OURO_VOICE_TTS", "1")
    monkeypatch.setenv("OURO_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("OURO_VOICE") == "0"  # voice chat ended
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "Ouro" / "composer-images"
...
# silently rewrote ~/.ouro/config.yaml top-to-bottom — top-level keys
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    OURO_HOME to the session's profile before rebuilding the system
    ...
    # to the profile home, not the root ~/.ouro.
