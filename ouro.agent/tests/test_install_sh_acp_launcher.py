"""`setup_path()` must also install a `ouro-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `ouro-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Ouro as not installed. `setup_path()` therefore
writes a `ouro-acp` launcher next to the `hermes` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/ouro-acp".*?'
    r'log_success "Installed ouro-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the ouro-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    script = (
        "set -e\n"
        f"OURO_BIN={hermes_bin}\n"
        f"OURO_ENTRYPOINT={entrypoint}\n"
    ...
    return command_link_dir / "ouro-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `ouro-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "ouro-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/ouro-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/ouro-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# ouro-agent launcher regression (#74819)
# ---------------------------------------------------------------------------

OURO_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/hermes-agent".*?'
    r'log_success "Installed ouro-agent launcher[^\n]*\n)',
    re.S,
)


def _extract_hermes_agent_shim_block() -> str:
    match = OURO_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
        "could not locate the ouro-agent launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)
...
def _run_hermes_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted ouro-agent block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: ouro-agent is NOT installed by this block (handled
    ...
    assert result.returncode == 0, (
        f"ouro-agent shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "ouro-agent"


def test_venv_install_writes_executable_hermes_agent_launcher(tmp_path):
    """venv install must write a user-executable ouro-agent launcher."""
    ...
    assert "unset PYTHONPATH" in text
    assert "unset PYTHONHOME" in text
    assert "run_agent.py" in text, "ouro-agent must dispatch to run_agent.py"


def test_hermes_agent_launcher_cleanup_on_uninstall(tmp_path):
    """uninstall.remove_wrapper_script() must remove ouro-agent alongside
    ouro and ouro-acp."""
    from hermes_cli.uninstall import remove_wrapper_script

    # Simulate a ouro-agent wrapper in the user-local location
    local_shim = tmp_path / ".local" / "bin" / "ouro-agent"
    ...
    assert local_shim in removed, "local ouro-agent wrapper must be removed"
