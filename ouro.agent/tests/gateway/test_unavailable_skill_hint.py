...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + OURO_HOME so the real user config is untouched."""
    home = tmp_path / ".ouro"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `ouro skills config` writes).
    monkeypatch.setattr(
    ...
    assert "ouro skills config" in msg
