...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the ouro ROOT, not the current
        # OURO_HOME — the current home may itself be a profile, and nesting
        ...
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            # consolidated layout a standalone `ouro -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Ouro home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``ouro pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Ouro layout."""
        monkeypatch.setenv("OURO_HOME", str(tmp_path))
    ...
    def test_profile_store_matches_profile_cli_home(self, tmp_path, monkeypatch):
        """Gateway and ``ouro -p`` must resolve the same pairing store."""
