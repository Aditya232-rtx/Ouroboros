...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.ouro."""
    monkeypatch.setattr(
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``ouro gateway run --replace`` takes the old process through the
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "ouro gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_OURO_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_hermes_slack_user_agent_prefix_format(self):
        """Module constant matches the Ouro/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._OURO_SLACK_USER_AGENT_PREFIX.startswith("Ouro/")
