...
OURO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env OURO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    (hermes_home / ".env").write_text(
        "OURO_MAX_ITERATIONS=90\nOPENROUTER_API_KEY=fresh-key\n",
        encoding="utf-8",
    )

    monkeypatch.setattr(gateway_run, "_hermes_home", hermes_home)
    monkeypatch.setenv("OURO_MAX_ITERATIONS", "9000")
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["OPENROUTER_API_KEY"] == "fresh-key"
    assert os.environ["OURO_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``ouro config get terminal.backend`` still says local.
    """
    hermes_home = tmp_path / ".ouro"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
