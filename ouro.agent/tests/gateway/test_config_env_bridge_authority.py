...
Guards against the 60-vs-500 bug where a stale `.env OURO_MAX_ITERATIONS=60`
...
def _run_gateway_import(hermes_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            "OURO_MAX_ITERATIONS",
            "OURO_AGENT_TIMEOUT",
            "OURO_AGENT_TIMEOUT_WARNING",
            "OURO_TURN_LEASE_TIMEOUT",
            "OURO_SESSION_STALL_TIMEOUT",
            "OURO_GATEWAY_BUSY_INPUT_MODE",
            "OURO_GATEWAY_BUSY_TEXT_MODE",
            "OURO_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "OURO_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    # otherwise intentionally clean so stale Ouro settings cannot leak in.
    for k in (
        "PATH",
        "PYTHONPATH",
        "VIRTUAL_ENV",
...
def test_config_gateway_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(hermes_home, {
        "OURO_AGENT_TIMEOUT": "60",
        "OURO_AGENT_TIMEOUT_WARNING": "30",
        "OURO_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("OURO_AGENT_TIMEOUT") == "1800"
    assert env.get("OURO_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("OURO_SESSION_STALL_TIMEOUT") == "300"
...
def test_config_turn_lease_timeout_wins_over_stale_env(hermes_home: Path) -> None:
    ...
    _write_env(
        hermes_home,
        {"OURO_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(hermes_home, initial_env={})

    assert env.get("OURO_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("OURO_TURN_LEASE_TIMEOUT") == "1800"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(hermes_home: Path) -> None:
    ...
    assert env.get("OURO_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"
...
def test_env_platform_connect_timeout_wins_over_config(hermes_home: Path) -> None:
    ...
    OURO_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        hermes_home,
        initial_env={"OURO_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("OURO_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
