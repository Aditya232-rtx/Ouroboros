...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    hermes_home.mkdir()
    monkeypatch.setattr(base, "_OURO_HOME", hermes_home)
    monkeypatch.setattr(base, "_OURO_ROOT", hermes_home)
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.ouro/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("OURO_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("OURO_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("OURO_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("OURO_KANBAN_HOME", str(tmp_path / "hermes"))
        monkeypatch.setenv("OURO_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("OURO_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("OURO_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("OURO_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.ouro/mcp-tokens/ must
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the OURO_HOME root (google_token.json)
        ...
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._OURO_HOME", hermes_dir)
        monkeypatch.setattr("gateway.platforms.base._OURO_ROOT", hermes_dir)
    ...
    def test_denylist_blocks_non_cache_file_under_hermes_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.ouro
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting OURO_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose OURO_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.ouro) emits
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._OURO_ROOT", hermes_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.ouro/cache/images/... (the agent_visible_image path
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.ouro/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "OURO_HUMAN_DELAY_MODE": "natural",
            "OURO_HUMAN_DELAY_MIN_MS": "oops",
            "OURO_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"OURO_MEDIA_DELIVERY_STRICT": "1",
                                     "OURO_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Ouro
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    SENSITIVE_PATH = "/home/jayne/.ouro/cache/media/sensitive_host_path_abc123.bin"
