...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "ouro-gateway"
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level OURO_HOME.
        ...
        the process-level OURO_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("OURO_HOME", str(process_home))
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``ouro gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "ouro gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "ouro -p coder gateway run --replace",
            "/opt/hermes/.venv/bin/hermes --profile coder gateway run --replace",
            "hermes_home=/opt/data/profiles/coder ouro gateway run --replace",
        ):
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders OURO_HOME with backslashes while the
        ...
        cmdline = r"hermes_home=c:\opt\data\profiles\coder ouro gateway run --replace"
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OURO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "ouro-gateway",
            "argv": ["/Users/user/.ouro/hermes-agent/hermes_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("OURO_HOME", "/opt/data")
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_hermes_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's OURO_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("OURO_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        OURO_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.ouro flap
    ...
    def test_consume_accepts_legacy_marker_without_hermes_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Ouro versions have no
...
class TestScopedLockTakeover:
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        replacer_home.mkdir()
        monkeypatch.setenv("OURO_HOME", str(replacer_home))
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``ouro gateway stop`` would be
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the OURO_HOME contextvar override (#56986), so the profile
