"""Global emergency stop (`ouro pause` / `ouro resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`ouro resume`)
...
def hermes_home(tmp_path, monkeypatch):
    """Point OURO_HOME at a temp dir and reset estop module log state."""
...
def test_is_engaged_fails_safe_on_stat_error(hermes_home, monkeypatch):
    ...
    hold even when OURO_HOME is misbehaving, matching the module's
...
async def test_gateway_slash_commands_bypass_estop(hermes_home):
    ...
    try:
        ...
        return
    assert reply is None or "ouro is paused" not in (reply or "").lower()
