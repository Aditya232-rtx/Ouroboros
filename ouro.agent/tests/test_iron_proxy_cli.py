...
def hermes_home(tmp_path, monkeypatch):
    """Point OURO_HOME at a temp dir so the wizard doesn't touch the
...
# ---------------------------------------------------------------------------
# _load_env_file_into_environ — setup discovers keys kept only in ~/.ouro/.env
# ---------------------------------------------------------------------------
...
def test_register_cli_uses_egress_command_dest():
    ...
    the inbound OAuth 'ouro proxy' subparser (dest='proxy_command').
    A future grep-and-refactor on proxy_command should not hit this
    subparser by accident."""

    parser = argparse.ArgumentParser(prog="ouro egress")
