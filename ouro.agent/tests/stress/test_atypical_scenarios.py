...
normal tests assume away:
  ...
  - Filesystem: OURO_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own OURO_HOME, collect failures.
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "OURO_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["OURO_HOME"]).resolve())
...
def _(home, kb):
    """OURO_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="ouro with spaces ")
    os.environ["OURO_HOME"] = weird
    ...
    try:
        ...
        print(f"  OURO_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    """OURO_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  OURO_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    """OURO_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same OURO_HOME share DB correctly")
    finally:
