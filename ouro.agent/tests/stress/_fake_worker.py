...
Reads OURO_KANBAN_TASK from env, heartbeats periodically, does short
work, completes via the CLI. Designed to be spawned by the dispatcher
exactly the way `ouro chat -q` would be, minus the LLM cost.
"""
...
def main():
    tid = os.environ["OURO_KANBAN_TASK"]
    workspace = os.environ.get("OURO_KANBAN_WORKSPACE", "")
