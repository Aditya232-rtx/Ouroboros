"""Shared fixtures for the ouro-agent test suite.
...
2. **Isolated OURO_HOME.** OURO_HOME points to a per-test tempdir so
   code reading ``~/.ouro/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".ouro"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No OURO_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox OURO_HOME before ANY test module is imported ──────────────────
...
# real `~/.ouro/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes OURO_HOME, but fixtures run
...
# the REAL Ouro root — capture it BEFORE the sandbox rewires OURO_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.ouro (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("OURO_KANBAN_HOME", "").strip()
_PRE_SANDBOX_OURO_HOME = os.environ.get("OURO_HOME", "")


def _hermes_home_points_at_production(value: str) -> bool:
    """True when a pre-set OURO_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    OURO_HOME=~/.ouro``) hand pytest the PRODUCTION home. Historically
    ...
    inside the real ``~/.ouro`` — the escape vector that landed pytest
    ...
    OURO_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".ouro").resolve()
    except Exception:
        return True
...
if _hermes_home_points_at_production(os.environ.get("OURO_HOME", "")):
    _SESSION_OURO_HOME = tempfile.mkdtemp(prefix="hermes-test-home-")
    os.environ["OURO_HOME"] = _SESSION_OURO_HOME
    atexit.register(shutil.rmtree, _SESSION_OURO_HOME, True)
...
# the child at the same moment the child lost the OURO_HOME redirect.
# OURO_TEST_ISOLATION is OUR marker: exported here (before any test module
...
# open a real DB must export OURO_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["OURO_TEST_ISOLATION"] = os.environ.get("OURO_HOME", "") or "1"

#: OURO_HOME as it stood when conftest was imported - i.e. before any test
...
OURO_HOME_AT_CONFTEST_IMPORT = os.environ.get("OURO_HOME", "")
...
# OURO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_OURO_BEHAVIORAL_VARS = frozenset({
    ...
    "OURO_MAX_ITERATIONS",
    "OURO_SESSION_PLATFORM",
    "OURO_SESSION_CHAT_ID",
    "OURO_SESSION_CHAT_NAME",
    "OURO_SESSION_CHAT_TYPE",
    "OURO_SESSION_THREAD_ID",
    "OURO_SESSION_SOURCE",
    "OURO_SESSION_KEY",
    "OURO_GATEWAY_SESSION",
    "OURO_CRON_SESSION",
    "_OURO_GATEWAY",
    "OURO_PLATFORM",
    "OURO_MODEL",
    "OURO_INFERENCE_MODEL",
    "OURO_INFERENCE_PROVIDER",
    "OURO_TUI_PROVIDER",
    "OURO_MANAGED",
    "OURO_MANAGED_DIR",
    "OURO_DEV",
    ...
    "OURO_TIMEZONE",
    "OURO_REDACT_SECRETS",
    "OURO_BACKGROUND_NOTIFICATIONS",
    "OURO_EXEC_ASK",
    "OURO_HOME_MODE",
    "OURO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.ouro/kanban.db instead of the per-test OURO_HOME.
    "OURO_KANBAN_DB",
    "OURO_KANBAN_BOARD",
    "OURO_KANBAN_HOME",
    "OURO_KANBAN_WORKSPACES_ROOT",
    "OURO_KANBAN_LOGS_ROOT",
    "OURO_KANBAN_TASK",
    "OURO_KANBAN_WORKSPACE",
    "OURO_KANBAN_RUN_ID",
    "OURO_KANBAN_CLAIM_LOCK",
    "OURO_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "OURO_DASHBOARD_OAUTH_CLIENT_ID",
    "OURO_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and OURO_HOME to per-test tempdirs so code that
    reads ``~/.ouro/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral OURO_* vars that could change test semantics.
    for name in _OURO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("OURO_HONCHO_HOST", "hermes")

    # 3. Redirect OURO_HOME to a per-test tempdir. Code that reads
    #    ``~/.ouro/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.ouro/*`` via
    #    ``Path.home() / ".ouro"`` instead of ``get_hermes_home()``
    ...
    monkeypatch.setenv("OURO_HOME", str(fake_hermes_home))
    ...
    monkeypatch.setenv("OURO_TEST_ISOLATION", str(fake_hermes_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("OURO_STATE_DB_GUARD_BYPASS", raising=False)
    ...
    monkeypatch.setenv("OURO_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.ouro/plugins/ (which, per step 3, is now empty — but the
    ...
    try:
        ...
        # previous test's tmp_path OURO_HOME can't leak forward. Paths
...
# invocation), kanban writes silently pollute the real ~/.ouro. This autouse
...
# allow-list because test-level fixtures legitimately move OURO_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session OURO_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``OURO_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Ouro root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_OURO_HOME and not _hermes_home_points_at_production(
        _PRE_SANDBOX_OURO_HOME
    ):
        # OURO_HOME was genuinely set to a CUSTOM root before the sandbox
    ...
    # No pre-existing OURO_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".ouro").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.ouro`` captured at import time. Hermetic tests that legitimately
    move OURO_HOME to sibling tempdirs are unaffected.
...
# path that resolves inside the REAL Ouro root. This fixture wires the
# test-side knobs:
...
#     installs where OURO_HOME is not ~/.ouro) into the guard's
...
def _state_db_write_guard(request, monkeypatch):
    ...
    if _PRE_SANDBOX_OURO_HOME and not _hermes_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_OURO_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Ouro home override redirects every later
...
@pytest.fixture()
def mock_config():
    """Return a minimal ouro config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``ouro-gateway`` process
...
#    ``check_output`` reject any ``systemctl ... <verb> ouro-gateway``
...
def _wal_is_usable() -> bool:
    """True when Ouro will actually put a database into WAL mode here.

    Ouro refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Ouro can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Ouro managed runtime on 3.53.1
    ...
    redirects ``OURO_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.ouro/state.db`` for the whole session. Tests then read live
...
#      process environment: ``os.environ["OURO_VOICE_TTS"] = "1"``. The
...
# OS gating
#
# Ouro runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.ouro``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Ouro falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Ouro uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart ouro-gateway"``,
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _OURO_TOKENS)
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live ouro-gateway systemd unit. Mock "
        ...
        # Block any subprocess that would run `ouro update` (or the
        ...
        if "update" in low and (
            # ouro update / ouro update --gateway / setsid bash -c ... ouro update
            ...
            raise RuntimeError(
                ...
                "`ouro update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_OURO_BEHAVIORAL_VARS``: the env blanking stops the flag
