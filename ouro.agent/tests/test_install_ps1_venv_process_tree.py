"""Windows installer regression for Ouro children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Ouro process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    install_dir = hermes_home / "ouro-agent"
    venv_scripts = install_dir / "venv" / "Scripts"
    runtime_dir = hermes_home / ".ouro-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Ouro launchers
