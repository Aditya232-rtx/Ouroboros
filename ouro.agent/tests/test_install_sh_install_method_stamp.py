...
($INSTALL_DIR), not into the shared $OURO_HOME.

Background (shared-$OURO_HOME bug)
------------------------------------
$OURO_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.ouro:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$OURO_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``ouro update``).
...
``~/.ouro/hermes-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$OURO_HOME/\.install_method"', text), (
        "install.sh must not stamp $OURO_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `ouro update`"
    )
