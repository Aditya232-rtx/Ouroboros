...
`docker exec <c> ouro login` would write /opt/data/auth.json as
...
failed: Ouro is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> ouro …`` (defaulting to root) gets dropped to the
   ouro user before the real binary runs.
2. ``docker exec --user ouro <c> ouro …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $OURO_HOME from a ``docker exec`` session land
   as hermes:ouro — the actual user-visible invariant.
4. The OURO_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``$OURO_HOME/logs/container-boot.log``: it is written by
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `ouro config set` which writes config.yaml under OURO_HOME.
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    invokes `s6-setuidgid ouro ouro <args>` which resolves `hermes`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> ouro login`` (root) wrote
    ...
    "Ouro is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `ouro config set`-style
    ...
    Specifically: pretend the operator ran `ouro login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the ouro UID. We use `ouro auth list` since that touches the
    ...
    # `ouro auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `ouro config set` first to
    # provoke a write into OURO_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # OURO_HOME that docker exec just wrote.
    r = subprocess.run(
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the ouro user "
