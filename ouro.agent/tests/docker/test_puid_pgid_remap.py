...
Build the real image and verify the actual runtime behavior:
  ...
  1. PUID/PGID env vars remap the ouro user UID/GID at boot
  2. OURO_UID/OURO_GID take precedence over PUID/PGID aliases
...
def test_puid_pgid_remaps_hermes_user(
    ...
    """PUID=1000 PGID=1000 must remap the ouro user to UID 1000."""
    ...
    assert r.stdout.strip() == "1000", (
        f"expected ouro UID 1000 after PUID remap, got: {r.stdout.strip()}"
    )
    ...
    assert r.stdout.strip() == "1000", (
        f"expected ouro GID 1000 after PGID remap, got: {r.stdout.strip()}"
    )
...
def test_nas_low_uid_accepted(
    ...
    assert r.stdout.strip() == "99", (
        f"expected ouro UID 99, got: {r.stdout.strip()}"
    )

    r = docker_exec_sh(container_name, "id -g hermes", timeout=10)
    assert r.stdout.strip() == "100", (
        f"expected ouro GID 100, got: {r.stdout.strip()}"
    )
...
def test_remap_enables_data_volume_writes(
    ...
    """After remap, the ouro user must be able to write to /opt/data."""
    start_container(built_image, container_name, "PUID=1000", "PGID=1000")
    ...
    assert "WRITE_OK" in r.stdout, (
        f"ouro user cannot write to /opt/data after remap: {r.stdout}"
    )