...
def test_dashboard_service_resets_home(
    ...
    container with OURO_DASHBOARD=1 and verify the dashboard process
    ...
    the process env.
    """
    start_container(built_image, container_name, "OURO_DASHBOARD=1", "OURO_DASHBOARD_HOST=127.0.0.1")

    # Check if the dashboard process is running and inspect its HOME.
    r = docker_exec_sh(
        container_name,
        # Find the dashboard process (ouro dashboard) and read its HOME
        # from /proc/<pid>/environ. If not running, verify the run script
        # itself exports HOME=/opt/data by grepping the script source.
        'pid=$(pgrep -f "ouro dashboard" | head -1); '
        'if [ -n "$pid" ]; then '
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    OURO_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "OURO_DASHBOARD=1", "OURO_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to hermes:ouro on every boot.
