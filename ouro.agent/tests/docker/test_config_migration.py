...
Build the real image and verify: a config.yaml present in $OURO_HOME
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $OURO_HOME must be migrated on boot by
    docker_config_migrate.py, running as the ouro user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $OURO_HOME: {r.stdout}"
    )
    ...
    # Verify config.yaml is owned by ouro (migration ran as hermes)
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() == "hermes", (
        f"config.yaml not owned by ouro (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
