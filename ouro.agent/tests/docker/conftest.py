...
Override the image with ``OURO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("OURO_TEST_IMAGE", "hermes-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``OURO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("OURO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged ouro user
# ---------------------------------------------------------------------------
#
# Background: every Ouro runtime path inside the container drops to UID
