...
package-lock.json. Without OURO_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV OURO_TUI_DIR=/opt/hermes/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the ouro user, return stdout."""
    inner = (
        "source /opt/hermes/.venv/bin/activate && "
        "cd /opt/hermes && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the ouro user (UID 10000) so we exercise the same path the
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    """OURO_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$OURO_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/hermes/ui-tui", (
        f"OURO_TUI_DIR={r.stdout.strip()!r} (expected /opt/hermes/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    # With OURO_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
