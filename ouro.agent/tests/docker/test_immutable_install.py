...
Build the real image and verify at runtime:
  ...
  1. /opt/hermes is not writable by the ouro user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and OURO_DISABLE_LAZY_INSTALLS are set
  3. /opt/hermes/.install_method contains "docker" (code-scoped stamp)
  4. $OURO_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $OURO_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The ouro user must not be able to modify /opt/hermes.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/hermes as the ouro user
        "touch /opt/hermes/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"ouro user can write to /opt/hermes (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    assert "WRITE_FAILED" in r.stdout, (
        f"ouro user can write to /opt/hermes/.venv: {r.stdout}"
    )
...
def test_hermes_disable_lazy_installs_and_dont_write_bytecode(
    ...
    OURO_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $OURO_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $OURO_HOME was not healed on boot: "
        f"{r.stdout}"
    )
