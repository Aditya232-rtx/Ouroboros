...
def test_arbitrary_user_uid_rejected(
    ...
    assert "OURO_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_hermes_uid_works(
    ...
    """docker run --user 10000:10000 (the ouro UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (ouro UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout