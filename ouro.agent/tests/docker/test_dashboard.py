"""Harness: dashboard opt-in via OURO_DASHBOARD.

Today (tini): dashboard starts once when OURO_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    """Without OURO_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "ouro dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without OURO_DASHBOARD"
    )
...
# whenever `OURO_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``OURO_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "OURO_DASHBOARD=1",
        "OURO_DASHBOARD_HOST=0.0.0.0",
        "OURO_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"OURO_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and OURO_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``OURO_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
