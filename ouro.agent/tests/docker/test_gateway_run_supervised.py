...
crash and a supervised dashboard alongside (when ``OURO_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``OURO_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``OURO_S6_SUPERVISED_CHILD`` sentinel breaks the loop.
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → ouro gateway run) can take
    ...
    # (main-wrapper.sh → python → ouro gateway run) can take well over
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The OURO_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``ouro gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``ouro gateway
    ...
    # (main-wrapper.sh → python → ouro gateway run) can take well
    ...
    # started either — so we'd see the CMD's `ouro gateway run`
    ...
    # `ouro gateway run`. If the recursion guard fails, s6 would
    ...
    assert n <= 1, (
        f"expected at most one supervised python `ouro gateway run` "
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``OURO_DASHBOARD=1`` is set, ``docker run <image> gateway
