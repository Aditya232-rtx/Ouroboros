...
``~/.ouro`` before the per-test ``OURO_HOME`` redirect, which makes tests
...
def test_conftest_does_not_import_hermes_state_at_collection():
    ...
    ~/.ouro, so tests read live production sessions instead of a tempdir.
    ...
    def guard(name, *args, **kwargs):
        if name == "hermes_state" or name.startswith("hermes_state."):
            ...
            raise AssertionError(
                ...
                "DEFAULT_DB_PATH from the real ~/.ouro during collection"
            )
