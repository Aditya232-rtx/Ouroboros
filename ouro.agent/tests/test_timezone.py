...
class TestHermesTimeNow:
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["OURO_TIMEZONE"] = "UTC"
        result = hermes_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["OURO_TIMEZONE"] = "America/New_York"
        result = hermes_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["OURO_TIMEZONE"] = "Europe/London"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When OURO_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) OURO_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("OURO_TIMEZONE=" + os.environ.get("OURO_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "OURO_TIMEZONE=NOT_SET" in result["output"], (
            "OURO_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When OURO_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("OURO_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Ouro tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Ouro tz
        ...
        # Use a Ouro timezone far behind UTC so that the numeric wall time
        ...
        os.environ["OURO_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hermes_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Ouro timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["OURO_TIMEZONE"] = "US/Eastern"
        _reset_hermes_time_cache()
