...
1. `ouro secrets bitwarden setup --help` works (dispatch path)
2. `ouro update --check` works (update path)
3. `ouro secrets bitwarden disable` works (handler execution)
4. `ouro secrets onepassword status` works (lazy backend loads on demand)
...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run ouro CLI as a subprocess from repo root.
    ...
    connectivity — but ``ouro update --check`` really does ``git fetch``
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_setup_help(self) -> None:
        """`ouro secrets bitwarden setup --help` must exit 0 and print usage.
    ...
    def test_bitwarden_status(self) -> None:
        """`ouro secrets bitwarden status` must exit 0 (runs lazy backend)."""
    ...
    def test_bitwarden_disable(self) -> None:
        """`ouro secrets bitwarden disable` must exit 0."""
    ...
    def test_onepassword_status(self) -> None:
        """`ouro secrets onepassword status` must exit 0 (1Password lazy backend)."""
    ...
    def test_onepassword_setup_help(self) -> None:
        """`ouro secrets onepassword setup --help` must exit 0."""
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `ouro update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`ouro update --check` must not load cryptography._rust."""
