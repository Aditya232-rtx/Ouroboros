...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`ouro skills list` reads the active profile's config via
    OURO_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from OURO_PLATFORM env."""
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `ouro skills search`
