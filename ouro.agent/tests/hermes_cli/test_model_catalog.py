...
def isolated_home(tmp_path, monkeypatch):
    """Isolate OURO_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".ouro"
    home.mkdir()
...
class TestFallbackChain:
    ...
    PRIMARY = "https://ouro.dev/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/Aditya232-rtx/ouro"
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``ouro model`` CLI — not the live
        ...
        # ``OURO_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` OURO_HOME directly instead.
        import importlib
        from hermes_cli import model_catalog
        ...
        try:
            ...
            active_home = Path(os.environ["OURO_HOME"])
