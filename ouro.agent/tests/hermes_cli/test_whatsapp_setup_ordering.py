...
Before the fix, ``ouro whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``ouro gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OURO_HOME", str(hermes))
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``ouro whatsapp`` with an existing paired session and
