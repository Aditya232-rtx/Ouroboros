...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh OURO_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("OURO_HOME", test_home)
    # Force-reimport so the fresh OURO_HOME is picked up.
    ...
    # gives each test its own monkeypatched OURO_HOME so no cross-test
    # contamination.
