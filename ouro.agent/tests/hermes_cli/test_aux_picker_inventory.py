...
Every aux picker (``ouro model`` → Configure auxiliary models, the
``ouro tools`` vision picker, and any future one) must route through
...
def configured_home(tmp_path, monkeypatch):
    """A OURO_HOME with one ``providers:`` entry and one legacy
