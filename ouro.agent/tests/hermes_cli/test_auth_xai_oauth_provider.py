"""Tests for xAI Grok OAuth — tokens stored in Ouro auth store (~/.ouro/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Ouro auth store at the given root."""
...
# ``XAI_BASE_URL`` / ``OURO_XAI_BASE_URL`` are a credential-leak vector
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``ouro model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``ouro model``
    ...
    assert "ouro model" not in rendered.lower()
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Ouro that didn't validate) must not be silently honored on the refresh
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `ouro model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `ouro auth list` reflects truth and
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``ouro auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `ouro auth remove xai-oauth 1`.
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Ouro invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``ouro model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``ouro model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``ouro auth
    ...
    (hermes_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("OURO_HOME", str(hermes_home))
    monkeypatch.delenv("OURO_XAI_BASE_URL", raising=False)
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`ouro auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``ouro auth status``, ``ouro setup``, and the
