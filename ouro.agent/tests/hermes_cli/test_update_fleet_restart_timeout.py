...
On hosts with many profile-backed ``ouro-gateway*.service`` units,
``ouro update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "ouro-gateway-xiaomo1",
            "ouro-gateway-xiaomo2",
            "ouro-gateway-xiaomo3",
            "ouro-gateway-xiaomo4",
            "ouro-gateway-xiaomo5",
            "ouro-gateway-xiaomo6",
            "ouro-gateway-xiaomo7",
            "ouro-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "ouro-gateway-xiaomo5":
        ...
        assert failed == ["ouro-gateway-xiaomo5"]
        assert restarted == [
            "ouro-gateway-xiaomo1",
            "ouro-gateway-xiaomo2",
            "ouro-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "ouro-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "ouro-gateway-coder.service loaded active running",
        ...
        assert seen == ["ouro-gateway-coder"]

    def test_hermes_serve_units_are_included(self):
        # #83438 — ouro update restarted ouro-gateway* units but left
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "ouro-gateway.service loaded active running",
                    "",
                ]
            ),
            process_unit=seen.append,
        ...
        assert seen == ["hermes-serve", "hermes-serve-work", "ouro-gateway"]
    ...
    def test_hermes_gateway_near_prefix_is_rejected(self):
        ...
        # ``ouro-gateway-<profile>``, so a hypothetical
        # ``ouro-gatewayd.service`` must not enter the restart path.
        seen: list[str] = []

        _for_each_systemd_gateway_unit(
            _list_units_stdout(["ouro-gatewayd", "ouro-gateway-coder"]),
            process_unit=seen.append,
            on_unit_timeout=lambda *_: pytest.fail("unexpected timeout"),
        )
...
class TestGracefulSigusr1Eligibility:
    def test_gateway_units_are_eligible(self):
        assert _service_unit_supports_graceful_sigusr1_restart("ouro-gateway")
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["ouro-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["ouro-gateway-xiaomo5", "ouro-gateway-xiaomo6", "ouro-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("ouro-gateway-xiaomo5") == 1
        assert "ouro-gateway-xiaomo6" in out
