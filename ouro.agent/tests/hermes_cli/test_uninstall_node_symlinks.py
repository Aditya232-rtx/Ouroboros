...
~/.local/bin pointing into $OURO_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Ouro-managed node dir.
"""
...
def _make_hermes_node(hermes_home: Path) -> Path:
    """Create a fake $OURO_HOME/node/bin/{node,npm,npx} tree."""
