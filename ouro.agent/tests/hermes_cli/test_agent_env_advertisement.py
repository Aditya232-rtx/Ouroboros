"""Tests for the AI_AGENT / OURO_AGENT harness-attribution env vars.
...
Ouro-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Ouro' id in the public agent-harness
registry (``ouro-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Ouro process env, and survives the
cross-session leak guard that strips ``OURO_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__OURO_CWD_testsession0__"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'OURO_AGENT="${OURO_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT HERMES=$OURO_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "OURO_AGENT")}
        out = subprocess.run(
        ...
        outer_env = dict(clean_env, AI_AGENT="pi", OURO_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
