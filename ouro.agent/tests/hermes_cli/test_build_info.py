...
The build SHA is written by the Dockerfile's ``OURO_GIT_SHA`` build-arg
into ``<project_root>/.ouro_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    missing = tmp_path / ".ouro_build_sha"  # never created
...
def test_get_build_sha_respects_short_argument(tmp_path):
    ...
    sha_file = tmp_path / ".ouro_build_sha"
