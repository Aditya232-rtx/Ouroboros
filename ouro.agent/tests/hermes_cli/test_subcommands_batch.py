...
def test_config_get_unset_subcommands_parse():
    """`ouro config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `ouro login` fails gracefully, not with argparse error ────
#
# `ouro login` is a removed command; its handler (`login_command` in
# `hermes_cli/auth.py`) prints a deprecation notice pointing at `ouro auth` /
# `ouro model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `ouro --help`.
