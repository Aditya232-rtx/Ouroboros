...
When Ouro runs in Docker with ``OURO_UID=1000`` / ``OURO_GID=911``,
the entrypoint chowns the top-level ``OURO_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("OURO_UID", "1000")
        monkeypatch.setenv("OURO_GID", "911")
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("OURO_UID", raising=False)
        monkeypatch.delenv("OURO_GID", raising=False)
        from hermes_cli import config as cfg
