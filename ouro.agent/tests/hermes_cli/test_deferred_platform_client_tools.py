...
loader so ``ouro chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``ouro tools`` checklist. The same tools worked in gateway/web processes only
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`ouro plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `ouro plugins list` under-report what the process is carrying, and
