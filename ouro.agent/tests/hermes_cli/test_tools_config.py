...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/Aditya232-rtx/ouro/issues/1247
    """
    config = {
        "platform_toolsets": {
...
class TestPlatformToolsetConsistency:
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """ouro-gateway includes list should cover all messaging platforms."""
        from hermes_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["ouro-gateway"]["includes"])
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/Aditya232-rtx/ouro/issues/6901
    """
    config = {
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool._find_agent_browser (PATH -> Homebrew/Ouro-managed
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Ouro-managed-Node-only setups: npx resolves via
        ...
        hermes_npx = "/home/user/.ouro/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool._chromium_installed", return_value=False
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $OURO_HOME/bin when only side installs exist."""
        with patch(
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `ouro tools` → "reconfigure existing" shows
...
#    reported as added/removed by `ouro tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `ouro tools` printed ``- kanban``
...
# `ouro tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `ouro tools` save ────────
#
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `ouro config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_platforms_whose_composite_excludes_it_are_left_narrow():
    ...
    composite (ouro-acp, hermes-webhook) that never carried the toolset."""
