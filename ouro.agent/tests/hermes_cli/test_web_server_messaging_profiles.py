...
profile's OURO_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedMessagingReads:
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        monkeypatch.setattr(
            ...
            # relying on process-level OURO_HOME resolution (#71211).
            lambda *a, **k: {
        ...
        assert payload["gateway_start_command"] == (
            "ouro -p worker_alpha gateway start"
        )
