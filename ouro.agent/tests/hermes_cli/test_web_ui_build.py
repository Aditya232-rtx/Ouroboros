...
(mirroring the desktop build), recorded in a stamp file under $OURO_HOME,
NOT mtime comparison — so ``git pull`` / ``ouro update`` that rewrite
...
def _isolated_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OURO_HOME", str(tmp_path / "_hermes_home"))
...
class TestWebUIBuildNeeded:
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``ouro update`` do) must NOT report stale."""
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_workspace_root_install_names_update_closure(self, tmp_path, monkeypatch):
        ...
        closure as `ouro update` (ui-tui + web + --include-workspace-root).
