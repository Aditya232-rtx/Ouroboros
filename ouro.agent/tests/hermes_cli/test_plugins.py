"""Tests for the Ouro plugin system (hermes_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    Pass *home* explicitly whenever the target Ouro home for this
    plugin isn't necessarily the current ``OURO_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``OURO_HOME``/``set_hermes_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <OURO_HOME>/config.yaml.
        # Config is always read from OURO_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            hermes_home = Path(home)
        else:
            import os
            hermes_home_str = os.environ.get("OURO_HOME")
            if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_portable_author_object_is_normalized_to_stable_string(
        ...
        bundled.mkdir()
        monkeypatch.setenv("OURO_HOME", str(home))
        monkeypatch.setenv("OURO_BUNDLED_PLUGINS", str(bundled))
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "hermes_test"))
...
class TestPluginLoading:
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $OURO_HOME/plugins/.
        hermes_home = tmp_path / "hermes_test"
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `ouro plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_plugin_manager_scoped_by_hermes_home_override(self, tmp_path):
        ...
        subagent/embedded callers: it swaps ``OURO_HOME`` via a
        ...
        ``OURO_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('OURO_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['OURO_HOME'] itself (simulating
...
class TestPluginDebugLogging:
    """OURO_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("OURO_PLUGINS_DEBUG", raising=False)
        from hermes_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from OURO_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """OURO_HOME at the root resolves to 'default'."""
        home = tmp_path / ".ouro"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """OURO_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".ouro" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OURO_HOME", str(prof))
