...
def _client():
    ...
    # Keep the state DB under the isolated OURO_HOME for any handler that
    # touches it.
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.ouro/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `ouro auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `ouro auth add`, which clears every suppression for the
...
class TestUpdateCheckEndpoint:
    ...
    ``POST /api/hermes/update`` runs ``ouro update``.
    """
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `ouro tools` configurator."""
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    _OURO_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    assert "_OURO_GATEWAY" not in captured["env"]
    assert captured["env"]["OURO_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    ...
    startup under OURO_DESKTOP=1 so the stale orphan is cleared first.
    """
    import hermes_cli.web_server as ws
