...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`ouro gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `ouro gateway restart` in the background; when a
        ...
        assert "ouro gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``OURO_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` OURO_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "ouro-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir OURO_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="OURO_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="OURO_HOME=/tmp/hermes-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="OURO_HOME={tmp_path}/hermes-home"\n'
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "ouro gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".ouro" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `ouro gateway start` cannot strip the FD floor and
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>OURO_HOME</key>"
                "<string>/Users/alice/.ouro</string></plist>"
            ),
        )
...
class TestSystemUnitHermesHome:
    """OURO_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".ouro" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".ouro")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_hermes = target_home / ".ouro"
        root_home = tmp_path / "root"
        root_hermes = root_home / ".ouro"
        ...
        monkeypatch.setenv("OURO_HOME", str(root_hermes))
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        monkeypatch.delenv("OURO_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'OURO_HOME=/home/alice/.ouro' in unit
        assert '/root/.ouro' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's OURO_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_hermes_home().resolve())
        assert f'OURO_HOME={hermes_home}' in unit


class TestSystemUnitRefreshSyncsHermesHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.ouro."""

    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_hermes = root_home / ".ouro"
        alice_hermes = alice_home / ".ouro"
        root_hermes.mkdir(parents=True)
        ...
        monkeypatch.delenv("OURO_RESTART_DRAIN_TIMEOUT", raising=False)

        # Correct installed unit (operator's OURO_HOME + drain timeout).
        monkeypatch.setenv("OURO_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited OURO_HOME (falls back to root).
        monkeypatch.setenv("OURO_HOME", str(root_hermes))
        ...
        assert os.environ["OURO_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned OURO_HOME *before* it reads/compares the unit.
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "ouro gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".ouro" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("OURO_HOME", str(root_profile))
        ...
        assert f'OURO_HOME={target_home / ".ouro" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".ouro" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OURO_HOME", str(profile_dir))
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".ouro" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".ouro" / "ouro-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".ouro" / "ouro-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".ouro" / "ouro-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("OURO_HOME", str(root_home / ".ouro"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".ouro")
        ...
        # WorkingDirectory is anchored at the target user's OURO_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.ouro" in unit
        assert "WorkingDirectory=/home/alice/.ouro/hermes-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "ouro-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "ouro-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'ouro gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``ouro-gateway.service``. After PR #5646 (signal recovery
    ...
    (``ouro-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          ...
          - ouro gateway run   (direct binary)
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "ouro gateway migrate-legacy" in out
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (ouro-gateway-coder.service)
        ...
        profile_unit = user_dir / "ouro-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "ouro-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `ouro gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `ouro gateway --help` shows it.
        import subprocess
        import sys
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "ouro-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "ouro-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "ouro-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``ouro gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "ouro-gateway")
        ...
        assert "sudo systemctl start ouro-gateway" in out
        assert "sudo ouro gateway uninstall --system" in out
        assert "ouro gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``ouro gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (OURO_HOME), never the source checkout / worktree, so a relocated or
