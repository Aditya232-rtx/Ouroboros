...
class TestIsSkillDisabled:
    ...
    @patch("hermes_cli.config.load_config")
    @patch.dict("os.environ", {"OURO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """OURO_SESSION_PLATFORM should be used when OURO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_hermes_platform_takes_precedence(self, tmp_path, monkeypatch):
        """OURO_PLATFORM should win over OURO_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
