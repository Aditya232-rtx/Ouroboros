...
def _make_agent(hermes_home: Path) -> Path:
    ...
    agent_root = hermes_home / "ouro-agent"
...
def _make_gui_build(hermes_home: Path) -> None:
    """Create the source-built GUI artifacts a `ouro desktop` run produces."""
    desktop = hermes_home / "ouro-agent" / "apps" / "desktop"
    ...
    (hermes_home / "ouro-agent" / "node_modules").mkdir(parents=True)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `ouro desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    hermes_home = tmp_path / ".ouro"
    _make_agent(hermes_home)
    icon = lde.icon_path(hermes_home / "ouro-agent")
    ...
    assert lde.icon_path(hermes_home / "ouro-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (hermes_home / "ouro-agent" / "hermes_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".ouro")
