"""`ouro update` must self-heal the ``ouro-acp`` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``ouro-acp`` command name on the login-shell PATH. Fresh installs get the
launcher from ``scripts/install.sh``; existing installs get it from
``_ensure_acp_launcher()`` during ``ouro update``.
"""
...
def test_unwritable_bin_dir_is_skipped(fake_home):
    ...
    try:
        ...
        assert not (fake_home / "ouro-acp").exists()
    finally:
