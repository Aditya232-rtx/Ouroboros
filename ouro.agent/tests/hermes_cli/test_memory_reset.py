"""Tests for the `ouro memory reset` CLI command.
...
- Profile-scoped reset (uses OURO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake OURO_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.ouro/hermes-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `ouro memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `ouro memory reset` subcommand."""
