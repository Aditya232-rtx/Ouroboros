"""Tests for Codex auth — tokens stored in Ouro auth store (~/.ouro/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Ouro auth store."""
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``ouro auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``ouro auth add openai-codex``.
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `ouro auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``ouro auth add openai-codex`` for accounts B and C
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``ouro auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Ouro auth store, not ~/.codex/."""
    hermes_home = tmp_path / "hermes"
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Ouro store
    assert not (codex_home / "auth.json").exists()

    # Ouro auth store should have the tokens
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run ouro auth".
    """
    from hermes_cli.auth import (
    ...
    assert "ouro auth" not in rendered
