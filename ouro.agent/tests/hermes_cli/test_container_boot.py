...
These tests run against a fake $OURO_HOME under tmp_path; no real
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running ouro containers, those
...
def _make_profile(
    ...
    *,
    ...
    if config:
        ...
        # `ouro profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    OURO_HOME root (the implicit default profile)."""
    if state is not None:
