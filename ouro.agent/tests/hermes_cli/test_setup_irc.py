"""Tests for IRC gateway configuration via `ouro setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Ouro install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Ouro install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `ouro setup gateway` experience with IRC present."""
