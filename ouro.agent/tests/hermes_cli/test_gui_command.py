"""Tests for ``ouro gui`` desktop launcher wiring."""
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("OURO_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Ouro.app" / "Contents" / "MacOS" / "Ouro"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Ouro.exe"
    else:
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Ouro-managed
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Ouro.app/Contents/MacOS/Ouro in place. Re-downloading Electron can't
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Ouro.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Ouro.app"
    _write_info_plist(app, "com.ouro.agent")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Ouro").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Ouro Helper.app"
    _write_info_plist(helper, "com.ouro.agent.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Ouro.app component, silently skipping every .node/.dylib/crashpad
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`ouro desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    assert launch_env["OURO_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["OURO_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("OURO_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["OURO_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "OURO_DESKTOP_PASSWORD_STORE" not in launch_env
