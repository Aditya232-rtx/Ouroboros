...
the SAME provider universe as ``ouro model`` (the CLI/TUI picker).
...
def test_every_hermes_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `ouro model` universe."""
    ...
    assert not missing, (
        "providers shown in `ouro model` but not configurable in the desktop "
