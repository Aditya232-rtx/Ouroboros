...
URLs and the bootstrap ``__OURO_BASE_PATH__``). The OAuth gate must
honour it too:
...
class TestForwardedPrefixNormalisation:
    def test_home_assistant_ingress_prefix_with_subpath_is_accepted(
        ...
        the SPA receives the correct __OURO_BASE_PATH__ and asset prefix."""
...
# ---------------------------------------------------------------------------
# OURO_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``OURO_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "OURO_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("OURO_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set OURO_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("OURO_DASHBOARD_PUBLIC_URL", "hermes.domain.com")
        ...
        assert any(
            "OURO_DASHBOARD_PUBLIC_URL" in m
