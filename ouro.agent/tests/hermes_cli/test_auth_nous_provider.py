...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OURO_HOME", str(hermes_home))
    monkeypatch.setenv("OURO_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `ouro model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `ouro auth add nous`
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Ouro must surface an
    ...
    re-authenticate" string led users to report this as a Ouro persistence
    ...
    assert "ouro auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect OURO_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("OURO_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without OURO_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.ouro/shared/`` across CI runs.
    """
    from hermes_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("OURO_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `ouro auth add nous
