...
1. The venv-repair import probes (early recovery + `ouro update`) must
...
2. ``ouro doctor`` must fail the certificate check in that state, and
   ``ouro doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `ouro update`'s venv repair must apply
...
# =========================================================================
# 2. ouro doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "ouro doctor --fix" in message
