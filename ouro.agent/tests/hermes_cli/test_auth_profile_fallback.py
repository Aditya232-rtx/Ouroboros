...
When ``OURO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.ouro/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.ouro            (has its own auth.json fixture)
    * Profile     -> tmp_path/.ouro/profiles/coder   (active, OURO_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Ouro is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (OURO_HOME != ~/.ouro, no profiles)."""
