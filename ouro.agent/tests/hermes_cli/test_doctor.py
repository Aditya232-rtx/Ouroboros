...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "recreate all Ouro containers" in hint
        assert "ouro update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `ouro update`" in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `ouro doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "OURO_HOME", hermes_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("OURO_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_hermes_home(self, tmp_path, provider=""):
        """Create a minimal OURO_HOME with config.yaml."""
        home = tmp_path / ".ouro"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        monkeypatch.setattr(doctor_mod, "OURO_HOME", home)
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`ouro doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp OURO_HOME.

        ``run_doctor`` reads the module-level ``OURO_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("OURO_HOME")`` alone
        leaves doctor probing the REAL ~/.ouro. On a dev machine with a
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale OURO_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"OURO_MAX_ITERATIONS={ghost}\n")
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("OURO_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("OURO_MAX_ITERATIONS", raising=False)
    ...
    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        ...
        assert "OURO_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "OURO_MAX_ITERATIONS=90" in (hermes_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale OURO_MAX_ITERATIONS" in out
        env_after = (hermes_home / ".env").read_text(encoding="utf-8")
        assert "OURO_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_hermes_tool_progress_warning_says_unsupported_since_floor(self):
        """OURO_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"OURO_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["OURO_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"OURO_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["OURO_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"OURO_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: OURO_TOOL_PROGRESS_MODE" in out
