"""Regression tests: `ouro dashboard` validates OURO_WEB_DIST before serving.

A custom OURO_WEB_DIST without --skip-build previously skipped BOTH the
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """OURO_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("OURO_WEB_DIST", str(empty_dist))
    ...
    assert "OURO_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("OURO_WEB_DIST", raising=False)
