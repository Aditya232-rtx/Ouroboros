...
def _install_example_plugin(_isolate_hermes_home):
    """Drop the example-dashboard fixture into the per-test OURO_HOME
    ...
    ``OURO_BUNDLED_PLUGINS`` override because the bundled dir is
    ...
    # plugin in exactly as a real operator would with `ouro plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down OURO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Ouro vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints OURO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("OURO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("OURO_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        """Create a TestClient and isolate the state DB under the test OURO_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `ouro update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `ouro update` an old-schema state.db used to
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        OURO_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide OURO_HOME honcho.json; snapshot and
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn ouro update")
    ...
    def test_update_hermes_spawns_with_action_id(self, monkeypatch):
        ...
        assert calls == [
            (["update"], "hermes-update", {"OURO_ACTION_ID": "a" * 32})
        ]
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Ouro provider — keep the user's aggregator instead of writing a
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``ouro model`` custom flow. Regression for the desktop loop where a
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.ouro/dashboard-themes/."""
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        # Pin the SessionDB to the isolated OURO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        installs it into the per-test ``OURO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into OURO_HOME by the class-level
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        OURO_HOME override scopes a request to another profile."""
        from hermes_constants import (
            reset_hermes_home_override,
            set_hermes_home_override,
        )
        ...
        monkeypatch.setenv("OURO_HOME", str(launch_home))
        from hermes_cli import web_server
        token = set_hermes_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets OURO_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the ouro root's plugins/ directory."""
        ...
        monkeypatch.setenv("OURO_HOME", str(profile_home))
        from hermes_cli import web_server
        plugins = web_server._discover_dashboard_plugins()
...
# or ``sh -c 'printf …'``) instead of the real ``ouro --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        assert env["OURO_PYTHON"] == command
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("OURO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        the per-test ``OURO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_hermes_home):
        ...
        monkeypatch.setenv("OURO_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under OURO_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("OURO_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
