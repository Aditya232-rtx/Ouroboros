"""Tests for the stale-dashboard handling run at the end of ``ouro update``.

``ouro update`` detects ``ouro dashboard`` processes left over from the
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12345, "ouro dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestWindowsWmicEncoding:
    ...
    `ouro update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
...
class TestSupervisedBackendRestart:
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: ouro update's systemd fleet-restart loop may
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.ouro/profiles/mini-cat"),
        ]) == []
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.ouro/profiles/coder"),
        ])
        assert out == [a]
...
class TestCmdlineCapture:
    ...
    def test_falls_back_to_ps_without_proc(self, monkeypatch):
        ...
        def fake_run(args, *a, **kw):
            ...
            return MagicMock(returncode=0, stdout="ouro serve --port 8300\n", stderr="")
...
class TestPostUpdateStaleModuleReload:
    ...
    ``ouro update`` runs in the PRE-pull Python process. When the update
