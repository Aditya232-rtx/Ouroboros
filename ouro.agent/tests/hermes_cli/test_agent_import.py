"""Tests for hermes_cli.agent_import — ``ouro import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Ouro stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Path.home() and OURO_HOME are redirected to tmp_path so nothing touches
the real ~/.ouro.
"""
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and OURO_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, OURO_HOME -> tmp/.ouro."""
...
class TestExistingConfigPreserved:
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "ouro config edit" in reason
