...
# unprivileged ouro user from every lifecycle op. These tests run
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``ouro gateway run``, an agent action, the Open WebUI helper) holds
    the per-OURO_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root OURO_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `ouro gateway run --replace` (no -p flag).
    assert "ouro gateway run --replace" in default_text
    assert "ouro -p default" not in default_text
...
#
# `ouro gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_hermes_without_chown(
    ...
    assert 's6-setuidgid ouro mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid ouro rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_hermes_idx = log_text.index('s6-setuidgid ouro mkdir -p "$log_dir"')
    rm_as_hermes_idx = log_text.index('s6-setuidgid ouro rm -f "$log_dir/lock"')
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside OURO_HOME the way
    # a real ouro uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'OURO_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that ouro cannot unlink a foreign
