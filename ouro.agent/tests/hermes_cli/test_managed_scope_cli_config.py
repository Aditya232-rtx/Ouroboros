...
silently ignores administrator-pinned values while `ouro config`/`doctor`
...
def _load_cli_config(home):
    ...
    singleton), so monkeypatching OURO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap OURO_HOME.)
    """
    import cli
