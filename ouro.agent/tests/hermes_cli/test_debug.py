"""Tests for ``ouro debug`` CLI command and debug utilities."""
...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated OURO_HOME with minimal logs."""
    home = tmp_path / ".ouro"
    home.mkdir()
...
class TestMissingLogNote:
    ...
    `ouro debug share` runs on the backend, so a desktop connected to a
    ...
    def test_client_written_log_names_its_writer_and_path(self, hermes_home):
        ...
        assert "Ouro Desktop" in snap.tail_text
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of OURO_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated OURO_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".ouro"
        home.mkdir()
        monkeypatch.setenv("OURO_HOME", str(home))
        ...
        monkeypatch.delenv("OURO_REDACT_SECRETS", raising=False)
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        input unchanged when OURO_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("OURO_REDACT_SECRETS", "false")

        from hermes_cli.debug import _capture_log_snapshot

        assert os.environ.get("OURO_REDACT_SECRETS", "") == "false"
...
class TestRunDebugShare:
    ...
    def test_share_uploads_five_pastes(self, hermes_home, capsys):
        ...
        with patch("hermes_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- ouro dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestScheduleAutoDelete:
    ...
    ``~/.ouro/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``ouro debug``
    invocation.
    """
...
class TestBuildDebugShare:
    ...
    Backs both ``ouro debug share`` (CLI) and ``POST /api/ops/debug-share``
...
class TestShareConsentGate:
    """`ouro debug share` requires explicit consent before uploading.
