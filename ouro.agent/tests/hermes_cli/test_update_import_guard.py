"""Tests for the post-update *import* guard in ``ouro update``.
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("ouro update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``ouro update`` can run under a different Python than the install's.
