...
- Bare `ouro setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `ouro setup --quick` runs the narrower "fill in missing items" flow.
- `ouro setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `ouro setup` on an existing install = full reconfigure wizard."""
