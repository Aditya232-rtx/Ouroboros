...
class TestMigrate:
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Ouro' managed block, re-running
...
class TestStripUnmanagedPluginTables:
    ...
    ``ouro codex-runtime migrate`` would re-emit them inside the managed
...
# ---- Bug C: OURO_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``OURO_HOME``
    ...
    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate OURO_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.ouro"
        monkeypatch.setenv("OURO_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("OURO_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When OURO_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever OURO_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "OURO_HOME" not in env, (
            f"OURO_HOME should not be set when env var is unset, got: "
            f"{env.get('OURO_HOME')!r}"
        )
