"""Tests for hermes_cli.dump._get_git_commit — git SHA resolution for ``ouro dump``.

``ouro dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``OURO_GIT_SHA`` build-arg.
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``ouro dump`` doesn't have to special-case
