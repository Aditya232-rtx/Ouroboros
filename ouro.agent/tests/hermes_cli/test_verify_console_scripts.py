...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        ouro = "hermes_cli.main:main"
        ouro-agent = "run_agent:main"
        ouro-acp = "acp_adapter.entry:main"
    """
        )
    )
...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("hermes", "ouro-agent", "ouro-acp"):
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        assert {"hermes.exe", "ouro-agent.exe", "ouro-acp.exe"} <= names
        assert "ouro-gateway.exe" in names
