"""Tests for the direct Ouro-to-Relay shared-metrics runtime."""
...
def direct_runtime(tmp_path, monkeypatch):
    fake = _Relay()
    monkeypatch.setenv("OURO_HOME", str(tmp_path / "hermes-home"))
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert "Ouro Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    monkeypatch.setenv("OURO_MANAGED_DIR", str(managed))
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    assert "Ouro shared-metrics task flush failed" in caplog.text
