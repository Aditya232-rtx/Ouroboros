...
def hermes_home(tmp_path, monkeypatch):
    """Point OURO_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.ouro/auth.json under pytest.
    """
...
# --- sibling readers of the same ~/.ouro/auth.json in other modules -------
#
# _load_auth_store lives in hermes_cli/auth.py, but several other modules read
# the same ~/.ouro/auth.json directly. They had the same UTF-8-vs-cp1252
...
class TestAuthJsonSiblingReaders:
    ...
    def test_auxiliary_nous_provider_reads_non_ascii_store(self, hermes_home, windows_default_encoding, monkeypatch):
        ...
        # OURO_HOME env from the fixture doesn't reach it — point it at
