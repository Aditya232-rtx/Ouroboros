"""Tests for ``ouro doctor --live`` — opt-in bounded real-call tool-backend probes.
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `ouro doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("hermes_cli.doctor.OURO_HOME", tmp_path / "home")
