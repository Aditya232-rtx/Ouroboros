...
`OURO_REDACT_SECRETS` at module-import time. `hermes_cli/main.py` at
...
in .env), the toggle was silently ignored in both `ouro chat` and
`ouro gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `OURO_REDACT_SECRETS`
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env OURO_REDACT_SECRETS takes precedence over config.yaml."""
    hermes_home = tmp_path / ".ouro"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hermes_home / ".env").write_text("OURO_REDACT_SECRETS=true\n")
