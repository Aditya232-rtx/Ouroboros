...
The Python launcher (`ouro --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Ouro session
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.setenv("OURO_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hermes", "--tui"])
        monkeypatch.delenv("OURO_TUI", raising=False)
        monkeypatch.delenv("OURO_TUI_NO_EARLY_DISABLE", raising=False)
