"""Tests for the Nous-Ouro-3/4 non-agentic warning detector.
...
@pytest.mark.parametrize(
    "model_name",
    [
        "NousResearch/Ouro-3-Llama-3.1-70B",
        "NousResearch/Ouro-3-Llama-3.1-405B",
        "hermes-3",
        "Ouro-3",
        "hermes-4",
        "hermes-4-405b",
...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    assert is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} to be flagged as Nous Ouro 3/4"
    )
    assert _check_hermes_model_warning(model_name) == _OURO_MODEL_WARNING
