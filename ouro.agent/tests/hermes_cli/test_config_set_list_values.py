"""``ouro config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``ouro config set platform_toolsets.discord '["file","web"]'``
...
def user_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("OURO_MANAGED_DIR", raising=False)
