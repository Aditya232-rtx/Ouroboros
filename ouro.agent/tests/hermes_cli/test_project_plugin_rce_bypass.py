...
Execution via the ``OURO_ENABLE_PROJECT_PLUGINS`` bypass in the web
...
1. ``hermes_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.ouro/plugins/`` source via
   ``os.environ.get("OURO_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
# ---------------------------------------------------------------------------
# Layer 1 — OURO_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.ouro/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("OURO_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("OURO_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"OURO_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``OURO_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_ENABLE_PROJECT_PLUGINS", "0")
