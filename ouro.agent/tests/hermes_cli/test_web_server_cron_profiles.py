...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    ``~/.ouro`` when the request carries no explicit ``profile`` (the
    Desktop app's pre-profileScoped clients sent none)."""
    from hermes_cli import web_server

    monkeypatch.setenv(
        "OURO_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """OURO_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from hermes_cli import web_server

    monkeypatch.setenv("OURO_HOME", str(isolated_profiles["default"]))
