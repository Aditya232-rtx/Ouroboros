...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """ouro -z bypasses HermesCLI, but recall still needs SessionDB."""
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["OURO_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["OURO_MODEL"] == "nous/hermes-test"
    assert env["OURO_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["OURO_TUI_PROVIDER"] == "nous"
    assert env["OURO_INFERENCE_PROVIDER"] == "nous"
    assert env["OURO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["OURO_TUI_ACTIVE_SESSION_FILE"])
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("OURO_TUI_DIR", raising=False)
