...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("OURO_HOME", str(tmp_path / "hermes"))
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `ouro auth add nous` must also populate providers.nous so the
    ...
    # raises "Ouro is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`ouro auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``ouro auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``ouro auth list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """ouro auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``ouro auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`ouro logout --provider openai-codex` must still clear model.provider.
...
# =============================================================================
# Unified credential-source stickiness — every source Ouro reads from has a
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hermes_pkce must not re-seed from ~/.ouro/.anthropic_oauth.json when suppressed."""
    hermes_home = tmp_path / "hermes"
    hermes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("OURO_HOME", str(hermes_home))
