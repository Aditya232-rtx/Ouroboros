...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    content = "OURO_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    """Known Ouro keys inherited from parent process are removed when absent
    ...
    monkeypatch.setenv("OURO_ACP_AUTH_METHOD", "cursor_login")
    ...
    # OURO_ACP_AUTH_METHOD and COPILOT_CLI_PATH are NOT in the profile .env → cleared
    assert "OURO_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    Empty ``OURO_ACP_AUTH_METHOD=`` tells the ACP adapter to skip
    ...
    home = tmp_path / "hermes"
    home.mkdir()
    (home / ".env").write_text("OURO_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "OURO_ACP_AUTH_METHOD" in os.environ
    assert os.environ["OURO_ACP_AUTH_METHOD"] == ""
...
def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    ...
    assert os.getenv("OURO_ACP_AUTH_METHOD") == "cursor_login"
    assert os.getenv("PATH") == "/usr/bin:/bin"


def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Ouro key that IS explicitly set in the profile .env survives
    the cleanup (overrides the inherited value).
    """
    home = tmp_path / "hermes"
    home.mkdir()
    (home / ".env").write_text(
        "OURO_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("OURO_ACP_AUTH_METHOD") == "claude_code_cli"


def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Ouro key defined with the bash-compatible ``export KEY=value``
    ...
    home.mkdir()
    (home / ".env").write_text(
        "export OURO_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    monkeypatch.setenv("OURO_ACP_AUTH_METHOD", "cursor_login")
    load_hermes_dotenv(hermes_home=home)
    assert os.getenv("OURO_ACP_AUTH_METHOD") == "claude_code_cli"
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process OURO_HOME (a different profile's
...
def test_config_yaml_terminal_backend_overrides_stale_env(tmp_path, monkeypatch):
    """Regression for #29186: a leftover TERMINAL_ENV=docker in ~/.ouro/.env
