...
Ouro.exe on the end user's machine. Before this gate, "build succeeded" was
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Ouro.exe"
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Ouro.exe").exists()
...
# ─── end-to-end: `ouro desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Ouro.exe cannot launch
    ...
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "Ouro.exe", PE_AMD64)
