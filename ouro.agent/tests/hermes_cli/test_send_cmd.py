"""Tests for the ``ouro send`` CLI subcommand.
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``ouro send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``ouro config set``) lives
...
def test_load_hermes_env_utf8_bom_preserves_first_key(tmp_path, monkeypatch):
    ...
    `ouro send` failed to authenticate.
    """
    import os
