"""Guard: every `ouro update` path that reports user-modified skills must
also tell the user how to find them.

`ouro update` keeps (does not overwrite) bundled skills the user edited and
...
at ``ouro skills list-modified`` so the count is actionable — otherwise,
...
_HINT_RE = re.compile(r"ouro skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`ouro skills list-modified` within the following lines — the "
