...
def test_windows_native_install_path_docs_match_installer() -> None:
    ...
    assert "%LOCALAPPDATA%\\ouro\\ouro-agent\\bin" in doc
    assert (
        "Get-Command ouro        # should print "
        "C:\\Users\\<you>\\AppData\\Local\\ouro\\ouro-agent\\bin\\ouro.exe"
    ) in doc
    # Installer exposes $InstallDir\bin, and must copy the launchers into it.
    assert '$hermesBin = "$InstallDir\\bin"' in install
    assert "ouro.exe" in install and "ouro-acp.exe" in install
