...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    ouro subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("OURO_KANBAN_TASK") == tid
    assert env.get("OURO_PROFILE") == "some-profile"
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `ouro kanban --help` output"
    )
