"""Tests for ouro backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.ouro directory structure for testing."""
    ...
    # ouro-agent repo (should be EXCLUDED)
    (root / "ouro-agent").mkdir(exist_ok=True)
    (root / "ouro-agent" / "run_agent.py").write_text("# big file\n")
    (root / "ouro-agent" / ".git").mkdir()
    (root / "ouro-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        ...
        assert _should_exclude(Path("ouro-agent/run_agent.py"))
        assert _should_exclude(Path("ouro-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)

        monkeypatch.setenv("OURO_HOME", str(hermes_home))
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside OURO_HOME."""
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Ouro backup."""
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Ouro configuration has been restored." in out
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".ouro"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("OURO_HOME", str(src_home))
        ...
        dst_home = tmp_path / "dest" / ".ouro"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("OURO_HOME", str(dst_home))
        ...
        # ouro-agent should NOT be present
        assert not (dst_home / "ouro-agent").exists()
...
class TestBackupEdgeCases:
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        """Backup handles empty ouro home (no files to back up)."""
        hermes_home = tmp_path / ".ouro"
        hermes_home.mkdir()
...
class TestImportAtomicWrites:
    """`ouro import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the OURO_HOME branch
    ...
    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside OURO_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        hermes_home = dst_home / ".ouro"
        hermes_home.mkdir()
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo ouro import`` onto a user-owned (or Docker/NAS volume-owned)
        OURO_HOME would hand every restored file to root. The uid/gid is
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``ouro import`` is the
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake OURO_HOME with critical state files."""
        home = tmp_path / ".ouro"
        home.mkdir()
...
# ---------------------------------------------------------------------------
# Pre-update backup (ouro update safety net)
# ---------------------------------------------------------------------------
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``ouro update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``ouro backup`` would, and should exclude the same directories."""
        ...
        # ouro-agent repo excluded
        assert not any(n.startswith("ouro-agent/") for n in names)
    ...
    def test_skips_symlinked_files(self, hermes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside OURO_HOME."""
        from hermes_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point OURO_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("OURO_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve OURO_HOME dynamically and their caches are
...
# ---------------------------------------------------------------------------
# Pre-migration backup (ouro claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``ouro claw migrate`` runs before mutating ~/.ouro/."""
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Ouro
        backup — `ouro import` should accept it."""
...
class TestRestoreCronJobsIfEmptied:
    """`ouro update` config migration can leave cron/jobs.json valid-but-empty,
...
# location, NOT under OURO_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------
...
class TestMemoryProviderExternalPaths:
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under OURO_HOME,
        ...
        # External state did NOT leak into OURO_HOME.
