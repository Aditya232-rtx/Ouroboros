...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from hermes_cli.config import ...`` silently makes `ouro --version`
...
def test_fast_version_parity_off_termux(tmp_path):
    home = tmp_path / ".ouro"
    home.mkdir()
    result = _run_version({"OURO_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Ouro v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"OURO_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
    assert "Ouro v" in result.stdout
