"""Tests for `ouro memory setup [provider]` routing.
...
``ouro memory setup honcho``) without the interactive picker — which matters
because the per-provider ``ouro <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "ouro memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets OURO_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"OURO_DISABLE_LAZY_INSTALLS": "0"}), \
