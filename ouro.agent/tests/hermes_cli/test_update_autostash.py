...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``ouro update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    marker = tmp_path / ".ouro-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by ouro update (hermes_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "hermes-update-autostash")

    assert marker.exists(), (
        ".ouro-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".ouro-bootstrap-complete" not in status
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "ouro-agent.rb").write_text("formula\n")
    ...
    try:
        ...
        assert (pkg / "ouro-agent.rb").read_text() == "formula\n"
    finally:
        os.chmod(pkg, 0o755)
