...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Ouro", folders=["/tmp/hermes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "ouro-agent"
    assert proj.name == "Ouro"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "ouro-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' OURO_HOME.
