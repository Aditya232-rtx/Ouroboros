...
Covers:
- Defense 1: gateway stop/restart refuse when _OURO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "ouro gateway stop",
        "ouro  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.hermes.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.ouro/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l ouro-gateway-restart-helper -- /bin/sh helper.sh",
        ...
        (
            ...
            "  -- /bin/sh ~/.ouro/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated ouro note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "ouro cron list",
        "ouro update",
        "ouro config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `ouro gateway start` is benign — starting a gateway from inside a
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hermes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade ouro then run ouro gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # OURO_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("OURO_HOME", str(tmp_path / ".ouro"))
        scripts_dir = tmp_path / ".ouro" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify ouro gateway stop/restart refuse when _OURO_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_OURO_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_OURO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _OURO_GATEWAY=1.

    Issue #37453: systemctl --user restart ouro-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart ouro-gateway",
        "systemctl --user restart ouro-gateway",
        "systemctl stop ouro-gateway.service",
        "ouro gateway restart",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart ouro-gateway", force=True
        ))
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run ouro gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("ouro gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under OURO_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("ouro gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 ouro gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        ...
        RuntimeError when neither OURO_HOME nor HOME resolves
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run ouro gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_HOME", str(tmp_path / ".ouro"))
        (tmp_path / ".ouro").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run ouro gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop ouro-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart ouro-gateway' /var/log/syslog",
        "rg 'ouro gateway restart' /home/user/.ouro/logs/",
        "journalctl -u ouro-gateway --grep 'systemctl restart ouro-gateway'",
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell ouro gateway restart"',
        'psql -c "\\! systemctl restart ouro-gateway"',
        "grep 'systemctl restart ouro-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart ouro-gateway",
        'grep "$(systemctl restart ouro-gateway)" f',
        "grep 'restart' log; systemctl restart ouro-gateway",
        'sqlite3 db "SELECT 1"; ouro gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "ouro gateway restart",
        "sudo systemctl stop ouro-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart ouro-gateway%'\""
        )
