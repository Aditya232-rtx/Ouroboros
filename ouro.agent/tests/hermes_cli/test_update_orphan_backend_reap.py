...
guard refused on it and the update dead-ended with "Ouro is still running"
...
reap (with its full child tree — the managed .ouro-runtime interpreter
...
def test_backend_with_live_parent_keeps_refusal():
    parent = _proc(50, ["Ouro.exe"], create_time=10.0)
...
def test_orphan_root_plus_managed_runtime_descendant_qualifies():
    ...
    # serve root and its .ouro-runtime interpreter child. The child's live
    # parent IS the orphan root, so the set is safe — only the root is
    # returned (taskkill /T reaps the descendant with it).
    backend = _proc(200, _SERVE_ARGV, ppid=999)
    child_argv = [
        "C:\\hermes\\.ouro-runtime\\python\\generation-1\\python.exe",
        "worker.py",
    ]
