...
Covers:
 - Normal path (tokens already in Ouro auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Ouro
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `ouro auth openai-codex`. The old
...
def hermes_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Ouro auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(hermes_auth_only_env):
    """openai-codex appears when tokens are already in Ouro auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Ouro
    auth store."""
    hermes_home = tmp_path / ".ouro"
    hermes_home.mkdir()
