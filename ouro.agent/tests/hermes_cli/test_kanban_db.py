...
def kanban_home(tmp_path, monkeypatch):
    """Isolated OURO_HOME with an empty kanban DB."""
    home = tmp_path / ".ouro"
    home.mkdir()
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("OURO_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("OURO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Ouro' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Ouro' DB/metadata/logs on task completion.
    """
...
# spawned with `ouro -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's OURO_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's OURO_HOME."""

    def _set_home(self, monkeypatch, tmp_path, hermes_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OURO_HOME", str(hermes_home))
        monkeypatch.delenv("OURO_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.ouro/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.ouro/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.ouro/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # OURO_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile OURO_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # OURO_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is OURO_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["OURO_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["OURO_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["OURO_KANBAN_TASK"] == "t_dispatch_env"
        assert env["OURO_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "OURO_SESSION_SOURCE":
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'ouro kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    monkeypatch.delenv("OURO_BIN", raising=False)
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "Ouro" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
