...
This saves 500-650ms on ``ouro --help``, ``ouro version``,
``ouro logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``ouro --help`` in-process and parse the subcommand block.
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``ouro photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    def _fake_loader():
        ...
        # top-level ouro <name> CLI command via PluginContext.
