...
dashboard-spawned ``ouro update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".ouro-update-in-progress"
...
def test_marker_path_follows_process_hermes_home(tmp_path, monkeypatch):
    ...
    All three resolve the *process* OURO_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("OURO_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".ouro-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``ouro update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Ouro is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``hermes-setup`` under ``~/.ouro`` is only refreshed by a full installer
