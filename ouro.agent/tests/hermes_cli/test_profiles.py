...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and OURO_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.ouro/profiles)
    * OURO_HOME  -> tmp_path/.ouro  (so get_hermes_home() agrees)
    * Creates the bare-minimum ~/.ouro directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".ouro"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("OURO_HOME", str(default_home))
    return tmp_path
...
class TestNoSkillsOptOut:
    """Tests for `ouro profile create --no-skills` and the opt-out marker."""
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `ouro update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".ouro" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_backend_scan_rejects_unrelated_hermes_prefixed_script(self, profile_env, monkeypatch):
        ...
        console-script entry points (hermes, ouro-agent, ouro-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (ouro-agent,
        ouro-acp -- see pyproject.toml [project.scripts]) must also be
        ...
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/hermes-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/ouro-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".ouro" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".ouro" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom OURO_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("OURO_HOME", str(custom))
        # With Docker-aware roots, a custom OURO_HOME is the default —
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".ouro" / "profiles" / "oldname"
...
class TestInternalHelpers:
    ...
    def test_default_hermes_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_hermes_home() returns OURO_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OURO_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under OURO_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OURO_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="ouro gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with OURO_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("OURO_HOME", str(get_profile_dir("coder")))
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured OURO_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("OURO_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No OURO_HOME: the platform default root applies (existing contract).
