...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".ouro"
    second = tmp_path / "b" / ".ouro"

    monkeypatch.setenv("OURO_HOME", str(first))
    cfg.ensure_hermes_home()

    # Profile switch: OURO_HOME moves → the new path is ensured too.
    monkeypatch.setenv("OURO_HOME", str(second))
    cfg.ensure_hermes_home()
