"""Orphan Desktop-local ``ouro serve`` reap at backend start.
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    ...
    assert _is_desktop_local_serve_cmdline(
        "ouro serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
...
def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "ouro serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "ouro serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("ouro gateway run --replace")
    assert not _is_desktop_local_serve_cmdline(
        "vim notes about ouro serve --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "ouro serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "ouro serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "ouro serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "ouro serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
    ...
    with (
        ...
        os.environ.pop("OURO_DESKTOP_CHILD_PID", None)
...
def test_reap_passes_child_pid_exclude_to_scan():
    with (
        ...
        patch("sys.platform", "darwin"),
        patch.dict(os.environ, {"OURO_DESKTOP_CHILD_PID": "999,111"}, clear=False),
    ):
...
def _valid_lock_payload(pid: int, ownership_id: str, spawn_nonce: str) -> dict:
    ...
    return {
        ...
        "hermesHome": "~/.ouro",
        "logPath": f"~/.ouro/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
...
def test_valid_lockfile_payload_rejects_wrong_owner_and_shape():
    ...
    bad_log["logPath"] = "~/.ouro/desktop-ssh/{oid}/other.log".format(oid=oid)
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "ouro serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "ouro serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    spares a matching orphaned serve even when OURO_DESKTOP_CHILD_PID is
    ...
    scanned = [(4242, "ouro serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
