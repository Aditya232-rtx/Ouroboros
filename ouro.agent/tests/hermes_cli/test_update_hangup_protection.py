"""Tests for SIGHUP protection and stdout mirroring in ``ouro update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``hermes_cli/main.py``.  These exist so
that ``ouro update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("OURO_ACTION_ID", "a" * 32)
...
def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("OURO_ACTION_ID", "not-safe\nforged")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        if hasattr(_cfg, "_OURO_HOME_CACHE"):
            _cfg._OURO_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "ouro update started" in contents
        finally:
