"""Tests for Bug #12905 fix — stale OAuth token detection in ouro model flow.

Bug 3: `ouro model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.ouro/.env but no valid
