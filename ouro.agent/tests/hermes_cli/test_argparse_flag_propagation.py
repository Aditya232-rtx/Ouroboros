...
subparser, placing the flag BEFORE the subcommand (e.g. 'ouro --yolo chat')
...
def _build_parser():
    """Build the ouro argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets OURO_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `ouro gateway run --accept-hooks` failed
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `ouro <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `ouro -t web chat` silently dropped the
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `ouro <flag> <value> chat`:\n  "
