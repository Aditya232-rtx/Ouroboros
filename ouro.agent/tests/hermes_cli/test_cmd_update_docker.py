"""Tests for ``ouro update`` / ``--check`` inside the Docker container.
...
fix, ``ouro update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Ouro, not an update to the running container, so the message
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``ouro update`` inside Docker → friendly message + exit 1, no git calls."""
...
def test_format_docker_update_message_contents():
    ...
    assert "OURO_HOME" in msg or "/opt/data" in msg, (
