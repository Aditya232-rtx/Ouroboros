...
def test_transport_registry_is_manager_and_profile_isolated(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("OURO_HOME", str(tmp_path / "profiles" / "work"))
    ...
    monkeypatch.setenv("OURO_HOME", str(tmp_path / "profiles" / "personal"))
...
def test_live_temp_home_fixture_plugin_routes_and_hardline_stays_core_owned(
    ...
    """Real discovery + config + guard path under an isolated OURO_HOME."""
...
def present(request):
    output = Path(os.environ["OURO_HOME"]) / "transport-invocations.jsonl"
