"""Regression test: `ouro dashboard --tui` must not hard-crash.

Older Ouro desktop app shells (<= 0.15.x) spawn the backend as::

    ouro dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Ouro couldn't
