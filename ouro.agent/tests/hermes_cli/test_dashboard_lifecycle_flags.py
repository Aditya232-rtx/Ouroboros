"""Tests for ``ouro dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``ouro update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No ouro dashboard processes running" in out

    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "ouro dashboard --port 9119"),
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 ouro dashboard process(es) running" in out
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``ouro dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``ouro dashboard --stop`` / ``--status`` actually parse."""
