"""Regression test: ``ouro mcp add --command`` must not clobber the
...
result, ``ouro mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the ouro parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`ouro mcp add foo --url ...` must keep args.command == "mcp".
