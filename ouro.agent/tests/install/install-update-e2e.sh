...
#
# Installs a real, earlier Ouro the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `ouro`.
#
...
# Keep sandbox state out of the default .ouro-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".ouro-sandbox-e2e-$ROUTE"
export OURO_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/hermes/.ouro/hermes-agent"   # user-level layout (sandbox default)
...
# both together.
UPSTREAM_URL="${OURO_DEV_SANDBOX_UPSTREAM:-https://github.com/Aditya232-rtx/ouro.git}"
...
LOG_DIR="${OURO_E2E_LOG_DIR:-$(mktemp -d -t hermes-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED ouro accept FLAG on `ouro update`?
#
...
# that silently rots. `ouro update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "ouro update --help 2>&1" | grep -qF -- "$flag"
}
...
require_hermes_works() {
  local when="$1" out
  out="$(in_sandbox "ouro --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "ouro --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "ouro runs $when"
}

# ── install the earlier Ouro ─────────────────────────────────────────────
...
case "$ROUTE" in
  update)
    ...
    # ouro which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="ouro update --yes"
    else
      update_cmd="ouro update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "ouro update failed ($update_cmd)"
    fi
    require_landed_on_target 'ouro update'
    require_hermes_works 'after ouro update'
    ;;
  installer)
