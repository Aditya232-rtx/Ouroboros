...
- ``ouro verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``ouro verify --json`` when the workspace
...
def hermes_home(tmp_path, monkeypatch):
    monkeypatch.setenv("OURO_HOME", str(tmp_path / ".ouro-home"))
...
def test_record_verify_run_marks_workspace_passed(hermes_home):
    ...
    assert status["evidence"]["canonical_command"] == "ouro verify"
...
def test_cli_run_uses_hermes_session_id_env(hermes_home, capsys, monkeypatch):
    monkeypatch.setenv("OURO_SESSION_ID", "sess-42")
    project = _workspace(hermes_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> ouro verify -> guard satisfied
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `ouro verify --json` suggestion
# ---------------------------------------------------------------------------
...
def test_nudge_mentions_hermes_verify_when_recipe_has_start(hermes_home):
    ...
    assert "ouro verify --json" in nudge
...
def test_nudge_mentions_hermes_verify_when_manifest_exists(hermes_home):
    # No start script, but a saved .ouro/environment.json qualifies.
    project = _workspace(
        hermes_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(hermes_home):
    ...
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "ouro verify" not in nudge
