...
class TestGetDefaultHermesRoot:
    ...
    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When OURO_HOME is not set, returns ~/.ouro."""
        monkeypatch.delenv("OURO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".ouro"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (OURO_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("OURO_HOME", str(profile))
        assert get_default_hermes_root() == docker_root

    @pytest.mark.windows_only
    def test_no_hermes_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\ouro, not ~/.ouro."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("OURO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_hermes_root() == local_appdata / "ouro"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; OURO_HOME / home changes invalidate.

        get_default_hermes_root() resolves OURO_HOME against the native
        ...
        (native home, OURO_HOME) compared for free each call.
        """
        # OURO_HOME set to a Docker-profile path: every call resolves the
        ...
        # OURO_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("OURO_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_hermes_root() == docker_root
        assert resolve_calls["n"] > before, (
            "OURO_HOME change must force a fresh resolution"
        )
...
class TestGetHermesHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When OURO_HOME is unset on Windows, use %LOCALAPPDATA%\\ouro."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("OURO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_hermes_home() == local_appdata / "ouro"
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Ouro-managed npm/node wrappers."""
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = hermes_constants._OURO_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".ouro"
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("OURO_HOME", str(home))
        monkeypatch.setenv(
            "OURO_NODE_TARGET_MAJOR",
            str(hermes_constants._OURO_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(hermes_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(hermes_constants._OURO_NODE_TARGET_MAJOR)
