...
"""
    ...
    assert "Tyler Williams > Active Projects: Ouro" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_OURO_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_OURO_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.ouro/config.yaml into a dotfiles repo.
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_OURO_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.ouro/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Ouro``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Ouro prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Ouro to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Ouro config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Ouro uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.ouro`` (Ouro home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "ouro should always respond concisely"
