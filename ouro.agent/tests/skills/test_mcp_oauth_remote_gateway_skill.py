...
SESSION_REVOKED / REFRESH_DEAD), the OURO_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, OURO_HOME=str(tokens_dir.parent))), \
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "Ouro"  # human credited first
