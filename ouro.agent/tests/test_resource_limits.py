...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    # cmd_dashboard(headless_backend=True) exports OURO_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("OURO_SERVE_HEADLESS", "0")
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert exec_call["env"]["OURO_HOME"] == str(tmp_path)
