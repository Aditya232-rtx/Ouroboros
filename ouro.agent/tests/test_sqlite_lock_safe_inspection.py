"""POSIX advisory locks must survive Ouro' own database inspection.
...
Ouro used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `ouro sessions
...
out across Ouro' inspection calls.
"""
