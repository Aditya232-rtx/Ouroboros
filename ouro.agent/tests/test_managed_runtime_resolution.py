"""Guard: Ouro-owned subprocesses must not resolve managed runtimes by bare PATH.

Ouro installs runtimes for itself — ``uv`` at ``$OURO_HOME/bin/uv``, Node at
``$OURO_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Ouro's own
...
* when a system copy also exists, the one Ouro does not own wins — which is
...
# Runtimes Ouro provisions into OURO_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Ouro-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Ouro-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$OURO_HOME/bin (uv) and $OURO_HOME/node are not on an "
