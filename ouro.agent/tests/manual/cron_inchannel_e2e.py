...
Uses a throwaway OURO_HOME so it never touches ~/.ouro. No real names.
"""
...
def _fresh_home():
    """Point OURO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hermes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["OURO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER OURO_HOME is set.
...
def main():
    ...
    print(f"OURO_HOME (throwaway): {HOME}")
