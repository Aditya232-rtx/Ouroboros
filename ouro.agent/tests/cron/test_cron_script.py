...
- Path resolution (absolute, relative to OURO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp OURO_HOME."""
    ...
    monkeypatch.setattr(jobs_mod, "OURO_DIR", hermes_home)
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Ouro provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _OURO_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_OURO_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.ouro/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("OURO_SESSION_PLATFORM") is None
        assert os.environ.get("OURO_SESSION_CHAT_ID") is None
        assert os.environ.get("OURO_SESSION_CHAT_NAME") is None
