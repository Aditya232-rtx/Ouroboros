...
which is already bounded by OURO_CRON_TIMEOUT — the worker thread submitted
...
timeout (OURO_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_CRON_SESSION_DB_TIMEOUT", "0.2")
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed OURO_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors OURO_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("OURO_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "OURO_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("OURO_CRON_SESSION_DB_TIMEOUT", raising=False)
