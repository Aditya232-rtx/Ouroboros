...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("OURO_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("OURO_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("OURO_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("OURO_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("OURO_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("OURO_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("OURO_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("OURO_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("OURO_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        monkeypatch.setenv("OURO_CRON_TIMEOUT", timeout_value)
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this OURO_HOME in ``_APPLIED_HOMES``, so the external-secret pull
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("OURO_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("OURO_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("OURO_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_OURO_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_OURO_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_OURO_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_OURO_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_OURO_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``OURO_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses OURO_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("OURO_MODEL", "env-model")
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """OURO_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("OURO_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``ouro cron create --deliver=<platform>`` silently
...
class TestFailureStreakNudge:
    ...
    def test_nudges_at_threshold(self):
        ...
        assert "ouro cron pause scout" in out
