...
resolve_runtime_provider against a temp OURO_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        assert "ouro cron edit pin-test --provider <provider> --model <model>" in blob
        ...
        assert "ouro cron edit pin-test --provider <provider> --model <model>" in delivered
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert agent_constructed is False
        assert error is not None
        assert "ouro cron edit pin-test --model <name>" in error
