...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED Ouro
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured Ouro timezone differs from the
...
class TestGetDueJobs:
    ...
    def test_run_claim_heartbeat_keeps_long_run_claimed_past_ttl(
        ...
        monkeypatch.delenv("OURO_CRON_TIMEOUT", raising=False)
...
class TestLateEnvRepointScopesStore:
    """A OURO_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("OURO_HOME", str(tmp_path / "env-home"))
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import OURO_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "OURO_DIR", sim_old_home)
        ...
        monkeypatch.setenv("OURO_HOME", str(new_home))
