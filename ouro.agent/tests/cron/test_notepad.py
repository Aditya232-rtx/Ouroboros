...
use the notepad, and the `ouro cron notepad` CLI handler.
"""
...
def cron_env(tmp_path, monkeypatch, notepad):
    """Isolated cron environment with temp OURO_HOME (mirrors test_cron_context_from)."""
...
class TestPromptInjection:
    def test_nonempty_notepad_rendered_into_prompt(self, cron_env, notepad):
        ...
        assert f"ouro cron notepad {job['id']} set" in prompt
