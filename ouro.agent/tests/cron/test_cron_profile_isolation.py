...
profile's OURO_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.ouro/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active OURO_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever OURO_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    OURO_HOME is a profile under it (``<root>/profiles/<name>``)."""
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile OURO_HOME (<root>/profiles/<name>), the cron store
    ...
    # cron/jobs.py computes OURO_DIR from get_hermes_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.OURO_DIR.resolve() == profile_home.resolve()
        assert (
