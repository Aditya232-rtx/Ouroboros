...
# 5. Symlinks the 'ouro' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/ouro-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink ouro into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up ouro command..."

OURO_BIN="$SCRIPT_DIR/venv/bin/ouro"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$OURO_BIN" "$COMMAND_LINK_DIR/ouro"
echo -e "${GREEN}✓${NC} Symlinked ouro → $COMMAND_LINK_DISPLAY_DIR/ouro"
...
# ============================================================================
# Seed bundled skills into ~/.ouro/skills/
# ============================================================================

OURO_SKILLS_DIR="${OURO_HOME:-$HOME/.ouro}/skills"
mkdir -p "$OURO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.ouro/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$OURO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  ouro status        # Check configuration"
if is_termux; then
    echo "  ouro gateway       # Run gateway in foreground"
else
    echo "  ouro gateway install # Install gateway service (messaging + cron)"
fi
echo "  ouro cron list     # View scheduled jobs"
echo "  ouro doctor        # Diagnose issues"
echo ""
