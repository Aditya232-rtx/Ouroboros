# Ouro Security Policy

This document describes Ouro's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Aditya232-rtx/ouro/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Ouro does not operate a bug
bounty program.**
...
- Environment details (`ouro version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Ouro access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Ouro's documentation
...
Ouro supports two OS-level isolation postures. They address
...
Ouro supports this in two ways:

- **Ouro's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Ouro's in-process heuristics
...
Ouro filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Ouro
itself. Bugs in Ouro's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Ouro does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Ouro's own documentation, or reasonable
  operator expectations would predict — including cases where
  Ouro has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  ouro-home security, and similar. Reports against those
...
- Ouro includes supply-chain guards for MCP server
