...
_UNSET = object()
_OURO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_OURO_HOME_OVERRIDE", default=_UNSET
)
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Ouro home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _OURO_HOME_OVERRIDE.set(value)


def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Ouro home override."""
    _OURO_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Ouro home override, if any."""
    override = _OURO_HOME_OVERRIDE.get()
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Ouro home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "ouro"
    return Path.home() / ".ouro"


def _hermes_home_from_env() -> Path:
    """Resolve OURO_HOME from the process environment only.

    Reads the ``OURO_HOME`` env var, falling back to the platform-native
    ...
    """
    val = os.environ.get("OURO_HOME", "").strip()
    if val:
        return Path(val)
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``OURO_HOME`` is
    ...
    if active and active != "default":
        ...
        msg = (
            f"[OURO_HOME fallback] OURO_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass OURO_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_hermes_home() -> Path:
    """Return the Ouro home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_hermes_home_override`) → ``OURO_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``OURO_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``OURO_HOME`` explicitly (see the systemd
    template in ``hermes_cli/gateway.py`` and the kanban dispatcher in
    ``hermes_cli/kanban_db.py``).  See https://github.com/Aditya232-rtx/ouro/issues/18594.
    """
    override = get_hermes_home_override()
    if override:
        return Path(override)

    if not os.environ.get("OURO_HOME", "").strip():
...
def hermes_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Ouro home/profile directory.
...
def get_process_hermes_home() -> Path:
    """Return the Ouro home for the running process, ignoring task overrides.
    ...
    process ``OURO_HOME`` env var (falling back to the platform default),
...
# OURO_HOME against the native home on every call (~80us of path
...
# Its result depends only on (OURO_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates OURO_HOME mid-process.
_default_hermes_root_memo: "tuple[str, str, Path] | None" = None


def get_default_hermes_root() -> Path:
    """Return the root Ouro directory for profile-level operations.

    In standard deployments this is the platform-native Ouro home
    (``~/.ouro`` on POSIX, ``%LOCALAPPDATA%\\ouro`` on native Windows).

    In Docker or custom deployments where ``OURO_HOME`` points outside
    ``~/.ouro`` (e.g. ``/opt/data``), returns ``OURO_HOME`` directly
    — that IS the root.

    In profile mode where ``OURO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.ouro/profiles/coder``) and Docker
    ...
    env_home = os.environ.get("OURO_HOME", "")
    ...
    if not env_home:
        ...
        try:
            ...
            # OURO_HOME is under ~/.ouro (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — OURO_HOME itself is the root
                result = env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``OURO_OPTIONAL_SKILLS``.
    """
    override = os.getenv("OURO_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``OURO_OPTIONAL_MCPS``.
    """
    override = os.getenv("OURO_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``OURO_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<OURO_HOME>/skills`` last-resort
    """
    override = os.getenv("OURO_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_hermes_dir(
    ...
    *,
    ...
    """Resolve a Ouro subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to OURO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to OURO_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Ouro home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local OURO_HOME.
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Ouro-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\ouro\\node``. POSIX installs use
    ``$OURO_HOME/node/bin``. Include both shapes on every platform so mixed
...
_OURO_NODE_TARGET_MAJOR = int(os.environ.get("OURO_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Ouro-managed Node trees live under ``$OURO_HOME/node`` (or a profile's
    ``OURO_HOME``). A partial upgrade or interrupted install can leave
    ...
    used to trust file presence alone, so ``ouro update`` would pick that
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Ouro-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Ouro-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `ouro update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%OURO_HOME%\\node`` on Windows.
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_OURO_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_OURO_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$OURO_HOME/node`` on POSIX.
    ...
    ``OURO_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a Ouro-managed Node tree and return its npm path.
    ...
    Ouro never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$OURO_HOME/node`` (the same tree a fresh install
...
def heal_hermes_managed_node() -> bool:
    """Redownload Ouro-managed Node when the tree exists but is broken.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OURO_HOME": str(get_hermes_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    ``latest-v{_OURO_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_hermes_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _OURO_NODE_TARGET_MAJOR
    return False


def find_hermes_node_executable(command: str) -> str | None:
    """Return a Ouro-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_OURO_NODE_TARGET_MAJOR``) heal the
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Ouro-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Ouro-managed installs.

    This is for Ouro-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Ouro-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``ouro update`` — leaving a **dangling symlink** that ``which`` still
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current OURO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.ouro``
        profile:  ``~/.ouro/profiles/coder``
        custom:   ``/opt/hermes-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.ouro``.  For code that needs a real ``Path``, use
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``OURO_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/Aditya232-rtx/ouro/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{OURO_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get("OURO_HOME") or os.getenv("OURO_HOME")
    if not hermes_home:
        return None
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("OURO_REAL_HOME") or os.getenv("OURO_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Ouro profile HOME.

    ``OURO_HOME`` scopes Ouro state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={OURO_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{OURO_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{OURO_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Ouro' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["OURO_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Ouro itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under OURO_HOME.
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under OURO_HOME."""
    return get_hermes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under OURO_HOME."""
...
# Top-level packages/modules that ship as part of Ouro itself. An ImportError
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Ouro.
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `ouro update` is exactly
    ...
    return [
        ...
        "    ouro update",
        "If that also fails, reinstall: https://ouro.dev",
    ]
