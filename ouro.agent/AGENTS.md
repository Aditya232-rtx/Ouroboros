# Ouro - Development Guide

Instructions for AI coding assistants and developers working on the ouro-agent codebase.

**Never give up on the right solution.**

## What Ouro Is

Ouro is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Ouro ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`ouro tools`, `ouro setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `OURO_HOME`. Mocks hide integration bugs.
...
- **New `OURO_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `ouro tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.ouro/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `ouro <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `ouro webhook`, `ouro cron`, `ouro tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.ouro/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Ouro Cloud. Only the first two are spawned
by us and carry `OURO_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
- **The toolset is the surface gate.** Keep the tools off `_OURO_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `OURO_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`ouro --tui` against that same backend) is
...
`$HOME/.ouro/hermes-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _OURO_CORE_TOOLS list
...
├── ui-tui/               # Ink (React) terminal UI — `ouro --tui`
...
**User config:** `~/.ouro/config.yaml` (settings), `~/.ouro/.env` (API keys only).
**Logs:** `~/.ouro/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_hermes_home()`.
Browse with `ouro logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Ouro: desktop, TUI, website, and future TS packages.
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.ouro/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `ouro --tui` or `OURO_TUI=1`.
...
### TUI in the Dashboard (`ouro dashboard` → `/chat`)

The dashboard embeds the real `ouro --tui` — **not** a rewrite.  See `hermes_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `hermes_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `ouro --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `ouro --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@hermes/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `ouro serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `OURO_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `hermes` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `ouro --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Ouro built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Ouro core.
Use the plugin route instead: create `~/.ouro/plugins/<name>/plugin.yaml`
and `~/.ouro/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Ouro tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_OURO_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_OURO_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_hermes_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `OURO_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_hermes_home()` for the base directory — never `Path.home() / ".ouro"`. This ensures each profile gets its own state.
...
| `load_config()` | `ouro tools`, `ouro setup`, most CLI subcommands | `hermes_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
~/.ouro/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Ouro gold/kawaii (the current look)
...
Users create `~/.ouro/skins/<name>.yaml`:
...
Ouro has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.ouro/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.ouro/plugins/`, `./.ouro/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  ...
  `ouro <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$OURO_HOME/plugins/`, `./.ouro/plugins/` (opt-in via
`OURO_ENABLE_PROJECT_PLUGINS`), and `hermes_agent.memory_providers` entry
...
it at argparse setup time and wires it into `ouro <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `ouro --help`.
...
into `~/.ouro/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `ouro memory setup` / `post_setup()` without
...
`~/.ouro/plugins/` (or via pip entry points). They register through
...
2. User: `$OURO_HOME/plugins/model-providers/<name>/`
...
[`hermes-example-plugins`](https://github.com/Aditya232-rtx/ouro-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `ouro skills install official/<category>/<skill>`. Adapter lives in
...
2. **Tools referenced in SKILL.md prose must be native Ouro tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Ouro" is the secondary collaborator. If the
   contributor's commit shows "Ouro" as author (because they
   used Ouro to draft the skill), replace it with their actual name
...
lives in the `ouro-agent-dev` skill at
...
`"messaging"`); `_OURO_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `ouro tools` (the curses UI) or the
...
Ouro returns a delegation id immediately and the result re-enters the
...
go to `~/.ouro/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `hermes_cli/curator.py` wires `ouro curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.ouro/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `ouro cron <verb>`
...
- File lock at `~/.ouro/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `ouro kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `hermes_cli/kanban.py` wires `ouro kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`ouro-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `OURO_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Ouro-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `OURO_BACKGROUND_NOTIFICATIONS` env var):
...
Ouro supports **profiles** — multiple fully isolated instances, each with its own
`OURO_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `hermes_cli/main.py` sets
`OURO_HOME` before any module imports. All `get_hermes_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_hermes_home()` for all OURO_HOME paths.** Import from `hermes_constants`.
   NEVER hardcode `~/.ouro` or `Path.home() / ".ouro"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".ouro" / "config.yaml"
   ```

2. **Use `display_hermes_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.ouro` for default or `~/.ouro/profiles/<name>` for profiles.
   ```python
   # GOOD
   ...
   print("Config saved to ~/.ouro/config.yaml")
   ```
...
4. **Tests that mock `Path.home()` must also set `OURO_HOME`** — since code now uses
   `get_hermes_home()` (reads env var), not `Path.home() / ".ouro"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"OURO_HOME": str(tmp_path / ".ouro")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not OURO_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".ouro" / "profiles"`, NOT `get_hermes_home() / "profiles"`.
   This is intentional — it lets `ouro -p coder profile list` see all profiles regardless
...
### DO NOT hardcode `~/.ouro` paths
Use `get_hermes_home()` from `hermes_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.ouro` breaks profiles — each profile
has its own `OURO_HOME` directory. This was the source of 5 bugs fixed in PR #3575.
...
with actual imports (not mocks) against a temp `OURO_HOME`.

### Tests must not write to `~/.ouro/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `OURO_HOME` to a temp dir. Never hardcode `~/.ouro/` paths in tests.
...
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OURO_HOME", str(home))
    return home
```
...
subprocess (`--file-retries`, default 1; `OURO_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.ouro/` | Your real config+auth.json                  | Temp dir per test                         |
...
Ouro supports Linux, macOS and native Windows, and plenty of its behaviour
