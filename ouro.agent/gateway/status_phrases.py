...
Ouro' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``OURO_HOME`` either by using
conventional paths::

    ~/.ouro/status_phrases.yaml
    ~/.ouro/status_phrases/*.yaml
...
# These are Ouro UI surfaces, not app/vendor/domain buckets.  Keep this
...
def classify_status_context(
    ...
    *,
    ...
    """Classify an internal gateway event into a Ouro UI-surface bucket."""
