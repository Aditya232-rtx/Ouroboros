...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{OURO_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``OURO_HOME`` is a **durable** store — on Ouro
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``ouro gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting OURO_HOME."""
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable OURO_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (OURO_HOME is a
    durable Fly volume on Ouro Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    OURO_HOME volume, or an expired same-epoch orphan (#85433), is
