...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{OURO_HOME}/pairing`` while newer code/docs may
    write ``{OURO_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own OURO_HOME using the same legacy/consolidated layout rules
    as ``ouro -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current OURO_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp OURO_HOME
        ...
        if profile:
            ...
            # ``ouro -p <profile> pairing ...`` process does. Merge the
        ...
        if path.exists():
            try:
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> ouro pairing approve ...` as root, "
                    "re-run with `docker exec -u ouro <container> ...` and "
                    "chown the existing file to the ouro user, or restart the "
