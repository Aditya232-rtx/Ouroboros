...
# ``OURO_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``OURO_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("OURO_AUTO_CONTINUE_FRESHNESS")
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `ouro tools` (opt-in, default
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `ouro tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `ouro tools` AND the bot
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Ouro session "
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.ouro/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.ouro/state.db "
                "and are shown by `ouro sessions list` and `/sessions`. "
