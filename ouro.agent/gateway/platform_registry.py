...
class PlatformEntry:
    ...
    # status displays (``ouro setup``, ``ouro status``, the dashboard
    ...
    # Env vars this platform needs (for ``ouro setup`` display).
    ...
    # built-ins).  Used by ``ouro gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved OURO_HOME and overlay the
        ...
        # invocation -- including plain `ouro chat`, which never touches any
        ...
        # `ouro setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``ouro setup``/``gateway status``, channel
