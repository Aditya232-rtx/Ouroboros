...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Ouro-Session-Id header; opt-in long-term memory scoping via X-Ouro-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Ouro-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists ouro-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Ouro sessions
- POST /api/sessions               — create an empty Ouro session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to ouro-agent
...
def _hermes_version() -> str:
    """Return the canonical Ouro version string.
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("OURO_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``ouro-agent``)
    ...
    ``direct_model_requests`` config value here, while Ouro-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Ouro-aware and is
    always honored.
    """
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted OURO_HOME (same filesystem
...
def _derive_chat_session_id(
    ...
    the same Ouro session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through ouro-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Ouro-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "ouro-agent")
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Ouro-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Ouro-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Ouro-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Ouro-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Ouro-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``ouro sessions list``
    ...
    def _create_agent(
        ...
        by the client (via ``X-Ouro-Session-Key``).  Unlike ``session_id``
        ...
        # (e.g. user ran `ouro auth add openai-codex` without `ouro model`),
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "ouro-agent", "version": _hermes_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list ouro-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Ouro provider/model inventory.
        ...
        Ouro provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Ouro version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Ouro AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Ouro-Session-Id",
                "session_key_header": "X-Ouro-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Ouro sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Ouro session row.
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Ouro-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Ouro-Session-Key"] = gateway_session_key
        runtime = {}
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Ouro-Session-Key.  This
        # is independent of X-Ouro-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Ouro-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Ouro-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Ouro-Session-Id rejected: "
            ...
            # conversation map to the same Ouro session.  The first user
        ...
        response_headers = {
            "X-Ouro-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Ouro-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Ouro-Completed"] = "false"
            response_headers["X-Ouro-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Ouro-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Ouro-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Ouro-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Ouro-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Ouro-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Ouro-Session-Id header.  Without this,
        ...
        response_headers = {"X-Ouro-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Ouro-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Ouro agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Ouro-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Ouro-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Ouro-Relay
            ...
            # ~/.ouro/config.yaml and plant persistence. Sandboxing (Docker /
