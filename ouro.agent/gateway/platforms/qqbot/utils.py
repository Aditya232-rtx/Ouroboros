...
def _get_hermes_version() -> str:
    """Return the ouro-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Ouro/<hermes_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Ouro/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    hermes_version = _get_hermes_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Ouro/{hermes_version})"
