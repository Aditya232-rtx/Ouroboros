...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.ouro to the active profile home."""
    ...
    if raw == "~/.ouro":
        return hermes_home
    if raw.startswith("~/.ouro/"):
        return hermes_home / raw.removeprefix("~/.ouro/")
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under OURO_HOME/scripts."""
    ...
    if raw_text == "~/.ouro" or raw_text.startswith("~/.ouro/"):
