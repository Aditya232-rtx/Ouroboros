...
There are two ways to add a platform to the Ouro gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.ouro/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Ouro code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `ouro gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Ouro core.
...
And add it to the `ouro-gateway` composite:
