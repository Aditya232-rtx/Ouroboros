...
# Audio file extensions Ouro recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Ouro but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Ouro' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Ouro-created Telegram private-chat
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``OURO_SESSION_PLATFORM`` contextvar — that contextvar is
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.ouro/.env manually."
)
...
_OURO_HOME = get_hermes_home()
_OURO_ROOT = get_default_hermes_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "OURO_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "OURO_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "OURO_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "OURO_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _OURO_HOME / "image_cache",
    _OURO_HOME / "audio_cache",
    _OURO_HOME / "video_cache",
    _OURO_HOME / "document_cache",
    _OURO_HOME / "browser_screenshots",
    ...
    _OURO_HOME / "cache" / "images",
    _OURO_HOME / "cache" / "audio",
    _OURO_HOME / "cache" / "videos",
    _OURO_HOME / "cache" / "documents",
    _OURO_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Ouro root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* OURO_HOME's cache, so a gateway running at
    the root (e.g. ``OURO_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when OURO_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _OURO_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("OURO_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _OURO_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Ouro profile and shared Ouro root both contain control
    ...
    # OURO_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.ouro
    ...
    for hermes_root in (_OURO_HOME, _OURO_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Ouro
    secrets (``~/.ouro/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Ouro cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.ouro/...``
...
def _translate_docker_container_media_path(candidate: Path) -> Optional[Path]:
    ...
    auto-mounted Ouro cache dirs (``/root/.ouro/...``), the default
    ...
    # In-process gateways (Desktop backend, `ouro serve`) may not have
    ...
    # are longer prefixes, so /root/.ouro/... still translates to the host
    ...
    if default_home is not None and not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.ouro/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.ouro"):
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``OURO_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Ouro-managed cache, under an operator-allowlisted root
    (``OURO_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Ouro root (~/.ouro/.env,
    ...
    # ``MEDIA:~/.ouro/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``OURO_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Ouro commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-OURO_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("OURO_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "OURO_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "OURO_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Ouro sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-OURO_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(ouro --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Ouro home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Ouro home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.ouro/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          OURO_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          OURO_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          OURO_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("OURO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("OURO_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("OURO_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                    ...
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # OURO_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile OURO_HOME isolation.
        """
