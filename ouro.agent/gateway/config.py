...
class PlatformConfig:
    ...
    # "Ouro is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single OURO_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.ouro/config.yaml (primary user-facing config)
    3. ~/.ouro/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # (e.g. via `ouro config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `ouro config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``ouro config set gateway.streaming.*``
