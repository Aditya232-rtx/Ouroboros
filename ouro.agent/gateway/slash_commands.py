...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: ouro gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Ouro commands.
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`ouro gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Ouro version."""
        from hermes_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Ouro default runtime
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `ouro cron` for durable schedules."
        )
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Ouro itself changed. ``--stat`` limits output to the summary.
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.ouro/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``ouro skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hermes_home
        from hermes_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `ouro skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.ouro/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `ouro bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``ouro debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Ouro to the latest version.

        Spawns ``ouro update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``ouro update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Ouro')}"
        ...
        # Spawn `ouro update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `ouro update --gateway`
