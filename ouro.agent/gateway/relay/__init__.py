"""Relay/connector support package for the Ouro gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``ouro gateway enroll`` writes them to
    ``~/.ouro/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``OURO_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``ouro gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # multi-agent scope it would prefix every reply "**Ouro:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``ouro gateway enroll`` CLI. Three modes, in precedence order:
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.ouro/.env``.
    ...
    sets neither ``OURO_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``ouro gateway enroll``: has a PINNED
