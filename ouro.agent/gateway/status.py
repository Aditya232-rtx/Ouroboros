...
The PID file lives at ``{OURO_HOME}/gateway.pid``.  OURO_HOME defaults to
``~/.ouro`` but can be overridden via the environment variable.  This means
separate OURO_HOME directories naturally get separate PID files — a property
...
def _get_process_hermes_home() -> Path:
    """Return the process-level OURO_HOME, skipping context-local overrides.
    ...
    ``get_hermes_home()`` honors ``_OURO_HOME_OVERRIDE`` contextvar used for
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Return a stable absolute OURO_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare OURO_HOME paths with the host platform's case semantics."""
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a OURO_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.ouro/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    OURO_HOME profile than the caller.  Prefers the explicit ``profile``
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting OURO_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("OURO_GATEWAY_LOCK_DIR")
    if override:
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Ouro' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Ouro gateway lifecycle subcommand from a command line.
    ...
    (``"C:\\Program Files\\...\\ouro-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Ouro's
    ...
    for token in tokens:
        ...
        if basename in ("ouro-gateway", "ouro-gateway.exe"):
            return "run"
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `ouro gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Ouro-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Ouro gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a OURO_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.ouro`` or ``$OURO_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``OURO_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a OURO_HOME= value on the argv
    ...
    # a non-matching explicit OURO_HOME= on the argv. OURO_HOME is usually
...
def _build_pid_record() -> dict:
    return {
        ...
        # OURO_HOME-local.  Persist the owning gateway's process home so an
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `ouro status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``OURO_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    OURO_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different OURO_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-OURO_HOME --replace while
    ...
    # ``ouro gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's OURO_HOME rather than the replacer's.  Same-home callers omit
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target OURO_HOME's gateway PID record, and the live
