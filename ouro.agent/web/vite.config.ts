...
const BACKEND = process.env.OURO_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `ouro dashboard` server injects a one-shot
 * session token into `index.html` (see `hermes_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__OURO_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hermesDevToken(): Plugin {
  const TOKEN_RE = /window\.__OURO_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__OURO_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;
        ...
        if (!match) {
          console.warn(
            `[hermes] Could not find session token in ${BACKEND} — ` +
              `is \`ouro dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__OURO_SESSION_TOKEN__="${match[1]}";` +
              `window.__OURO_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[hermes] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`ouro dashboard\` or set OURO_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `ouro dashboard` must serve these; Vite has no
