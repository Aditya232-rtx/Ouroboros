/**
 * ChatPage — embeds `ouro --tui` inside the dashboard.
 *
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__OURO_SESSION_TOKEN__ &&
    !window.__OURO_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `ouro dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__OURO_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Ouro TUI has enabled xterm mouse-events
    ...
    //      payloads upload to OURO_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to OURO_HOME/images, then drive `/image`
    ...
    // WebSocket. In gated mode (``window.__OURO_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets OURO_HOME pointed at the
    ...
    // forwarding them into Ouro. Keyboard input, paste, and resize still
    // behave normally.
