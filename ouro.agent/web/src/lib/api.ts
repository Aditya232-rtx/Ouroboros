...
// injects ``window.__OURO_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__OURO_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const OURO_BASE_PATH = readBasePath();
const BASE = OURO_BASE_PATH;
...
declare global {
  interface Window {
    __OURO_SESSION_TOKEN__?: string;
    __OURO_BASE_PATH__?: string;
    ...
    __OURO_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Ouro-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__OURO_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``ouro update``, ``ouro gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__OURO_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__OURO_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__OURO_SESSION_TOKEN__`` is valid, so the next
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__OURO_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Ouro-Session-Token`` header.
 ...
 * ``window.__OURO_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``ouro update``. */
  ...
  /** NS-656: disk-usage rollup for the OURO_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the OURO_HOME volume — no staleness dimension, so no
 * sampled_at. */
