...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind OURO_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `OURO_SESSION_SOURCE` values, and older installs with
        ...
        with _session_resume_lock:
            ...
            try:
                ...
                if sid in _sessions:
                    ...
                    # Remember the profile home so each turn re-binds OURO_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``ouro --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``ouro gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Ouro profile home
    # (~/.ouro/sessions/saved/) rather than the project/workspace CWD, and
