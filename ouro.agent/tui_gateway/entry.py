...
# from shadowing Ouro's own top-level modules.  ``hermes_bootstrap`` lives at
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `OURO_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("OURO_TUI_SIDECAR_URL")
...
# tested; override via ``OURO_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("OURO_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``OURO_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``ouro --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # session profile's OURO_HOME first), so a launch profile with no
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``ouro --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``ouro dashboard`` spawn theirs via
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's OURO_HOME override — the shared owner in
...
def main():
    ...
    # Live-apply skins Ouro activates mid-conversation.
