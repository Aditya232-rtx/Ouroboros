...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.ouro/.env`` into the gateway process via
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind OURO_HOME to the session's profile so get_skill_commands()
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("OURO_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("OURO_BASE_URL", "") or cfg.get("base_url", "")
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off OURO_HOME, so scoping the
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``ouro journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `ouro mcp` offers, so
...
# param that scopes OURO_HOME via set_hermes_home_override (omitted/None = the
...
def _(rid, params: dict) -> dict:
    ...
    the SAME interactive MCP OAuth machinery ``ouro mcp login`` uses
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``ouro plugins`` / the dashboard, so the three surfaces
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Ouro plugin.
