...
def _(rid, params: dict) -> dict:
    """List Ouro profiles (name, path, model, description, skill count).
...
def _(rid, params: dict) -> dict:
    ...
    ``ouro setup`` in that flow to recover. A profile spawned as an
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local OURO_HOME override — the same
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the OURO_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `ouro tools`
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `ouro tools` / Settings is where
