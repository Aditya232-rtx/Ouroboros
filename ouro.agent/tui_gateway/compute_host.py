    ...
    def __init__(
        ...
        *,
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("OURO_COMPUTE_HOST_HEARTBEAT_SECS") or "15")
        )
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("OURO_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["OURO_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit(
        {
            ...
            "cwd": os.getcwd(),
            "hermes_home": os.environ.get("OURO_HOME", ""),
        }
    )
