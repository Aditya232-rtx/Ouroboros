...
# from shadowing Ouro's own top-level modules.  This worker is spawned as
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``OURO_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("OURO_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("OURO_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    ...
    Each slash_worker child is its own process — the parent ``ouro serve``
...
def main():
    ...
    os.environ["OURO_SESSION_KEY"] = args.session_key
