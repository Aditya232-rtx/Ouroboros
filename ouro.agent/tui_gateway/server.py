...
# appends every unhandled exception to ~/.ouro/logs/tui_gateway_crash.log
...
try:
    _slash_timeout = float(os.environ.get("OURO_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("OURO_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("OURO_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Ouro' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Ouro-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Ouro agent → needs provider credentials.
        ...
        # gateway's launch OURO_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["OURO_HOME"] assignment.
        ...
        env = build_subprocess_env(
            ...
            extra={"OURO_HOME": str(profile_home)} if profile_home else None,
        )
        # Prepend the Ouro venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Ouro-managed CLIs
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `ouro --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("OURO_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `ouro --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its OURO_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s OURO_HOME around a pet RPC handler.
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``ouro dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("OURO_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # OURO_HOME so config/skills/model resolve to it, and hand the
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``hermes`` / ``ouro --tui`` / CLI) is started from a
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read OURO_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # OURO_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["OURO_GATEWAY_SESSION"] = "1"
    os.environ["OURO_EXEC_ASK"] = "1"
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``ouro config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Ouro activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _resolve_model() -> str:
    env = (
        os.environ.get("OURO_MODEL", "")
        or os.environ.get("OURO_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``OURO_DESKTOP=1`` and ``OURO_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``OURO_DESKTOP_TERMINAL=1`` → "tui"
        (``ouro --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``ouro --tui``.)
    """
    if is_truthy_value(os.environ.get("OURO_DESKTOP")) and not is_truthy_value(
        os.environ.get("OURO_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads OURO_MODEL /
    OURO_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`ouro --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads OURO_MODEL /
    # OURO_INFERENCE_MODEL, which `ouro --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("OURO_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("OURO_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind OURO_HOME to the session's profile so load_soul_md() and
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("OURO_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_OURO_CORE_TOOLS`` — every other
    ...
    paths run with ``OURO_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Ouro desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("OURO_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Ouro): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `ouro --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] OURO_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown OURO_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in OURO_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid OURO_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _OURO_CORE_TOOLS (every other
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid OURO_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (OURO_MODEL /
    # OURO_INFERENCE_MODEL / OURO_TUI_PROVIDER / OURO_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `ouro --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # OURO_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("OURO_DEV_CREDITS")):
        try:
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Ouro window, and answers
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("OURO_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("OURO_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("OURO_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("OURO_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("OURO_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("OURO_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped OURO_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / OURO_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The OURO_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Ouro has to support both the legacy 8-row petdex atlas and the current
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("OURO_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $OURO_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=OURO_SESSION_KEY`` (see
    ...
    # DB when OURO_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``OURO_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn OURO_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            # Run while any profile-specific OURO_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile OURO_HOME
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile OURO_HOME override, while the docker/ssh
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("OURO_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["OURO_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("OURO_YOLO_MODE", None)
                    nv = "0"
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under OURO_HOME (~/.ouro by
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    OURO_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: OURO_HOME itself and the dirs in
...
def _discover_repos_payload(
    ...
    repos even with zero ouro sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (ouro home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Ouro sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Ouro
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`ouro setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`ouro gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`ouro sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`ouro config edit` needs $EDITOR in a real terminal"
    return None
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Ouro" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("OURO_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("OURO_VOICE_TTS", "").strip() == "1"
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["OURO_VOICE"] = "0"
                    os.environ["OURO_VOICE_TTS"] = "0"
                    try:
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["OURO_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["OURO_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
