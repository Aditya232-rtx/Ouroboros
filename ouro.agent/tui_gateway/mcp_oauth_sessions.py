...
The underlying token machinery is the *same* one the CLI ``ouro mcp login``
...
def _start_loopback_listener(flow) -> "http.server.HTTPServer":
    ...
    class _Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — stdlib naming
            ...
            body = b"<h1>Authorization received</h1><p>You can close this tab and return to Ouro.</p>"
            status = 200
            try:
...
def _worker(session_id: str, hermes_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    OURO_HOME override + secret-scope + force_interactive_oauth +
