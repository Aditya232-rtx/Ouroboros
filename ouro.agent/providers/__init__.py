...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with ouro-agent)
2. User plugins: ``$OURO_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$OURO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$OURO_HOME/plugins/model-providers/`` if it exists."""
    try:
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple OURO_HOME profiles don't alias each other.
...
def _discover_entry_point_providers() -> None:
    ...
    * **Provider targets only.** The ``hermes_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$OURO_HOME``) keep their
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$OURO_HOME/plugins/model-providers/<name>/``
    ...
    #    filesystem steps means a bundled or ``$OURO_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with ouro-agent.
    ...
    # 2. User plugins — under $OURO_HOME/plugins/model-providers/<name>/.
