...
Registry and ABC for every inference provider Ouro knows about.
...
`$OURO_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$OURO_HOME/plugins/model-providers/` for a private plugin).
