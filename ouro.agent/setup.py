...
Ouro (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``OURO_NIX_BUILD=1`` on the
Ouro package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("OURO_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for ouro-agent is not supported.\n"
    "Ouro is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://ouro.dev/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Ouro Nix derivation sets OURO_NIX_BUILD=1. If it does, file a bug."
)
