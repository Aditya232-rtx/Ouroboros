"""Unified provider-credential lifecycle across every store Ouro reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.ouro/.env``                     — the canonical secret store
    2. ``~/.ouro/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.ouro/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `ouro auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``ouro auth add``.
    """
