...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Ouro'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Ouro' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Ouro for
        ...
        try:
            ...
            # Permissions + Ouro tool callback are always-on production
            ...
            if "hermes-tools" in mig_report.migrated:
                msg_lines.append(
                    "Ouro tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Ouro runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Ouro tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Ouro runtime.")
