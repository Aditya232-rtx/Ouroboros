...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect OURO_HOME?"""
...
def _resolved_home() -> str:
    hermes_home = os.environ.get("OURO_HOME", "").strip()
    if hermes_home:
        return hermes_home
    return os.path.join(os.path.expanduser("~"), ".ouro")
...
def container_mode_may_be_active() -> bool:
    ...
    default_home = os.path.join(os.path.expanduser("~"), ".ouro")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect OURO_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    print(f"Ouro v{__version__} ({__release_date__})")
    ...
    print("Run 'ouro version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``ouro --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the OURO_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("OURO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
