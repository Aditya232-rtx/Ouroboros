"""Safe Ouro Console command engine.

This module backs ``ouro console`` and is intentionally narrower than the
full Ouro CLI. It exposes a curated set of native adapters that can later be
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'ouro doctor'")
        and last.startswith("Run 'ouro setup'")
    ):
        return text.rstrip()
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `ouro "):
        return ""
    return summary
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `ouro {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `ouro {usage}`?",
        )


class HermesConsoleEngine:
    """Curated line-command executor for Ouro Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Ouro Console does not run shell syntax. Use one supported "
                    "Ouro command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Ouro component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Ouro logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Ouro configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Ouro Console."""
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Ouro configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Ouro Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Ouro Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`ouro {first}` is not available in Ouro Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Ouro Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Ouro Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Ouro Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Ouro Console.",
            ("skills", "publish"): "`skills publish` is not available in Ouro Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Ouro Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Ouro Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Ouro Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Ouro Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Ouro Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Ouro Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Ouro Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Ouro Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Ouro Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Ouro Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Ouro Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Ouro Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Ouro Console.")
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from ouro console")
...
def run_console_repl(
    *,
    ...
    """Run the local ``ouro console`` REPL."""
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Ouro Console. Type `help` for commands, `exit` to quit.", file=stdout)
