"""Ouro update pipeline — extracted from ``hermes_cli/main.py``.
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``ouro update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``ouro update`` runs in the PRE-pull Python process. After ``git pull``
...
# Critical files that Ouro must be able to import immediately after an
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``ouro update`` can be driven by a
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``ouro update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `ouro update`.
    ...
    print("  Preview now:  ouro curator run --dry-run")
    print("  Pause it:     ouro curator pause")
    print(
        "  Docs:         https://ouro.dev/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `ouro update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `ouro sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    ouro sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  ouro sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``ouro update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``ouro update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: ouro curator status)"
    )
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    ouro dashboard --port <port>")
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.ouro-update-staging"
    backup = f"{dst}.ouro-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `ouro update`" advice with
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("OURO_ACTION_ID", "")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    """Update Ouro by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `ouro update "
            f"--branch {branch}`, or update against main with `ouro update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/Aditya232-rtx/ouro/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for ouro-agent itself
        ...
        print(
            "  Re-run `ouro update` to retry; if the agent won't start, "
            "reinstall from https://ouro.dev"
        )
        _m().sys.exit(1)
    finally:
    ...
    if not import_ok:
        ...
        print("  Re-run `ouro update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: ouro skills list-modified  "
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Ouro behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes ouro completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Ouro couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Ouro couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Ouro behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Ouro couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Ouro couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/Aditya232-rtx/ouro.git",
    "git@github.com:Aditya232-rtx/ouro.git",
    "https://github.com/Aditya232-rtx/ouro",
    "git@github.com:Aditya232-rtx/ouro",
}

OFFICIAL_REPO_URL = "https://github.com/Aditya232-rtx/ouro.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Ouro repository.")
        print("  This means you may miss updates from Aditya232-rtx/ouro.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Aditya232-rtx/ouro.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Aditya232-rtx/ouro.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``ouro update``, every profile is now current.
    """
    homes = []
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Ouro Desktop, exit any open `hermes` REPLs, and")
    lines.append("  stop the gateway (`ouro gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `ouro update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``ouro tools``."""
    try:
        from hermes_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Ouro Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``ouro tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Ouro Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Ouro Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``ouro update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `ouro update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `ouro update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Ouro' editable-install extras or
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$OURO_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `ouro update` is exactly the step
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    ouro update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `ouro update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `ouro update`.")
            failed = []
            if any(
    ...
    # Ouro profile using this checkout. Keep one per-checkout cache under the
    # shared Ouro root rather than rerunning npm once per named profile.
    ...
    # `ouro update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `ouro update`.")
        return list(labels)
    ...
    # `ouro update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.ouro/logs/update.log`` only, never the terminal.

    During ``ouro update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``ouro update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``ouro update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``ouro update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Ouro — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``ouro-acp`` launcher next to the ``hermes`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``ouro update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``ouro-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Ouro as not installed even when it is.
    ...
    No-op on Windows (install.ps1 copies ``hermes.exe`` + ``ouro-acp.exe``
    ...
    so ``ouro-acp.exe`` already resolves) and wherever a ``ouro-acp`` is
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        hermes_cmd = bin_dir / "ouro"
        acp_cmd = bin_dir / "ouro-acp"
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Ouro — ACP launcher (written by `ouro update`).\n"
            ...
            continue
        print(f"  ✓ Installed ouro-acp launcher → {acp_cmd}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of OURO_HOME under
      ``backups/`` (restorable via ``ouro import``; the #48200 wrong-path
    ...
    # Render path using display_hermes_home so the user sees ~/.ouro/...
    try:
    ...
    print(f"  Restore:  ouro import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``ouro update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``ouro update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run ouro from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.ouro-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".ouro-bootstrap-complete",
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Ouro processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Ouro Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Ouro desktop app / other Ouro terminals, then re-run:"
    )
    lines.append("    ouro update")
    lines.append("  (or use `ouro update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``ouro update``
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning ouro-setup, and the update-in-progress marker
    ...
    the update with "Ouro is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Ouro backend (``hermes_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.ouro-runtime`` interpreter child)
...
def _stop_process_trees(pids: list[int]) -> None:
    ...
    ``.ouro-runtime`` interpreter child alive and holding the install open
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Ouro...")
    try:
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: ouro gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``ouro gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``ouro-gateway*.service``/``hermes-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not (
            unit == "ouro-gateway.service"
            or unit.startswith("ouro-gateway-")
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``ouro-gateway*`` units run ``gateway/run.py``, which installs the
    ...
    (``ouro-gateway-helper`` is fine — profile units are
    ``ouro-gateway-<profile>`` — but ``ouro-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "ouro-gateway" or svc_name.startswith("ouro-gateway-")
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``ouro update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``ouro-setup.exe``) executes
    ``$OURO_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``ouro update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``ouro update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `ouro desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    #
    # Start the build subprocess with the Ouro-managed Node on PATH: when
    # `ouro update` runs inside the desktop updater chain (Desktop →
    # ouro-setup → ouro update), the shell PATH customizations are lost,
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (non-fatal; run `ouro desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # (most commonly the Desktop app's `ouro serve` backend) will keep .pyd
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .ouro-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Ouro is still
    ...
    # process by design (a CLI `ouro update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Ouro windows/gateways and re-run: ouro update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Ouro gateways, Desktop backends, or other "
        ...
        # every user who ran ``ouro update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``ouro update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``ouro update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``ouro doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && ouro update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        if not import_ok:
            ...
            print("    Run `ouro update` again — if it persists, reinstall:")
            print("    https://ouro.dev")
        ...
        # current — so copy it straight over ~/.ouro/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit OURO_HOME so
        # it is not affected by sync_skills()'s module-level OURO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's OURO_HOME env var points at the default or a named profile.
        try:
        ...
        # ``ouro update`` process is the PRE-pull Python process — its
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'ouro config migrate' to retry.")
            ...
            if assume_yes:
                ...
                try:
                    ...
                    print(
                        ...
                        "Run 'ouro config migrate' manually to configure."
                    )
                    response = "n"
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: ouro config migrate")
            else:
                print()
                print("Skipped. Run 'ouro config migrate' later to configure.")
        else:
        ...
        # check `ouro curator status`. Self-stamps after printing so it
        ...
        # Self-heal the ouro-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Ouro on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("ouro-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``ouro update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `ouro update`
                # must stay fast; `ouro computer-use install --upgrade`
        ...
        # When running as ``ouro update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... ouro-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... ouro-gateway*)
            ...
            # Discover all ouro-gateway* units (default + profiles) plus
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: ouro gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `ouro update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: ouro gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: ouro -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Ouro gateway unit files are still installed.
        # When both hermes.service (from a pre-rename install) and the
        # current ouro-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `ouro update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Ouro gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (hermes.service) fight the current")
                print("  ouro-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    ouro gateway migrate-legacy")
                print()
        ...
        print("  ouro model              # Select provider and model")
