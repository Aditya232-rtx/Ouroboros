"""Shell completion script generation for ouro CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Ouro bash completion
# Add to ~/.bashrc:
#   eval "$(ouro completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.ouro/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.ouro/profiles" ]]; then
        profiles+=($HOME/.ouro/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'ouro command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Ouro fish completion",
        "# Add to your config:",
        "#   ouro completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.ouro/profiles",
        "        for d in $HOME/.ouro/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c ouro -f -s p -l profile"
