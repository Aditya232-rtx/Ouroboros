"""Runtime-backed validation behind ``ouro plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary OURO_HOME
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "OURO_HOME": str(home),
                "OURO_BUNDLED_PLUGINS": str(bundled),
                "OURO_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Ouro discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    try:
        ...
        pass
    candidates.append(Path.cwd() / ".ouro" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Ouro supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Ouro never auto-installs plugin dependencies; "
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Ouro' real scanner and registration path."""
    try:
