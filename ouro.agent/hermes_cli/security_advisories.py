"""
Security advisory checker for Ouro.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``ouro doctor --ack <id>``; the ack is persisted to
...
1. ``ouro doctor`` (and ``ouro doctor --ack <id>``)
...
run in environments where the rest of Ouro failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including ouro when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.ouro/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: ouro doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Ouro requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'ouro doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.ouro/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``ouro doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `ouro doctor` on the gateway host for details.")
