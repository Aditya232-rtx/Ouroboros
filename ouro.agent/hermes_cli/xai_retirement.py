...
Pure logic: walks a Ouro config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `ouro doctor` and a future `ouro migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Ouro config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Ouro config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `ouro migrate xai` reports nothing to migrate
