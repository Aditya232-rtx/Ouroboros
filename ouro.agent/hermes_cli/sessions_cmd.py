"""``ouro sessions`` command — extracted from ``hermes_cli/main.py``.
...
#: Default age floor for `ouro sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`ouro sessions prune --never-active` — drop leaked/dead keyed rows.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    ouro sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        ...
        # `ouro sessions prune`: no time window and no filters at all
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `ouro sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: ouro sessions pin <session_id>"
            )
            return
        ...
        # Launch ouro --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`ouro sessions optimize` later to reclaim freed space.)")
