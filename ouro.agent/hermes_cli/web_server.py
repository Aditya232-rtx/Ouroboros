...
try:
    ...
    # running `ouro dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["OURO_WEB_DIST"]) if "OURO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``ouro gateway run`` — but the
    desktop app spawns a ``ouro dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same OURO_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `ouro update` current before the
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `ouro update` kept 500ing every
    ...
    # Desktop-spawned backends (OURO_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("OURO_DESKTOP") == "1":
...
app = FastAPI(title="Ouro", version=__version__, lifespan=_lifespan)
...
# OURO_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return (
        os.environ.get("OURO_DASHBOARD_SESSION_TOKEN")
...
_SESSION_HEADER_NAME = "X-Ouro-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Ouro-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `ouro egress setup` "
            "and `ouro egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Ouro (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during ouro update. "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Ouro provider slug — fine.
    ...
    1. Vendor-name → Ouro-provider mapping: when the provider string is
       not a known Ouro provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Ouro
...
_MANAGED_FILES_ROOT_ENV = "OURO_DASHBOARD_FILES_ROOT"
...
# OURO_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to OURO_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Ouro credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no OURO_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("OURO_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``ouro update``.
    ...
    a container — e.g. the hermes-webui image sharing the Ouro source tree),
    the dashboard's ``ouro update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Ouro root is actually /opt/data
    # (the container/hosted layout) or when OURO_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Ouro inside an xterm.js PTY. Browser
    ...
    ``OURO_HOME/images/`` — the same directory ``clipboard.paste`` /
...
async def fs_write_text(payload: FsWriteText):
    ...
    tmp = target.with_name(f".{target.name}.ouro-tmp-{os.getpid()}")
    try:
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # OURO_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # and which providers are registered so ``ouro status`` and the
        ...
        # disk-exhaustion incidents). One statvfs call on OURO_HOME's
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Ouro Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the ouro home root).
    """
...
# streamed to a per-action log file under ``~/.ouro/logs/<action>.log`` so
...
def _spawn_hermes_action(
    ...
    *,
    ...
    """Spawn ``ouro <subcommand>`` detached and record the Popen handle.
    ...
    # _OURO_GATEWAY=1. Inheriting it makes a spawned `ouro gateway restart`
    ...
    action_env = {**os.environ, "OURO_NONINTERACTIVE": "1"}
    action_env.pop("_OURO_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("OURO_ACTION_ID")
    if action_id:
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``ouro gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``ouro gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``ouro gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``OURO_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    """Kick off ``ouro update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Ouro updates are managed outside this dashboard in "
    ...
    try:
        proc = _spawn_hermes_action(
            ["update"],
            "hermes-update",
            env_overrides={"OURO_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn ouro update")
...
async def check_hermes_update(force: bool = False):
    """Report whether a Ouro update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hermes/update`` actually runs ``ouro update``.

    Returns:
        install_method: 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Ouro version string
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.ouro/config.yaml`` under ``tts.``.
    """
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Ouro supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # (OURO_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `ouro model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.ouro/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``ouro model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``ouro model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `ouro model` provider — would be
        ...
        # Vertex — a `ouro model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `ouro model` picker uses (not desktop-only prefix guesses).
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level OURO_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Ouro from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Ouro to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Ouro from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Ouro to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Ouro in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Ouro through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Ouro via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Ouro through an IMAP/SMTP mailbox.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Ouro to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Ouro inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Ouro to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Ouro through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Ouro to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Ouro to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Ouro.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Ouro from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Ouro over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Ouro through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Ouro over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Ouro to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Ouro as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Ouro via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://ouro.dev/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Ouro Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in ouro gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in ouro gateway setup",
...
# hermes_cli/setup_hidden_env.py. Shared with the `ouro setup gateway`
...
def _messaging_platform_payload(
    ...
    # OURO_HOME contextvar override (#56986 / #69143), so the profile's
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.ouro.dev"
_TELEGRAM_ONBOARDING_USER_AGENT = f"OuroDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Ouro").strip() or "Ouro"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Ouro appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the OURO_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized OURO_HOME)
...
# the canonical ``ouro auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.ouro/.anthropic_oauth.json`` — Ouro-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Ouro PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Ouro even
    when they also have a separate Ouro-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Ouro never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "ouro auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "ouro auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "ouro auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "ouro auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "ouro auth add xai-oauth",
        "docs_url": "https://ouro.dev/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "ouro auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Ouro, so the disconnect
    ...
    user sees exactly what executes, and Ouro then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Ouro — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``ouro model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `ouro model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"ouro auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `ouro model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Ouro-managed PKCE file and auth-store entry.
...
#          → persists to ~/.ouro/.anthropic_oauth.json AND credential pool
...
# Guarded so ouro web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Ouro file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``ouro auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_hermes_oauth_file
    oauth_file = _get_hermes_oauth_file()
    payload = {
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("OURO_PORTAL_BASE_URL")
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `ouro auth add xai-oauth`: first credential may become
            ...
            # prior ``ouro auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``ouro model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Ouro "
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `ouro update` until the first message forced a writable open.
    """
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``ouro serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `ouro sessions prune`)."""
    has_window = (
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (OURO_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.ouro``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    OURO_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, OURO_HOME)."""
    from hermes_cli import profiles as profiles_mod
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # test_config_read_guard.py). The OURO_HOME override scopes
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``ouro gateway stop`` persists ``"stopped"``; start and
...
# servers managed here show up under `ouro mcp list` and vice versa.  Secrets
...
# spawn the real `ouro gateway <verb>` so behaviour matches the CLI exactly.
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `ouro auth remove`) can seed again.
        # Mirrors the `ouro auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``ouro auth remove`` uses: each source cleans up its
...
def _profile_setup_command(name: str) -> str:
    ...
    return "ouro setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local OURO_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local OURO_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the OURO_HOME override. Returns the
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a OURO_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Ouro,
...
#
# The endpoint spawns the same ``ouro --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``ouro --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``OURO_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``OURO_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``OURO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``OURO_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``OURO_HOME`` at the profile dir in the child env. Every spawned
    ...
    — the same propagation ``ouro -p <name>`` performs. The in-process
    ``OURO_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    # Ouro TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides OURO_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("OURO_TUI_DISABLE_MOUSE", "1")
    env.setdefault("OURO_TUI_INLINE", "1")
    ...
    env["OURO_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["OURO_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["OURO_TUI_RESUME"] = resume

    if sidecar_url:
        env["OURO_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile OURO_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["OURO_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``OURO_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("OURO_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
# ---------------------------------------------------------------------------
# /api/console — safe Ouro Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Ouro CLI
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Ouro Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Ouro inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("OURO_TUI_RESUME") or raw_resume
...
# OURO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Ouro Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__OURO_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `ouro serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("OURO_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (ouro serve): web UI disabled — use "
            "`ouro dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__OURO_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__OURO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__OURO_BASE_PATH__="{prefix}";'
                f"window.__OURO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__OURO_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__OURO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__OURO_BASE_PATH__="{prefix}";'
                f"window.__OURO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``OURO_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Ouro Teal canvas, then
    ...
    # (the MC Pages app), not the Ouro backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Ouro Teal",         "description": "Classic dark teal — the canonical Ouro look"},
    {"name": "default-large", "label": "Ouro Teal (Large)", "description": "Ouro Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.ouro/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.ouro/dashboard-themes/*.yaml for user-created themes.
    ...
    live under the server's own ``OURO_HOME``.
    """
...
async def get_dashboard_themes():
    ...
    from `~/.ouro/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.ouro/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.ouro/plugins/  (only if OURO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # OURO_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``OURO_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the ouro root (``~/.ouro/plugins``). Scan the default
    # root as well (``get_default_hermes_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``OURO_HOME`` unchanged when it *is* the root), mirroring how
    ...
    if env_var_enabled("OURO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".ouro" / "plugins", "project"))
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.ouro/plugins/``) ship with the CWD and are
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``OURO_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("OURO_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``OURO_PARENT_PID`` is the Electron Desktop PID, not necessarily this
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``OURO_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("OURO_PARENT_PID")
    start_marker = os.environ.get("OURO_PARENT_START_MARKER")
    nonce = os.environ.get("OURO_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("OURO_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``OURO_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing OURO_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Ouro exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `ouro dashboard register` (Nous Portal) or "
            ...
            try:
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`ouro plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # OURO_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            # for standalone `ouro serve` (no OURO_PARENT_PID env).
            ...
            ready_token = "OURO_BACKEND_READY" if headless else "OURO_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Ouro backend listening on {host}:{actual_port}")
            else:
                print(f"  Ouro Web UI → http://{host}:{actual_port}")
