"""
Gateway subcommand for ouro CLI.

Handles: ouro gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `ouro update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "ouro CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `ouro update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``ouro gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``ouro gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    # Forward slashes on both sides of the OURO_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. OURO_HOME
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``ouro update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``ouro update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Ouro profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Ouro profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``ouro update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Ouro's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose OURO_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``ouro update`` users
    # with no running gateway until they re-invoke ``ouro gateway``
    ...
    # interpreter and captures the stable cwd + env overlay (OURO_HOME,
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / OURO_HOME) the windowless
...
def _hermes_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``OURO_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``OURO_HOME``.

    Under ``sudo``, ``OURO_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.ouro`` — the wrong
    profile. The unit file pins ``OURO_HOME`` for the actual gateway
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("OURO_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("OURO_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["OURO_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}ouro gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}ouro gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: ouro gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `ouro gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``ouro gateway status`` output so users with
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            ...
            # dormant on every standard `ouro gateway install` deployment.
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (OURO_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Ouro Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``ouro gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``OURO_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("OURO_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "ouro-gateway"
SERVICE_DESCRIPTION = "Ouro Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current OURO_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (OURO_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary OURO_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when OURO_HOME is a named profile.

    For ``~/.ouro/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit OURO_HOME path. Defaults to the current
            ``get_hermes_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Ouro root to compare against. Used when
...
def _profile_arg_for_target_user(hermes_home: str, target_home_dir: str) -> str:
    ...
    target_root = Path(target_home_dir) / ".ouro"
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this OURO_HOME.

    Default ``~/.ouro`` returns ``ouro-gateway`` (backward compatible).
    Profile ``~/.ouro/profiles/coder`` returns ``ouro-gateway-coder``.
    Any other OURO_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Ouro installs that predate the
# ouro-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (ouro-gateway-*.service) and unrelated third-party
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Ouro gateway units.

    Detects unit files installed by older Ouro versions that used a
    different service name (e.g. ``hermes.service`` before the rename to
    ``ouro-gateway.service``). When both a legacy unit and the current
    ``ouro-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``ouro-gateway-coder.service`` and unrelated third-party
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Ouro gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Ouro gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Ouro gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current ouro-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    ouro gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Ouro gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Ouro gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Ouro gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: ouro gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo ouro gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    ouro gateway uninstall")
    print_info("    sudo ouro gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: ouro gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``ouro setup`` (end of wizard)
    and ``ouro import``: if this host supports a service manager and no
    ...
    ``ouro gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   ouro gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: ouro gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: ouro gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Ouro often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.ouro`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.ouro/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.ouro/hermes-agent  -> /home/alice/.ouro/hermes-agent
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current OURO_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.ouro                    → /home/alice/.ouro
      /root/.ouro/profiles/coder     → /home/alice/.ouro/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes_raw = os.environ.get("OURO_HOME", "").strip()
    current_hermes = (
    ...
    # different OURO_HOME into the generated service unit.
    current_default = Path.home() / ".ouro"
    target_default = Path(target_home_dir) / ".ouro"

    # Default ~/.ouro → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.ouro → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.ouro) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``ouro update`` later relocates/removes — the path
    ...
    ``OURO_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if OURO_HOME
...
def _append_node_dir_for_service(
    ...
    The Ouro-managed Node under ``$OURO_HOME/node`` goes first when it
    ...
    *hermes_root* is the Ouro home the unit will run against. System units
    ...
    # target Ouro home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Ouro home is known — probing here would stat the calling
    ...
    if system:
        ...
        # Anchor cwd to the target user's OURO_HOME (stable, always exists)
...
Environment="OURO_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned OURO_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo ouro gateway … --system``, OURO_HOME is often stripped
    # and falls back to ``/root/.ouro``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / OURO_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir OURO_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved OURO_HOME
    ...
    exported a throwaway ``OURO_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``ouro gateway restart``
    with ``OURO_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="OURO_HOME=..."`` lines and launchd
    ``<key>OURO_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'OURO_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>OURO_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp OURO_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: OURO_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported OURO_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single OURO_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only OURO_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp OURO_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="OURO_HOME=..."``. Writing that to the developer's
    ...
    # OURO_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Ouro install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo ouro gateway uninstall --system")
    print_info("         ouro gateway install")
    print_info("         ouro gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("OURO_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("OURO_RESTART_AFTER_TURN_TIMEOUT")
...
def systemd_install(
    ...
    # before installing the new ouro-gateway.service. If both remain, they
    ...
    # Existing system units already pin OURO_HOME; adopt it before any
    ...
    # ``sudo ouro gateway install --system --force`` would bake /root/.ouro
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}ouro gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}ouro gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}ouro gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # OURO_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `ouro gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `ouro gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}ouro gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}ouro gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}ouro gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}ouro gateway start{scope_flag}"
        )
...
# process invocation (each ``ouro gateway start/stop/status`` call).
...
#      background process (the `nohup ouro gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    ...
    Profile-aware: honors the active OURO_HOME via `_profile_arg()` so the
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``ouro update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup ouro gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup ouro gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `ouro gateway start`)
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.ouro/logs/launchd-reload.log, which the health watchdog
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Ouro install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  ouro gateway status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `ouro gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `ouro update`
...
def launchd_status(deep: bool = False):
    ...
    # Ouro PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Ouro PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Ouro install")
    else:
        print("⚠ Service definition is stale relative to the current Ouro install")
        print("  Run: ouro gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: ouro gateway stop")
            else:
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``OURO_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``ouro gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``ouro gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``ouro gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other OURO_HOME roots.
    """
    ...
    print("  Use 'ouro gateway restart' to replace it,")
    print("  or 'ouro gateway stop' first.")
    print("  Or use 'ouro gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("OURO_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Ouro gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Ouro command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$OURO_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set OURO_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `ouro gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set OURO_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("OURO_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `ouro gateway restart` which already
    ...
    # `ouro gateway start/restart` — leaving the gateway vulnerable to
    ...
    # OURO_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("OURO_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``OURO_GATEWAY_MAX_STARTS`` /
    # ``OURO_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("OURO_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("OURO_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``ouro gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Ouro delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Ouro connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: ouro pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Ouro will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``ouro setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run ouro under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.ouro/plugins/ still require
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'ouro pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: ouro pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'ouro gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Ouro will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Ouro will store the returned account_id/token in ~/.ouro/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `ouro gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `ouro pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `ouro pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "Ouro"')
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.ouro/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.ouro/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: ouro gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: ouro gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: ouro gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo ouro gateway install --system"
                        )
                    print_info("  Or run in foreground:  ouro gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: ouro gateway run")
                print_info(
                    "  For persistence:   tmux new -s ouro 'ouro gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: ouro gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup ouro gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'ouro gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    if profile is None:
        ...
        # OURO_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``ouro gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``ouro gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``ouro gateway run`` are unaffected.
      2. ``OURO_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``ouro gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``OURO_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("OURO_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("OURO_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${OURO_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if OURO_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or OURO_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: ouro gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: ouro gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s ouro 'ouro gateway run'"
                )
                print()
            ...
            print()
            print(
                "  ouro gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s ouro 'ouro gateway run'         # persistent via tmux"
            )
            print(
                "  nohup ouro gateway run > ~/.ouro/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ouro profile create <name>     # creates the s6 service slot")
                print("  ouro -p <name> gateway start   # bring it up via s6")
                print("  ouro status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: ouro gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: ouro gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: ouro gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ouro profile delete <name>     # tears down the s6 service slot")
                print("  ouro -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `ouro -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: ouro gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_OURO_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `ouro gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_OURO_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `ouro gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: ouro gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  ouro gateway install")
                else:
                    print("To install as a service:")
                    print("  ouro gateway install")
                    print("  sudo ouro gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  ouro gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup ouro gateway run > ~/.ouro/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s ouro 'ouro gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  ouro gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  ouro gateway install  # Install as user service")
                    print(
                        "  sudo ouro gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Ouro gateway unit files from
