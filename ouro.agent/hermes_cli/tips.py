...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or ouro -c.",
    ...
    "Ctrl+Z suspends Ouro to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "ouro -c resumes your most recent CLI session. ouro -c \"project name\" resumes by title.",
    "ouro -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "ouro -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "ouro chat -t web,terminal enables only specific toolsets for a focused session.",
    "ouro chat -s github-pr-workflow preloads a skill at launch.",
    "ouro chat -q \"query\" runs a single non-interactive query and exits.",
    "ouro chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "ouro chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "ouro --yolo bypasses all dangerous command approval prompts for the entire session.",
    "ouro chat --source telegram tags the session for filtering in ouro sessions list.",
    "ouro -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "ouro doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "ouro dump outputs a compact setup summary — great for bug reports.",
    "ouro config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "ouro config edit opens config.yaml in your default editor.",
    "ouro config check scans for missing or stale configuration options.",
    "ouro sessions browse opens an interactive session picker with search.",
    "ouro sessions stats shows session counts by platform and database size.",
    "ouro sessions prune --older-than 30 cleans up old sessions.",
    "ouro skills search react --source skills-sh searches the skills.sh public directory.",
    "ouro skills check scans installed hub skills for upstream updates.",
    "ouro skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "ouro skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "ouro mcp add github --command npx adds MCP servers from the command line.",
    "ouro mcp serve runs Ouro itself as an MCP server for other agents.",
    "ouro auth add lets you add multiple API keys for credential pool rotation.",
    "ouro completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "ouro logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "ouro backup creates a zip backup of your entire Ouro home directory.",
    "ouro profile create coder creates an isolated profile that becomes its own command.",
    "ouro profile create work --clone copies your current config and keys to a new profile.",
    "ouro update syncs new bundled skills to ALL profiles automatically.",
    "ouro gateway install sets up Ouro as a system service (systemd/launchd).",
    "ouro memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "ouro webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: ouro tools disables unused tools, ouro skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "ouro models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as ouro -w).",
    ...
    "OURO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Ouro tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'ouro profile create coder' creates the 'coder' command.",
    "ouro profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Ouro prints a resume command with session ID and stats.",
    "ouro sessions export backup.jsonl exports all sessions for backup or analysis.",
    "ouro -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "ouro skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .ouro/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: ouro cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Ouro runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "ouro gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Ouro via a local macOS server.",
    ...
    "Context files (.ouro.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Ouro loads project context from .ouro.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "ouro mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "ouro mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "ouro mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — ouro tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.ouro/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "ouro chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "ouro plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "ouro acp runs Ouro as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "OURO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "ouro auth add nous or ouro auth add openai-codex sets up OAuth-based providers.",
    ...
    "ouro status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.ouro/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Ouro your own.",
    ...
    "Before a gateway session reset, Ouro auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.ouro/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Ouro state via OURO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "OURO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in OURO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.ouro/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'ouro profile create ls' would be rejected.",
    "ouro profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "ouro claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "OURO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via ouro tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.ouro/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.ouro/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Ouro auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Ouro config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Ouro is working.',
    ...
    '/reload reloads ~/.ouro/.env into the running session — pick up new API keys without restarting.',
    ...
    "OURO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.ouro/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.ouro/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'ouro curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "ouro curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'ouro curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'ouro auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'OURO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "OURO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'ouro dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'ouro dashboard embeds the full Ouro TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.ouro/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.ouro/dashboard-plugins/ — no npm build required.',
    ...
    'OURO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'OURO_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'OURO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'OURO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.ouro/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.ouro/skills/ so drop-in skills appear without restarting the session.',
    ...
    'ouro -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'ouro chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'ouro chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'ouro chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "ouro chat --source tool tags programmatic chats so they don't clutter ouro sessions list.",
    'ouro dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'ouro sessions rename <ID> "new title" renames any past session; ouro sessions delete <ID> removes one.',
    'ouro import restores a full Ouro backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'ouro fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'ouro pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'ouro setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'ouro status --deep runs the full health sweep across every component; plain ouro status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'OURO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'OURO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.ouro/plugins/ — trust-gated by design.',
    "OURO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'OURO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'OURO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'OURO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'OURO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'OURO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'OURO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'OURO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'OURO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'OURO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'OURO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Ouro block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from ouro sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.ouro/dashboard-plugins/.',
]
