"""On-demand supply-chain audit for Ouro installs.

Scans three surfaces a Ouro user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Ouro venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.ouro/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.ouro/plugins``.
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `ouro security audit`."""
