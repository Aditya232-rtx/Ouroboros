...
_TASK_DESCRIPTION = "Ouro Gateway - Messaging Platform Integration"
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Ouro-owned paths under the configured OURO_HOME spelling.
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Ouro profile."""
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("OURO_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("OURO_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("OURO_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["OURO_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["OURO_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["OURO_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("OURO_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("OURO_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("OURO_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_script_path() -> Path:
    ...
    (or ``<OURO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Ouro installs stay self-contained).
    """
    _assert_windows()
    from hermes_cli.config import get_hermes_home
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``OURO_HOME`` whenever it
    ...
    source checkout only if ``OURO_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports OURO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    to that terminal like a normal foreground ``ouro gateway run``.
    ...
    lines.append(f'set "OURO_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "OURO_GATEWAY_DETACHED=1"')
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('OURO_HOME')}) = {_quote_vbs_string(hermes_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('OURO_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Ouro builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    env_overlay = {
        "OURO_HOME": hermes_home,
        "PYTHONIOENCODING": "utf-8",
        "OURO_GATEWAY_DETACHED": "1",
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (OURO_HOME, VIRTUAL_ENV,
    ...
    if hermes_home:
        env_overlay["OURO_HOME"] = hermes_home
    _prepend_pythonpath(
        env_overlay,
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("OURO_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("OURO_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `ouro -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"ouro {profile_arg} gateway start" if profile_arg else "ouro gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: ouro gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Ouro gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: ouro gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: ouro gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Ouro gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: ouro gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: ouro gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: ouro gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `ouro gateway status`."
        )
