...
configured for "cli" in `ouro tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (OURO_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `ouro chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"ouro -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "ouro -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"ouro -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "ouro -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "ouro -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to OURO_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("OURO_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "ouro -z: --provider requires --model (or OURO_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"ouro -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("ouro -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``ouro -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when ouro is invoked for
    ...
    env_model = os.getenv("OURO_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("OURO_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                OURO_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via OURO_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via OURO_YOLO_MODE=1
