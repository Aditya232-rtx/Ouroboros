"""Helpers for loading Ouro .env files consistently across entrypoints."""
...
# Used by setup / `ouro model` flows to label detected credentials so
...
# OURO_HOME paths we've already pulled external secrets for during this
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Ouro env-var keys.
...
# Behavioral routing keys a parent Ouro process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Ouro keys absent from ``.env``.
    ...
    parent Ouro process injects and that silently change *which provider
...
def reset_secret_source_cache() -> None:
    """Forget which OURO_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `ouro setup` (or edit the "
...
def load_hermes_dotenv(
    *,
    ...
    """Load Ouro environment files with user config taking precedence.

    Behavior:
    - `~/.ouro/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("OURO_HOME", Path.home() / ".ouro"))
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Ouro keys
    ...
    #   EnvironmentFile=-/path/to/.ouro/.op.env
    ...
    # 2. A fresh ``ouro update`` retry just completed a deferred dependency
    ...
    # TERMINAL_ENV=docker left in ~/.ouro/.env (e.g. written by an older
    # `ouro setup` before the user switched terminal.backend in config.yaml)
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Scoped to the process OURO_HOME: the shared bridge reads the
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of OURO_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Ouro reads
    ...
    owns the once-per-OURO_HOME guard, the post-apply ASCII
    ...
    # a generic dict entry must not force crypto load on every ouro launch.
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `ouro model`
...
def _process_hermes_home() -> Path:
    """The OURO_HOME the shared config cache is keyed to."""
    try:
