"""
Ouro Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``ouro desktop`` / ``ouro gui``)
     Built inside the agent checkout under ``$OURO_HOME/hermes-agent/``:
       ...
       - ``$OURO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Ouro.app`` or ``~/Applications/Ouro.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Ouro`` (NSIS per-user)
...
the app name ("Ouro"), separate from ``$OURO_HOME``:
  - macOS:   ``~/Library/Application Support/Ouro``
  - Windows: ``%APPDATA%\\Ouro``
  - Linux:   ``$XDG_CONFIG_HOME/Ouro`` (default ``~/.config/Ouro``)
...
``ouro uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(hermes_home: Path) -> Path:
    ...
    return hermes_home / "ouro-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Ouro"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Ouro"
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``ouro desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``ouro-agent/`` source + ``venv/`` and never
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Ouro" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Ouro.app"),
            home / "Applications" / "Ouro.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Ouro).
            local_base / "Programs" / "Ouro",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Ouro")
    else:
        ...
        paths += [
            # The launcher entry `ouro desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Ouro.desktop",
        ]
    return paths


def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under OURO_HOME.
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``ouro-agent/hermes_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$OURO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Ouro disappears from the launcher.
        try:
