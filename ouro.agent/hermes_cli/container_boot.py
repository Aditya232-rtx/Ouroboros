...
``$OURO_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $OURO_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$OURO_HOME``,
    not under ``profiles/``). The dispatcher in ``hermes_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``ouro gateway start`` (no ``-p``) targets. Without it,
    bare ``ouro gateway start`` inside the container would land on
    ...
    ``$OURO_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's OURO_HOME (typically /opt/data).
    ...
    # ``ouro gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `ouro profile create` (config.yaml
            # is not — that comes later via `ouro setup`). Use it as the
            ...
            # via ``ouro -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("OURO_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the ouro args.
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    ``init`` prefix. The wrapper re-execs ``ouro <subcommand>``, so an
    ...
    # The wrapper re-execs `ouro <subcommand>`; peel an explicit hermes.
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``ouro dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted OURO_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `ouro -p <profile> gateway start`
        ...
        # (all dispatched as the ouro user) won't hit EACCES. See
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $OURO_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted OURO_HOME, both race to flock() the same s6-log lock
