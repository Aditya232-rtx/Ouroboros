...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``ouro model`` wizard — not a
    ...
    while ``ouro model`` still live-probed the full ``/v1/models`` list.
...
_OURO_MODEL_WARNING = (
    "Nous Research Ouro 3 & 4 models are NOT agentic and are not designed "
    "for use with Ouro. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Ouro 3 / Ouro 4 chat families.
...
#   NousResearch/Ouro-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   hermes-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_OURO_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Ouro 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_OURO_NON_AGENTIC_RE.search(model_name))


def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Ouro 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _OURO_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``ouro config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'ouro model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'ouro doctor' — config issues detected:"
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Ouro provider IDs to prefetch (e.g. ``["openrouter",
...
def _collect_authed_provider_slugs(
    ...
    from hermes_cli.providers import OURO_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # --- Section 1: Ouro-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    ...
    # --- Section 2: Ouro-only providers (OURO_OVERLAYS) ---
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in OURO_OVERLAYS.items():
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Build curated model lists keyed by ouro provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://ouro.dev/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Ouro release. Falls back to the in-repo
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (OURO_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Ouro may not route yet.
        ...
        # /model picker sees the SAME list `ouro model` would build, with
    ...
    # --- 2. Check Ouro-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import OURO_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Ouro provider ID.
    # OURO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Ouro IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in OURO_OVERLAYS.items():
        ...
        # Resolve Ouro slug — e.g. "github-copilot" → "copilot"
        hermes_slug = _mdev_to_hermes.get(pid, pid)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `ouro model` picker deliberately shows ONLY the curated agentic
    ...
    # in PROVIDER_TO_MODELS_DEV or OURO_OVERLAYS (keeps /model in sync
    # with `ouro model`).
    try:
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Ouro writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Ouro's own writer uses (mirrors section-4 grouping).
            ...
            # ``ouro model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``ouro model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Ouro's own writer (main.py::_save_custom_provider)
        ...
        for grp in groups.values():
            ...
            #   ``ouro model``. Pin a dict catalog with
