"""``ouro doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from hermes_cli.doctor import OURO_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (OURO_HOME / "node" / "bin",
                          OURO_HOME / "node",
                          OURO_HOME / "node_modules" / ".bin"):
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``ouro mcp test``.
    """
