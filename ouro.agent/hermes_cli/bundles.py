"""Implementation of the ``ouro bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `ouro bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  ouro bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``ouro bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `ouro bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``ouro bundles <subcommand>`` to the right handler."""
