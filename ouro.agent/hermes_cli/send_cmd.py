"""CLI subcommand: ``ouro send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"ouro send: {file_path} is not a text file. --file reads the "
                ...
                f'  ouro send --to telegram "MEDIA:{file_path}"\n'
                f'  ouro send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  ouro send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"ouro send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"ouro send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"ouro send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"ouro send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `ouro send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"ouro send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `ouro gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.ouro/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.ouro/.env`` AND bridge top-level
    ...
    startup that ``ouro send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.ouro/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.ouro/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``ouro send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.ouro/.env and ~/.ouro/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "ouro send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  ouro send --to telegram \"hello\"\n"
            "  ouro send --to discord:#ops --file report.md\n"
            "  ouro send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "ouro send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `ouro send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Ouro "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.ouro/.env + ~/.ouro/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  ouro send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `ouro send --list telegram`.",
    )
