"""
MCP Server Management CLI — ``ouro mcp`` subcommand.

Implements ``ouro mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.ouro/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.ouro/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``ouro mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  ouro mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  ouro mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  ouro mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: ouro mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `ouro mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  ouro mcp add <name> --url <endpoint>')
        _info('  ouro mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``ouro mcp login`` and
    ``ouro mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `ouro mcp remove` + `ouro mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `ouro mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Ouro desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `ouro mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``ouro mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``ouro mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: ouro mcp reauth <name>   |   ouro mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'ouro mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``ouro mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `ouro plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("ouro mcp                                    Open the catalog picker (default)")
        _info("ouro mcp catalog                            List Nous-approved MCPs")
        _info("ouro mcp install <name>                     Install a catalog MCP")
        _info("ouro mcp serve                              Run as MCP server")
        _info("ouro mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("ouro mcp add <name> --command <cmd>         Add a stdio server")
        _info("ouro mcp add <name> --preset <preset>       Add from a known preset")
        _info("ouro mcp remove <name>                      Remove a server")
        _info("ouro mcp list                               List configured servers")
        _info("ouro mcp test <name>                        Test connection")
        _info("ouro mcp configure <name>                   Toggle tools")
        _info("ouro mcp login <name>                       Re-authenticate OAuth")
        _info("ouro mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
