...
The provider list shown by ``ouro model`` (CLI/TUI) and the desktop Settings
...
the same universe ``ouro model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``ouro model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `ouro model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``ouro model`` universe.
    ...
    # Ouro overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = OURO_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Ouro overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
