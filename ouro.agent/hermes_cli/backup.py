"""
Backup and import commands for ouro CLI.

`ouro backup` creates a zip archive of the entire ~/.ouro/ directory
(excluding the ouro-agent repo and transient files).

`ouro import` restores from a backup zip, overlaying onto the current
OURO_HOME root.
"""
...
# ``ouro-agent`` is special-cased to root level only in ``_should_exclude``
...
# OURO_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "ouro-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under OURO_HOME) —
...
# File names that ``ouro import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE OURO_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Ouro backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Ouro backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside OURO_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to ouro root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``ouro-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/hermes-agent/`` — must be preserved.
        if part == "ouro-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside OURO_HOME.
...
# to the user as a hung `ouro update` (#70553 follow-up). Sessions databases
...
def run_backup(args) -> None:
    """Create a zip backup of the Ouro home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Ouro home directory not found at {hermes_root}")
        sys.exit(1)
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``ouro-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "ouro-agent" and not is_root)
        ]
    ...
    # outside OURO_HOME, so the walk above never sees it. Ask the active
    ...
    if not errors:
        print(f"\nRestore with: ouro import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Ouro backup.
    ...
    # Look for telltale files that a ouro home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Ouro backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.ouro/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a ouro dir name
        if prefix in {".ouro", "hermes"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``ouro import``, which is the
    ...
    so without the chown a ``sudo ouro import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``ouro import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Ouro' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Ouro configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under OURO_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: ouro profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "ouro-agent").is_dir():
            print("Note: The ouro-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: ouro update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  ouro -p {pname} gateway install")
        ...
        print("Done. Your Ouro configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and ouro backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to OURO_HOME).
...
# — `ouro update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $OURO_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside OURO_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: ouro snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside OURO_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``ouro update``.
    ...
    Args:
        ...
        hermes_home: Override for the Ouro home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for ouro backup --quick."""
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `ouro claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of OURO_HOME under ``backups/`` before a
    ``ouro claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``ouro import <archive>``.  Writes to
    ``<OURO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `ouro import` and the
