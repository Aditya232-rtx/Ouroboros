...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Ouro. Use 'ouro debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    ouro debug share              Upload debug report (asks for confirmation)
    ouro debug share --yes        Skip confirmation (for scripts/CI)
    ouro debug share --lines 500  Include more log lines
    ouro debug share --expire 30  Keep paste for 30 days
    ouro debug share --local      Print report locally (no upload)
    ouro debug share --no-redact  Disable upload-time secret redaction
    ouro debug share --nous       Upload to Nous-internal storage (private)
    ouro debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'ouro debug share'",
    )
