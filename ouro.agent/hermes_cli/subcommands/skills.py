...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.ouro/skills/.bundled_manifest) "
            "so future 'ouro update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `ouro update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `ouro update` reports as user-modified and skips. "
            "Use `ouro skills diff <name>` to see changes and `ouro skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`ouro update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `ouro update`. Pass --sync to re-seed now."
        ),
    )
