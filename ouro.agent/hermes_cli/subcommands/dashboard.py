"""``ouro dashboard`` / ``ouro serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `ouro dashboard` / `ouro serve` cmdlines and SIGTERM them directly —
    # the same path `ouro update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Ouro web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Ouro web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Ouro web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Ouro desktop app shells (<= 0.15.x) spawn the
    # backend as `ouro dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Ouro couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Ouro Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Ouro backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Ouro backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports OURO_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `ouro dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.ouro/.env.
    # Nested subparser so bare `ouro dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "OURO_DASHBOARD_OAUTH_CLIENT_ID into ~/.ouro/.env, and prints "
            "how to engage the login gate. Requires being logged in (ouro setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "portal. Also settable via OURO_DASHBOARD_PORTAL_URL. Mainly for "
