...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``ouro login`` was removed in favor of ``ouro auth`` / ``ouro model``
    ...
    old scripts/aliases invoking ``ouro login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``ouro --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``ouro login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``ouro model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `ouro auth` to manage credentials, "
            "`ouro model` to select a provider, or `ouro setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `ouro model`",
    )
    login_parser.add_argument(
