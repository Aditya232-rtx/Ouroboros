"""``ouro sync`` subcommand parser — Skill Sync.
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    ouro sync status                 show gate/opt-in/head state
    ouro sync pull                   pull and materialize opted-in skills
    ouro sync push                   push opted-in skills
    ouro sync now                    reconcile: pull then push
    ouro sync enable <skill>         opt a skill into sync
    ouro sync disable <skill>        opt a skill out of sync
    ouro sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    ouro sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  ouro sync status            what is synced, and from where\n"
            "  ouro sync enable my-skill   include a skill in your sync\n"
            "  ouro sync now               pull, then push\n"
            "  ouro sync propose my-skill  share a skill with your team\n"
        ),
