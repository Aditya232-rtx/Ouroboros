"""
Dump command for ouro CLI.

Outputs a compact, plain-text summary of the user's Ouro setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.ouro/.env.
    ...
    exports. ``ouro debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.ouro_build_sha`` by the Dockerfile's
    ``OURO_GIT_SHA`` build-arg (see ``hermes_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``ouro dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.ouro/.env: a managed
        ...
        # A credential added via `ouro auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `ouro auth list` shows it (#42130).
