...
# commands at execution time when ``_OURO_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: ouro gateway install", Colors.DIM))
    print(color("                    sudo ouro gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  ouro cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'ouro cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `ouro cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: ouro gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec ouro "
                        "ouro cron ...`). Fix ownership to match the "
        ...
        print("    ouro gateway install    # Install as a user service")
        print("    sudo ouro gateway install --system  # Linux servers: boot-time system service")
        print("    ouro gateway            # Or run in foreground")
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (OURO_SESSION_KEY);
...
def cron_notepad(args) -> int:
    """Handle ``ouro cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: ouro cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: ouro cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: ouro cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: ouro cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
