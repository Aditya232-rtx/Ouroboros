"""
Unified self-relaunch for Ouro CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``ouro sessions browse`` or post-setup relaunch
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh ouro invocation.
    ...
    relaunch contract wants: "run ouro again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "ouro exited, then
    new ouro started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                f"\nOuro relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run ouro.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
