"""
Unified tool configuration for Ouro.

`ouro tools` and `ouro setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.ouro/config.yaml under
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``ouro tools post-setup <key>`` child. On Windows, every console child
...
# They're still in _OURO_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `ouro tools` → Video Generation, which walks
...
# `ouro tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `ouro tools` for provider/API-key
...
# Platform-scoped toolsets: only appear in the `ouro tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``ouro tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``ouro tools`` checklist actually offers
    for ``platform``.
    ...
    so ``ouro tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Ouro routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set OURO_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "OURO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "OURO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "OURO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "OURO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "OURO_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `ouro model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("OURO_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``OURO_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Ouro telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Ouro' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $OURO_HOME/bin is never on PATH, so a bare which()
    # misses the uv Ouro installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``ouro update`` and
      by ``ouro computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``ouro update`` sets
    ...
    a further ~600s wait on Windows). ``ouro computer-use install
    ...
    ``ouro update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `ouro update` calls this
    ...
    override = os.environ.get("OURO_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    OURO_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Ouro' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Ouro' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `ouro update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Ouro cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("OURO_CUA_DRIVER_CMD", "").strip():
            _print_info(
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `ouro computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `ouro update` (require_confirmed_update=True) keeps the
    ...
    # `ouro computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: ouro computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: ouro computer-use doctor")
            return False
    if ok and before:
        try:
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `ouro update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `ouro update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.ouro/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Ouro process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Ouro-managed ``$OURO_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Ouro installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Ouro-managed install)
        ...
        # (PATH -> Homebrew/Ouro-managed node -> npx) rather than a bare
        # shutil.which — Ouro-managed-Node-only setups resolve agent-browser
        ...
        # Ouro-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Ouro-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.ouro/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.ouro/config.yaml")
        ...
        # Run the full `ouro auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.ouro/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: ouro auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `ouro auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: ouro auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: ouro plugins enable observability/langfuse")
        _print_info("    Restart Ouro for tracing to take effect.")
        _print_info("    Verify: ouro plugins list")
        ...
        try:
            ...
            _print_info("    Run later: ouro auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `ouro auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: ouro auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: ouro auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``ouro tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``ouro tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: ouro tools post-setup <key>")
        return 2
...
#:
#: Saving ``ouro tools`` (or one toggle in the desktop Toolsets UI) replaces
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``ouro tools``
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (ouro-acp, hermes-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # "hermes-cli" (which include all _OURO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # through ``ouro tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `ouro tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `ouro tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `ouro tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by ouro tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `ouro config set` or a
    ...
    # at runtime, not only during the next `ouro update`/`ouro doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `ouro tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `ouro tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `ouro tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``ouro tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``ouro tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    # The install hook runs in a spawned ``ouro tools post-setup`` process,
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `ouro model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``ouro model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``ouro model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `ouro model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `ouro model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `ouro tools` and `ouro setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Ouro Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://ouro.dev/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, ouro mcp
