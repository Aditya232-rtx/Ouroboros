"""ouro import-agent — import Claude Code / Codex CLI setups into Ouro.

Usage:
    ouro import-agent                       # auto-detect ~/.claude or ~/.codex
    ouro import-agent claude-code           # import from ~/.claude
    ouro import-agent codex                 # import from ~/.codex
    ouro import-agent claude-code --dry-run # preview only, no changes
    ouro import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``ouro claw migrate`` /
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in OURO_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → OURO_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in OURO_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in OURO_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → OURO_HOME/skills/codex-imports/<name>/
...
deliberately via ``ouro setup`` or config.yaml.
"""
...
# Same entry delimiter as the Ouro memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`ouro config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Ouro command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Ouro glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Ouro equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → OURO_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``ouro import-agent`` (invoked from hermes_cli.main)."""
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: ouro import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: ouro import-agent claude-code   or   ouro import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Ouro — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: ouro import-agent "
    ...
    print_info("Secrets:     never imported — run 'ouro setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: ouro import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'ouro setup' "
               "to configure providers, or add them to ~/.ouro/.env.")
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'ouro setup' or ~/.ouro/.env.")
        print()
