...
manual BotFather token copy-paste. Ouro talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.ouro.dev"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Ouro Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Ouro Telegram onboarding service.")
