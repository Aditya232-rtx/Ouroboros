"""``ouro plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.ouro/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Ouro subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.ouro/.env``."""
...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Ouro never auto-installs plugin pip
    ...
    with tempfile.TemporaryDirectory(prefix=".install-", dir=plugins_dir) as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Ouro.",
                ) from None
        ...
        if target.exists() and not force:
            raise PluginOperationError(
                ...
                f"or run `ouro plugins update {plugin_name}`."
            )
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `ouro plugins search <term>` to "
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Ouro plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `ouro plugins enable {installed_name}` to activate.[/dim]",
        )
    ...
    console.print("[dim]  ouro gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`ouro plugins install {recorded_source} --force "
        ...
        if allowed is not True:
            ...
            if scan_result.verdict == "dangerous":
                ...
                console.print(
                    ...
                    f"findings, then re-enable with `ouro plugins enable {name}` "
                    f"if you trust them.",
                )
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``ouro setup`` / ``ouro plugins disable basic`` can park the plugin
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``ouro plugins enable`` /
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Ouro core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
    ...
    *,
    ...
    if not (sys.stdin.isatty() and sys.stdout.isatty()):
        console.print(
            ...
            f"`ouro plugins capabilities {plugin_id}` to review and "
            f"`ouro plugins enable {plugin_id}` to grant interactively."
        )
        return False
    ...
    console.print(
        ...
        f"`ouro plugins enable {plugin_id}` to grant later.[/dim]"
    )
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``ouro plugins capabilities [<id>]`` — declared vs granted."""
    from rich.console import Console
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`ouro plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    plugin directory under ``~/.ouro/plugins``. Include package metadata here
    so ``ouro plugins list`` can show and enable them.
    """
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``ouro plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] ouro plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] ouro plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] ouro plugins")
    console.print("[dim]Enable/disable:[/dim] ouro plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Ouro
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] ouro plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``ouro plugins enable web/firecrawl`` cleared
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.ouro/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.ouro/plugins/<name>``."""
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `ouro plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``ouro plugins update`` path: drop bytecode
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``ouro update``
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.ouro/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def cmd_search(
    ...
    *,
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: ouro plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")


def plugins_command(args) -> None:
    """Dispatch ouro plugins subcommands."""
