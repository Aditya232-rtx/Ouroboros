"""
Top-level argparse construction for the ouro CLI.
...
# argparse runs (it sets ``OURO_HOME`` and strips itself from ``sys.argv``),
...
_EPILOGUE = """
Examples:
    ouro                        Start interactive chat
    ouro chat -q "Hello"        Single query mode
    ouro --tui                  Launch the modern TUI (or set display.interface: tui)
    ouro --cli                  Force the classic REPL (overrides display.interface: tui)
    ouro -c                     Resume the most recent session
    ouro -c "my project"        Resume a session by name (latest in lineage)
    ouro --resume <session_id>  Resume a specific session by ID
    ouro --resume latest        Resume the most recent session (same as -c)
    ouro --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    ouro setup                  Run setup wizard
    ouro logout                 Clear stored authentication
    ouro auth add <provider>    Add a pooled credential
    ouro auth list              List pooled credentials
    ouro auth remove <p> <t>    Remove pooled credential by index, id, or label
    ouro auth reset <provider>  Clear exhaustion status for a provider
    ouro model                  Select default model
    ouro fallback [list]        Show fallback provider chain
    ouro fallback add           Add a fallback provider (same picker as `ouro model`)
    ouro fallback remove        Remove a fallback provider from the chain
    ouro config                 View configuration
    ouro config edit            Edit config in $EDITOR
    ouro config set model gpt-4 Set a config value
    ouro gateway                Run messaging gateway
    ouro -s ouro-agent-dev,github-auth
    ouro -w                     Start in isolated git worktree
    ouro gateway install        Install gateway background service
    ouro sessions list          List past sessions
    ouro sessions browse        Interactive session picker
    ouro sessions rename ID T   Rename/title a session
    ouro logs                   View agent.log (last 50 lines)
    ouro logs -f                Follow agent.log in real time
    ouro logs errors            View errors.log
    ouro logs --since 1h        Lines from the last hour
    ouro debug share             Upload debug report for support
    ouro console                Open the safe Ouro command console
    ouro update                 Update to latest version
    ouro dashboard              Start web UI dashboard (port 9119)
    ouro dashboard --stop       Stop running dashboard processes
    ouro dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="ouro",
        description="Ouro - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `ouro chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via OURO_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `ouro setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to OURO_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.ouro/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Ouro",
    )
    ...
    # top-level parser: when the user writes `ouro -m foo chat`, argparse
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also OURO_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.ouro/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Ouro itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
