"""CLI handlers for ``ouro secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so Ouro expects
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Ouro resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]ouro secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]ouro secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]ouro secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]ouro secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Ouro will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]ouro secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]ouro secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Takes effect on the next Ouro invocation."
    )
    if not op_cfg.get("enabled"):
        console.print(
            "[yellow]Note: the 1Password integration is currently disabled — "
            "run `ouro secrets onepassword setup` to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "`ouro secrets onepassword setup` first.[/yellow]"
        )
        return 1
    ...
    if not references:
        console.print(
            ...
            "`ouro secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "next Ouro invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]ouro secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them."
    )
    return 0
