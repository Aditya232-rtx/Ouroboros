...
entries via ``ouro mcp catalog`` or the interactive ``ouro mcp picker``,
and install them with ``ouro mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into ouro-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``ouro mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.ouro/.env`` (the
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.ouro/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Ouro install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Ouro understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Ouro is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Ouro
        understands. Update Ouro to install this entry.
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.ouro/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.ouro/.env via save_env_value()."""
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``ouro mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `ouro mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `ouro mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `ouro mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `ouro mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `ouro auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`ouro auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Ouro session to load its tools.",
        Colors.GREEN,
    ))
