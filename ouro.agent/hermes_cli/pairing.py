...
Usage:
    ouro pairing list              # Show all pending + approved users
    ouro pairing approve <platform> <request-id|code>  # Approve a pairing request
    ouro pairing revoke <platform> <user_id> # Revoke user access
    ouro pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle ouro pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: ouro pairing {list|approve|revoke|clear-pending}")
        print("Run 'ouro pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: ouro pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.ouro/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'ouro pairing list' to see pending requests.\n")
