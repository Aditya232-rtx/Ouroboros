...
def active_session_limit_message(
    ...
    return (
        f"Ouro is at the active session limit ({active_count}/{max_sessions})."
...
class ActiveSessionLease:
    ...
    # root ``OURO_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile OURO_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``ouro dashboard`` / ``serve``) never trips that
