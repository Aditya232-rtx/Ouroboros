"""ouro webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.ouro/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: ouro gateway run
"""
...
def webhook_command(args):
    """Entry point for 'ouro webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: ouro webhook {subscribe|list|remove|test}")
        print("Run 'ouro webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (ouro gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: ouro webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from ouro webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (ouro gateway run)")
