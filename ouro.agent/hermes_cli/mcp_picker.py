"""MCP picker — interactive `ouro mcp picker` (also the default `ouro mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``ouro mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `ouro plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Ouro session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `ouro mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: ouro mcp install <name>    Picker: ouro mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Ouro is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Ouro — run `ouro update` "
...
def show_catalog() -> None:
    """`ouro mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`ouro mcp picker` (and default `ouro mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`ouro mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `ouro mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
