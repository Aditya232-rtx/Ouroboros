"""``ouro debug`` debug tools for Ouro.

Currently supports:
    ouro debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.ouro/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[ouro debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.ouro/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``ouro debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``ouro debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `ouro debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "ouro-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``ouro debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.ouro/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``ouro debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ouro-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
# desktop app talks to a remote/docker/SSH backend, `ouro debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Ouro Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``ouro dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``ouro dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``ouro debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "ouro debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `ouro debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  ouro debug delete <url>")

    print("\nShare these links with the Ouro team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Ouro version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``ouro debug share --nous``: upload the bundle to Nous-S3.
    ...
    if redact:
        logger.info(
            "ouro debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `ouro debug share --local` to print the report instead, "
            "or `ouro debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: ouro debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'ouro debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``ouro debug`` call.
    ...
    # best-effort — any failure is swallowed so ``ouro debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: ouro debug <command>")
        print()
        print("Commands:")
