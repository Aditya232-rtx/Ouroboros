...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with ouro-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.ouro/plugins/<name>/``
3. **Project plugins** – ``./.ouro/plugins/<name>/`` (opt-in via
   ``OURO_ENABLE_PROJECT_PLUGINS``)
...
# ---------------------------------------------------------------------------
#
# Set ``OURO_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.ouro/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("OURO_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When OURO_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Ouro loggers.
    """
    ...
    logger.debug(
        "OURO_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Ouro' shipped guidance lives in the evidence-based
    ...
    # separate `ouro -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `ouro kanban dispatch`),
    ...
    # ``ouro kanban dispatch``); on_kanban_task_updated fires in whichever
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Ouro home."""
...
# Highest manifest schema version this Ouro understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Ouro "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `ouro plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.ouro/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``ouro plugins list``. For a flat plugin at
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Ouro never
    ...
    # (``ouro plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Ouro paths, traversal syntax, and the security-
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Ouro config writers still retain their existing
    ...
    def profile_name(self) -> str:
        """Return the active Ouro profile name (e.g. ``"default"``).

        Derived from ``OURO_HOME`` via
        ...
        running under ``~/.ouro/profiles/<name>``, or ``"custom"`` when
        ``OURO_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Ouro core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``ouro honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``ouro <subcommand>``
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.ouro/.env`` loads, before Ouro reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Ouro' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``ouro model → Configure auxiliary models`` picker
    ...
    def register_skill(
        ...
        ``~/.ouro/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # by resolved ouro home). The ledger is therefore keyed per manager
        ...
        # contributed, so `ouro plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("OURO_SAFE_MODE"):
                logger.info("OURO_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with ouro and must
            ...
            # invocation — including plain `ouro chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Ouro ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `ouro plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 3. Project plugins (./.ouro/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("OURO_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".ouro" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set OURO_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("OURO_SAFE_MODE"):
            return False
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Ouro process (e.g. a pip
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``ouro plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`ouro plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``ouro tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `ouro plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Ouro validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Ouro does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `ouro plugins list` would
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Ouro home (profile switch reusing a slug
...
# Keyed cache: resolved Ouro home -> PluginManager. Ouro supports
# multiple profiles via different OURO_HOME directories, and a single
...
def _plugin_home_key() -> Path:
    ...
    can temporarily switch Ouro home (env var *or* the context-local
    ``set_hermes_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Ouro home instead of
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Ouro profile/home.
    ...
    profile switch — via ``OURO_HOME`` or the context-local
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``ouro tools`` TUI so plugin-provided toolsets appear
