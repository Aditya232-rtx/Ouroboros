...
user's ``~/.ouro/config.yaml`` and ``~/.ouro/.env`` on a per-leaf-key basis.

This is DISTINCT from ``hermes_cli.config.is_managed()`` / ``OURO_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``OURO_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$OURO_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("OURO_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
