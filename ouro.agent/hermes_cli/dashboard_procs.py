...
def _scan_dashboard_processes(
    *,
    ...
    ``ouro dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``ouro update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Ouro Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``ouro serve`` as
    ...
    environment variable ``OURO_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`ouro serve`) is the same long-lived server
...
def _hermes_home_for_pid(pid: int) -> str | None:
    """Best-effort ``OURO_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("OURO_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"OURO_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Ouro Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``ouro update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    ...
    """Stable owner key: ``OURO_HOME`` when known, else ``--profile`` / ``-p``.

    ``OURO_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.ouro`` roots) keep a resolved ``home:`` key so
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends to respawn after ``ouro update``.

    Each candidate is ``(pid, argv, hermes_home)``.

    Rules (#78821):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``OURO_DESKTOP_CHILD_PID``) owns their
    ...
    3. Cap at most one managed backend per profile / ``OURO_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``ouro update`` respawn detaches with ``start_new_session=True``, so
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``ouro dashboard`` / ``ouro serve`` processes.

    Called at the end of ``ouro update`` (default ``reason``) and also
    from ``ouro dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``ouro update`` path), a detected
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``ouro update``'s systemd
    ...
    # When the Ouro Desktop Electron app spawns this dashboard as a
    # backend child, it sets OURO_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("OURO_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `ouro update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot OURO_HOME before the kill so per-profile caps
        ...
        if already_restarted_units:
            # Already handled directly by the caller (e.g. ouro update's
    ...
    #    remote backend (ouro serve) under Restart=on-failure never comes
...
def _detect_concurrent_hermes_instances(
    ...
    The Ouro Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    so during ``ouro update`` the user-invoked process and the desktop's
    ...
    shims (``hermes.exe`` / ``ouro-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``ouro update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``ouro update``
    ...
    #      hermes.exe sitting *under* a non-Ouro parent (e.g. a Ouro
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        ouro serve --host 127.0.0.1 --port 0
        ouro serve --isolated --host 127.0.0.1 --port 0 ...
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (OURO_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("OURO_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``ouro serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``ouro serve`` on a Mac
...
def _hermes_home_dir() -> Path:
    """Resolved Ouro home (OURO_HOME override or ~/.ouro)."""
    override = os.environ.get("OURO_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    # only check the suffix so a relocated OURO_HOME (different leading path)
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local ouro serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    lock_owned_pids_fn=None,
) -> dict[str, list]:
    """Kill leftover Desktop-local ``ouro serve`` backends with no parent.
    ...
    running under OURO_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never OURO_DESKTOP_CHILD_PID entries
