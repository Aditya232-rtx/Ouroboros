"""``ouro approvals suggest`` — mine approval history into allowlist proposals.

Ouro has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.ouro/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, ouro self-management, …) are eligible.
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  ouro approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.ouro/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``ouro approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``ouro approvals <subcommand>``."""
    ...
    print(
        "usage: ouro approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `ouro approvals <subcommand> -h` for details."
    )
    return 1
