"""``ouro project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$OURO_HOME/projects.db`` store
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Ouro'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``ouro project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: ouro project <action> [options]\n"
                "Run 'ouro project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `ouro project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `ouro project add-folder`.",
            file=sys.stderr,
        )
        return 1
