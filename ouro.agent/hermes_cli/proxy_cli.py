...
The top-level command is ``ouro egress``.  Note that the inbound OAuth
reverse-proxy command (``ouro proxy``) lives elsewhere in
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    # inbound OAuth ``ouro proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`ouro egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `ouro secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.ouro/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `ouro egress setup` finds the same keys the
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Ouro sandbox.  They will start "
    ...
    # `ouro egress reload` apply future ruleset changes without a
    ...
    # rotation), running `ouro egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]ouro egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]ouro egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]ouro egress start[/cyan]\n"
        "  Restart: [cyan]ouro egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]ouro egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]ouro egress status[/cyan]\n"
        "  Stop:    [cyan]ouro egress stop[/cyan]\n"
        "  Disable: [cyan]ouro egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `ouro egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `ouro egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`ouro egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `ouro secrets bitwarden setup` to configure the "
                "project, or switch back via `ouro egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``ouro egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `ouro egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `ouro egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `ouro egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]ouro egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `ouro egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.ouro/.env`` into ``os.environ``.

    ``ouro egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.ouro/.env`` (which the agent
