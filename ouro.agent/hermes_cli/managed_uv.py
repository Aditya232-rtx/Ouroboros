"""Ouro-managed uv and Python runtime repair.

Ouro owns its own uv binary at ``$OURO_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$OURO_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Ouro
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.ouro-runtime/python``. A
...
_RUNTIME_DIR_NAME = ".ouro-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Ouro keeps *its* uv binary.

    ``$OURO_HOME/bin/uv`` on POSIX, ``$OURO_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Ouro-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Ouro keeps databases "
            "out of WAL mode on this SQLite build. The next `ouro update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``ouro update`` runs
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``ouro update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under OURO_HOME
...
def update_managed_uv(
    *,
    ...
    Call this during ``ouro update`` so the managed copy stays current.
...
def _reload_hermes_constants():
    ...
    ``ouro update`` imports ``hermes_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        (~/.ouro/hermes-agent/hermes_constants.py)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Ouro generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `ouro update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    ...
    if holders:
        ...
        return True, f"other Ouro processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Ouro-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``ouro update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``ouro update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$OURO_HOME/bin/`` instead of ``~/.local/bin/``.
    """
