"""``ouro logs`` — view and filter Ouro log files.
...
Usage examples::
    ...
    ouro logs                    # last 50 lines of agent.log
    ouro logs -f                 # follow agent.log in real time
    ouro logs errors             # last 50 lines of errors.log
    ouro logs gateway -n 100    # last 100 lines of gateway.log
    ouro logs gui -f            # follow gui.log (dashboard/pty/ws)
    ouro logs desktop -f        # follow desktop.log (Electron app boot/backend)
    ouro logs --level WARNING    # only WARNING+ lines
    ouro logs --session abc123   # filter by session ID substring
    ouro logs --component tools  # only tool-related lines
    ouro logs --since 1h         # lines from the last hour
    ouro logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Ouro runs — try 'ouro chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'ouro chat' to generate logs)")
