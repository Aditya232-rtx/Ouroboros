...
Add, remove, or reorder entries here — both `ouro setup` and
...
_OURO_USER_AGENT = f"hermes-cli/{_OURO_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `ouro model`
...
# $OURO_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Ouro-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $OURO_HOME/models_dev_cache.json directly (no network). Falls
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Ouro that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Ouro that doesn't ship
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$OURO_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  ouro model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `ouro model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `ouro model` TUI
...
#
# Some vendors expose several Ouro provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `ouro model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``ouro model``, the
...
# In-repo fallback for the model Ouro silently lands on when the user never
...
# ``ouro model``) uses the richer free/paid-tier-aware resolver — see
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``ouro auth add openai-codex`` without
    ``ouro model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``ouro model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    ouro-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — ouro-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Ouro lands on when none is selected.
            desc = "default"
        else:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``ouro model``, ``/model``, etc.).
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Ouro' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Ouro should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``ouro auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.ouro/.env``.
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Ouro release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Ouro release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Ouro release.
...
#
# Cache strategy:
#   - One JSON file at $OURO_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$OURO_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `ouro config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _OURO_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Ouro' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Ouro' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _OURO_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"ouro-agent/{_OURO_VERSION}"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `ouro moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Ouro will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Ouro cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Ouro actually ships.  (#46850)
            #
