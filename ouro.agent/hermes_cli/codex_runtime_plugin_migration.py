"""Migrate Ouro' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Ouro' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Ouro mcp_servers.<n>.command/args/env  → codex stdio transport
  Ouro mcp_servers.<n>.url/headers       → codex streamable_http transport
  Ouro mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Ouro mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Ouro-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Ouro' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Ouro config.")
...
# Ouro keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Ouro' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Ouro MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Ouro' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_OURO_KEYS:
            skipped.append(f"{key} (unknown Ouro key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (OURO_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Ouro)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Ouro' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Ouro' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Ouro
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a OURO_HOME pointing
    ...
    ``~/.ouro`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Ouro' own
    ...
    so a ouro installed under /opt/, /usr/local/, or a venv all work.
    OURO_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # OURO_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever OURO_HOME its launcher sets
    ...
    # — that would override the launcher's OURO_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("OURO_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("OURO_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["OURO_HOME"] = hermes_home
    # PYTHONPATH passes through so a worktree-launched ouro finds the
    ...
    env["OURO_QUIET"] = "1"
    env["OURO_REDACT_SECRETS"] = env.get("OURO_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate Ouro mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.ouro/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Ouro' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Ouro for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Ouro config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Ouro' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Ouro for the tools codex
