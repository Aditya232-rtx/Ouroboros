"""ouro memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``ouro memory setup`` — if the update-time refresh only reinstalled
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as ouro tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``ouro update`` heals the active memory provider
    ...
    # data volume (OURO_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'ouro memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.ouro/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.ouro/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.ouro/plugins/")
