...
- Keep data: Remove code but keep ~/.ouro/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Ouro PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing ouro-agent or ouro PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Ouro" comment and following line
                if '# Ouro' in line or '# ouro-agent' in line:
                    skip_next = True
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Ouro-owned file, and
...
def remove_wrapper_script():
    """Remove the ouro wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "ouro",
        Path.home() / ".local" / "bin" / "ouro-acp",
        Path.home() / ".local" / "bin" / "ouro-agent",
        Path("/usr/local/bin/ouro"),
        Path("/usr/local/bin/ouro-acp"),
        Path("/usr/local/bin/ouro-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'ouro-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Ouro home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``ouro gateway run`` processes
...
#
#   1. Sets User-scope env vars ``OURO_HOME`` and ``OURO_GIT_BASH_PATH``
...
#   4. On the ``ouro dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Ouro-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare ouro-agent install dir.
    markers = [root + "\\ouro-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if OURO_HOME was customised to somewhere else — find-and-nuke
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Ouro-owned entries from User-scope PATH in the registry.
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete OURO_HOME and OURO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("OURO_HOME", "OURO_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its OURO_HOME directory.

    We shell out to ``ouro -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current OURO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's OURO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``ouro uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$OURO_HOME`` config/sessions/.env, and
    ...
    print(color("│         ⚕ Ouro Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Ouro Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Ouro agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Ouro agent at {hermes_home / 'ouro-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Ouro agent is still installed. Run 'hermes' to use the CLI,")
    print("or 'ouro uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.ouro/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.ouro/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie OURO_HOMEs
    ...
    # full wipe (code + ~/.ouro data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Ouro Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their OURO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Ouro data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Ouro code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Ouro PATH entries from shell configs / Windows User PATH")
    print("  • Ouro wrapper scripts and Ouro-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Ouro config/data: {hermes_home}")
        ...
        print(f"  • Keep Ouro config/data: {hermes_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    wipe ``$OURO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Ouro-owned PATH entries in User environment")

        log_info("Removing OURO_HOME / OURO_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Ouro-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing ouro command...")
    ...
    #     (only when they still point into this Ouro home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Ouro-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Ouro-managed node/npm/npx symlinks found")
    ...
    #     but the packaged app + Electron userData live OUTSIDE OURO_HOME and
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.ouro/, just remove the ouro-agent subdir
    ...
    #     under OURO_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.ouro/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile OURO_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://ouro.dev/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://ouro.dev/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Ouro! ⚕")
    print()
