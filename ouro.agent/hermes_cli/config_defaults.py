"""Default configuration data for Ouro.
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    # SQLite journal mode used by every Ouro database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Ouro server processes.
    ...
    "session": {
        # Per-terminal `ouro -c`: each CLI session drops a breadcrumb file
        # under $OURO_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # an interactive `ouro gateway restart` must never block for hours
        ...
        # the Ouro-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Ouro
        ...
        # identity slot (SOUL.md). Empty by default. The OURO_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Ouro adds a coding operating brief
        ...
        # `ouro status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``OURO_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             OURO_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force OURO_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Ouro auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Ouro sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Ouro
        ...
        # bundled Ouro image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Ouro sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Ouro should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``ouro chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.ouro/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: ouro sweeps the checkpoint base at startup
        ...
        # ``ouro checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .ouro.md, or .cursorrules before Ouro applies
    ...
    # Single-query (``ouro -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Ouro truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Ouro' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); ouro =
                                      # Ouro' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Ouro' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Ouro does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``ouro kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``ouro kanban decompose`` and the kanban
        ...
        # ``ouro profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `ouro model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hermes` (and `ouro chat`) launches by default:
        ...
        # REPL and `--tui` (or OURO_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `ouro --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `ouro -c` muscle memory.  Default off so existing
        # users aren't surprised.  OURO_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `ouro --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to OURO_HOME-relative YAML files/directories
        ...
        # CLI, TUI, and desktop app. Manage with `ouro pets`. Disabled until
    ...
    "dashboard": {
        ...
        # ``OURO_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``OURO_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``OURO_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``OURO_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``OURO_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Ouro appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.ouro/cache/piper-voices/
    ...
    # "Hey Ouro" hands-free wake word. Always-on, on-device hotword
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.ouro/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.ouro/cache/delegation/
    ...
    # response. If not, Ouro feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Ouro auto-pauses the goal and
    ...
    # always goes to ~/.ouro/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${OURO_SKILL_DIR} and ${OURO_SESSION_ID} in SKILL.md
        ...
        # ~/.ouro/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.ouro/.curator_backups/blobs/. Enables `ouro curator ledger`
        # and single-mutation `ouro curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `ouro curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `ouro curator run --consolidate` overrides
        ...
        # normally restored on every `ouro update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `ouro curator purge` deletes
        ...
        # skipped), snapshot ~/.ouro/skills/ into
        # ~/.ouro/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `ouro curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Ouro*" header.
    ...
    # dangerous command. -q runs export OURO_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; OURO_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Ouro' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.ouro/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or OURO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .ouro config) always require human
        ...
        # longer trigger the startup banner. Add via `ouro doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Ouro to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $OURO_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via OURO_CRON_MAX_PARALLEL env var.
        ...
        # OURO_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # OURO_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `ouro -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # decomposition is manual via `ouro kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     ouro-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Ouro tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.ouro/logs/.
    ...
    # update model picker lists without shipping a ouro-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://ouro.dev/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `ouro model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Ouro provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal OURO_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.ouro/sessions/sessions.json entirely.
        ...
        # the OURO_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `ouro update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``OURO_GATEWAY_MAX_STARTS`` / ``OURO_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.ouro/.env,
        ...
        # behavior: files must live under the Ouro cache, under
        ...
        # user. Bridged to OURO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Ouro
        # cache (~/.ouro/cache/{documents,images,audio,video,screenshots})
        ...
        # to OURO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # OURO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to OURO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.ouro/state.db.
    ...
    "sessions": {
        ...
        # the default of ``ouro sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.ouro/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `ouro sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `ouro update` prints a one-line notice with
        ...
        # ~/.ouro/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to OURO_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # OURO_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `ouro sessions export`. Checked
    ...
    # ``ouro doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `ouro doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``ouro update`` behaviour.
    "updates": {
        ...
        #     DBs) into <OURO_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``ouro backup``-style zip
        #     of OURO_HOME into <OURO_HOME>/backups/, restorable with
        #     ``ouro import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `ouro update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<OURO_HOME>/lsp/bin/`` on first use.
    ...
    # `ouro tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.ouro/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.ouro/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.ouro/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Ouro may use this encrypted
            ...
            # ~/.ouro/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.ouro/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Ouro disables it for our users unless they
        # explicitly opt in here. When false (default), Ouro sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Ouro also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Ouro computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Ouro passes the
    ...
    #
    # Configure with `ouro egress setup`.  Disabled by default — the rest of
    # Ouro works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.ouro/bin/ on
    ...
    # Ouro Desktop (Electron app) launch options. These only affect
    # `ouro desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the OURO_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the OURO_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Ouro mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'ouro model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Ouro auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Ouro' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Ouro, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. OURO_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'ouro-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Ouro API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Ouro API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Ouro API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # OURO_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to OURO_TOOL_PROGRESS_MODE for backward
    ...
    # OURO_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "OURO_PREFILL_MESSAGES_FILE": {
