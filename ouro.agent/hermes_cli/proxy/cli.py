"""CLI handlers for the ``ouro proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "ouro proxy requires aiohttp. Run `ouro setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"ouro auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Ouro proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Ouro proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: ouro proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``ouro proxy <subcommand>``."""
    ...
    print(
        "ouro proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  ouro proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
