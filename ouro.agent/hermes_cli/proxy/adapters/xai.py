...
# xAI's public API is OpenAI-compatible for the endpoints Ouro commonly
# uses. The Responses endpoint is included because Ouro' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Ouro-managed OAuth credentials."""

    auth_hint = "ouro auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`ouro auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`ouro auth reset xai-oauth` or re-authenticate with "
                    "`ouro auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `ouro auth add xai-oauth --type oauth`."
            )
