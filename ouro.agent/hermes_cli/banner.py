...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `ouro ... > file`) raises
...
# (e.g. nix-built ouro — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Aditya232-rtx/ouro.git"
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `ouro update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Ouro update is available.

    Two paths: if ``OURO_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("OURO_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # OURO_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    if embedded_rev:
        ...
        # $OURO_HOME/hermes-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "ouro-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Ouro git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$OURO_HOME/hermes-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
_RELEASE_URL_BASE = "https://github.com/Aditya232-rtx/ouro/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Ouro checkout. Cached per-process. Release URL always points at the
    canonical Aditya232-rtx/ouro repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Ouro v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else OURO_CADUCEUS
    except Exception:
        _bskin = None
        _hero = OURO_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or ouro setup[/]"
            )
        else:
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else OURO_AGENT_LOGO
        console.print(_logo)
        console.print()
