"""
ouro fallback — manage the fallback provider chain.
...
Subcommands:
  ouro fallback [list]   Show the current fallback chain (default when no subcommand)
  ouro fallback add      Pick provider + model via the same picker as `ouro model`,
                           then append the selection to the chain
  ouro fallback remove   Pick an entry to delete from the chain
  ouro fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.ouro/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `ouro model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  ouro fallback add")
        print()
        return
    ...
    print("  Docs: https://ouro.dev/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `ouro model`, then append the selection to the chain."""
    ...
    print("  `ouro model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `ouro fallback list` to view, or `ouro fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``ouro fallback [subcommand]``."""
