#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Ouro Skills Hub.

Powers both:
  - `ouro skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `ouro skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.ouro/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.ouro/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    c.print(table)
    c.print("[dim]Use: ouro skills inspect <identifier> to preview, "
            "ouro skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # The identifier is what you pass to `ouro skills install`. Browse used
    ...
    c.print("[dim]Tip: 'ouro skills inspect <identifier>' to preview, "
            "'ouro skills install <identifier>' to install, "
            "'ouro skills search <query>' to search deeper[/]\n")
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]ouro skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with ouro-agent but is not activated by default.\n"
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="ouro skills install <id> to install"))
...
def do_list(source_filter: str = "all",
    ...
    config — ``ouro -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``OURO_HOME`` at process
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``ouro update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: ouro skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `ouro update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `ouro update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   ouro skills diff <name>[/]")
    c.print("[dim]Resume updates: ouro skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: ouro skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: ouro skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: ouro skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: ouro skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: ouro skills publish <path> --to github --repo owner/repo\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Ouro Skills Hub.\n\n"
                        f"This skill was scanned by the Ouro Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `ouro skills <subcommand>` — called from hermes_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: ouro skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: ouro skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: ouro skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'ouro skills <command> --help' for details.\n")
