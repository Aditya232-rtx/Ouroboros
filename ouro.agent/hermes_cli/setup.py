"""
Interactive setup wizard for Ouro.
...
_DOCS_BASE = "https://ouro.dev/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during ouro update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Ouro Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Ouro using environment variables or config commands:")
    print_info("  ouro config set model.provider custom")
    print_info("  ouro config set model.base_url http://localhost:8080/v1")
    print_info("  ouro config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'ouro setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``OURO_NONINTERACTIVE=1`` (see ``hermes_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("OURO_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``OURO_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'ouro setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Ouro cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    ouro model            (pick any provider/model)")
        print_info("    ouro setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'ouro setup' to configure"))
    ...
    # Video generation — opt-in via `ouro tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'ouro setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'ouro setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'ouro tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'ouro setup terminal'"))
    ...
    # Spotify (OAuth via ouro auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'ouro setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('ouro setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('ouro setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('ouro setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('ouro setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('ouro setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('ouro config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('ouro config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('ouro config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('ouro gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('ouro doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `ouro tools` and `ouro setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``ouro model``)
    ...
    provider added to ``ouro model`` is automatically available here.
    ...
    # Delegate to the shared ouro model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: ouro model")
    ...
    # on demand via `ouro auth add`, `ouro setup` vision, and
    # `ouro setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``ouro model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.ouro/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Ouro auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via ouro setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'ouro setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Ouro runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `ouro setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `ouro setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `ouro egress setup` then `ouro egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `ouro egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'ouro-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $OURO_HOME/bin is never on PATH, so a bare
            # which() misses the uv Ouro installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'ouro-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into OURO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("OURO_MAX_ITERATIONS")
    ...
    print_info("  Run `ouro setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # OURO_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.ouro/logs/tool_calls.log (gateway only)")
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when OURO_HOME is a profile dir."""
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Ouro delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Ouro to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://ouro.dev/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://ouro.dev/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  ouro config edit")
    print_info("   Open config in your editor:  ouro config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Ouro from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'ouro setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     ouro config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `ouro import` or `ouro setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `ouro setup tools` and `ouro tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Ouro profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and Ouro.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Ouro to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Ouro at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Ouro at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Ouro at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Ouro at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Ouro equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Ouro config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Ouro.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Ouro's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Ouro can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: ouro claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: ouro claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Ouro configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Ouro config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Ouro (use ouro claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``ouro setup --portal`` and ``ouro portal``. This is the
    ...
    Ouro session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``ouro setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``ouro model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``ouro portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Ouro Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `ouro portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `ouro portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `ouro portal`: %s", exc)
        print()
    ...
    print_info("  Run `ouro portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ouro setup           — full or quick (auto-detected)
      ouro setup model     — just model/provider
      ouro setup tts       — just text-to-speech
      ouro setup terminal  — just terminal backend
      ouro setup gateway   — just messaging platforms
      ouro setup tools     — just tool configuration
      ouro setup telemetry — just local shared metrics
      ouro setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Ouro Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Ouro Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Ouro installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Ouro configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'ouro setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Ouro?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'ouro config edit'")
    ...
    # Tune later with `ouro setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``ouro setup <section>``
    or switch providers with ``ouro model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'ouro setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `ouro import` from another machine).
        from hermes_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    ouro setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  ouro setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``ouro tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``ouro setup agent`` /
    ``ouro config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `ouro update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        ouro tools")
        print_info("  Seed skills:         ouro skills opt-in --sync")
        print_info("  Add MCP servers:     ouro mcp add")
        print_info("  Enable plugins:      ouro plugins")
        print_info("  Tune agent settings: ouro setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`ouro update` runs from re-injecting them. Opt back in any")
            print_info("time with `ouro skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `ouro tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `ouro plugins list` / `ouro plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `ouro plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `ouro mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `ouro mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   ouro tools")
    print_info("  Seed skills:         ouro skills opt-in --sync")
    print_info("  Add MCP servers:     ouro mcp add")
    print_info("  Tune agent settings: ouro setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'ouro setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Ouro to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'ouro setup gateway'.")
