...
``ouro desktop`` builds and launches the Electron app. On Linux, a
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``ouro desktop``.

    Prefer the real ``hermes`` executable (argv[0] or PATH). When Ouro
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Ouro\n"
        "GenericName=Ouro Desktop\n"
        "Comment=Launch Ouro Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Ouro desktop entry. Return its path.
