"""``ouro verify`` — detect a project's run recipe and smoke-test it.
...
``.ouro/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``ouro verify`` satisfies
