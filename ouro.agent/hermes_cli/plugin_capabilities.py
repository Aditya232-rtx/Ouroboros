...
this. Capabilities govern the *host API surfaces* Ouro hands out (which
...
until the user re-consents (``ouro plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Ouro may declare ids this build doesn't know; they can
