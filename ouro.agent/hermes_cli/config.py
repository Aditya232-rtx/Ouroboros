"""
Configuration management for Ouro.

Config files are stored in ~/.ouro/ for easy access:
- ~/.ouro/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.ouro/.env         - API keys and secrets

This module provides:
- ouro config          - Show current configuration
- ouro config edit     - Open config in editor
- ouro config get      - Print a resolved configuration value
- ouro config set      - Set a specific value
- ouro config unset    - Remove a user configuration value
- ouro config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``ouro config set`` (which rewrites
    ...
    ``config.yaml`` in place: ouro never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.ouro/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``ouro logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  ouro config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Ouro makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Ouro itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``ouro update``, the TUI build.
...
#   on every plugin install / ``ouro update``.
...
# * ``OURO_HOME`` / ``OURO_PROFILE`` / ``OURO_CONFIG`` /
#   ``OURO_ENV`` — Ouro runtime location flags. Writing these into
...
#
# IMPORTANT: ``OURO_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (OURO_LANGFUSE_PUBLIC_KEY,
# OURO_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Ouro runtime location — never via dashboard env writer.
    # NOT a OURO_* blanket: integration credentials (OURO_GEMINI_*,
    # OURO_LANGFUSE_*, OURO_SPOTIFY_*, ...) ARE allowed.
    "OURO_HOME", "OURO_PROFILE", "OURO_CONFIG", "OURO_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Ouro runtime location "
            "(OURO_HOME, OURO_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # OURO_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `ouro plugins enable
    # observability/langfuse` or `ouro tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "OURO_LANGFUSE_ENV",
    "OURO_LANGFUSE_RELEASE",
    "OURO_LANGFUSE_SAMPLE_RATE",
    "OURO_LANGFUSE_MAX_CHARS",
    "OURO_LANGFUSE_CAPTURE",
    "OURO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    ...
    "OURO_ACP_AUTH_METHOD",
    "OURO_ACP_AUTO_APPROVE",
    "OURO_COPILOT_ACP_COMMAND",
    "OURO_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
# from `nix run` / `nix profile install` (which don't set OURO_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("OURO_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    """Check if Ouro is running in package-manager-managed mode.

    Two signals: the OURO_MANAGED env var (set by the systemd service),
    or a .managed marker file in OURO_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Ouro through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    the running interpreter, NOT of ``$OURO_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Ouro was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$OURO_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. OURO_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.ouro``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$OURO_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.ouro:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$OURO_HOME``, a
    ...
    ``docker`` and ``ouro update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $OURO_HOME.
    try:
    ...
    #    host install whose shared $OURO_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``ouro update``.
    try:
        method = (
    ...
    # detect Nix installs that don't set OURO_MANAGED (e.g. ``nix run``,
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$OURO_HOME``, so that two installs sharing one data directory
...
# Long-form text for ``ouro update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side Ouro, it doesn't update
...
✗ ``ouro update`` doesn't apply inside the Docker container.

Ouro runs as a published image (nousresearch/hermes-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/hermes-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate ouro-agent
...
Notes:
  ...
  • Your config and session history live under ``$OURO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``ouro update`` inside Docker.
...
def format_managed_message(action: str = "modify this Ouro installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("OURO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Ouro installation is managed by NixOS "
            f"(OURO_MANAGED={env_hint}).\n"
            "Edit services.hermes-agent.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Ouro installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Ouro."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from OURO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or OURO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("OURO_DEV") == "1":
        return None
    ...
    container_name = info.get("container_name", "ouro-agent")
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the OURO_UID / OURO_GID env vars set by Docker deployments.

    Docker containers running Ouro commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level OURO_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("OURO_UID", "").strip()
    gid_str = os.environ.get("OURO_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``OURO_UID:OURO_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the ouro group can
    share state with the gateway service.

    The mode can be overridden via the OURO_HOME_MODE environment variable
    (e.g. OURO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse OURO_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``OURO_UID``/``OURO_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("OURO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Ouro runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("OURO_CONTAINER") or os.environ.get("OURO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set OURO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into OURO_HOME, upgrading legacy empty templates.
...
_OURO_HOME_ENSURED: set = set()


def ensure_hermes_home():
    """Ensure ~/.ouro directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``OURO_HOME`` we only re-run
    ...
    if key in _OURO_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``ouro profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old OURO_HOME would resurrect an empty skeleton
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"OURO_HOME {home} does not exist. "
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `ouro config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `ouro update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Ouro' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Ouro versions) may write
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'ouro setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Ouro won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, ouro send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'ouro doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "ouro update / ouro -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ ouro config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: ouro config set <key> <value>")
    ...
    # `ouro update`, but only the version bump is persisted.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.ouro/.env); "
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``ouro config set``
    ...
    backends) — while display paths (``ouro status``/``dump``) read ``name``
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``OURO_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.ouro/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.ouro/config.yaml.
    ...
    (which change ``OURO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Ouro host rewrites it to the host or container
...
#   openai-codex (OAuth — ouro auth) — OpenAI Codex
#   nous         (OAuth — ouro auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.ouro/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Ouro writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.ouro/.env.
    ...
    interactive menu render (`ouro tools`, `ouro setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `ouro tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.ouro/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.ouro/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``OURO_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"OURO_CUSTOM_{slug}_API_KEY" if slug else "OURO_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.ouro/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.ouro/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Ouro — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Ouro vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.ouro/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.ouro/.env`` over ``os.environ``.

    Used for Ouro-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Ouro Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale OURO_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("OURO_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale OURO_MAX_ITERATIONS={_env_ghost} "
                f"(run 'ouro doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  ouro config edit     # Edit config file", Colors.DIM))
    print(color("  ouro config set <key> <value>", Colors.DIM))
    print(color("  ouro setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``OURO_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("OURO_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `ouro cron list`, then pin the "
        "intended values with `ouro cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # `ouro config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``ouro config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``ouro config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``ouro config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (ouro config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if not isinstance(_default_value_for_key(key), str):
        if value.lower() in {'true', 'yes', 'on'}:
            ...
            #   ouro config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   ouro config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``ouro config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``ouro config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    ouro config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    ouro config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `ouro config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `ouro config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `ouro config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: ouro config get <key> [--json]")
            print()
            print("Examples:")
            print("  ouro config get model")
            print("  ouro config get terminal.backend")
            print("  ouro config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: ouro config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  ouro config set model anthropic/claude-sonnet-4")
            print("  ouro config set terminal.backend docker")
            print("  ouro config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: ouro config unset <key>")
            print()
            print("Examples:")
            print("  ouro config unset model")
            print("  ouro config unset terminal.backend")
            print("  ouro config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'ouro config migrate' to add them")
        ...
        print("  ouro config           Show current configuration")
        print("  ouro config edit      Open config in editor")
        print("  ouro config get <key>          Print a resolved config value")
        print("  ouro config set <key> <value>   Set a config value")
        print("  ouro config unset <key>        Remove a config value")
        print("  ouro config check     Check for missing/outdated config")
        print("  ouro config migrate   Update config with new options")
        print("  ouro config path      Show config file path")
        print("  ouro config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``ouro config`` UI so users
