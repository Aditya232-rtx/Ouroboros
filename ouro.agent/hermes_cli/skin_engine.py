"""Ouro skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Ouro itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.ouro/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.ouro/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Ouro"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Ouro "       # Response box header label
...
.. code-block:: python
    ...
    print(skin.get_branding("agent_name"))  # "Ouro"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.ouro/skins/
...
Drop a YAML file in ``~/.ouro/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces OURO_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces OURO_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Ouro — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same Ouro gold.
        "colors": {
        ...
        "branding": {
            "agent_name": "Ouro",
            "welcome": "Welcome to Ouro! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Ouro ",
            "prompt_symbol": "❯",
