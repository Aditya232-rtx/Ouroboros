"""Slash command definitions and autocomplete for the Ouro CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Ouro config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Ouro works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Ouro' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Ouro is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.ouro/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Ouro to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Ouro version", "Info", aliases=("v",),
...
# Telegram allows up to 100 BotCommands. Ouro ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Ouro
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``ouro skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``ouro skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``ouro skills list`` and
...
_SLACK_VIA_OURO_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/ouro <verb>`` subcommand.
    ...
    can still reach them via ``/ouro <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/ouro`` is always reserved as the first entry so the
    catch-all ``/ouro <subcommand>`` form keeps working for anything that
    gets dropped by the clamp or for free-form questions.  The legacy
    ``/hermes`` name is still routed by the adapter for older manifests.
    """
    overrides = _resolve_config_gates()
    entries: list[tuple[str, str, str]] = []
    seen: set[str] = set()

    # Reserve /ouro as the catch-all top-level command (the legacy /hermes name
    # is still routed by the adapter for older workspace manifests).
    entries.append(("ouro", "Talk to Ouro or run a subcommand", "[subcommand] [args]"))
    seen.add("ouro")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_OURO_ONLY:
            # Intentionally Slack-via-/hermes only (see _SLACK_VIA_OURO_ONLY).
            return
...
def slack_app_manifest(request_url: str = "https://ouro.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /ouro handler.

    Maps both canonical names and aliases so /ouro bg do stuff works
    the same as /ouro background do stuff.

    Plugin-registered slash commands are included so ``/ouro <plugin-cmd>``
