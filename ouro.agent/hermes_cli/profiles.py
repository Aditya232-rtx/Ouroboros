"""
Profile management for multiple isolated Ouro instances.

Each profile is a fully independent OURO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.ouro/profiles/<name>/`` by default.

The "default" profile is ``~/.ouro`` itself — backward compatible,
zero migration needed.

Usage::

    ouro profile create coder          # fresh profile + bundled skills
    ouro profile create coder --clone  # also copy config, .env, SOUL.md, skills
    ouro profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    ouro -p coder chat                 # or via flag
    ouro profile use coder             # set as sticky default
    ouro profile delete coder          # remove profile + alias + service
"""
...
# Marker file written by `ouro profile create --no-skills`.  When present in
...
# `ouro skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Ouro infrastructure directories that only the default profile
         (``~/.ouro``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.ouro) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "ouro-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when OURO_HOME equals the
...
# into the export. The set below identifies the *known Ouro profile
# artifacts* at the root of OURO_HOME; everything else is excluded.
...
# Ouro subcommands that cannot be used as profile names/aliases
_OURO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the ouro root, NOT to the current OURO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where OURO_HOME points outside
    ``~/.ouro``, profiles live under ``OURO_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) OURO_HOME path.

    In standard deployments this is ``~/.ouro``.
    In Docker/custom deployments where OURO_HOME is outside ``~/.ouro``
    (e.g. ``/opt/data``), returns OURO_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Ouro installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its OURO_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, ouro subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _OURO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a ouro subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hermes_exe = shutil.which("ouro") or "ouro"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Ouro. If we
    ...
    try:
        ...
        # not be migrated. The next `ouro doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``ouro -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``ouro profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``ouro -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``ouro profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``OURO_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Ouro config
...
def profiles_to_serve(
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own OURO_HOME. When
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``ouro update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.ouro)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.ouro/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `ouro -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Ouro profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `ouro update`'s
    ...
    # Cloned configs can be older than the running Ouro (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `ouro profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `ouro -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches OURO_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``ouro profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "OURO_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Ouro *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Ouro backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``OURO_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `ouro -p <canon> profile
    ...
    hermes_markers = ("hermes_cli.main", "ouro-gateway", "tui_gateway")
    ...
    # interpreter.
    _OURO_CONSOLE_SCRIPT_NAMES = frozenset({"ouro", "ouro-agent", "ouro-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Ouro process: either an entrypoint marker in argv, a
            ...
            if not is_hermes and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                ...
                # (pyproject.toml [project.scripts]: hermes, ouro-agent,
                # ouro-acp) rather than a bare `startswith("hermes")` --
                ...
                is_hermes = script_stem in _OURO_CONSOLE_SCRIPT_NAMES
            if not is_hermes:
                continue
            ...
            # ...or by OURO_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("OURO_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.ouro).\n"
            "To remove everything, use: ouro uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``OURO_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set OURO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("OURO_HOME")
    try:
        os.environ["OURO_HOME"] = str(profile_dir)
        ...
        if old_home is not None:
            os.environ["OURO_HOME"] = old_home
        elif "OURO_HOME" in os.environ:
            del os.environ["OURO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.ouro/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: ouro profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from OURO_HOME.

    Returns ``"default"`` if OURO_HOME is not set or points to ``~/.ouro``.
    Returns the profile name if OURO_HOME points into ``~/.ouro/profiles/<name>``.
    Returns ``"custom"`` if OURO_HOME is set to an unrecognized path.
    """
    from hermes_constants import get_hermes_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      OURO_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Ouro file the
      user may have lying alongside OURO_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``ouro sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``OURO_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.ouro itself — its parent is ~/ and its
        # directory name is ".ouro", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: ouro profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.ouro itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.ouro). "
            "Specify a different name: ouro profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a OURO_HOME path string.

    Called early in the CLI entry point, before any ouro modules
    are imported, to set the OURO_HOME environment variable.

    When OURO_HOME is already set, the configured spelling IS the
    ...
    it to match Ouro-owned PYTHONPATH entries written in the same
    ...
    validate_profile_name(canon)
    env_home = os.environ.get("OURO_HOME", "").strip()
    if env_home:
