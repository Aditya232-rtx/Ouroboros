"""`ouro checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.ouro/checkpoints/``.  Actions:

    ouro checkpoints               # same as `status`
    ouro checkpoints status        # total size, project count, breakdown
    ouro checkpoints list          # per-project checkpoint counts + workdir
    ouro checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    ouro checkpoints clear [-f]    # nuke the entire base (asks first)
    ouro checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    ouro checkpoints
    ouro checkpoints prune --retention-days 3 --max-size-mb 200
    ouro checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: ouro checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``ouro checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `ouro checkpoints` → status
