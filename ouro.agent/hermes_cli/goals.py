"""Persistent session goals — the Ralph loop for Ouro.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Ouro feeds a
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current OURO_HOME.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.ouro/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``ouro -p <profile> chat -q "work kanban task <id>"``). The worker's
