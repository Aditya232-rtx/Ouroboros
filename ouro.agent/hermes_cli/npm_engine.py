...
Scope of the repair is deliberately narrow. Ouro only upgrades an npm that
lives inside its **own** managed Node tree (``$OURO_HOME/node``), installing
...
and their other projects; Ouro never modifies those. When the failing npm is
one of those foreign installs, Ouro instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Ouro-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$OURO_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$OURO_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Ouro-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Ouro-managed Node.js tree is in use by a "
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Ouro could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Ouro-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$OURO_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Ouro-managed Node.js runtime "
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Ouro-managed install, or
    ...
    if prefix is not None:
        # Ouro owns this npm — upgrade it in place. Only an npm-range
