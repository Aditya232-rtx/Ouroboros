...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `ouro tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `ouro computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``ouro tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `ouro tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``ouro tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.ouro/.env`` via ``save_env_value`` —
    the same store ``ouro tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``ouro tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``ouro -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same OURO_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``ouro computer-use permissions grant`` as a background action.
