...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the OURO_HOME
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Ouro and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Ouro for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Ouro.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `ouro mcp catalog` / `ouro mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds OURO_HOME-derived paths in the child.
