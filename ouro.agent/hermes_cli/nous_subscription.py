...
# `ouro tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Ouro-managed Node dirs (Windows installer / POSIX $OURO_HOME/node)
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("OURO_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `ouro model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("OURO_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("OURO_LOCAL_STT_COMMAND"))
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`ouro tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``ouro tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``ouro model``'s Nous
    login, this:
