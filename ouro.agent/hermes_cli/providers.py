"""
Single source of truth for provider identity in Ouro.
...
2. **Ouro overlays** — transport type, auth patterns, aggregator flags,
...
# -- Ouro overlay ----------------------------------------------------------
# Ouro-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Ouro-specific provider metadata layered on top of models.dev."""
...
OURO_OVERLAYS: Dict[str, HermesOverlay] = {
    "moa": HermesOverlay(
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="OURO_QWEN_BASE_URL",
    ),
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Ouro overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Ouro overlay
    ...
    overlay = OURO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + ouro extra
    ...
    if overlay is not None:
        # Ouro-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Ouro overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in OURO_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Ouro provider IDs must win over LOSSY alias collapsing.
