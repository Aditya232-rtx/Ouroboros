"""
Status command for ouro CLI.

Shows the status of all Ouro components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``ouro config``'s
...
def _estop_status_line():
    """One-line pause banner for `ouro status`, or None when not paused.

    Cheap: a single stat on $OURO_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `ouro resume` to lift)"


def show_status(args):
    """Show status of all Ouro components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Ouro Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `ouro status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: ouro portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: ouro model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: ouro auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: ouro auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: ouro model)"
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'ouro-agent[vercel]')"
    ...
    print(color("  Run 'ouro doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'ouro setup' to configure", Colors.DIM))
    print()
