"""ouro hooks — inspect and manage shell-script hooks.

Usage::

    ouro hooks list
    ouro hooks test <event> [--for-tool X] [--payload-file F]
    ouro hooks revoke <command>
    ouro hooks doctor

Consent records live under ``~/.ouro/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.ouro/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``ouro hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: ouro hooks {list|test|revoke|doctor}")
        print("Run 'ouro hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.ouro/config.yaml.")
        print("See `ouro hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.ouro/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `ouro hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `ouro hooks test` and `ouro hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Ouro wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `ouro hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `ouro hooks doctor` would execute
