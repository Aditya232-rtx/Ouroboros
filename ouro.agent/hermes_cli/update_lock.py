"""Cross-process mutual exclusion for in-flight Ouro updates.

Three different surfaces can start an update of the same install tree:

* ``ouro update`` from a terminal,
...
* the desktop's Update button, which hands off to the Tauri
  ``ouro-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``ouro update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <OURO_HOME>/.ouro-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``ouro update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Ouro is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``ouro-setup`` binary under ``~/.ouro`` is only refreshed by a full
  ...
  ancestry check those users get exit 2 ("Ouro is still running") on every
  GUI update forever, with no Ouro process actually running.
"""
...
MARKER_NAME = ".ouro-update-in-progress"

# Set by an orchestrating updater (the Tauri `ouro-setup --update` flow) to
# its own pid before spawning `ouro update` as a child stage. The parent
...
HANDOFF_PID_ENV = "OURO_UPDATE_HANDOFF_PID"
...
# to show "Ouro is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Ouro home (never the context-local profile override):
    the Rust updater resolves ``$OURO_HOME`` or the platform default, and the
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``ouro update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``ouro-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Ouro update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `ouro update` as a stage): we run under ITS claim
