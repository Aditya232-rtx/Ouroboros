...
def _is_hermes_gateway_run_argv(command: Sequence[str]) -> bool:
    """True for Ouro ``gateway run`` argv this wrapper is allowed to upgrade.

    The wrapper is generic. Only historical/current Ouro gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``ouro update``
