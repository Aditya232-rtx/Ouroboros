"""CLI handlers for ``ouro secrets bitwarden ...``.
...
# that process is ``ouro update``, the self-lock preflight detects the
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Ouro process."
    )
    console.print(
        "  Status:  [cyan]ouro secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]ouro secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]ouro secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]ouro secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Ouro will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `ouro secrets bitwarden setup` to pick a "
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "run `ouro secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`ouro secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]ouro secrets bitwarden setup[/cyan] and pick EU or "
