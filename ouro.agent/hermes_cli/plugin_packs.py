...
* Capability consent is NEVER bulk-granted: after each plugin installs,
  ...
  as a normal ``ouro plugins install`` (#64228).
...
def render_pack_review(console, pack: PluginPack, resolved: List[ResolvedPackPlugin]) -> None:
    ...
    if pack.skills:
        ...
        console.print(
            "[dim]Install them manually, e.g. `ouro skills install <id>`.[/dim]"
        )
    console.print(
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-hermes-pack") -> tuple[str, List[str]]:
    ...
    doc: dict[str, Any] = {
        "name": pack_name,
        "description": "Exported by `ouro plugins pack export`.",
        "version": "1.0.0",
        "plugins": entries,
    }
    if config:
...
def cmd_pack_show(source: str) -> None:
    """``ouro plugins pack show <path-or-url>`` — dry-run review."""
    ...
    console.print("\n[dim]Dry run only. Install with `ouro plugins pack install ...`.[/dim]")


def cmd_pack_install(source: str, *, force: bool = False) -> None:
    """``ouro plugins pack install <path-or-url>``.
...
def cmd_pack_export(*, enabled_only: bool = False, name: str = "my-hermes-pack") -> None:
    """``ouro plugins pack export [--enabled-only]`` — pack YAML on stdout."""
    from rich.console import Console
...
def pack_command(args) -> None:
    """Dispatch ``ouro plugins pack <action>``."""
    ...
    if action == "install":
        ...
        Console().print(
            "[red]Error:[/red] Usage: ouro plugins pack {install|export|show}"
        )
        sys.exit(1)
