...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `ouro --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``ouro auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``ouro auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`ouro auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Ouro reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `ouro auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `ouro auth` is called bare."""
