...
3. Running inside a container with no persistent volume mount over the
   OURO_HOME data dir (state is ephemeral — lost on container restart).
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Ouro as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("OURO_DESKTOP_CHILD_PID"):
