...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local OURO_HOME override (profile
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``ouro -z "..."`` / ``-q``), the larger
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``ouro dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``ouro -z``) can construct
