"""Pydantic request/response models for the Ouro dashboard web server.
...
class ModelAssignment(BaseModel):
    ...
    # ``ouro model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `ouro import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`ouro -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the OURO_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under OURO_HOME.
    output: str = ""
