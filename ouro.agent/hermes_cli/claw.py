"""ouro claw — OpenClaw migration commands.

Usage:
    ouro claw migrate              # Preview then migrate (always shows preview first)
    ouro claw migrate --dry-run    # Preview only, no changes
    ouro claw migrate --yes        # Skip confirmation prompt
    ouro claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    ouro claw migrate --no-backup  # Skip pre-migration snapshot
    ouro claw cleanup              # Archive leftover OpenClaw directories
    ouro claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Ouro may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Ouro gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Ouro gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'ouro gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: ouro claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Ouro")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'ouro claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Ouro migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: ouro claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.ouro/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: ouro claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Ouro home ─────────
    ...
    # restorable with `ouro import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: ouro import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Ouro home."
            )
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: ouro claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: ouro claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  ouro claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  ouro claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  ouro config set OPENROUTER_API_KEY sk-or-v1-...")
