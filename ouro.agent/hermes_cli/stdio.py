...
on Japanese locales, etc.).  Ouro's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Ouro is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``OURO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("OURO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Ouro-managed Git install directories so
    ...
    # install without Ouro-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Ouro's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Ouro's
    ``ouro config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Ouro (User env var in Windows Settings, or
    export in a PowerShell profile) and Ouro picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Ouro-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Ouro spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Ouro-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Ouro venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "hermes", "ouro-agent", "venv", "Scripts"),
