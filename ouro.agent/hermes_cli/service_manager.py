...
``ouro gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``ouro gateway start/stop/restart``).
    """
    ...
    # `ouro gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every Ouro runtime call inside the
    container drops to ouro via ``s6-setuidgid``, that silent failure
...
# active (set via the ``ouro -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `ouro profile create`
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's OURO_HOME blindly;
    derive the shared profile root from the current OURO_HOME and map the
    ...
    hermes_home = Path(os.environ.get("OURO_HOME", "/opt/data"))
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the ouro user.
    ...
    (the ``hermes`` user — UID 10000 — running every Ouro runtime
    ...
    is dispatched as the ouro user.
    ...
    with ouro ownership before triggering ``s6-svscanctl -a``,
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _OURO_UID, _OURO_GID)
        except PermissionError:
            # Running as the ouro user already — directory is hermes-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _OURO_UID, _OURO_GID)
        except PermissionError:
            pass
    ...
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _OURO_UID, _OURO_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``ouro -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`ouro profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources OURO_HOME (and any extra env) via with-contenv —
             so e.g. ``-e OURO_HOME=/data/hermes`` is honored at run
          ...
          4. Drops to the ouro user and exec's
             ``ouro -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``ouro gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root OURO_HOME profile" (the implicit profile that exists at
        the top of $OURO_HOME, not under profiles/). It must be
        ...
        would instead look up ``$OURO_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``OURO_HOME``,
        ...
        # guard.
        lines.append("export OURO_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's OURO_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``ouro gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-OURO_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The OURO_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own OURO_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "ouro gateway run --replace"
        else:
            gateway_cmd = f"ouro -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid ouro {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${OURO_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the OURO_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e OURO_HOME=/data/hermes``
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``ouro logs`` reads and what
        ...
        return (
            ...
            f': "${{OURO_HOME:=/opt/data}}"\n'
            f'log_dir="$OURO_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as ouro when
            ...
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid ouro mkdir -p "$log_dir"\n'
            f'  s6-setuidgid ouro rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid ouro s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``ouro gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``ouro -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with ouro ownership
            ...
            # the ouro user) won't hit EACCES on root-owned 0700
