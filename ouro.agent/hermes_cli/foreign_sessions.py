...
``ouro sessions import`` (and ``--resume @claude`` / ``--resume @codex``)
let a user pull a conversation they started in another agent CLI into
Ouro and continue it here.
...
role-alternation invariant Ouro enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Ouro SessionDB.

    Returns the new Ouro session id.  The foreign file is only read.
...
def pick_foreign_session(
    ...
    if not sys.stdin.isatty():
        print(
            ...
            "  ouro sessions import --from claude|codex <path>"
        )
        return None
    try:
...
def run_sessions_import(args, db=None) -> Optional[str]:
    """`ouro sessions import` entry point. Returns new session id or None."""
    ...
    print(f"  Continue it with:  ouro --resume {session_id}")
    return session_id
