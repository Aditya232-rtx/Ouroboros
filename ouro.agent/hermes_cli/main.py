...
Usage:
    ouro                     # Interactive chat (default)
    ouro chat                # Interactive chat
    ouro gateway             # Run gateway in foreground
    ouro gateway start       # Start gateway as service
    ouro gateway stop        # Stop gateway service
    ouro gateway status      # Show gateway status
    ouro gateway install     # Install gateway service
    ouro gateway uninstall   # Uninstall gateway service
    ouro setup               # Interactive setup wizard
    ouro logout              # Clear stored authentication
    ouro status              # Show status of all components
    ouro cron                # Manage cron jobs
    ouro cron list           # List cron jobs
    ouro cron status         # Check if cron scheduler is running
    ouro doctor              # Check configuration and dependencies
    ouro honcho setup                    # Configure Honcho AI memory integration
    ouro honcho status                   # Show Honcho config and connection status
    ouro honcho sessions                 # List directory → session name mappings
    ouro honcho map <name>               # Map current directory to a session name
    ouro honcho peer                     # Show peer names and dialectic settings
    ouro honcho peer --user NAME         # Set user peer name
    ouro honcho peer --ai NAME           # Set AI peer name
    ouro honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    ouro honcho mode                     # Show current memory mode
    ouro honcho mode [hybrid|honcho|local]  # Set memory mode
    ouro honcho tokens                   # Show token budget settings
    ouro honcho tokens --context N       # Set session.context() token cap
    ouro honcho tokens --dialectic N     # Set dialectic result char cap
    ouro honcho identity                 # Show AI peer identity representation
    ouro honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    ouro honcho migrate                  # Step-by-step migration guide: OpenClaw native → Ouro + Honcho
    ouro version             Show version
    ouro update              Update to latest version
    ouro uninstall           Uninstall Ouro
    ouro acp                 Run as an ACP server for editor integration
    ouro sessions browse     Interactive session picker with search

    ouro claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``ouro update``
...
# this guard, ouro crashes on import and the user can't run
# ``ouro update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
# a prior ``ouro update`` left a recovery marker and a core package's import
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``ouro tools`` or
    ...
    try:
        ...
        setproctitle.setproctitle("ouro")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"ouro", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"ouro")
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("OURO_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".ouro", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``OURO_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``ouro … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("OURO_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``OURO_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("OURO_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`ouro --tui … >log`, CI capture):
...
def _try_ultrafast_version() -> bool:
    """Handle ``ouro --version`` before config/logging imports."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (ouro tools, ouro setup, ouro model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'ouro {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any ouro module import.
#
# Many modules cache OURO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("OURO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.ouro/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set OURO_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `ouro mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Ouro' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo ouro -p <name>` against the invoking user's home.
        ...
        candidate = home / ".ouro" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`ouro chat -p coder`), so keep scanning broadly.
    ...
    # 1.5 If OURO_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.ouro/profiles/coder or
    # /opt/data/profiles/coder).  If OURO_HOME points to the ouro root
    # instead (e.g. systemd hardcodes OURO_HOME=/root/.ouro), we must
    # still read active_profile — the user may have switched profiles via
    # `ouro profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("OURO_HOME", "")
    if profile_name is None and hermes_home_env:
        if Path(hermes_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the ouro root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as OURO_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``ouro gateway run`` to mean
    # "the root OURO_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("OURO_S6_SUPERVISED_CHILD"):
        try:
    ...
    # 3. If we found a profile, resolve and set OURO_HOME
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent ouro from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["OURO_HOME"] = hermes_home
...
# Load .env from ~/.ouro/.env first, then project root as dev fallback.
...
# Bridge security.redact_secrets from config.yaml → OURO_REDACT_SECRETS env
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "OURO_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["OURO_REDACT_SECRETS"] = str(_early_redact).lower()
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("OURO_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("OURO_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Ouro itself has been explicitly configured (model
    ...
    # Only count these if Ouro has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Ouro to use their tokens.
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``ouro -c`` from repo A continues repo A's last session rather than the
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo ouro {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo ouro {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'ouro sessions list' to see available sessions, or "
...
def _print_tui_exit_summary(
    ...
    print(f"  ouro --tui --resume {target}")
    if title:
        print(f'  ouro --tui -c "{title}"')
    print()
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``OURO_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("OURO_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``OURO_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("OURO_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "OURO_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``ouro update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("OURO_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Ouro checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `ouro update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Ouro checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `ouro --tui`\n"
        "If the checkout is still inconsistent, run `ouro update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (OURO_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("OURO_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $OURO_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Ouro installed,
    ...
    ext_dir = os.environ.get("OURO_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with OURO_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset OURO_TUI_DIR (e.g. `unset OURO_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Ouro-managed
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("OURO_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["OURO_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("OURO_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["OURO_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("OURO_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["OURO_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["OURO_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a ouro process: propagate the profile-home contract via
    ...
    env["OURO_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["OURO_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["OURO_MODEL"] = model
        env["OURO_INFERENCE_MODEL"] = model
    if provider:
        env["OURO_TUI_PROVIDER"] = provider
        env["OURO_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["OURO_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["OURO_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["OURO_TUI_SKILLS"] = value
    if query:
        env["OURO_TUI_QUERY"] = query
    if image:
        env["OURO_TUI_IMAGE"] = image
    if checkpoints:
        env["OURO_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["OURO_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["OURO_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["OURO_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["OURO_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["OURO_ACCEPT_HOOKS"] = "1"
    ...
    # OURO_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `ouro --tui` try to
    ...
    # still set OURO_TUI_RESUME themselves.
    env.pop("OURO_TUI_RESUME", None)
    if resume_session_id:
        env["OURO_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `ouro update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``OURO_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``ouro kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``ouro kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("OURO_KANBAN_BOARD"):
        return
    try:
        from hermes_cli.kanban_db import get_current_board

        os.environ["OURO_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.ouro/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Ouro — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Ouro still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``ouro … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'ouro sessions list' to see available sessions.")
            sys.exit(1)
    ...
    # Code / Codex CLI) and resume the newly created Ouro session.
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: ouro --resume {_imported_id})")
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'ouro doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Ouro isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  ouro setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'ouro setup' at any time to configure.")
        sys.exit(1)
    ...
    # rare launch right after `ouro update` changed a bundled skill.
    ...
    if getattr(args, "yolo", False):
        os.environ["OURO_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.ouro/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["OURO_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["OURO_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["OURO_SESSION_SOURCE"] = args.source
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Ouro?")
        print()
    ...
    # has no creds.json.  Every subsequent `ouro gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: ouro gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `ouro gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  ouro gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Ouro'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  ouro gateway")
        ...
        print()
        print("  Or install as a service: ouro gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'ouro whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``ouro whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``ouro model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("OURO_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'ouro model' for "
                "available providers, or run 'ouro doctor' to diagnose config "
                "issues."
            )
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``ouro model``
    ...
    # a leftover OPENAI_BASE_URL in ~/.ouro/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.ouro/.env if the active provider is not 'custom'.
...
#
# Ouro uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `ouro model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `ouro model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Ouro only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``ouro model`` flow, then route aux tasks
    to them here.
    """
    from hermes_cli.config import load_config
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Ouro URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# fast paths like `ouro --version` and slash-command dispatch that never
...
# every `hermes` invocation, including fast paths like `ouro --version`
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``ouro setup`` / ``ouro model``.
    ...
    recover from a malformed paste without editing ``~/.ouro/.env`` by hand.
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `ouro setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Ouro will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Ouro CLI with a provider."""
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.ouro/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `ouro sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set OURO_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "OURO_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="ouro sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="ouro sync now")
...
def cmd_slack(args):
    ...
    Dispatches ``ouro slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: ouro slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `ouro slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_approvals(args):
    """Dispatch `ouro approvals <subcmd>`."""
...
def cmd_backup(args):
    """Back up Ouro home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Ouro backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Ouro (or just the Chat GUI with --gui)."""
...
# validated against. Lives next to the checkout (NOT in OURO_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``ouro update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``ouro update``
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``ouro update``
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $OURO_HOME."""
    from hermes_constants import get_hermes_home
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the ouro venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — ouro venv python3, no nix-shell overhead
    ...
    # a ouro venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``ouro update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Ouro-managed install, or a freshly provisioned managed npm when
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``ouro web``).
    ...
    # as `ouro update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (ouro web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``ouro update`` can unconditionally call ``ouro desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``ouro desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $OURO_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $OURO_HOME."""
    from hermes_constants import get_hermes_home
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Ouro.app/Contents/MacOS/Ouro → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``ouro desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``ouro update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Ouro.app/Contents/MacOS/Ouro"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Ouro.exe",
            release_dir / "win-ia32-unpacked" / "Ouro.exe",
            release_dir / "win-arm64-unpacked" / "Ouro.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Ouro",
            release_dir / "linux-arm64-unpacked" / "hermes",
            release_dir / "linux-arm64-unpacked" / "Ouro",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Ouro.exe
...
#
# The desktop self-update chain (Desktop → ouro-setup --update →
# `ouro update` → `ouro desktop --build-only` → relaunch) rebuilds
# Ouro.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Ouro.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Ouro.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 ouro-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Ouro.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Ouro.exe from backup.")
        print("    Your existing version was kept and still works. Run `ouro desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `ouro desktop --force-build` to rebuild, or re-run the Ouro")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Ouro'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Ouro.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Ouro.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Ouro.exe: Access is
    ...
    an unrelated "Ouro" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Ouro.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Ouro is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Ouro.app/Contents/MacOS/Ouro  ->  app bundle = .../Ouro.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``ouro desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Ouro Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Ouro Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `ouro desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the OURO_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Ouro Desktop (Linux only, best-effort).
    ...
    shell. ``ouro uninstall --gui`` removes it.
    """
    try:
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["OURO_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["OURO_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "hermes_root", None):
        env["OURO_DESKTOP_OURO_ROOT"] = str(Path(args.hermes_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["OURO_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["OURO_DESKTOP_CWD"] = os.getcwd()
    ...
    # `OURO_DESKTOP_DISABLE_GPU=... ouro desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "OURO_DESKTOP_DISABLE_GPU" not in os.environ:
        env["OURO_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `OURO_DESKTOP_PASSWORD_STORE=... ouro desktop` keeps working.
    if sys.platform == "linux" and "OURO_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["OURO_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  ouro gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Ouro-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → ouro-setup →
            # ouro update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `ouro update` path).
            ...
            if not source_mode:
                ...
                # holds Ouro.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Ouro.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Ouro.exe, close any")
                    print("  running Ouro desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> ouro desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Ouro is
                ...
                # success on a Ouro.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Ouro shows up
    ...
    if source_mode:
        print("→ Launching Ouro Desktop from source build...")
    ...
    print(f"→ Launching packaged Ouro Desktop: {' '.join(launch_command)}")
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Ouro installs Linux services in the
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but ouro launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``ouro update``.
...
# =========================================================================
# Fork detection and upstream management for `ouro update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $OURO_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``ouro update``.
    ...
    protocol stream (``ouro acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `ouro update` was interrupted mid-install — "
    ...
    try:
        ...
        if self_locked:
            print(
                "  Ouro is still running from the launcher that needs "
                "replacing. Close other Ouro windows, restart from a "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``ouro update`` is still progressing even if pip/uv itself is silent.
    """
...
def _quarantine_running_hermes_exe(
    ...
    when ``ouro update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    the next ouro invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Ouro Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Ouro Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Ouro Desktop, exit other `hermes` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `ouro update`."
        )
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every ouro invocation. The .old files are unlocked once
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `ouro update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hermes.exe`` / ``ouro-gateway.exe`` shims
    ...
    # hours later inside ``ouro update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    ``hermes.exe`` shim is locked during ``ouro update``, or when uv/distlib
    skips a launcher write. The symptom is ``ouro-agent.exe`` and
    ``ouro-acp.exe`` present but ``hermes.exe`` missing, so ``hermes`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `ouro update --force` after "
            "closing other ouro processes."
        )
        return
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``ouro update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `ouro update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `ouro update --force` after closing other ouro processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``ouro update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.ouro/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``ouro update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``ouro update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.ouro/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``ouro update --gateway``) the update is already
...
def cmd_update(args):
    """Update Ouro to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `ouro update --branch=<x>` would actually pull.
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``ouro -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `ouro profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → ouro -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.ouro)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  ouro profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     ouro -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `ouro profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → ouro -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  ouro -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      ouro -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `ouro profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"ouro -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Ouro {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Ouro {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No ouro dashboard processes running.")
        return 0

    print(f"{len(live)} ouro dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``ouro dashboard register`` for OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `ouro dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "OURO_DASHBOARD_OAUTH_CLIENT_ID into ~/.ouro/.env for you.\n"
            "  Docs: https://ouro.dev/docs/"
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under $HOME/.ouro/desktop-ssh: a
    # literal "~/.ouro/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of OURO_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_hermes_home(): a non-default sticky profile (or any OURO_HOME pointing
    ...
    token_root = _Path.home() / ".ouro" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``OURO_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``ouro dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No ouro dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `ouro update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with ouro serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with OURO_DESKTOP=1 plus
    # OURO_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # OURO_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `ouro dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited OURO_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # OURO_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (OURO_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets OURO_SERVE_HEADLESS below.
    if os.environ.get("OURO_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("OURO_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("OURO_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("OURO_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → OURO_HOME points into
    ...
    if (
        ...
        and os.environ.get("OURO_DESKTOP") != "1"
    ):
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: OURO_HOME is explicitly pinned to the
        ...
        # OURO_HOME.  We must resolve the root explicitly instead of just
        # dropping OURO_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV OURO_HOME=/opt/data`), so an unset
        # OURO_HOME falls back to $HOME/.ouro = /opt/data/.ouro — an
        ...
        # returns the root for both layouts: ~/.ouro for a standard install
        ...
        try:
            from hermes_constants import get_default_hermes_root
            env["OURO_HOME"] = str(get_default_hermes_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop OURO_HOME) rather than block the reroute.
            env.pop("OURO_HOME", None)
    ...
    # the same logs directory as every other Ouro surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["OURO_SERVE_HEADLESS"] = "1"
    elif "OURO_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["OURO_WEB_DIST"])
            if "OURO_WEB_DIST" in os.environ
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom OURO_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "OURO_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # OURO_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["OURO_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ OURO_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset OURO_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads OURO_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["OURO_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from OURO_WEB_DIST: {_dist_root}")
...
def cmd_logs(args):
    """View and filter Ouro log files."""
...
def cmd_console(args):
    """Open the safe Ouro command console."""
...
# so that in ``ouro -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``ouro -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``ouro <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``ouro photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("OURO_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that OURO_YOLO_MODE is set before ANY
...
def _try_fast_chat_launch() -> bool:
    ...
    ``hermes`` / ``ouro -w -s foo --yolo`` / ``ouro chat`` don't need the
    ...
    """
    if os.environ.get("OURO_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("OURO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["OURO_DEFER_AGENT_STARTUP"] = "1"
            os.environ["OURO_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["OURO_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `ouro --tui` is the hot path on phones. The full parser setup imports
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: ouro-agent[otlp])")
...
def _advertise_agent_env() -> None:
    ...
    (``ouro-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``OURO_AGENT`` is the Ouro-specific marker. setdefault: never
    clobber an outer harness (e.g. Ouro running inside another agent's
    terminal).
    """
    os.environ.setdefault("AI_AGENT", "ouro-agent")
    os.environ.setdefault("OURO_AGENT", "true")


def main():
    """Main entry point for ouro CLI."""
    ...
    # ``ouro update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # If the checkout changed since the last launch (ouro update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``ouro update``
    ...
    # ``ouro skills install update``) merely defers recovery one launch;
    # under-matching (missing ``ouro -p work update``) would race a recovery
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://ouro.dev/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `ouro model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.ouro/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://ouro.dev/docs/user-guide/secrets/"
        ),
    )
    ...
    # `ouro proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://ouro.dev/docs/user-guide/egress/iron-proxy"
        ),
    )
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``ouro proxy`` subparser (dest='proxy_command').
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `ouro whatsapp` which sets up "
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.ouro/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo ouro uses to snapshot working directories before "
    ...
    # subcommand — ``ouro --help``, ``ouro version``, ``ouro logs``,
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Ouro shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `ouro computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `ouro tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `ouro computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Ouro app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            override = _os.environ.get("OURO_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                ...
                # to repair an install that `ouro computer-use install` will
                # (correctly) refuse to touch, with nothing pointing at the
                # env var that actually selected the binary.
                origin = " [custom binary from OURO_CUA_DRIVER_CMD]" if override else ""
                if version:
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by OURO_CUA_DRIVER_CMD, or unset "
                            "the override and run: ouro computer-use install --upgrade"
                        )
                    else:
                        print("    Run: ouro computer-use install")
                    return 1
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: ouro computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: ouro computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: ouro computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: ouro computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: ouro computer-use permissions grant")
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <ouro home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Ouro",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Ouro session store "
            "so it can be resumed with 'ouro --resume <id>'. The foreign "
    ...
    # for one release. The Ouro-Setup.exe success screen tells users to
    # run `ouro desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``ouro -c Pokemon Agent Dev`` → ``ouro -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'ouro -c model' where 'model' is consumed as the
    ...
    # --yolo: set OURO_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (ouro hooks list, cron
    ...
    # ``ouro egress start`` refusing when credential_source=bitwarden
