...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Ouro already vendors
    (``~/.ouro/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".ouro" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``ouro acp`` speaks JSON-RPC on stdout);
...
def recover_if_needed(
    ...
    recovery marker from a prior ``ouro update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `ouro update`: CLI and Desktop retries preserve
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``ouro update`` (or the self-lock preflight, #83569) left the
    ...
    failure does not block every launch (``ouro acp`` included).
    ...
    try:
        ...
        try:
            ...
            print(
                "  The next launch will retry; ouro will keep working from "
