...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Ouro accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Ouro' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Ouro should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Ouro state.

    The app-server owns the compacted thread context, so Ouro should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Ouro deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Ouro UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Ouro tool loop. Without this bridge gateway
...
# Codex item types that map to a Ouro tool_call in the projector (and
...
# Internal MCP server that wraps Ouro' native tools for codex. When
...
# vision_analyze, ...) since the user thinks of these as Ouro tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "hermes-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Ouro tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Ouro' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Ouro' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Ouro'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Ouro' standard prompt flow if a
        ...
        # explicitly opted out of Ouro approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / OURO_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Ouro UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into Ouro' gateway UI callbacks
