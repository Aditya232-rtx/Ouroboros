...
* First-use consent is gated by the allowlist under
  ``~/.ouro/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``OURO_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Ouro-canonical

    # Inject context for pre_llm_call:
    {"context": "Today is Friday"}

    # Modify tool input for pre_tool_call (Ouro-canonical):
...
def register_from_config(
    ...
    *,
    ...
    setting.  ``OURO_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / OURO_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("OURO_SAFE_MODE"):
        logger.info("OURO_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / OURO_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``ouro hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _evaluate_result(
    ...
    * non-JSON / unparseable stdout on a ``fail_closed`` blocking hook —
    ...
    test helper (:func:`run_once`) so ``ouro hooks test`` reflects
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Ouro wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Ouro
    ...
    if event == "pre_verify":
        # "continue" (Ouro) / "block" (Claude-Code Stop: block the stop) both
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / OURO_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Ouro is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``OURO_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("OURO_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `ouro hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``ouro hooks test`` and ``ouro hooks doctor``.
    ...
    produce — otherwise scripts tested via ``ouro hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Ouro-wire-shape response — including exit-code-2
    blocking and ``fail_closed`` semantics, so what ``ouro hooks test``
