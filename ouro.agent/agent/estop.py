...
``ouro pause`` writes a sentinel file at ``$OURO_HOME/ESTOP``;
``ouro resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Ouro is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.ouro/ESTOP``.
...
def _hermes_home() -> Path:
    """Resolve the active OURO_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active OURO_HOME."""
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on OURO_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Ouro is paused ({reason}). New work is on hold; "
            "run `ouro resume` to pick things back up."
        )
    return (
        "⏸️ Ouro is paused. New work is on hold; "
        "run `ouro resume` to pick things back up."
    )
