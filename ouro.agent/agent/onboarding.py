...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Ouro starts and finds ``~/.openclaw/``.

    Points users at ``ouro claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``ouro claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Ouro, run "
        "`ouro claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `ouro claw cleanup` (renames it to ~/.openclaw.pre-migration — "
