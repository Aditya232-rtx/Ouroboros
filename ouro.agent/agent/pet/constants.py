...
row taxonomy is inferred from the atlas shape so Ouro can render both legacy
...
# rows (1536x1872), while older Ouro/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Ouro' activity state names. They are not always identical to the
...
# Legacy Ouro/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Ouro activity names -> accepted row-name aliases in descending
