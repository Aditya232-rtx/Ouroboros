...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when OURO_DEV_CREDITS_FIXTURE selects a fixture state, /usage
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so OURO_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — OURO_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Ouro's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `ouro auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `ouro auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Ouro doesn't keep the credential
