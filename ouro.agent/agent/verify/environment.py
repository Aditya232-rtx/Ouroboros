...
The manifest lives at ``<project>/.ouro/environment.json`` and is the
...
MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".ouro") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``ouro verify``.
    """
