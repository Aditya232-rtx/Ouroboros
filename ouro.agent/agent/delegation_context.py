...
The parent Ouro process may itself be a Kanban dispatcher worker with
OURO_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's OURO_KANBAN_* vars are legitimately in os.environ (cron
...
DELEGATED_CHILD_ENV_MARKER = "OURO_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``OURO_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$OURO_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``OURO_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``OURO_KANBAN_*`` vars while losing the ContextVar in
