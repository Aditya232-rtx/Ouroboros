...
configured under ``monitoring.export.otlp``. Lets an operator stream Ouro
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install ouro-agent[otlp]``), imported lazily so the
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'ouro-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
