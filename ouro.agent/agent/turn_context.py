...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``ouro logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Ouro only initiates compaction in "hermes" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Ouro preflight compression for codex app-server "
                "(mode=%s); Ouro will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Ouro (via _emit_status), so it always
