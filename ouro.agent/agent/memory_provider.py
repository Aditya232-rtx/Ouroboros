...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `ouro backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - hermes_home (str): The active OURO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.ouro``.
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'ouro memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], hermes_home: str) -> None:
        ...
        Called by 'ouro memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``hermes_home`` is the active OURO_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE OURO_HOME.

        ``ouro backup`` only walks OURO_HOME, so any provider state kept
        ...
        of the archive, and ``ouro import`` restores them to their original
