...
Data resolution order:
  ...
  2. Disk cache (~/.ouro/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Ouro ↔ models.dev
# ---------------------------------------------------------------------------

# Ouro provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Ouro' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Ouro ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Ouro provider ids that map to *mdev_id* (may be [])."""
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``ouro config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Ouro provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Ouro id.
...
def _get_provider_models(
    ...
    """Resolve a Ouro provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``ouro model``), which is an interactive user-facing flow
...
# Gemma models whose TPM quotas are too small for normal Ouro agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Ouro provider ID (e.g. "kilocode") or a models.dev
...
def get_model_info(
    ...
    Accepts Ouro or models.dev provider ID.  Tries exact match then
