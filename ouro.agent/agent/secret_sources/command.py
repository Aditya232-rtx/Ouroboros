...
* The requested key is passed to the child ONLY via the ``OURO_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``OURO_SECRET_KEY``) — it is never called per-key in a loop, so a
...
def _run_helper(
    ...
    The key is passed as DATA via ``OURO_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["OURO_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``OURO_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # OURO_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
    ...
    def remediation(self, kind, cfg: dict) -> str:
        ...
        if kind == ErrorKind.INTERNAL:
            return (
                ...
                "Ouro discards helper stderr so diagnostics can't leak "
                "secret material."
            )
