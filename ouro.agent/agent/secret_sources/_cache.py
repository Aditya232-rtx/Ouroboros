...
cache problem must never block Ouro startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Ouro home used for cache paths.

    ``home_path`` is whatever ``load_hermes_dotenv()`` already resolved;
    falling back to ``$OURO_HOME`` / ``~/.ouro`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if OURO_HOME is ever made traversable.
            try:
