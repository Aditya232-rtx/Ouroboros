...
Ouro pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.ouro/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  Ouro pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.ouro/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block Ouro startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `ouro chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.ouro/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    callers; falls back to `$OURO_HOME` / `~/.ouro` when home is None.
    """
...
def _hermes_bin_dir() -> Path:
    """Where Ouro stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``ouro secrets bitwarden setup``
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Ouro
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `ouro secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `ouro secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `ouro secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `ouro secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`ouro secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`ouro secrets bitwarden token`) so the
