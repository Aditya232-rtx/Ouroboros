...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Ouro does
...
class EntraIdentityConfig:
    ...
    Captures the Ouro-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Ouro-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.ouro/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Ouro processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``ouro doctor`` /
    ``ouro auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``ouro doctor`` and the wizard preflight — never
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `ouro doctor` or `az login` to recover.",
                exc,
            )
