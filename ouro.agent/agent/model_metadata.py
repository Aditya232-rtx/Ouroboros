...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``OURO_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("OURO_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Ouro.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Ouro from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Ouro' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Ouro minimum.
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Ouro grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    # ``Ouro-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Ouro sends to providers:
