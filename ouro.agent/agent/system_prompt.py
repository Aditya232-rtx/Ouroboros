...
upstream prefix cache warm.  See ``ouro-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``OURO_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("OURO_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the OURO_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound OURO_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Ouro never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want OURO_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the OURO_HOME ContextVar
    ...
    # Pointer to the ouro-agent skill + docs for user questions about Ouro itself.
    stable_parts.append(OURO_AGENT_HELP_GUIDANCE)
    ...
    # OURO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Ouro, any interactive coding surface in a code
    ...
    # `ouro -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    ...
    # Active-profile hint — names the Ouro profile the agent is running
    # under so it doesn't conflate ~/.ouro/skills/ (default profile) with
    # ~/.ouro/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient OURO_HOME: on a build thread that lost the ContextVar this
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Ouro profile: default. Other profiles (if any) live "
        ...
        post_workspace_parts.append(
            f"Active Ouro profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Ouro). Every other surface (desktop chat panel,
