"""Profile-scoped NeMo Relay runtimes owned by the Ouro agent core."""
...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when Ouro finalization forwards runtime metadata. Filter to
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Ouro session."""
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Ouro-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                ...
                logger.warning(
                    "Ouro Relay segment close failed (session=%s segment=%d); "
            ...
            try:
                ...
                logger.warning(
                    "Ouro Relay segment open failed (session=%s segment=%d); "
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Ouro Relay session without creating one."""
        with self._sessions_lock:
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Ouro session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Ouro Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Ouro Relay session context is unavailable")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Ouro session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before Ouro authorizes a tool call."""
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "Ouro Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "Ouro Relay drained %d orphaned scope(s) before closing %s",
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Ouro Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Ouro Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Ouro profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Ouro Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one Ouro turn or top-level task."""
...
# Set >0 while the native Relay pipeline is mid-dispatch of a Ouro callback
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Ouro core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Ouro Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                logger.warning(
                    "Ouro Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Ouro Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Ouro turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Ouro turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Ouro Relay segment rotation failed", exc_info=True
                )
            try:
                ...
                logger.warning("Ouro Relay turn initialization failed", exc_info=True)
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "Ouro Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
                ...
                try:
                    ...
                    logger.warning(
                        "Ouro Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "Ouro Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Ouro Relay compaction notification failed", exc_info=True
            )
    ...
    def _finish_logical_calls(
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            logger.warning("Ouro Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a Ouro tool). Nested managed execution here is structurally
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Ouro session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Ouro Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at Ouro's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Ouro core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Ouro Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Ouro session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Ouro Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Ouro Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Ouro session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Ouro Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active Ouro profile."""
