...
_SENSITIVE_OURO_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_dirs.extend(hermes_home / rel for rel in _SENSITIVE_OURO_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Ouro path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.ouro/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Ouro path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
