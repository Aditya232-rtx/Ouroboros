...
# Org mirrors live under ~/.ouro/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    OURO_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``OURO_KANBAN_TASK`` / ``OURO_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("OURO_KANBAN_TASK") or os.getenv("OURO_KANBAN_BOARD"):
        ...
        # The Ouro Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``OURO_PLATFORM`` or
            ``OURO_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("OURO_PLATFORM")
        or get_session_env("OURO_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``ouro config set`` and JSON-mode editor saves store lists as quoted
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.ouro/skills/`` are silently skipped.
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against OURO_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.ouro/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.ouro/skills/`` (including via symlinks) or configured
    ...
    # ~/.ouro/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Ouro can discover and view
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Ouro metadata, VCS, virtualenv/dependency, cache, and skill
