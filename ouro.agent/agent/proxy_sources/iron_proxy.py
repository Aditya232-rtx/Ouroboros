...
* The ``iron-proxy`` binary is auto-installed into ``<hermes_home>/bin/iron-proxy``
  on first use.  Ouro pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<hermes_home>/proxy/ca.{crt,key}`` is generated on
  first ``ouro egress setup``.  Sandboxes trust this CA so iron-proxy can
...
* The proxy runs as a managed subprocess (``ouro egress start``), pidfile
...
# request needs the bearer key below.  ``ouro egress reload`` is the
# client.
#
...
_MGMT_API_KEY_ENV = "OURO_IRON_PROXY_MGMT_KEY"
...
# credential (Ouro' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.ouro/proxy/`` just to
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``ouro proxy install`` surface lets them
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "ouro-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``ouro egress
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    The daemon receives it via the ``OURO_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``ouro egress reload`` reads the same file to authenticate.
    """
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `ouro egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `ouro egress setup` and use "
            "`ouro egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `ouro egress setup`, "
            "then `ouro egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``ouro egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `ouro egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    ...
    try:
        import yaml  # PyYAML is already a Ouro dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``ouro egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``ouro egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_OURO_IRON_PROXY_NONCE_ENV = "OURO_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_OURO_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `ouro egress install`."
        )
    ...
    env[_OURO_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Ouro
    # process.
    #
    ...
    # ``ouro egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``ouro egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `ouro egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`ouro egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `ouro secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
