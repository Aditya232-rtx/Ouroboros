"""Anthropic Messages API adapter for Ouro.

Translates between Ouro's internal OpenAI-style message format and
...
try:
    ...
    _OURO_VERSION = str(_hermes_cli.__version__)
except Exception:
    _OURO_VERSION = "0.0.0"
...
# Ouro effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + Ouro User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://ouro.dev")
        headers.setdefault("X-Title", "Ouro")
        headers.setdefault("User-Agent", f"Ouro/{_OURO_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Ouro loads that into the process env from
    # ``~/.ouro/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Ouro notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Ouro historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``ouro models``) never mutates ``~/.ouro/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Ouro)
      ...
      5. Anthropic credential_pool OAuth entry (~/.ouro/auth.json)
    ...
    # 1. Ouro-managed OAuth/setup token env var
    ...
    # 5. Ouro credential_pool OAuth entry.
...
# ── Ouro-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.ouro/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Ouro-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Ouro with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Ouro-managed OAuth credentials from ~/.ouro/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Ouro OAuth credentials: %s", e)
    return None
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See ouro-agent: extended-thinking + parallel
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Ouro", "Claude Code")
                text = text.replace("Ouro agent", "Claude Code")
                text = text.replace("ouro-agent", "claude-code")
        ...
        #      a) bare Ouro-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Ouro surfaces in its CLI. We
