...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.ouro/.env``. See Aditya232-rtx/ouro#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'ouro memory status'. Note: "
        "systemd/gateway services do not inherit ~/.ouro/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from hermes_cli.providers import OURO_OVERLAYS, get_provider

        overlay = OURO_OVERLAYS.get(provider)
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: ouro config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$OURO_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$OURO_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .ouro.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / OURO_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.ouro/SOUL.md as the primary
    ...
    # report cumulative micros spent.  Surfaced behind OURO_DEV_CREDITS.
    ...
    # both live under ~/.ouro/logs/.  Idempotent, so gateway mode
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `ouro model` to "
                        "select a provider, or run `ouro setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different OURO_HOME after
    ...
    # worker (kanban_show tool is present iff OURO_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["OURO_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.ouro/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.ouro/sessions/session_{sid}.json)
    ...
    # `ouro --resume <id>` can restore the bypass (CLI resume paths read
    ...
    # management API (the historical Ouro behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Ouro' built-in
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Ouro.  Choose a model with at least "
    ...
    # Nous Ouro 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            ...
            if _hermes_warn:
                _user_msg = (
                    "⚠ Nous Research Ouro 3 & 4 models are NOT agentic — they "
