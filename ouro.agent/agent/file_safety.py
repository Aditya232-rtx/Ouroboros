...
def _hermes_home_path() -> Path:
    """Resolve the active OURO_HOME (profile-aware) without circular imports."""
    try:
        from hermes_constants import get_hermes_home  # local import to avoid cycles
        return get_hermes_home()
    except Exception:
        return Path(os.path.expanduser("~/.ouro"))


def _hermes_root_path() -> Path:
    """Resolve the Ouro root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_safe_write_roots() -> set[str]:
    """Return resolved OURO_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("OURO_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside OURO_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Ouro path.

    Three categories are blocked:

      * Internal Ouro cache files under ``OURO_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under OURO_HOME and the global Ouro
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.ouro/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active OURO_HOME (profile-aware) AND the global
    # Ouro root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (OURO_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Ouro cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Ouro credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Ouro MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Ouro MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Ouro read (see
...
#
# Ouro profiles are separate OURO_HOME dirs under
...
# edited skills under both ``~/.ouro/profiles/hermes-security/skills/``
# AND ``~/.ouro/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under OURO_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from OURO_HOME.
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Ouro scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Ouro entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Ouro profile {info['target_profile']!r}, but the "
...
#
#   <OURO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.ouro/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.ouro/…`` segment and warns
# regardless of which Ouro profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.ouro/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.ouro`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.ouro/…`` and returns the
    index where the inner Ouro-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .ouro / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".ouro":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Ouro state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.ouro``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.ouro`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Ouro resolver to
    succeed, so it works correctly even when called from contexts where
    OURO_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Ouro process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real OURO_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.ouro/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.ouro/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host OURO_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Ouro state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Ouro "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real OURO_HOME. Use the "
