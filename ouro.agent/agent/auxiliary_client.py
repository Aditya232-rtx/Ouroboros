...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.ouro/auth.json active provider)
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``OURO_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `ouro update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Ouro owns auxiliary retry + provider/model fallback policy (the
    ...
    # Ouro sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Ouro control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Ouro triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``OURO_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``OURO_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("OURO_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("OURO_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Ouro)",
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("OURO_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Ouro plumbing (_reasoning_config
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.ouro/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``ouro auth
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Ouro auth store.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `ouro setup` or `ouro model` and enter a valid http(s) base URL."
        ) from exc
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``ouro config aux reset``)."""
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Ouro' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `ouro model` but the
    #    old OPENAI_BASE_URL lingers in ~/.ouro/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: ouro model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.ouro/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Ouro owns
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: ouro auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: ouro model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: ouro model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: ouro doctor for "
                "diagnostics)"
            )
            return None, None
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Ouro reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Ouro reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `ouro model`."
                    )
