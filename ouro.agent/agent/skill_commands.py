...
#
# When a user invokes a /skill (or /bundle), Ouro expands the turn into a
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``OURO_PLATFORM`` env var and
    ``OURO_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("OURO_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective Ouro home the skill scan should be scoped to.
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.ouro/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core Ouro slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Ouro command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Ouro home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.ouro/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``ouro -s <skill>`` or
    a deployment's ``OURO_TUI_SKILLS`` env var could force-load a skill an
