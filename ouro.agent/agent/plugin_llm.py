...
Plugins built on Ouro often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Ouro
...
embedded slugs, no shorthands. This mirrors Ouro' main config
...
handles every provider, fallback chain, and per-task override Ouro
supports.
"""
