"""Upload a Ouro session transcript to Hugging Face as an agent trace.

Ouro stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``ouro trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Ouro' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Ouro OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Ouro' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Ouro conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.ouro/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `ouro trace upload`)."
)
