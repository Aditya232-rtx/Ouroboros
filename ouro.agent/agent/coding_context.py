"""Coding-context awareness — base Ouro, every interactive surface.

When the user runs Ouro inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Ouro shifts into a **coding posture**. This module is the
...
# in _OURO_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``OURO_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Ouro should operate in its coding posture right now."""
