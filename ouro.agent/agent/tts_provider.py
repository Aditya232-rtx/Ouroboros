...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Ouro with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.ouro/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Ouro
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``ouro tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``ouro setup`` for
