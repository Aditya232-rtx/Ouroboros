"""Core NeMo Relay adapters for physical Ouro provider attempts."""
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Ouro turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Ouro turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Ouro's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Ouro-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Ouro."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Ouro Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
