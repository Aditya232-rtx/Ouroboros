...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the OURO_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Ouro runtime / deployment
    "OURO_HOME", "OURO_PROFILE", "OURO_GATEWAY_LOCK_DIR",
    "OURO_MAX_ITERATIONS", "OURO_MAX_TOKENS", "OURO_API_TIMEOUT",
    "OURO_REDACT_SECRETS", "OURO_NOUS_TIMEOUT_SECONDS",
    "_OURO_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "OURO_KANBAN_DB", "OURO_KANBAN_WORKSPACES_ROOT", "OURO_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "OURO_KANBAN_",
    "OURO_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Ouro writes
    ...
    # Parse values with the canonical Ouro parser: save_env_value
