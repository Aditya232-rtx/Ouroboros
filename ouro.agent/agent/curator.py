...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.ouro/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `ouro update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``ouro curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``ouro update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``ouro curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `ouro curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Ouro' background skill CURATOR. This is an "
    ...
    "into ~/.ouro/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.ouro/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.ouro/logs/curator/``)
    ...
    authored skill data in ``~/.ouro/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every ouro install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: ouro curator status
        keep an umbrella stable: ouro curator pin document-tools
    ...
    lines.append("full report: ouro curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: ouro curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.ouro/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.ouro/skills/.archive/` for "
            "safety and can be restored via `ouro curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.ouro/skills/.archive/`. "
            "Restore any via `ouro curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `ouro curator restore <name>`")
    lines.append("- All archives live under `~/.ouro/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``ouro curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `ouro curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `ouro update` only if "
        ...
        # recorded in state so `ouro curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``ouro model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `ouro model` → auxiliary
