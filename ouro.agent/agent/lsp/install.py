...
appropriate.  All installs go to a Ouro-owned bin staging dir,
``<OURO_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``ouro lsp
  status``.
...
# ``<OURO_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `ouro lsp status` reports the host's presence.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the Ouro-owned bin staging dir for LSP servers."""
...
def _install_npm(
    ...
    """
    # Managed npm first: $OURO_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Ouro installed and
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <OURO_HOME>/lsp/
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``ouro lsp status`` CLI to give users a quick
