...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<OURO_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<OURO_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    inner = (
        ...
        f"-HostName Ouro -HostProfileId ouro -HostVersion 1.0.0 "
