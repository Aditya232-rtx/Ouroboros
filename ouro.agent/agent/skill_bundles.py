...
Storage
-------
Bundles live in ``~/.ouro/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``OURO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``ouro bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under OURO_HOME.

    Honors ``OURO_BUNDLES_DIR`` for tests; falls back to
    ``<OURO_HOME>/skill-bundles``.
    """
    override = os.environ.get("OURO_BUNDLES_DIR")
    if override:
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `ouro bundles` CLI subcommand.
# ---------------------------------------------------------------------------
