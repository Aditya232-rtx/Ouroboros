...
Owns one Codex thread per Ouro session. Drives `turn/start`, consumes
...
# Ouro' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_OURO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Ouro/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Ouro session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _OURO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("OURO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Ouro' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Ouro' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Ouro' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Ouro
        approval-mode/timeout logic. The Ouro-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Ouro approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Ouro approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Ouro returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Ouro approval mode/timeout resolution
    ...
    # "prompt expired"; the Ouro-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort Ouro version string for codex's userAgent line."""
    try:
