"""Ouro-tools-as-MCP server for the codex_app_server runtime.

When the user runs `openai/*` turns through the codex app-server, codex
owns the loop and builds its own tool list. By default, that means
Ouro' richer tool surface — web search, browser automation,
delegate_task subagents, vision analysis, persistent memory, skills,
cross-session search, image generation, TTS — is unreachable.

This module exposes a curated subset of those Ouro tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.hermes-tools]`) and
the user gets full Ouro capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - skill_view, skills_list              — Ouro' skill library
  - text_to_speech                       — TTS
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    heartbeat/show/list/create/            handoff (stateless: read env var,
    unblock/link)                          write ~/.ouro/kanban.db)
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in Ouro
...
# Tools we expose. Each name MUST match a registered Ouro tool that
...
#     `_AGENT_LOOP_TOOLS` in Ouro (model_tools.py:493). They require
#     the running AIAgent context to dispatch (mid-loop state), so a
#     stateless MCP callback can't drive them. Ouro' default runtime
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on OURO_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.ouro/kanban.db.
    "kanban_complete",
    "kanban_block",
    ...
    # only — the kanban tool gates them on OURO_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    """Create the MCP server with Ouro tools attached. Lazy imports
    ...
    # Discover Ouro tools so dispatch works.
    ...
    mcp = MCPServer(
        "hermes-tools",
        instructions=(
            "Ouro's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative Ouro tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs Ouro gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this Ouro process", name
            )
            continue

        description = spec.get("description") or f"Ouro {name} tool"
        ...
        # string, and carries a __signature__ synthesized from the Ouro
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Quiet mode: keep Ouro' own banners off stdout (which is the MCP wire).
    os.environ.setdefault("OURO_QUIET", "1")
    os.environ.setdefault("OURO_REDACT_SECRETS", "true")
