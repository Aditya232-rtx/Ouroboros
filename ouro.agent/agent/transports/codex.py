...
# Wire-name used when Ouro keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Ouro's configured provider
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if params.get("is_xai_responses", False):
            ...
            # out at high. max/ultra are Ouro ladder aliases for "this
        ...
        if (params.get("provider") or "").strip().lower() == "actual":
            ...
            # line 1 column 1"). Clamp Ouro' wider set to the supported one.
            _effort_clamp.update({"xhigh": "high", "ultra": "max"})
        reasoning_effort = _effort_clamp.get(reasoning_effort, reasoning_effort)

        response_tools = _responses_tools(tools)

        # xAI server-side web search vs Ouro web providers.
        #
        ...
        #    keep Ouro dispatch so ``web.backend`` / ``web.search_backend``
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Ouro dispatches
