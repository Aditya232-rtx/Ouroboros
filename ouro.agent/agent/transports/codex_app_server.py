...
projection into Ouro' display, approval bridging, transcript projection into
...
"codex_app_server"`. Ouro' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Ouro secret — gateway
        ...
        if spawn_env.get("OURO_KANBAN_TASK"):
            kanban_db = spawn_env.get("OURO_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "OURO_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("OURO_HOME", os.path.expanduser("~/.ouro")),
                        "kanban",
                    ),
                )
            )
