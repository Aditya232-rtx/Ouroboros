"""Public, plugin-safe lifecycle API for delegated Ouro subagents.
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active Ouro parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("Ouro failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because Ouro delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. Ouro always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
