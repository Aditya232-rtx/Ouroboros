...
as ``kind: backend``) or ``~/.ouro/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``ouro tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$OURO_HOME/cache/videos/``, creating parents as needed."""
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$OURO_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$OURO_HOME/cache/videos/``.
