...
# un-throttled log fires on *every* selection. On Windows several Ouro
...
# readiness handshake ("Timed out connecting to Ouro backend after
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``ouro model`` / ``ouro auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Ouro process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``ouro auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Ouro
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``OURO_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "OURO_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "OURO_XAI_REFRESH_TIMEOUT_SECONDS"
        )
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Ouro
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Ouro process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Ouro profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `ouro model` / `ouro auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `ouro model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``ouro auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `ouro auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `ouro auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Ouro PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `ouro setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.ouro/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `ouro setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `ouro auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (ouro auth remove copilot gh_cli) must not pay either on
        ...
        # Ouro auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.ouro/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `ouro auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `ouro auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Ouro auth store.  Without this gate the removal is instantly
        ...
        # Ouro owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``ouro model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``ouro auth list`` reflects the logged-in state and so the pool
...
# Prefer ~/.ouro/.env over os.environ — the user's config file is the
# authoritative source for Ouro credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `ouro auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.ouro/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.ouro/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `ouro auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Ouro
