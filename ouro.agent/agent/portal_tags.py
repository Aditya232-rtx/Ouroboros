...
Every Ouro request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Ouro and bucket it by client release.
...
def _hermes_version() -> str:
    """Return the current Ouro release version, e.g. ``"0.13.0"``.
...
def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Ouro session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Ouro conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.ouro/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Ouro
    ...
    """
    tags = ["product=ouro-agent", hermes_client_tag()]
