...
_OURO_MD_NAMES = (".ouro.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.ouro.md`` or ``HERMES.md``.
    ...
    # could pick up a .ouro.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _OURO_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Ouro, an intelligent AI assistant created by Nous Research. "
...
OURO_AGENT_HELP_GUIDANCE = (
    "You run on Ouro (by Nous Research). When the user needs help with "
    "Ouro itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://ouro.dev/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `ouro-agent` skill with skill_view(name='ouro-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.ouro/kanban.db`. Your task id is in "
    "`$OURO_KANBAN_TASK`; your workspace is `$OURO_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $OURO_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $OURO_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${OURO_KANBAN_BRANCH:-wt/$OURO_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$OURO_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`ouro profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `ouro kanban <verb>` for board operations. Use "
...
#
# The ouro-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to ouro-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Ouro YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Ouro launches that runtime.\n\n"
        ...
        "run `ouro computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Ouro "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Ouro floating over another application, so the
    user is rarely asking about Ouro — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Ouro "
        ...
        "rather than anything inside Ouro. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Ouro terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Ouro desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Ouro WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Ouro itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Ouro runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Ouro itself is running. The host OS, home, and cwd "
                f"of the Ouro process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Ouro "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Ouro
    ...
    extra = (os.getenv("OURO_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("OURO_PLATFORM") or os.environ.get("OURO_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.ouro/skills/`` directory.  External dirs
    ...
    # OURO_HOME would otherwise fall back to the launch (default) home and
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Ouro itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `ouro-agent` skill "
            "first. It has the actual commands (e.g. `ouro config set …`, `ouro tools`, "
            "`ouro setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: ouro setup, ouro setup tools, ouro setup terminal, ouro status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from OURO_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the OURO_HOME ContextVar falls
    ...
    try:
        ...
        logger.debug("Could not ensure OURO_HOME before loading SOUL.md: %s", e)
...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.ouro.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        return _truncate_content(
            result, ".ouro.md", context_length=context_length,
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .ouro.md / HERMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from OURO_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Ouro install/source
    ...
    # explicitly configured cwd is honored verbatim — the Ouro tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Ouro in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Ouro install tree (%s) — set terminal.cwd to "
    ...
    # SOUL.md from OURO_HOME only — skip when already loaded as identity
    if not skip_soul:
