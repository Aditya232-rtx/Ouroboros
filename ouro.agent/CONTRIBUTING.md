...
Thank you for contributing to Ouro! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Ouro to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo Aditya232-rtx/ouro "<your terms>"
  gh search prs --repo Aditya232-rtx/ouro --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/Aditya232-rtx/ouro/issues?q=) · [PRs (all states)](https://github.com/Aditya232-rtx/ouro/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Ouro itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Ouro install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `ouro skills browse` (labeled "official") and install it with `ouro skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `ouro skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.ouro/plugins/` (or via a pip entry point).
...
- Integrate with `ouro memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Ouro ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.ouro/plugins/`, project `.ouro/plugins/`, or a pip entry point) — see [Build an Ouro Plugin](https://ouro.dev/docs/guides/build-a-hermes-plugin)
...
The installer creates the Ouro venv, wires the `ouro` command, stamps the
install method for `ouro update`, and clones the full git project into
`$OURO_HOME/ouro-agent` (usually `~/.ouro/ouro-agent`). That keeps your
...
Use this only if you intentionally do not want Ouro' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `ouro` entrypoint from this venv; running the
...
export VIRTUAL_ENV="$HOME/.ouro/venvs/ouro-dev"
...
```bash
mkdir -p ~/.ouro/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.ouro/config.yaml
touch ~/.ouro/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.ouro/.env
```

### Run

```bash
# The standard installer already put `ouro` on PATH.
ouro doctor
ouro chat -q "Hello"
```

If you used the manual clone fallback, run `./ouro` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/ouro" ~/.local/bin/ouro
```
...
│
├── skills/                   # Bundled skills (copied to ~/.ouro/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (ouro.dev)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.ouro/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.ouro/`)

| Path | Purpose |
|------|---------|
| `~/.ouro/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.ouro/.env` | API keys and secrets |
| `~/.ouro/auth.json` | OAuth credentials (Nous Portal) |
| `~/.ouro/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.ouro/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.ouro/state.db` | SQLite session database |
| `~/.ouro/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.ouro/cron/` | Scheduled job data |
| `~/.ouro/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.ouro/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_OURO_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
The user may skip setup and keep loading the skill. Ouro only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `ouro setup` or update `~/.ouro/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Ouro tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Ouro" is the secondary collaborator. If the contributor's commit shows "Ouro" as author because they used Ouro to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Ouro tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `ouro --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Ouro uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.ouro/skins/<name>.yaml`:
...
Ouro runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the ouro wrapper (it has a stdlib fallback
...
Ouro has terminal access. Security matters.
...
2. **Test manually**: Run `ouro` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Aditya232-rtx/ouro/issues)
- Include: OS, Python version, Ouro version (`ouro version`), full error traceback
