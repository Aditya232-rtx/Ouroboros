...
ouro current          93.3%*  26.7%* 33.3%    30.0%  45.8%   ~162K
ouro lean closed-book 40.0%   60.0%  23.3%    36.7%  40.0%   ~49K
ouro lean+recovery    70.0%   80.0%  43.3%    80.0%  68.3%   ~49K
...
4. **What exact error message is returned when 'gh pr merge --auto' is attempted on the Aditya232-rtx/ouro repository?**  
...
9. **Which two properties shape almost every design decision in Ouro according to the Development Guide?**  
...
10. **What error message does the live-checkout git guard display when blocking a history-rewriting command?**  
   gold: `Blocked: `git <op>` would rewrite Ouro's live source checkout (/home/teknium/.ouro/hermes-agent) and can mix module `
...
1. **What is the PR number for the authored fix addressing mid-turn message ordering bugs in Ouro Desktop?**  
   gold: `#86617`
...
8. **What error message does the Ouro terminal tool return when a git command is blocked to prevent rewriting the live source checkout?**  
   gold: `Blocked: `git <op>` would rewrite Ouro's live source checkout`
9. **What is the core design principle regarding 'Narrow Waist' in Ouro development?**  
...
6. **What exact error message did the agent receive when attempting to checkout a worktree while in the live source directory?**  
   gold: `Blocked: `git checkout` would rewrite Ouro's live source checkout (/home/teknium/.ouro/hermes-agent) and can mix mod`
