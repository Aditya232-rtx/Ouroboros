"""Run the read-tool eval through the REAL Ouro AIAgent.

For each task: fresh temp OURO_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    hermes_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".ouro"
    hermes_home.mkdir(parents=True)
    build_workspace(ws)

    old_env = dict(os.environ)
    os.environ["OURO_HOME"] = str(hermes_home)
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.ouro/.env; set +a  — then relaunch."
        )
