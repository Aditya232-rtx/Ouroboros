"""Windows UTF-8 bootstrap for Ouro entry points.
...
should be imported at the very top of every Ouro entry point
(``hermes``, ``ouro-agent``, ``ouro-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Ouro modules.

    Ouro ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Ouro source root
    ...
    """
    root = src_root or os.environ.get("OURO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``OURO_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("OURO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
