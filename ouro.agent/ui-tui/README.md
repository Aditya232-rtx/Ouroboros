...
React + Ink terminal UI for Ouro. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `OURO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `ouro setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.ouro/.ouro_history` or under `OURO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `ouro setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external ouro setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.ouro/tui-parent.log
