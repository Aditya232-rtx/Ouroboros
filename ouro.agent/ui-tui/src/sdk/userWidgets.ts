...
/**
 * User widget apps — Ouro authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$OURO_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.ouro/plugins/`: files under OURO_HOME execute
...
const widgetsDir = () => join(process.env.OURO_HOME?.trim() || join(homedir(), '.ouro'), 'tui-widgets')
 ...
 *  every change, so a widget Ouro writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Ouro) ever writes must hot-load too — a 10s poll here read
