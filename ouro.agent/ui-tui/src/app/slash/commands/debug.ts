...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $OURO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see OURO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['OURO_TUI_BACKGROUND', process.env.OURO_TUI_BACKGROUND ?? '(unset)'],
            ['OURO_TUI_THEME', process.env.OURO_TUI_THEME ?? '(unset)'],
