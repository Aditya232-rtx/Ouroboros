...
// Profile-aware: the Python launcher exports OURO_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.ouro matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.OURO_HOME ?? join(homedir(), '.ouro'), 'tui-theme-boot.json')

// Never touch the user's real ~/.ouro from test runs (the TS suite has no
// OURO_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into OURO_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.OURO_TUI_THEME || env.OURO_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.OURO_TUI_BACKGROUND
  ) {
    env.OURO_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.OURO_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.OURO_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into OURO_TUI_THEME.
