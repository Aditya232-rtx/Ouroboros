...
//
// Both gate on OURO_DEV_PERF=1 and dump JSON-lines (default ~/.ouro/perf.log,
// override OURO_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// OURO_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.OURO_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.OURO_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.OURO_DEV_PERF_LOG?.trim() || join(homedir(), '.ouro', 'perf.log')
