...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.OURO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Ouro on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via OURO_TUI_TRUECOLOR=1.
