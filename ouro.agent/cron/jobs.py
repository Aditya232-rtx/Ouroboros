...
Jobs are stored in ~/.ouro/cron/jobs.json
Output is saved to ~/.ouro/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own OURO_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same OURO_HOME — so a job authored in profile
# `coder` lives in `~/.ouro/profiles/coder/cron/jobs.json` and executes with
...
# into one shared `jobs.json` and run them under whatever OURO_HOME the
...
OURO_DIR = get_hermes_home().resolve()
...
CRON_DIR = OURO_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``ouro cron status`` process share it
...
# threshold in `ouro cron status` (hermes_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_hermes_home()
       (context-local override, then the OURO_HOME env var) — so a test
       or embedder that re-points OURO_HOME after this module was
...
# the cron inactivity timeout is disabled (OURO_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. OURO_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``OURO_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec ouro ouro cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Ouro timezone, not the server's local
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Ouro configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Ouro timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Ouro/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall ouro-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `ouro cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``ouro cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `ouro cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``ouro cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``ouro cron
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.ouro/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.ouro/scripts/, .sh/.bash via bash, else Python). Each
    ...
    # covered, not just `ouro cron create`.
...
def _machine_id() -> str:
    ...
    Uses ``OURO_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("OURO_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from OURO_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one OURO_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from OURO_CRON_TIMEOUT),
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``OURO_HOME/skills`` or an
