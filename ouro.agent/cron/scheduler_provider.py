...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global OURO_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `ouro cron
        ...
        # Heartbeat once before the first sleep so `ouro cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `ouro cron status`/`list` (separate processes) can show
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `ouro cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
