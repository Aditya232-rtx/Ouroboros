...
``ouro gateway restart`` (or ``launchctl kickstart ai.hermes.gateway``
or ``systemctl restart ouro-gateway``).  When the cron fires, the
...
``ouro cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``ouro gateway``, ``launchctl ... ouro-gateway``,
``systemctl ... ouro-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_OURO_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``ouro gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `ouro gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:hermes\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a ouro-gateway label. macOS launchd
    # labels look like `ai.hermes.gateway` / `ouro-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated ouro services (e.g.
    ...
    # Branch C: systemctl ops on a ouro-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bhermes[.\-]?gateway)"
    # Branch D: pkill / kill targeting the ouro gateway process. Both
...
# for `systemctl restart ouro-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart ouro-gateway' f | sh`); never mask
# such a line.
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart ouro-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart ouro-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".ouro"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<OURO_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.ouro/scripts/restart.sh``) but is passed as the bare name
    ...
    try:
        ...
        # neither OURO_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.ouro/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `ouro gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `ouro gateway restart` from a shell outside "
