...
Uses a file-based lock (~/.ouro/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `ouro update` reloads
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `ouro fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `ouro cron pause {job_ref}` "
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Ouro, pin it explicitly: "
                f"`ouro cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``ouro tools`` config for the ``cron`` platform.
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``OURO_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("OURO_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid OURO_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
# (OURO_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse OURO_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("OURO_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid OURO_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.ouro/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``ouro update`` / ``ouro gateway stop`` / systemd restart, or an
...
def _get_hermes_home() -> Path:
    """Resolve Ouro home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active OURO_HOME at call time
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("OURO_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid OURO_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within OURO_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Ouro-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against OURO_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within OURO_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("OURO_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `ouro setup`), or pin a "
            "working provider via `ouro cron edit "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`ouro setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    #
    # Bounded with its own timeout (separate from OURO_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("OURO_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid OURO_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    # gateway message. Do not seed OURO_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads OURO_SESSION_PLATFORM
    #     and OURO_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via OURO_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     OURO_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read OURO_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the OURO_CRON_AUTO_DELIVER_* vars set
    # below, so clearing OURO_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # OURO_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ OURO_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "OURO_CRON_AUTO_DELIVER_PLATFORM",
        "OURO_CRON_AUTO_DELIVER_CHAT_ID",
        "OURO_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["OURO_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `ouro chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with OURO_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $OURO_KANBAN_TASK -- letting
        ...
        # recorded this OURO_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        if delivery_target:
            _VAR_MAP["OURO_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["OURO_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["OURO_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > OURO_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``ouro cron edit --model``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"OURO_MODEL={os.getenv('OURO_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`ouro cron edit {job_id} --model <name>` or set a "
                "default with `ouro model <name>`."
            )
        ...
        prefill_file = (
            os.getenv("OURO_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject OURO_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Ouro "
                        "pin it explicitly: "
                        f"`ouro cron edit {job_id} --provider <provider> "
        ...
        agent = AIAgent(
            ...
            # OURO_HOME. When a workdir is configured, also inject project
        ...
        # override via OURO_CRON_TIMEOUT env var.  0 = unlimited.
        #
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`ouro pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `ouro resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `ouro cron run` that claimed a job
        ...
        # Set OURO_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("OURO_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid OURO_CRON_MAX_PARALLEL value; defaulting to unbounded")
