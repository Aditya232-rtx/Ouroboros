...
has to be thrown away or summarized. Ouro has always done this in one batch:
...
Ouro folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Ouro's normal session flush is
...
With `micro_compact` unset or `false` Ouro behaves exactly as it always has:
...
Defaults to `$OURO_HOME/logs/agent.log`. It reports passes, outcome mix, net
