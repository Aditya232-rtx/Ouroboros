...
`ouro config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.ouro/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `OURO_HOME`, secret scope, and
