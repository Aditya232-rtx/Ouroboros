...
Supersedes: PR #31781 (prompt_toolkit `ouro profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `ouro profile wizard` in the terminal.
...
needs, and a profile is just a OURO_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.ouro/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — OURO_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with OURO_HOME override |
...
### Seam #1 — hub-skill install cannot use the OURO_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = OURO_HOME / "skills"` at **module
...
A fresh subprocess re-imports `skills_hub` with the profile's OURO_HOME bound
...
2. write model (synchronous, OURO_HOME override)
3. write selected MCP servers (synchronous, OURO_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `ouro -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, OURO_HOME override
...
page afterward. Mirror `_write_profile_model`'s OURO_HOME-override helper for
...
- Backend E2E with isolated OURO_HOME: POST a full create body
