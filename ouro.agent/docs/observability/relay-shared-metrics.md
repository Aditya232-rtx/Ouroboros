...
Ouro includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Ouro and does not require `ouro plugins enable
observability/nemo_relay`. Ouro remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Ouro execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `ouro-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Ouro requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Ouro installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Ouro process so configured interceptors can
...
Collection remains off unless Ouro policy enables it:
...
Ouro core owns one Relay host and one isolated Relay session scope per Ouro
...
marks do not require Ouro plugin registration. Shared-metrics marks must
...
```text
Ouro turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Ouro shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Ouro sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Ouro used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; Ouro
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Ouro processes share the SQLite latch, so simultaneous
...
the owning Ouro session. The start counter contains only bounded execution
...
provider attempts for the same Ouro API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Ouro tool-call
...
cancellations. Active task ownership follows the task ID if Ouro rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. Ouro
...
tool or plugin names. Ouro does not infer retries from repeated tool names or
...
state, and a use-count bucket. Ouro derives reuse and patch-generation
...
```text
$OURO_HOME/telemetry/shared_metrics/metrics.sqlite3
$OURO_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Ouro version,
...
schema field name, its current scope is one `OURO_HOME`, so it is more
...
Deleting `$OURO_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `OURO_HOME`. To reset it, stop Ouro
processes and remove `$OURO_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Ouro CLI turn against the deterministic local model server:
