...
Ouro observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Ouro' planner, model providers, memory, tool registry,
...
Ouro also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Ouro catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Ouro only builds
