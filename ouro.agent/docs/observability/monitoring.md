...
Ouro gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Ouro-owned subscribers.
...
```bash
ouro monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'ouro-agent[otlp]'`),
...
a plain `ouro gateway run`: the exporter lives in the gateway process, so
...
Ouro-owned Relay transport health remains in scope. A separate gateway
...
```bash
ouro monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `ouro monitoring` CLI intentionally exposes `status` only. This first
release covers only Ouro-owned service-health and operational-diagnostic
signals, including Ouro-owned Relay transport health. Team Gateway's
