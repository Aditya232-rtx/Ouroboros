...
## 2026-07-13: Scope plugin manager state by Ouro home/profile (keyed cache)

Status: Accepted

Context:
Ouro supports multiple profiles via different Ouro home directories.
Homes are switched two ways in a running process: the `OURO_HOME`
...
1. Switching homes via `set_hermes_home_override()` was invisible to a
   naive "did `OURO_HOME` change" check, so the singleton silently kept
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Ouro home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `OURO_HOME` or `set_hermes_home_override()`.
