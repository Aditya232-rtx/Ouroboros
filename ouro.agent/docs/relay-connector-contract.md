...
This document is the formal interface between the **Ouro gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `ouro gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`ouro gateway enroll` → connector `/relay/enroll`, or
