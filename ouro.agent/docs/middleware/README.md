...
Ouro middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Ouro'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Ouro calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Ouro retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Ouro guardrails,
...
Ouro stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Ouro runs
them as a nested chain in registration order. Middleware failures are fail-open:
Ouro logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Ouro applies middleware in this order:
...
For each tool call, Ouro applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Ouro pre-execution path against the effective arguments:
...
For isolated local testing, use one `OURO_HOME` for plugin enablement and the
agent run:

```bash
export OURO_HOME=/tmp/hermes-middleware-test
mkdir -p "$OURO_HOME"
ouro plugins enable <plugin-name>
ouro chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run ouro plugins enable <plugin-name>
uv run ouro chat --query 'Reply exactly ok'
```
...
Return the same response shape Ouro expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Ouro treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Ouro preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Ouro does not convert downstream
