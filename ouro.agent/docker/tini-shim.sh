...
# Ouro WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: OURO_TINI_SHIM_TARGET overrides the `/init` path (and
# OURO_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${OURO_TINI_SHIM_TARGET:-/init}"
WRAPPER="${OURO_TINI_SHIM_WRAPPER:-/opt/hermes/docker/main-wrapper.sh}"
