...
OURO_HOME="${OURO_HOME:-/opt/data}"
INSTALL_DIR="/opt/hermes"

# Drop to ouro via s6-setuidgid, but skip it when already non-root.
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid ouro "$@"; }
...
# $OURO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass OURO_UID/OURO_GID — or the PUID/PGID aliases — which the
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e OURO_UID=\$(id -u) -e OURO_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the ouro user to that UID/GID at boot and chowns the data
...
# --- Bootstrap OURO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid ouro mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom OURO_HOME paths whose parents only
# root can create (e.g. `OURO_HOME=/home/hermes/.ouro` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for OURO_UID/OURO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  OURO_UID/
# OURO_GID still win when both are set.  See #15290, salvages #25872.
OURO_UID="${OURO_UID:-${PUID:-}}"
OURO_GID="${OURO_GID:-${PGID:-}}"

if [ -n "${OURO_UID:-}" ] && validate_uid_gid "$OURO_UID" && [ "$OURO_UID" != "$(id -u hermes)" ]; then
    echo "[stage2] Changing ouro UID to $OURO_UID"
    usermod -u "$OURO_UID" hermes
fi
if [ -n "${OURO_GID:-}" ] && validate_uid_gid "$OURO_GID" && [ "$OURO_GID" != "$(id -g hermes)" ]; then
    echo "[stage2] Changing ouro GID to $OURO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$OURO_GID" ouro 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised ouro user (UID 10000)
...
# ouro process with `Groups: 10000` (998 gone); after this hook adds
...
#   - ouro is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G ouro 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] ouro already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" ouro 2>/dev/null; then
        echo "[stage2] Added ouro to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group ouro failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When OURO_UID is remapped or the top-level $OURO_HOME isn't owned by
# the runtime ouro UID, restore ownership to ouro — but ONLY for the
# directories ouro actually writes to. The full $OURO_HOME may be a
...
chown_hermes_tree() {
    ...
    chown -R hermes:ouro "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_hermes_owner() {
    target="$1"
    find "$target" \( ! -user ouro -o ! -group ouro \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$OURO_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $OURO_HOME (targeted) to ouro ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $OURO_HOME: chown the directory itself (not its contents)
    # so ouro can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$OURO_HOME"; then
        :
    else
        chown hermes:ouro "$OURO_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $OURO_HOME failed (rootless container?) — continuing"
    fi
    # Ouro-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by ouro (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$OURO_HOME/$sub" ] && tree_has_non_hermes_owner "$OURO_HOME/$sub"; then
            chown_hermes_tree "$OURO_HOME/$sub"
        fi
    done
fi
...
# ouro user. Hosted/container instances keep mutable state under
# $OURO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# OURO_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hermes root-owned and
...
# $OURO_HOME/lazy-packages dir on the data volume (Dockerfile sets
# OURO_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# Always reset ownership of $OURO_HOME/profiles to ouro on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> ouro …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as ouro and walks
...
if [ -d "$OURO_HOME/profiles" ] && tree_has_non_hermes_owner "$OURO_HOME/profiles"; then
    chown_hermes_tree "$OURO_HOME/profiles"
fi

# Always reset ownership of $OURO_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged ouro runtime even
...
# gate as profiles/).
if [ -d "$OURO_HOME/cron" ] && tree_has_non_hermes_owner "$OURO_HOME/cron"; then
    chown_hermes_tree "$OURO_HOME/cron"
fi
...
# top-level $OURO_HOME is mis-owned, so a warm volume with hermes-owned
# OURO_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid ouro mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as hermes.
if [ -d "$OURO_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$OURO_HOME/logs/gateways"; then
        :
    else
        chown hermes:ouro "$OURO_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# ouro pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged ouro gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $OURO_HOME is
...
# and cron/.
if [ -d "$OURO_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$OURO_HOME/platforms/pairing"; then
    chown_hermes_tree "$OURO_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$OURO_HOME/pairing" ] && tree_has_non_hermes_owner "$OURO_HOME/pairing"; then
    chown_hermes_tree "$OURO_HOME/pairing"
fi
...
# *subdirectories*; loose state files living directly under $OURO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> ouro …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged ouro runtime then hits
...
# sweep so host-owned files in a bind-mounted $OURO_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$OURO_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$OURO_HOME/$f"; then
            :
        else
            chown hermes:ouro "$OURO_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the ouro runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$OURO_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$OURO_HOME/config.yaml"; then
        :
    else
        chown hermes:ouro "$OURO_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$OURO_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as ouro user ---
# Run as ouro via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $OURO_HOME values
...
as_hermes mkdir -p \
    ...
    "$OURO_HOME/home" \
    "$OURO_HOME/pairing" \
    "$OURO_HOME/platforms/pairing" \
    "$OURO_HOME/lazy-packages"
...
# $OURO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $OURO_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.ouro:/opt/data) and sometimes shared with a
...
# 'ouro update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $OURO_HOME if one is present (the host install's
...
if [ -f "$OURO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$OURO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$OURO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$OURO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$OURO_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$OURO_HOME/$dest"
        fi
    fi
}
...
if [ -f "$OURO_HOME/.env" ] && ! grep -q '^API_SERVER_KEY=..*' "$OURO_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$OURO_HOME/.env"; then
        :
    else
        ...
        if [ -n "$_gen_key" ]; then
            ...
            sed -i '/^API_SERVER_KEY=$/d' "$OURO_HOME/.env" 2>/dev/null || true
            printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$OURO_HOME/.env"
...
if [ -f "$OURO_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$OURO_HOME/.env"; then
        :
    else
        chown hermes:ouro "$OURO_HOME/.env" 2>/dev/null || true
        chmod 600 "$OURO_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $OURO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `ouro update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set OURO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$OURO_HOME/config.yaml" ]; then
    s6-setuidgid ouro "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$OURO_HOME/auth.json" ] && [ -n "${OURO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$OURO_HOME/auth.json"; then
        :
    else
        printf '%s' "$OURO_AUTH_JSON_BOOTSTRAP" > "$OURO_HOME/auth.json"
        chown hermes:ouro "$OURO_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$OURO_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via OURO_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$OURO_HOME/auth.json" ] && [ -n "${OURO_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$OURO_HOME/auth.json"; then
        :
    else
        s6-setuidgid ouro "$INSTALL_DIR/.venv/bin/python" \
...
# can set OURO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# OURO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$OURO_HOME/gateway_state.json" ] && \
        [ "${OURO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$OURO_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$OURO_HOME/gateway_state.json"
        chown hermes:ouro "$OURO_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$OURO_HOME/gateway_state.json"
    fi
fi
...
# directory. agent-browser (the runtime CLI Ouro spawns for the
