...
# with-contenv before touching OURO_HOME / PATH. On the non-PID-1
...
#   first arg is anything else    → exec `ouro <args>` (subcommand passthrough)
#
# Drop to ouro via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${OURO_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${OURO_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export OURO_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset OURO_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid ouro "$@"; exec "$@"; }
...
# baked image dirs (owned by the ouro build UID) are unwritable, so fail
...
# to the ouro user's home before dropping privileges so libraries that
...
_hermes_orig_cwd="${OURO_ORIG_CWD:-$PWD}"
...
# the user's command so `ouro chat` starts in the Docker -w
