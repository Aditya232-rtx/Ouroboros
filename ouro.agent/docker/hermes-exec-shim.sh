...
# `hermes` user (UID 10000). When an operator runs `docker exec <c> ouro ...`
# the default UID is root (0), and any file the command writes under
# $OURO_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> ouro login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Ouro is not logged into Nous
...
# When invoked as root, it drops to the ouro user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $OURO_HOME is uid-aligned with the supervised processes. When invoked
...
#
# Opt-out: set OURO_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the ouro CLI. Default is to drop.

set -e

if [ -x /opt/hermes/.venv/bin/ouro ]; then
    REAL=/opt/hermes/.venv/bin/ouro
...
if [ ! -x "$REAL" ]; then
    echo "ouro-shim: $REAL not found or not executable" >&2
    exit 127
fi
...
case "${OURO_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the ouro user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user ouro or set OURO_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the ouro user's home before dropping privileges. Without
