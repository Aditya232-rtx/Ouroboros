...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``ouro -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.ouro`` even when a test fixture later
    redirects ``OURO_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_hermes_home()`` fresh so a runtime
       ``OURO_HOME`` redirect takes effect regardless of import order.
    """
...
# ~/.ouro/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects OURO_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without OURO_HOME, a stale worktree
# without the re-pin, or a developer shell that exports OURO_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "OURO_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.ouro``).  The test conftest injects the pre-sandbox production
#: root here so custom-``OURO_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Ouro root for the guard.
    ...
    try:
        if sys.platform == "win32":
            ...
            root = Path(os.path.expanduser("~")) / ".ouro"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``OURO_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Ouro test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``OURO_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "OURO_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``OURO_HOME`` redirect: that child aims at the production DB *and*
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Ouro home *root*.
    ...
    under ``~/.ouro/hermes-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``OURO_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``OURO_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Ouro root {root}). "
                "Tests must run against a temporary OURO_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "OURO_HOME. If this test genuinely needs the live "
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``ouro sessions optimize`` (FTS merge + VACUUM)
...
def _wal_reset_repair_hint() -> str:
    ...
    ``ouro update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Ouro-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Ouro"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Ouro-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `ouro doctor`. This warning fires once per "
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``ouro doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Ouro host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``ouro serve`` backend, interactive CLI sessions,
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Ouro home tree (``get_hermes_home()``) — the safe repair scope:
      Ouro owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Ouro needs read-write access to open the database. "
...
# (``~/.ouro/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``ouro sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("OURO_FTS5_CJK_SO")
    if env:
...
def _cjk_fts_config_enabled() -> bool:
    ...
    return os.getenv("OURO_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `ouro snapshot list` / "
                "`ouro snapshot restore <id>`.",
                path,
            )
            return None
...
# ── Read-only health/stats probes (ouro doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple ouro processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Ouro
    ...
    # `ouro update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `ouro snapshot list` / "
                    f"`ouro snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`ouro db optimize`),
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `ouro update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `ouro sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Ouro process held the "
    ...
    #
    # `optimize_fts_storage()` (the `ouro sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`ouro sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``ouro sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``ouro --resume`` into a fresh
    ...
    def search_sessions(
        ...
        ``ouro -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``ouro sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``OURO_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Ouro, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Ouro session.

        A Ouro session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Ouro session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `ouro sessions vacuum` runs in a transient
