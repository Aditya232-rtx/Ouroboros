...
def _build_child_agent(
    ...
    # child Ouro/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.ouro/logs/subagent-<sid>-<ts>.log``
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("OURO_UI_SESSION_ID", "") or None
            except Exception:
...
def delegate_task(
    ...
    # OURO_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Ouro-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("OURO_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Ouro-Session-Id once the batch completes.
            ...
            # OURO_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `ouro -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("OURO_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("OURO_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and OURO_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'ouro auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Ouro config.

    Prefer the shared persistent loader because it follows the active
    OURO_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``OURO_IGNORE_USER_CONFIG=1`` (``ouro chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("OURO_IGNORE_USER_CONFIG") == "1"
