"""Progressive tool disclosure ("tool search") for Ouro.
...
``tool_call`` — and surfaced on demand. Core Ouro tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._OURO_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_OURO_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_OURO_CORE_TOOLS``. Core tools are never deferred
