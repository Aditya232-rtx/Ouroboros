...
`ouro skills publish` for sharing. No new source type, no new store, no new
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    ouro = meta.get("hermes") if isinstance(meta, dict) else None
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    to ``ouro skills publish`` to share. ``body`` is the plain-language
