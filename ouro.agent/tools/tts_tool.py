...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.ouro/config.yaml``. Ouro
...
Configuration is loaded from ~/.ouro/config.yaml under the 'tts:' key.
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``ouro auth add <provider_id>``).
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.ouro/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.ouro/config.yaml.
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``ouro tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Ouro without any Python code
...
#           output_format: wav
#
# Ouro writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__OURO_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Ouro secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Ouro secrets (salvage of #56332) while still
...
# file, so retired models disappear from ouro the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `ouro setup` to configure, "
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `ouro model` or set XAI_API_KEY.")
    ...
    # output_format when Ouro actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Ouro client context following Gemini's partner
        # integration guidance:
        # https://ai.google.dev/gemini-api/docs/partner-integration
        headers["X-Goog-Api-Client"] = f"ouro-agent/{_hermes_version}"
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Ouro caches Piper voice models.

    Resolves to ``~/.ouro/cache/piper-voices/`` under the active
    OURO_HOME so voice downloads follow profile boundaries.
    """
    from hermes_constants import get_hermes_dir
...
def _text_to_speech_single(
    ...
    *,
    ...
    platform = get_session_env("OURO_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `ouro setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run ouro setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'ouro setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'ouro tools' and select Piper under TTS, or install manually: "
