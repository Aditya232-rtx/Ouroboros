...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``ouro tools`` repaint, tool-registration scans,
    ...
    2. ``~/.ouro/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``ouro auth add xai-oauth``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.ouro/.env`` first, then ``os.environ``.
...
def hermes_xai_user_agent() -> str:
    """Return a stable Ouro-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Ouro-Agent/{__version__}"
...
def hermes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Ouro,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Ouro config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Ouro home, then mark it seen."""
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``OURO_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    ...
    """
    override = str(
        get_env_value("OURO_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Ouro-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.ouro/.env`` (the standard Ouro location) are honored —
    ...
    honors ``OURO_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
