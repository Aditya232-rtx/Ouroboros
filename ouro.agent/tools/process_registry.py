...
# ---------------------------------------------------------------------------
# When Ouro runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Ouro gateway runtime.

    Both supervisor markers and ``_OURO_GATEWAY`` are inherited by every
...
class ProcessRegistry:
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `ouro update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Ouro desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.ouro/processes.json with the raw command
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Ouro'}"
    elif _reason == "lost":
