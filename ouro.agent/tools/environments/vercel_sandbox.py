...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Ouro'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``OURO_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Ouro
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.ouro"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.ouro"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.ouro"
        )
        archive_member = remote_hermes.lstrip("/")
        remote_tar = f"/tmp/.ouro_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
