"""Base class for all Ouro execution environment backends.
...
# OURO_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("OURO_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {OURO_HOME}/sandboxes/.
    """
...
def _cwd_marker(session_id: str) -> str:
    return f"__OURO_CWD_{session_id}__"
...
# the FIRST session's OURO_SESSION_ID into the snapshot makes every LATER
...
# OURO_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Ouro' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is OURO_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (OURO_SESSION_|OURO_UI_SESSION_ID|OURO_CRON_AUTO_DELIVER_|OURO_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``OURO_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!OURO_SESSION_*} ${!OURO_CRON_AUTO_DELIVER_*} "
        # AI_AGENT / OURO_AGENT are per-command attribution markers
        ...
        "AI_AGENT OURO_AGENT "
        f"OURO_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Ouro backends.
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_OURO_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under Ouro via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Ouro-specific
        # ``OURO_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``ouro-agent`` — see huggingface.js
        ...
        # inherited from the Ouro process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Ouro running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-ouro-agent}" '
            'OURO_AGENT="${OURO_AGENT:-true}"'
        )
        ...
        # Restrict Ouro metadata files without changing the user's command
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"OURO_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via OURO_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __OURO_CWD_{session}__ marker from stdout output.
