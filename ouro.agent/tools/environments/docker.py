...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.ouro/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Ouro profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``label=ouro-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Ouro process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=hermes-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a ouro process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Ouro
    process leaves its container behind permanently because there's no
    subsequent Ouro process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=ouro-agent=1", "--filter", "status=exited"]
    if profile_filter:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``OURO_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("OURO_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using OURO_DOCKER_BINARY override: %s", override)
        return override
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``OURO_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `ouro egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `ouro egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `ouro egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `ouro egress setup` to mint provider "
    ...
    env_overrides: dict[str, str] = {
        ...
        "_OURO_EGRESS_NODE_OPTIONS_APPEND": "--use-openssl-ca",
    }
    ...
    # through the proxy.  Keep the OURO_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"OURO_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``ouro-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.ouro/sandboxes/). Non-persistent
        ...
        # ``_OURO_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_OURO_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. ouro-agent:latest) already use /init as PID 1
        ...
        #   * the orphan reaper (`ouro-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`hermes-task-id`, `hermes-profile`)
        #   * operators running `docker ps --filter label=ouro-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Ouro profile is captured at
        ...
        # container shared across sessions").  If a prior Ouro process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic Ouro secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Ouro-internal dynamic secrets
        ...
        forward_keys = explicit_forward_keys | (_implicit_forward - _OURO_PROVIDER_ENV_BLOCKLIST)
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # reused after the operator runs "ouro egress disable",
        ...
        # container) but can happen if a previous Ouro process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Ouro exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Ouro process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Ouro startup. That covers the
        ...
        # outlives Ouro processes, processes inside it stay alive, and
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``ouro /quit``
