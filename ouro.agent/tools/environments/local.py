...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Ouro was already launched from (for example ``ouro-agent``
    while the process cwd is ``~/.ouro/hermes-agent``), passing it through
    unchanged makes the wrapper run ``cd ouro-agent`` *inside* the project
    ...
    # Common recovery for config values like ``ouro-agent`` when Ouro was
...
# Ouro-internal env vars that should NOT leak into terminal subprocesses.
_OURO_PROVIDER_ENV_FORCE_PREFIX = "_OURO_FORCE_"

# Ouro-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Ouro inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Ouro-managed inference credential — Claude subscription auth is not a
    # working Ouro provider path.  Stripping it broke agent-spawned
...
_OURO_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Ouro venv path instead of the project's own
# ``.venv`` — silently clobbering the Ouro environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Ouro venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Ouro venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Ouro itself treats PYTHONHOME as contamination in its own
...
# _strip_hermes_owned_pythonpath() which removes only Ouro-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for Ouro-internal secrets injected under *dynamic* names.

    ``_OURO_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Ouro-internal dynamic secret"
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Ouro home override into subprocess env."""
    try:
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (OURO_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Ouro-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_OURO_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_hermes_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _OURO_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_OURO_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_OURO_PROVIDER_ENV_FORCE_PREFIX):]
    ...
    # the separate Ouro runtime venv.  The filter validates that relationship
...
# legitimate child Ouro spawns needs them, and they are the highest-value
...
# narrow subset of _OURO_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_OURO_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Ouro spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_OURO_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Ouro-internal dynamic secrets
    ...
    if not inherit_credentials:
        ...
        for key in _OURO_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)
    ...
    # copies os.environ, whose OURO_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's OURO_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    propagation (``OURO_HOME`` / subprocess ``HOME`` contract) and the
    Ouro secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      ``OURO_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Ouro home override into ``OURO_HOME`` and apply the
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``OURO_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs OURO_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("OURO_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale OURO_GIT_BASH_PATH pointing at one) must not
    ...
    # OURO_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "OURO_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Ouro requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set OURO_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Ouro. If the override is blocked or later re-applied, "
...
_SENTINEL = object()
_OURO_BIN_DIR: "str | None | object" = _SENTINEL
...
def _resolve_hermes_bin_dir() -> str | None:
    ...
    ouro install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    ...
    """
    global _OURO_BIN_DIR
    if _OURO_BIN_DIR is not _SENTINEL:
        return _OURO_BIN_DIR  # type: ignore[return-value]
    ...
    _OURO_BIN_DIR = candidate
    return candidate


def _prepend_hermes_bin_dir(existing_path: str) -> str:
    """Prepend the ouro install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Ouro-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Ouro installs for
    itself, so on a machine where Ouro provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$OURO_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$OURO_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Ouro-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Ouro
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_OURO_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_OURO_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_hermes_internal_secret(real_key):
                continue
            run_env[real_key] = v
            ...
            if k in _OURO_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the ouro install dir is reachable so plugins can shell out
...
def _build_hermes_repo_root_aliases(
    ...
    """Return exact repo-root spellings emitted by Ouro launchers.

    ``gateway_windows._preserve_hermes_home_path`` maps a physical path under
    the resolved OURO_HOME back onto the configured OURO_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of OURO_HOME as Ouro-owned.
    ...
    # while still never matching arbitrary descendants of OURO_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\hermes\ouro-agent
    # -> C:\...\ouro-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Ouro-owned.  Fail-closed: a
...
# --- Ouro venv / repo-root detection (module-level, computed once) ---

#: The Ouro repository root - three levels up from this file
...
#: etc.  Subprocesses that are NOT the Ouro backend don't need it and it
#: can shadow local packages.
_hermes_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Ouro launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Ouro-owned paths under
#: the configured OURO_HOME spelling (which may be a junction to another
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Ouro repo>/venv`` layout and a real venv marker; require both before
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Ouro runtime.
...
def _strip_hermes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Ouro-owned PYTHONPATH entries, then the runtime marker vars.
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    """Remove Ouro-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Ouro repo root and the Ouro venv's
    ...
    user entries, so only entries proven Ouro-owned are removed:
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Ouro-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Ouro venv site-packages ---
        ...
        # resolved and unresolved (OURO_HOME/junction) spellings count as
        # Ouro-owned.
        if not should_strip:
            should_strip = any(
    ...
    if stripped:
        logger.debug(
            "Stripped Ouro-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Ouro trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``OURO_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under OURO_HOME.  Using
