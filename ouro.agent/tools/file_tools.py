...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where ouro previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Ouro.
...
def _get_hermes_config_resolved() -> str | None:
    """Return the resolved absolute path of the Ouro config file (cached)."""
    ...
    try:
        ...
        try:
            _hermes_config_resolved = str(Path(_expand_tilde("~/.ouro/config.yaml")).resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Ouro config file directly.
    ...
    if hermes_config and (resolved == hermes_config or normalized == hermes_config):
        return (
            f"Refusing to write to Ouro config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.ouro/config.yaml directly or use 'ouro config' instead."
        )
    return None
...
# .cursorrules (or a project-local .ouro config tree) outlives the current
...
def _get_real_hermes_home() -> str | None:
    """Return the realpath of the authoritative Ouro home (cached)."""
    ...
    try:
        ...
        try:
            _real_hermes_home_cached = os.path.realpath(_expand_tilde("~/.ouro"))
        except Exception:
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.ouro home is governed by its own guards
    ...
    # ``.ouro`` component rule below, which would otherwise match the
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .ouro config dirs (e.g. <repo>/.ouro/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.ouro`` — matching
        # any ancestor named .ouro would gate every write inside a
        # checkout that happens to live under ~/.ouro (e.g. the
        # ouro-agent repo itself at ~/.ouro/hermes-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".ouro":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Ouro mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Ouro
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Ouro state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.ouro/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Ouro process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.ouro/…`` path.

    Returns ``None`` when the write is in-scope or outside Ouro scope.
    ...
    # in a session that cd'd into ``~/.ouro/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Ouro internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under OURO_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == OURO_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Ouro-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Ouro-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Ouro profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Ouro profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
