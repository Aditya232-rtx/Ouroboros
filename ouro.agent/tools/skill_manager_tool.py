...
~/.ouro/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `ouro config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.ouro/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch OURO_HOME and
    ...
    live profile-scoped OURO_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`ouro curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`ouro curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `ouro curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `ouro curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.ouro/skills/) first, then any
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `ouro sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `ouro sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `ouro sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `ouro sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Ouro profiles.
    ...
    # Default profile (~/.ouro/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.ouro/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`ouro -p "
            ...
            base += (
                ...
                f"Switch profiles (`ouro -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.ouro/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `ouro curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `ouro curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `ouro curator unpin <name>`. "
