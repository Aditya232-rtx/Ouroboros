...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Ouro exit
...
# display_hermes_home imported lazily at call site (stale-module safety during ouro update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of ouro directories
        total_bytes = 0
        import glob
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("OURO_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("OURO_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("OURO_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (OURO_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (OURO_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Ouro password prompt or the sudo -S password-pipe path.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``ouro-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Ouro processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Ouro processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Ouro processes a generous grace
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.ouro/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    be stale from ``ouro setup``). Environment values for omitted terminal
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Ouro processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Ouro tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # restart|stop targeting ouro-gateway) must never run inside the
        ...
        # never restart. This mirrors the `ouro gateway restart` guard in
        # hermes_cli/gateway.py and the cron-path guard in hermes_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_OURO_GATEWAY") == "1":
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Ouro cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `ouro gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # ouro-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in ouro-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `ouro -z`, a "
                        ...
                        _gw_platform = _gse("OURO_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("OURO_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("OURO_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("OURO_SESSION_USER_ID", "")
                            _gw_user_name = _gse("OURO_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("OURO_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("OURO_INTERACTIVE"):
                    output += (
