...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Ouro's live source checkout "
        ...
        "checkout, stop Ouro, run the command externally, then restart "
        "Ouro."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(hermes_home).expanduser() if hermes_home else Path.home() / ".ouro"
