...
transport, discovers their tools, and registers them into the ouro-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.ouro/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.ouro/logs/mcp-stderr.log), tagged with the
...
def sdk_httpx():
    ...
    side by side with Ouro' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Ouro' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Ouro explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Ouro and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "OURO_HOME", os.path.join(os.path.expanduser("~"), ".ouro")
                )
            )
            candidates = [
                ...
                # image (which the Ouro Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Ouro MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Ouro gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Ouro document cache; returns a marker with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Ouro profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Ouro' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (OURO_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit OURO_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Ouro' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `ouro setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Ouro process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Ouro' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Ouro is fully
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``ouro mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Ouro' pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `ouro mcp login {server_name}` (or delete the tokens "
        f"file under ~/.ouro/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Ouro processes (e.g.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local OURO_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local OURO_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Ouro config file.
    ...
    ``os.environ`` (which includes ``~/.ouro/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Ouro' MEDIA: tag convention and out to messaging
            ...
            # Ouro' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Ouro's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Ouro uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Ouro registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``ouro tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Ouro registry.
