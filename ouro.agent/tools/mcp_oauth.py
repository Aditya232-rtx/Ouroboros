    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Ouro"
"""
...
def _get_token_dir(hermes_home: str | Path | None = None) -> Path:
    ...
    Uses OURO_HOME so each profile gets its own OAuth tokens.
    Layout: ``OURO_HOME/mcp-tokens/``
    """
...
def _cached_redirect_port(storage: "HermesTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Ouro restarts and chooses a
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``ouro mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `ouro mcp login <server>` interactively to (re)authorize, "
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        OURO_HOME/mcp-tokens/<server_name>.json         -- tokens
        OURO_HOME/mcp-tokens/<server_name>.client.json   -- client info
        OURO_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Ouro records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Ouro.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://ouro.dev/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `ouro mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Ouro")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Ouro is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: ouro mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Ouro auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Ouro is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`ouro mcp login {server_name}` interactively first to complete "
