...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Ouro' — mcp 2.0 builds on httpx2,
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``ouro mcp reauth``.
...
_OURO_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `ouro mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``ouro mcp remove <name>`` and (indirectly) by
        ``ouro mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
