...
def _hermes_root(home: Path) -> Path:
    """Root ~/.ouro for both the default profile and named profiles."""
...
def _build_section(home: Path) -> str:
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Ouro profile is an agent teammate with "
        ...
        "```\n"
        f'ouro -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from 🤖 {handle} (@{handle}): your message"\n'
        "```\n"
        ...
        "which agent it came from. Run `ouro profile list` for the LIVE teammate "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient OURO_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("OURO_HOME") or os.path.expanduser("~/.ouro"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("OURO_HOME") or os.path.expanduser("~/.ouro")))
    surface: dict = {}
    try:
