...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Ouro-managed provider credential (API key,
    token, or similar) per ``_OURO_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Ouro API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Ouro credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Ouro-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_hermes_internal_secret(name):
        return True
    return name in _OURO_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Ouro-managed provider credentials (from
    ``_OURO_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Ouro-managed
    ...
    Non-Ouro third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Ouro provider "
                "credential %r (blocked by _OURO_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Ouro-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Ouro "
                        "provider credential %r from config.yaml (blocked "
                        "by _OURO_PROVIDER_ENV_BLOCKLIST). Operator "
