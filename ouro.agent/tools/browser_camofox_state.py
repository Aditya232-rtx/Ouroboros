"""Ouro-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Ouro
...
def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Ouro-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Ouro profile = same userId).
