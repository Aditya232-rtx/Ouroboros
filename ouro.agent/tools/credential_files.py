...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.ouro",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``OURO_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside OURO_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because OURO_HOME is exactly where
    ...
    # Reject absolute paths — they bypass the OURO_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to OURO_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape OURO_HOME.
    ...
    # inside OURO_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.ouro/{rel}"
                        result.append({
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.ouro``,
    SSH ``<remote_home>/.ouro``, etc.) and whether translation is wanted.
