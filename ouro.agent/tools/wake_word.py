"""Wake-word ("Hey Ouro") detection — hands-free session trigger.
...
wake word, Ouro opens a fresh session and captures voice via the existing
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the Ouro backend "
...
# OURO_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `ouro tools` "
