#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Ouro Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.ouro/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "Aditya232-rtx/ouro"
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``ouro update``. Rather than telling them to update first, resolve
...
# ---------------------------------------------------------------------------
# Ouro centralized index source
# ---------------------------------------------------------------------------

OURO_INDEX_URL = "https://ouro.dev/docs/api/skills-index.json"
OURO_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for OURO_INDEX_TTL seconds to avoid repeated
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Ouro index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Ouro index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Ouro index fetch failed: %s", e)
            return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Ouro Skills Index.
