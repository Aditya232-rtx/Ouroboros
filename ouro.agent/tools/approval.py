...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("OURO_YOLO_MODE", ""))
...
# Ouro session id (observability identity, distinct from the gateway
...
# os.environ["OURO_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export OURO_INTERACTIVE.
...
def set_hermes_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["OURO_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``OURO_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``OURO_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hermes_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("OURO_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Ouro session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("OURO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("OURO_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("OURO_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("OURO_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("OURO_CRON_SESSION")


def _is_single_query_approval_context() -> bool:
    """True when the current approval decision is from a single-query (-q) session.

    ``ouro chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``OURO_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("OURO_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("OURO_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set OURO_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind OURO_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds OURO_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("OURO_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``OURO_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $OURO_HOME, or by the resolved absolute
# active profile home path such as /home/hermes/.ouro/config.yaml. The
# resolved-absolute form is folded into the ~/.ouro/ patterns at detection
...
# go stale when OURO_HOME is set after this module is imported, e.g. under the
...
# ~/.ouro/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _OURO_ENV_PATH; matches the OURO_HOME override form as
# well as ~/.ouro/.
_OURO_CONFIG_PATH = (
    r'(?:~\/\.ouro/|'
    r'(?:\$home|\$\{home\})/\.ouro/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Ouro: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Ouro
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Ouro secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `hermes` and `gateway` (e.g. `ouro -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart ouro gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "ouro update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `ouro gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `ouro ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart ouro-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart ouro-gateway')"),
    ...
    # the `ouro gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.hermes.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(hermes|ai\.ouro)\b', "stop/restart ouro launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Ouro file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.ouro/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Ouro-managed security file (~/.ouro/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_OURO_CONFIG_PATH}|{_OURO_ENV_PATH})', "in-place edit of Ouro config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_OURO_CONFIG_PATH}|{_OURO_ENV_PATH})', "in-place edit of Ouro config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_OURO_CONFIG_PATH}|{_OURO_ENV_PATH})', "in-place edit of Ouro config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.ouro/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / OURO_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Ouro home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hermes), so folding the user home
    # first would eat the prefix the Ouro-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / OURO_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.ouro``); the matched
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Ouro home prefix to ``~/.ouro/``.

    Resolves the active ``OURO_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.ouro/`` so the static ``_OURO_CONFIG_PATH`` / ``_OURO_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/hermes/.ouro/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$OURO_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.ouro")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``ouro send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Ouro ad-hoc temp script."""
    try:
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Ouro YOLO.
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["OURO_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "OURO_SPINNER_PAUSE" in os.environ:
            del os.environ["OURO_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Ouro approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``OURO_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            OURO_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export OURO_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set OURO_INTERACTIVE or "
                "OURO_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set OURO_INTERACTIVE or "
            "OURO_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("OURO_EXEC_ASK"):
        ...
        # can see (OURO_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``OURO_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("OURO_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # OURO_EXEC_ASK routes through the gateway decision loop (no human
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (OURO_EXEC_ASK) still takes this path even when INTERACTIVE
