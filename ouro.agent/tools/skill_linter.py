...
Design contract (matches the Ouro "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    if author and author.strip().lower() in ("hermes", "agent", "ouro agent") and (
        author != "Ouro"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Ouro' (proper caps) "
