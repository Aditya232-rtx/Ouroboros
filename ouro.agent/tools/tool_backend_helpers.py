...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `ouro model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``ouro auth add
    ...
    2. The environment / ``~/.ouro/.env``. Under a multiplexed gateway turn
    ...
    3. The credential pool / auth store for ``provider_id`` (``ouro auth
    ...
    try:
        ...
        # `ouro auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``ouro auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.ouro/.env`` (via
