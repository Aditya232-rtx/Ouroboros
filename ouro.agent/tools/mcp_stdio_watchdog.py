...
<url>``) is spawned as a direct child of the Ouro process. Ouro's own
...
Ouro process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Ouro startup's opt-in
