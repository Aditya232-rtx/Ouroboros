...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Ouro convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("OURO_CHECKPOINT_TIMEOUT", 30)))
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Ouro wrote
...
def _git_env(
    ...
    The shared store is internal Ouro infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``ouro checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `ouro checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Ouro Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``OURO_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at OURO_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "OURO_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Ouro just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Ouro last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Ouro last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Ouro'
        last write or that Ouro never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Ouro has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Ouro never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Ouro wrote it: restoring it back is
                # safe — its last content was Ouro-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Ouro changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Ouro changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Ouro' last write — per the agent-write ledger —
        are left untouched, and only Ouro-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Ouro-created files absent from it (delete to restore state).
...
# ---------------------------------------------------------------------------
# Public helpers for `ouro checkpoints` CLI
# ---------------------------------------------------------------------------
