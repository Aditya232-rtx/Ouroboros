...
Lets the LLM write a Python script that calls Ouro tools via RPC,
...
# available on every platform Ouro itself runs on.
...
# match a safe prefix, the operational OURO_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Ouro
# code can lose the DB-layer Kanban mutation guard while still inheriting
# OURO_HOME.
#
# NB: the broad "OURO_" prefix was deliberately removed (#27303) — it leaked
# OURO_*-named config that lacks a secret substring (e.g. OURO_BASE_URL,
# OURO_KANBAN_DB, OURO_*_WEBHOOK).  The child only needs the few
# location/profile vars in _OURO_CHILD_ALLOWED below; OURO_RPC_SOCKET /
# OURO_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational OURO_* vars the child legitimately needs by exact name — these
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational OURO_* vars (_OURO_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret OURO_* vars dropped by the tightened allowlist (#27303). The
    # broad "OURO_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like OURO_KANBAN_DB / OURO_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _OURO_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("OURO_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped OURO_* var.
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted OURO_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Ouro-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Ouro-profile soft guard."""',
...
"""Auto-generated Ouro tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["OURO_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("OURO_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Ouro tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("OURO_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"OURO_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"OURO_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("OURO_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from OURO_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  OURO_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["OURO_RPC_SOCKET"] = rpc_endpoint
        child_env["OURO_RPC_TOKEN"] = rpc_token
        ...
        # OURO_TIMEZONE is an internal Ouro setting and must not leak
        # into child processes.
        _tz_name = os.getenv("OURO_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("OURO_TIMEZONE", None)
        ...
        # Ouro's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Ouro's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Ouro-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Ouro-
        ...
        if _uses_hermes_python_environment(_child_python):
            ...
            logger.info(
                "execute_code: child interpreter %s is outside the Ouro "
                "environment; ouro root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.ouro/.env')).
...
def _load_config() -> dict:
    ...
    ``ouro --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (ouro-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the Ouro environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the ouro root from every
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Ouro's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``ouro tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and ouro-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.ouro/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Ouro tools programmatically. "
