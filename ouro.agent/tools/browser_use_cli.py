...
_PRIVATE_BROWSER_SENTINEL = "_OURO_BU_PRIVATE_BROWSER"
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Ouro's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Ouro's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Run `ouro tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Ouro' own bin dir ($OURO_HOME/bin) — where install.sh puts uv/uvx
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Ouro' managed uv (bootstrapped on demand via
    ``hermes_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$OURO_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Ouro-managed copy, or resolution
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `ouro tools` → Browser Automation."
        )
