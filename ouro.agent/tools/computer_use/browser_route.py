...
* the driver session id is injected by the adapter, never accepted from the
  model;
* refs are usable only from the latest snapshot in this Ouro session;
...
ordinary Ouro tool approval never substitutes for protected consent.
"""
        ...
        if profile_mode == "existing_profile":
            ...
            if permission_mode != "bounded" and not grant_existing_profile:
                return _refusal(
                    ...
                    "true` in config.yaml. Ouro cannot grant this at "
