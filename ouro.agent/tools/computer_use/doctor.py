"""
`ouro computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Ouro side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Ouro provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Ouro-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    Honors `OURO_CUA_DRIVER_CMD` via the shared runtime resolver, so the
    ...
    if json_output:
        ...
        # attach Ouro identity under hermes_identity so existing parsers
