...
`health_report` (surfaced by `ouro computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Ouro runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Ouro-session cached backends. Each backend owns its own cua-driver
...
# See Aditya232-rtx/ouro#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Ouro's explicit approval bypass onto Cua's immutable mode.

    Ouro has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown OURO_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Ouro YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Ouro sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Ouro process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `ouro computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (Aditya232-rtx/ouro#67052).
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Ouro sessions.
...
# "foreground" (it doesn't know Ouro has a typed page route), so the model
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of Aditya232-rtx/ouro#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$OURO_HOME/cache/vision/``
...
def check_computer_use_requirements() -> bool:
    ...
    `ouro computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
