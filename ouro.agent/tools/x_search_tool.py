...
* ``XAI_API_KEY`` is set in ``~/.ouro/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``ouro auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `ouro auth add xai-oauth` "
