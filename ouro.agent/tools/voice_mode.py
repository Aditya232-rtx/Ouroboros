...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled Ouro venv at
    # ~/.ouro/profiles/<name>/hermes-agent/venv/), `pip install` on the
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.ouro/config.yaml during a
...
# playback-phase barge transcript is treated as a self-capture of Ouro'
...
def is_tts_echo(
    ...
    Ouro just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match Ouro' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Ouro TTS calls from colliding on
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("OURO_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    OURO_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
