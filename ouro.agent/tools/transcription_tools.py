...
LOCAL_STT_COMMAND_ENV = "OURO_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "OURO_LOCAL_STT_LANGUAGE"
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``OURO_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the Ouro process user cannot "
...
#
# The single-env-var ``OURO_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Ouro's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__OURO_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set OURO_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `ouro plugins list` to see "
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-local-stt-") as output_dir:
            ...
            # Scrub Ouro secrets from the child env (sibling path to #56332 /
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``OURO_LOCAL_STT_LANGUAGE`` (env).
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `ouro model` or set XAI_API_KEY",
        }
