#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Ouro desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Ouro window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Ouro desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "Ouro desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other Ouro "
        "windows are skipped: the nearest non-Ouro window is reported. "
