...
# means a compromised transitive dependency could read every Ouro secret
...
def _build_browser_env() -> dict:
    ...
    Strips Ouro-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``ouro doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.ouro/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Ouro profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.ouro/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Ouro silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Ouro-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Ouro CLI where sys.stdout and sys.stderr both report
...
# Hard ceiling for a daemon whose owning ouro process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed ouro processes — this way every clean ouro exit sweeps
    ...
    # Sweep orphans from other crashed ouro processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live ouro processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current ouro PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live ouro process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    ouro restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning ouro process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced ouro PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent ouro instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live ouro process and must not be touched — but "owner alive"
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Ouro-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Ouro-managed one with no recovery — every candidate
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Ouro-managed
    ...
    # node_modules binary, which disappears on the next ``ouro update`` and
    ...
    # Build an extended search PATH including Ouro-managed Node, macOS
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``ouro update`` (or ``ouro doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Ouro holds, while running registry-fetched
    npm code on every ``ouro update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `ouro update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this ouro PID as the session owner (cross-process safe
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Ouro
