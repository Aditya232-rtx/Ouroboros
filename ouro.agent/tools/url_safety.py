...
Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    ...
    connection. Ouro-owned direct httpx request paths should use
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for Ouro-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``OURO_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("OURO_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``OURO_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
