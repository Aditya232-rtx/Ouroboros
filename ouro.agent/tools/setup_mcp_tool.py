...
``ouro mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Ouro desktop app. Use the "
            "terminal instead: `ouro mcp install <name>` for catalog entries, "
            "`ouro mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Ouro desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `ouro mcp catalog` "
