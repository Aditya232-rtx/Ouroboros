...
//!   macOS:   ~/.ouro
//!   Linux:   ~/.ouro  (override via $OURO_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.ouro on macOS — there
...
/// Returns the canonical Ouro home directory, respecting $OURO_HOME if set.
pub fn hermes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("OURO_HOME") {
    ...
    {
        // %LOCALAPPDATA%\ouro — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("hermes");
        }
    }

    // macOS + Linux + fallback: ~/.ouro (matches Python get_hermes_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".ouro");
    }
...
/// OURO_HOME so it survives repo checkout deletion (unlike anything under
/// ouro-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\ouro\ouro-setup.exe`; on other
...
///
/// Lives directly under OURO_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves OURO_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    hermes_home().join(".ouro-update-in-progress")
}
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to OURO_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_OURO_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".ouro-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under OURO_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[ouro-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("OURO_BOOTSTRAP_LOG")
