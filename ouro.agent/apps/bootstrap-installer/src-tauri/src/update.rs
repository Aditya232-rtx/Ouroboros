...
//!
//! Driven when the installer is launched as `Ouro-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Ouro desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `ouro update`
//!      or repair bootstrap can race locked files),
//!   2. run `ouro update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in hermes_cli/main.py),
//!   3. run `ouro desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `ouro update` already handles macOS/Linux (git/pip).
...
/// `ouro update` exit code meaning "another ouro process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// hermes_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `ouro update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `ouro update` claims
...
/// overwriting — the pre-fix clobber is what let a dashboard `ouro update`
...
/// owner that is itself and aborts ("Another Ouro update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `ouro update`) still blocks.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let install_root = hermes_home.join("ouro-agent");
    ...
    // updater — most often a dashboard-spawned `ouro update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Ouro update is already running (PID {}, started {} ago). \
    ...
    let ouro = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the ouro CLI under {}. Is Ouro installed? \
    ...
    // async on Windows. If it still holds the venv shim, `ouro update`
    ...
    // ---- stage 2: ouro update -----------------------------------------
    // Pass --branch so `ouro update` targets the branch this installer was
    ...
    // without --branch, `ouro update` switches the checkout to `main` (a
    ...
    // --force skips `ouro update`'s Windows running-exe guard (which would
    ...
    // time `ouro update` runs there is no legitimate hermes.exe to protect,
    // and the guard would only produce a false "Ouro is still running" stop.
    //
    ...
    // below surfaces the correct "close all Ouro windows" message.
    ...
    // Retry-once for the update-boundary crash. `ouro update` lazily imports
    ...
    // second `ouro update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Ouro" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Ouro is still running. Close all Ouro windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "ouro update failed (exit {:?}). See {} for details.",
                other,
    ...
    // ---- stage 3: ouro desktop --build-only ----------------------------
    // `ouro update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `ouro update` already does above, and `ouro update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `ouro desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Ouro manually."),
            );
        }
    } else if let Err(err) =
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Ouro to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend hermes.exe (or the desktop Ouro.exe
            ...
            // force-kill every Ouro.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Ouro still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Ouro.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Ouro.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Spawn `ouro <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv ouro shim under an install root, regardless of existence.
...
/// Resolve the ouro CLI to drive. Prefer the venv shim in the install we
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `ouro update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `ouro update` child claims that SAME lock (hermes_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Ouro is still running")
    ...
    envs.push((
        "OURO_UPDATE_HANDOFF_PID".to_string(),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Ouro.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.ouro-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.ouro-update-old", target_app.display()));
...
mod tests {
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/hermes-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "OURO_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the ouro update child claims the same marker we hold; without our pid \
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".ouro-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `ouro update` and install-mode bootstrap mutate one tree
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Ouro.app"]),
            Some(PathBuf::from("/Applications/Ouro.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Ouro.app");
        let tmp = base.join("Ouro.app.hermes-update-new");
        let old = base.join("Ouro.app.hermes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Ouro.app");
        let tmp = base.join("Ouro.app.hermes-update-new"); // intentionally absent
        let old = base.join("Ouro.app.hermes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Ouro.app");
        let tmp = base.join("Ouro.app.hermes-update-new"); // absent
        let old = base.join("Ouro.app.hermes-update-old");
        write_marker(&target, "OLD");
