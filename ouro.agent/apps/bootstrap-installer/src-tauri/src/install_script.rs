...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $OURO_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Ouro-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Ouro-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("OURO_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "ouro-setup/0.0.1")
        .send()
        .await
