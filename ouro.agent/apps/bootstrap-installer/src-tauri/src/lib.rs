...
/// desktop app handing off via `Ouro-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under OURO_HOME/logs/ so install
    ...
    // Ouro is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Ouro installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Ouro is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Ouro" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Ouro" shortcuts that install.ps1 creates, and a
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hermes_home().join("ouro-agent");
                if bootstrap::hermes_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            ...
                            tracing::info!(
                                "ouro already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running Ouro Setup");
}
