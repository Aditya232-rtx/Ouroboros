...
//! OURO_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for OURO_HOME. Tests use this; production
...
/// Spawn the locally-built Ouro desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Ouro desktop at {}. The desktop build step \
             may have been skipped or failed. Run `ouro desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Ouro desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Ouro
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Ouro.exe"),
            ("win-arm64-unpacked", "Ouro.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Ouro.app/Contents/MacOS", "Ouro"),
            ("mac-arm64/Ouro.app/Contents/MacOS", "Ouro"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../Ouro.app/Contents/MacOS/Ouro -> .../Ouro.app
...
/// path so a bare re-open just opens Ouro instead of re-running setup.
pub(crate) fn hermes_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".ouro-bootstrap-complete").exists()
...
fn write_bootstrap_complete_marker(install_root: &Path, pin: &Pin) -> Result<serde_json::Value> {
    ...
    let tmp_path = install_root.join(".ouro-bootstrap-complete.tmp");
    {
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Ouro desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\ouro-agent. Mirrors hermes_constants.
    let hermes_home = args
        .ouro_home
        .clone()
        .unwrap_or_else(|| crate::paths::hermes_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&hermes_home).join("ouro-agent");
    ...
    // Copy ourselves to OURO_HOME/ouro-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into OURO_HOME (non-fatal)");
        emit_log(&format!(
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Ouro"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Ouro.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Ouro.exe");
    ...
    // what the updater ditto's over /Applications/Ouro.app). A regression in
    ...
    fn bootstrap_complete_marker_uses_desktop_compatible_schema() {
        ...
        let marker_path = root.join(".ouro-bootstrap-complete");
    ...
    fn bootstrap_complete_marker_is_published_atomically() {
        ...
        let marker_path = root.join(".ouro-bootstrap-complete");
        let tmp_path = root.join(".ouro-bootstrap-complete.tmp");
        assert!(
            marker_path.is_file(),
    ...
    fn hermes_is_installed_treats_marker_existence_as_sufficient() {
        ...
        make_release_tree(&root);
        std::fs::write(root.join(".ouro-bootstrap-complete"), b"").unwrap();
    ...
    fn marker_write_failure_leaves_no_final_marker() {
        ...
        assert!(
            !not_a_dir.join(".ouro-bootstrap-complete").exists(),
            "failed write must not leave a final marker that enables the fast path"
        );
        assert!(
            !not_a_dir.join(".ouro-bootstrap-complete.tmp").exists(),
