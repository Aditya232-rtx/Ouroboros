 ...
 * product name (`Ouro`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Ouro'
 *
 * This is a hard failure with no obvious cause for the user — `ouro desktop`
 ...
 * (the mac-unpacked Ouro.app bundle) and Windows (win-unpacked), so we
 ...
 * Ouro.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `ouro desktop --build-only` (hermes_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Ouro.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Ouro'}.exe`
