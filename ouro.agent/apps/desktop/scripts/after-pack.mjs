 ...
 *
 * Stamps the Ouro icon + identity onto the packed Windows Ouro.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `ouro desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Ouro')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Ouro'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Ouro.exe keeps the stock Electron icon`)
  }
}
