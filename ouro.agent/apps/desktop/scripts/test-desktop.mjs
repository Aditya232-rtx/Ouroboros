...
// no longer bundles the Ouro Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Ouro.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Ouro'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Ouro.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Ouro'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default OURO_HOME for non-sandboxed runs -- matches main.ts's
// resolveHermesHome(). On Windows it's %LOCALAPPDATA%\hermes; elsewhere
// it's ~/.ouro. The fresh-install sandbox launchFresh() sets its own
// OURO_HOME and never touches this.
const DEFAULT_OURO_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hermes')
  }
  return path.join(os.homedir(), '.ouro')
})()
const VENV_ROOT = path.join(DEFAULT_OURO_HOME, 'ouro-agent', 'venv')
...
function ensurePackagedApp() {
  if (process.env.OURO_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Ouro-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Ouro-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Ouro-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Ouro-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.OURO_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.OURO_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  env.OURO_DESKTOP_CWD = cwd
  env.OURO_DESKTOP_IGNORE_EXISTING = '1'
  env.OURO_DESKTOP_TEST_MODE = 'fresh-install'
  env.OURO_DESKTOP_USER_DATA_DIR = userDataDir
  env.OURO_HOME = hermesHome
  delete env.OURO_DESKTOP_HERMES
  delete env.OURO_DESKTOP_OURO_ROOT
  ...
  console.log(`  OURO_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'ouro-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Ouro Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'ouro-agent', 'hermes_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Ouro
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + OURO_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  OURO_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
