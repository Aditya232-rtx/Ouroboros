...
//   · its own OURO_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · OURO_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated OURO_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate OURO_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  if (seedConfig && !hermesHome) {
    seedConfigFrom(join(homedir(), '.ouro'), home)
  }
  ...
  try {
    ...
    // OURO_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set OURO_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      OURO_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.OURO_DESKTOP_DEV_SERVER = devUrl
    }
