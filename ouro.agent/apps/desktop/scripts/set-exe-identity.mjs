#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Ouro icon + version metadata onto the
// built Ouro.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Ouro.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `ouro desktop`, the installer's
...
// path (which rebuilds via `ouro desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Ouro.exe>
//
...
// Stamp the Ouro icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Ouro',
      FileDescription: 'Ouro',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Ouro icon + identity stamped')
}
