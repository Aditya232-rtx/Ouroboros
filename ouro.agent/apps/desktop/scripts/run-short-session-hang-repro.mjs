...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        OURO_DESKTOP_APP_NAME: `HermesShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        OURO_DESKTOP_OURO_ROOT: target.targetRoot,
        OURO_DESKTOP_IGNORE_EXISTING: '1',
        OURO_DESKTOP_USER_DATA_DIR: userData,
        OURO_HOME: hermesHome,
