...
const OURO_PATHS_MIME = 'application/x-hermes-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(OURO_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(OURO_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (ouro --tui, vim) grab the mouse, so a plain drag
