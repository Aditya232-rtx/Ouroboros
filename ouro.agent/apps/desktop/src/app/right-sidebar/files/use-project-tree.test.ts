...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as any).ouroDesktop = { readDir }
})
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop = { readDir, sanitizeWorkspaceCwd }
