...
beforeEach(() => {
  ...
  ;(window as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForHermes) → app sits in the connecting combo, then fails', async () => {
    ...
    ;(window as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop = desktop
    ...
    await act(async () => {
      rejectConn(new Error('Ouro backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Ouro couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Ouro')
    expect($currentCwd.get()).toBe('C:\\Ouro')
  })
