  ...
  useI18n: () => ({
    t: {
      ...
      profiles: {
        ...
        connectGateway: 'Connect another Ouro gateway…',
...
describe('ProfileRail multi-gateway entry point', () => {
  it('deep-links to Settings → Connections from the rail', () => {
    ...
    const pill = screen.getByRole('button', { name: 'Connect another Ouro gateway…' })
  ...
  it('keeps the entry point visible for single-profile users', () => {
    ...
    expect(screen.getByRole('button', { name: 'Connect another Ouro gateway…' })).toBeTruthy()
