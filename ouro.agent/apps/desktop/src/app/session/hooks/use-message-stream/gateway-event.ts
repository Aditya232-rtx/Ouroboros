 ...
 * billing-specific toast — never the generic "Ouro error" — with a smart CTA
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        // A runtime skin switch (Ouro activating an authored skin, or `/skin`
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Ouro reported an error',
        ...
        // native OS notification when Ouro is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Ouro reported an error'
