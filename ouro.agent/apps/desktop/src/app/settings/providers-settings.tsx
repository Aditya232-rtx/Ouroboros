...
//      `ouro model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Ouro can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Ouro can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Ouro never deletes creds another tool owns behind a silent API call.
