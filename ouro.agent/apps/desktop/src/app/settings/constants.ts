...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Ouro & Nous-trained models',
  ...
  { prefix: 'OURO_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Ouro stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Ouro can read from one file request.',
  approvals: {
    mode: 'How Ouro handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Ouro updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
