...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Ouro Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Ouro docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Ouro Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Ouro docs', url: 'https://example.com/docs' }
    ])
  })
})
...
describe('buildToolView browser_navigate title', () => {
  it('shows failed title when navigate returns success=false', () => {
    ...
    expect(view.title).toBe('Failed to open ouro.dev/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://ouro.dev/docs' },
        result: { success: true, url: 'https://ouro.dev/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened ouro.dev/docs')
  })
})
