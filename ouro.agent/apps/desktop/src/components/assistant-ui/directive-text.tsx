...
const OURO_REF_TYPES = WIRE_REFERENCE_KINDS
type HermesRefType = (typeof OURO_REF_TYPES)[number]
...
const OURO_DIRECTIVE_RE = referenceRe()
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(OURO_DIRECTIVE_RE)).map(match => {
...
/**
 * Renders text containing Ouro directives (`@file:...`, `@image:...`) as
