...
describe('collidesWithWorkspace', () => {
  it('suppresses a skill named exactly like the cwd folder', () => {
    expect(collidesWithWorkspace('ouro-agent', '/Users/b/www/hermes-agent')).toBe(true)
  })

  it('suppresses inside worktree-suffixed folders too', () => {
    expect(collidesWithWorkspace('ouro-agent', '/Users/b/www/hermes-agent-suggest')).toBe(true)
  })
  ...
  it('never collides when detached (empty cwd)', () => {
    expect(collidesWithWorkspace('ouro-agent', '')).toBe(false)
  })
})
...
describe('skillTouchedInMessages', () => {
  ...
  it('matches qualified skill names (category/name, plugin:name)', () => {
    expect(
      skillTouchedInMessages('ouro-agent-dev', [toolCall('skill_view', { name: 'github/ouro-agent-dev' })])
    ).toBe(true)
    expect(
