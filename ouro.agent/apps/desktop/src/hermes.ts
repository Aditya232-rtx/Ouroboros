...
// "Timed out connecting to Ouro backend" that hangs the UI (#48504).
//
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Ouro gateway connection closed',
      connectErrorMessage: 'Could not connect to Ouro gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Ouro gateway is not connected',
...
// the call; each pooled backend already has its own OURO_HOME, so no backend
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hermesDesktop?.api) {
    throw new Error('Ouro desktop bridge unavailable')
  }
...
// Cron jobs are stored per-profile (<OURO_HOME>/cron/jobs.json), and the
...
// curation `ouro model` does — for Nous it honors the free/paid tier so a
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `ouro skills`
...
// `ouro mcp` and the dashboard MCP page). Raw JSON editing stays in
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `ouro memory` / `ouro curator`).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `ouro doctor` / `ouro security
// audit` / `ouro backup` / `ouro debug share` and the dashboard System
