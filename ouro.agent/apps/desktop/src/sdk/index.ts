 ...
 *    object as `window.__OURO_PLUGIN_SDK__` and maps the import to it, so a
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Ouro profile served by that route. */
...
export const host = {
  ...
  connections: async () => {
    ...
    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Ouro Desktop.')
    }
  ...
  agents: async () => {
    ...
    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Ouro Desktop.')
    }
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Ouro Desktop connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Ouro gateway unavailable')
    }
...
/** Ouro' reasoning levels + their compact labels, so a plugin surfacing a
