...
declare global {
  interface Window {
    ouroDesktop?: Window['hermesDesktop']
    hermesDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`ouro --tui
      ...
      connections: {
        ...
        // Fan out `ouro update` to every eligible registered connection;
      ...
      // Ouro Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new OURO_HOME (reloads the window). Pass null to
      ...
      // Local Desktop runtime-plugin root (<OURO_HOME>/desktop-plugins),
      ...
      // Local AGENT-plugin root (<OURO_HOME>/plugins), same Electron-local
...
export interface DesktopUpdateApplyResult {
   ...
   *  `ouro update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  hermesRoot?: string
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Ouro Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Ouro Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Ouro Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the ouro gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Ouro Cloud agent — the trimmed DTO from NAS GET /api/agents.
