...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Ouro doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // this provider identity — the SAME one `ouro model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `ouro model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Ouro simply carries none of it. See store/profile-share.ts.
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Ouro, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `ouro tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Ouro); elsewhere, driver health from `cua-driver doctor`. `null`
