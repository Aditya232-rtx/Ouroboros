...
test('security: delegated routine arguments remain literal shell values', () => {
  ...
  const command = prompt.slice(prompt.indexOf('ouro '), prompt.lastIndexOf('\n\nIf the command'))
...
test('security: upgrade pauses persisted delegated routines before they can execute', async () => {
  ...
  const oldPrompt =
    ...
    `ouro -p research chat -c "Routine: ${title}" -q ${JSON.stringify(`[Scheduled routine] ${instruction}`)}\n\n` +
  ...
  const command = recreated.slice(recreated.indexOf('ouro '), recreated.lastIndexOf('\n\nIf the command'))
  const script = `hermes() { printf '%s\\037' "$@"; }\n${command}`
