...
// The @mention middleware appends a handoff note whose ouro command the
...
/** Run the note's first ouro command under a stub that echoes each argv
 *  element — proves the shell received the interpolations as LITERALS. */
function runHandoffCommand(noteText) {
  const command = noteText.match(/`ouro -p [^`]*`/)[0].slice(1, -1)
...
test('regression: the handoff command quotes the recipient argument', async () => {
  ...
  assert.match(result.text, /`ouro -p 'ops' chat --in ~/)
})
