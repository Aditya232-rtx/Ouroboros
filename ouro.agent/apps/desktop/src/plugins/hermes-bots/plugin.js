/**
 * Ouro Bot Mode — a "one chat per agent" roster for the Ouro desktop.
 *
 * Left pane "Bots": one row per Ouro profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Ouro cron jobs) scoped to the
 ...
 * bot-initiated sends use `ouro -p <bot> chat --in ~ -c "Bot Chat"`.
 */
...
/** Permanently delete a bot's Ouro profile, then remove plugin-local state
...
if (typeof document !== 'undefined' && !document.getElementById('hermes-bots-roster-css')) {
  ...
  style.textContent =
    '.ouro-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.ouro-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes hermes-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.ouro-bots-pulse { animation: hermes-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run ouro mcp / Settings" hint. profile is
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Ouro), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `ouro pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (ouro-agent bot_mode_probe).
...
function useRoster() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (ouro-agent #86875) also expose the union
...
function displayName(bot, meta) {
  ...
  // that reads like nobody bothered. Present it as Ouro (the agent it is)
...
// turn in its OWN persistent per-group Ouro session and is fed only the
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'ouro -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `ouro profile list` for the LIVE',
  ...
  const open = async () => {
    ...
    if (bot.remoteSource) {
      if (typeof host.ensureAgent !== 'function') {
        host.notifyError?.(
          new Error('Update Ouro Desktop to chat with agents on other connections.'),
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Ouro).'
    })
  }
...
// https://ouro.dev/docs/skills?embed=picker hides the
...
const HUB_ORIGIN = 'https://ouro.dev'
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Ouro).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// ouro -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `ouro -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Ouro-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Ouro schedule string from picker state. */
...
async function openProfileSession(botName, storedId, gatewayGeneration) {
  ...
  if (typeof host.openSession !== 'function') {
    throw new Error('This Ouro Desktop version cannot open stored sessions')
  }
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Ouro and restart the gateway.`
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Ouro profile at ',
      ...
      data: {
        handler: async draft => {
          ...
          const note =
            ...
            mentioned.map(n => '`ouro -p ' + shellQuote(n) + ' chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + shellDoubleQuote(senderName) + ' (@' + shellDoubleQuote(botHandle(active)) + '): <your composed message>"`').join('\n') +
