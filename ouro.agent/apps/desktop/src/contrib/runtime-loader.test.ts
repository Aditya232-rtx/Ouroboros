...
const getStatus = vi.fn(async () => ({ hermes_home: '/remote/box/.ouro' }))
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { hermesDesktop?: unknown; ouroDesktop?: unknown }).ouroDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend hermes_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.ouro/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.ouro/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.ouro/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.ouro/plugins')
    // The remote backend's hermes_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.ouro/desktop-plugins')
  })
  ...
  it('probes desktop/plugin.js inside agent-plugin packages (unified packaging)', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.ouro/plugins'
        ? { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.ouro/plugins/my-feature' }] }
        : { entries: [] }
    )
    ...
    expect(readFileText).toHaveBeenCalledWith('/local/.ouro/plugins/my-feature/desktop/plugin.js')
    // The Python half's files must never be probed as a desktop entry.
    expect(readFileText).not.toHaveBeenCalledWith('/local/.ouro/plugins/my-feature/plugin.js')
  })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir =>
      dir === '/local/.ouro/plugins'
        ? { entries: [{ isDirectory: true, name: 'uni', path: '/local/.ouro/plugins/uni' }] }
        : { entries: [] }
    )
    ...
    try {
      ...
      // ~/.ouro/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend hermes_home', async () => {
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.ouro/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.ouro/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.ouro/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
