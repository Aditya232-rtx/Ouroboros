 ...
 * and the two on-disk doors — `<ouro home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<ouro home>/plugins/<name>/desktop/
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.ouro/plugins` keeps its
...
//  - `<ouro home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<ouro home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.ouro/plugins` is installed-but-inert until the
