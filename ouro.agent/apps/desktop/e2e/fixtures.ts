/**
 * Shared E2E fixtures for the Ouro desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → ouro serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `ouro desktop --source`. The
 ...
 *
 * Key env vars:
 *  - OURO_HOME → sandbox hermes-home (isolated config/sessions)
 *  - OURO_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - OURO_DESKTOP_IGNORE_EXISTING=1 → don't pick up `hermes` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - OURO_DESKTOP_OURO_ROOT → REPO_ROOT (dev checkout resolution)
 *  - OURO_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    OURO_HOME: sandbox.hermesHome,
    OURO_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    OURO_DESKTOP_IGNORE_EXISTING: '1',
    OURO_DESKTOP_OURO_ROOT: REPO_ROOT,
    OURO_DESKTOP_APP_NAME: `HermesE2E-${Date.now()}`,
    ...
    OURO_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated OURO_HOME + userData
 * @param env      - the process environment (already has OURO_HOME etc.)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via OURO_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`ouro serve`
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { OURO_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Ouro backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Ouro.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Ouro.app', 'Contents', 'MacOS', 'Ouro')
  }
 ...
 * progress without spawning a real Ouro backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * OURO_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    OURO_DESKTOP_BOOT_FAKE: '1',
    OURO_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + hermes-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).OURO_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).OURO_DESKTOP_HERMES
  delete (env as Record<string, string | undefined>).OURO_DESKTOP_OURO_ROOT
