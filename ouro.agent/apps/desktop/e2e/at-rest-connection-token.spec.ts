 ...
 * dir (macOS `~/Library/Application Support/Ouro/connection.json`, Windows
 * `AppData\Roaming\Ouro\connection.json`). Anything that can read the file
 ...
 * `X-Ouro-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add Ouro desktop app (#20059)"), the commit
...
interface FakeGateway {
  url: string
  /** Every `X-Ouro-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Ouro gateway. It serves the public
 ...
 * backend, so we skip spawning `ouro serve` (no Python needed, ~3s launch,
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    OURO_DESKTOP_APP_NAME: STABLE_APP_NAME,
    OURO_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as any).ouroDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Ouro should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as any).ouroDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // OURO_HOME (desktop.log lives there).
    expect(
    ...
    expect(
      ...
      'the gateway session token leaked into a OURO_HOME file (logs included)',
    ).toEqual([])
