 ...
 * packaged binary) with a real `ouro serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → ouro serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Ouro title', async () => {
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → ouro serve is spawned → WS connects → config
