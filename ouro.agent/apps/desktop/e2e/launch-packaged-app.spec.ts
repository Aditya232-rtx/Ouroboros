...
/**
 * E2E smoke tests for the packaged Ouro desktop app.
 *
 ...
 * (credential stripping, isolated OURO_HOME + userData, unique app name).
 *
...
test('window opens with the Ouro title', async () => {
  const title = await fixture!.page.title()
  expect(title).toContain('Ouro')
})
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      hermesDesktop?: { hud?: any }; ouroDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).ouroDesktop?.hud?.open({ sessionId: null })
  )
