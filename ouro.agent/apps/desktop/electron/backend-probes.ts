 ...
 *     issue #61764 death-loop) with OURO_PROBE_TIMEOUT_MS override
 ...
 * Honours OURO_PROBE_TIMEOUT_MS when it parses as a positive integer.
 */
function resolveProbeTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.OURO_PROBE_TIMEOUT_MS
...
function execProbeSync(
  ...
  try {
    ...
    // One cold-cache / AV miss should not force ouro-setup --update (#61764).
    execFileSync(command, args, options)
  }
}

/**
 * Return the Python snippet used to verify Ouro can import far enough to
...
/**
 * Return true iff the Ouro runtime import probe exits 0.
 *
 ...
 * its immutable, matching Ouro package; it must never fall through to the
