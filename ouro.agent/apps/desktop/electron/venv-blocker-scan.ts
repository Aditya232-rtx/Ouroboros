 ...
 * preview servers. Unknown Python/Ouro processes are deliberately ignored.
 */
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Ouro process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Ouro installation is free.\n' +
    '\n' +
    'Close other Ouro windows and terminals, then retry.  If the problem\n' +
    'persists, run `ouro update` in a terminal for detailed diagnostics.'
  )
}
