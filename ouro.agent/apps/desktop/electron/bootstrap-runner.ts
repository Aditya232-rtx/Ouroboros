 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Ouro by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.ouro-bootstrap-complete'), 'utf8')
...
// checkout under ~/.ouro/hermes-agent. Used as a last-resort fallback when
...
function installedAgentInstallScript(hermesHome) {
  ...
  const candidate = path.join(hermesHome, 'ouro-agent', 'scripts', installScriptName())
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Aditya232-rtx/ouro/${ref}/scripts/${scriptName}`
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass OURO_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          OURO_HOME: hermesHome || process.env.OURO_HOME || ''
        }
      })
    )
