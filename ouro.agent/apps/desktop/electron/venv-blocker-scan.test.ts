...
describe('formatProbeFailedMessage', () => {
  it('suggests retry and ouro update', () => {
    const msg = formatProbeFailedMessage()
    assert.ok(msg.includes('ouro update'))
    assert.ok(msg.includes('retry'))
  })
})
        ...
        processes: [
          {
            pid: 47484,
            name: 'python.exe',
            cmdline: 'C:\\Ouro\\venv\\Scripts\\python.exe -m http.server 8766 --directory C',
            kind: 'local-preview',
            safeToStop: true,
...
describe('stopSafeVenvBlockers', () => {
  it('stops only blockers explicitly classified as safe local previews', async () => {
    ...
    const outcome = await stopSafeVenvBlockers(
      ...
      exec,
      () => 'C:\\Ouro\\venv\\Scripts\\python.exe'
    )

    assert.deepEqual(calls, [
      {
        command: 'C:\\Ouro\\venv\\Scripts\\python.exe',
