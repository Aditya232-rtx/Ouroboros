...
const READY_RE = /^(?:OURO|HERMES)_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    '$candidates+=(Join-Path $hermesHome "ouro-agent\\venv\\Scripts\\hermes.exe")',
    '$candidates+=(Join-Path $HOME "ouro-agent\\.venv\\Scripts\\hermes.exe")',
    '$hermes=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $hermes){throw "Ouro is not installed on the remote Windows host."}',
    'if($explicit -and $ouro -ne $explicit){throw "The configured Ouro path is not an executable file."}',
    '$python=Join-Path (Split-Path $hermes) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Ouro Python runtime was not found."}',
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error('Update Ouro on the remote Windows host before connecting with Desktop SSH.')
  ...
  rememberLog(`[ssh-lifecycle] located ouro at ${runtime.hermesPath}`)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Ouro SSH"', 'powershell.exe -NoLogo')
