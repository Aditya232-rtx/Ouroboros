...
//   OURO_WIN_SSH_HOST   ssh alias/host of the Windows box
//   OURO_WIN_SSH_USER   remote user
//   OURO_WIN_SSH_HERMES absolute path to the remote hermes.exe under test
const liveHost = process.env.OURO_WIN_SSH_HOST || ''
const liveUser = process.env.OURO_WIN_SSH_USER || ''
const configuredHermes = process.env.OURO_WIN_SSH_HERMES || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Ouro-Session-Token': token } }).then(async response => {
    if (!response.ok) {
