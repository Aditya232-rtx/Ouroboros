 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Ouro dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Ouro install on the remote (login-shell probe),
 ...
 *     `OURO_DASHBOARD_READY port=<n>` readiness line,
 ...
 *
 * The minted OURO_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^(?:OURO|HERMES)_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.ouro/desktop-ssh'
...
// Resolve the remote ouro executable. An EXPLICIT path is honored strictly
...
async function locateHermes(ssh, remoteHermesPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The ouro binary or wrapper script
    ...
    //     the Ouro version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/Aditya232-rtx/ouro/issues/74411
    return candidate
  }
  ...
  if (remoteHermesPath) {
    ...
    const err: any = new Error(
      `The Ouro path you set is not an executable on the remote host: "${remoteHermesPath}". ` +
        'Check the path (it must be the full path to the `ouro` binary on the remote, e.g. ' +
        '~/hermes-agent/.venv/bin/ouro), or clear it to auto-detect.'
    )
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v ouro')}`)).trim()
  ...
  candidates.push('~/.local/bin/ouro')
  candidates.push('/usr/local/bin/ouro')
  candidates.push('~/.ouro/hermes-agent/venv/bin/ouro')
  // Pre-rebrand installs exposed `hermes`; keep them resolvable.
  candidates.push('~/.local/bin/hermes')
  candidates.push('/usr/local/bin/hermes')
  candidates.push('~/.ouro/hermes-agent/venv/bin/hermes')
  ...
  const err: any = new Error(
    'Ouro is not installed on the remote host (could not find a `ouro` executable). ' +
      'Install it on the remote with:  curl -fsSL https://ouro.dev/install.sh | sh  ' +
      '— or set the Ouro path explicitly in the SSH connection settings.'
  )
...
// e.g. "Ouro v0.18.2 ..."), or '' on failure. Surfaces WHICH ouro a
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Ouro Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The OURO_HOME the remote dashboard will use (explicit env wins, else
// ~/.ouro). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteHermesHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${OURO_HOME:-$HOME/.ouro}"')).trim().split('\n').pop()

    return out || '~/.ouro'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Ouro home.')
...
function buildSpawnCommand(hermesPath, profile, opts: any = {}) {
  const ouro = expandRemotePath(hermesPath)
  ...
  const dashCmd =
    ...
    `exec env OURO_DESKTOP=1 ${ouro} ${profileArgs}${subCmd}`
...
async function spawnRemoteDashboard(ssh, { hermesPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Ouro install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Ouro on the remote host to continue using Desktop SSH mode.'
    )
...
async function connect(deps) {
  ...
  log(`located ouro at ${hermesPath}`)
  const hermesVersion = await probeHermesVersion(ssh, hermesPath)

  if (hermesVersion) {
    log(`remote ouro version: ${hermesVersion}`)
  }
