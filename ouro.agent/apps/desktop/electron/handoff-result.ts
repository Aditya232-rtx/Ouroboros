 ...
 * console. It writes OURO_HOME/.ouro-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Ouro within 30 minutes.
...
export function handoffResultPath(hermesHome: string): string {
  return path.join(hermesHome, '.ouro-update-result.json')
}
