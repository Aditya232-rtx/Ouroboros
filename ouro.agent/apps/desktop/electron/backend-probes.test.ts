...
test('ouro runtime import probe checks config dependencies', () => {
...
test('explicit Ouro override is authoritative', () => {
  assert.equal(shouldTrustHermesOverride('/nix/store/abc/bin/hermes'), true)
})

test('empty Ouro override is not authoritative', () => {
...
test('verifyHermesCli returns true when --version exits 0', () => {
  ...
  // it through node. This stands in for a working ouro binary --
...
test('default probe timeout is 15s (not the old 5s death-loop value)', () => {
  ...
  // matches the default (tests run without OURO_PROBE_TIMEOUT_MS).
  assert.equal(PROBE_TIMEOUT_MS, DEFAULT_PROBE_TIMEOUT_MS)
})

test('resolveProbeTimeoutMs honours OURO_PROBE_TIMEOUT_MS', () => {
  assert.equal(resolveProbeTimeoutMs({}), DEFAULT_PROBE_TIMEOUT_MS)
  assert.equal(resolveProbeTimeoutMs({ OURO_PROBE_TIMEOUT_MS: '30000' }), 30_000)
  assert.equal(resolveProbeTimeoutMs({ OURO_PROBE_TIMEOUT_MS: '0' }), DEFAULT_PROBE_TIMEOUT_MS)
  assert.equal(resolveProbeTimeoutMs({ OURO_PROBE_TIMEOUT_MS: 'nope' }), DEFAULT_PROBE_TIMEOUT_MS)
  // Cap runaway values
  assert.equal(resolveProbeTimeoutMs({ OURO_PROBE_TIMEOUT_MS: '999999' }), 120_000)
})
