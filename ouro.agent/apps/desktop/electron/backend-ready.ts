...
// `ouro serve` announces OURO_BACKEND_READY; the legacy `ouro dashboard`
// backend announces OURO_DASHBOARD_READY (or HERMES_ equivalents). Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^(?:OURO|HERMES)_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
 ...
 * OURO_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.OURO_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `OURO_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Ouro backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Ouro backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
