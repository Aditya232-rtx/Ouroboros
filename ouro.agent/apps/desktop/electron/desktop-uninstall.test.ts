...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Ouro.app/Contents/MacOS/Ouro', 'darwin'),
    '/Applications/Ouro.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Ouro.app/Contents/MacOS/Ouro', 'darwin'),
    '/Users/x/Applications/Ouro.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Ouro\\Ouro.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Ouro'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Ouro.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Ouro.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_HermesXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/Ouro.AppImage' }),
    '/home/x/Apps/Ouro.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Ouro.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Ouro.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.ouro/hermes-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.ouro/hermes-agent',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/hermes/linux-unpacked',
    hermesHome: '/home/x/.ouro'
  })
  ...
  assert.match(script, /export OURO_HOME='\/home\/x\/\.ouro'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.ouro/hermes-agent',
    agentRoot: '/home/x/.ouro/hermes-agent',
  ...
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.ouro\/hermes-agent'/)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Ouro',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Ouro" >nul 2>&1/)
