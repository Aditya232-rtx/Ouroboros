...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Ouro-Session-Token': { encoding: 'plain', value: 'token' },
...
test('cookiesHavePrivySession does NOT treat ouro gateway cookies as a portal session', () => {
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Ouro backend after 8000ms'),
