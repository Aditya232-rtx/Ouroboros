...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'ouro serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  ...
  assert.equal(backendCommandMatches('"C:\\Ouro Runtime\\hermes.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('ouro chat --query serve'), false)
