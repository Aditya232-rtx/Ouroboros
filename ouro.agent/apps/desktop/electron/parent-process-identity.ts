export type ParentWatchdogEnv = {
  OURO_PARENT_PID: string
  OURO_PARENT_START_MARKER?: string
  OURO_PARENT_NONCE?: string
}
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { OURO_PARENT_PID: String(pid) }
  ...
  env.OURO_PARENT_START_MARKER = startMarker
  env.OURO_PARENT_NONCE = nonce
