...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(OURO_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Ouro-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.ouro/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  return path.join(os.homedir(), '.ouro', 'desktop-ssh')
}
