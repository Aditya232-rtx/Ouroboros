...
test('desktop backend PATH adds Ouro-managed bins and missing POSIX sane entries', () => {
  const result = buildDesktopBackendPath({
    hermesHome: '/Users/test/.ouro',
    venvRoot: '/Users/test/.ouro/hermes-agent/venv',
  ...
  assert.deepEqual(entries.slice(0, 3), [
    '/Users/test/.ouro/node/bin',
    '/Users/test/.ouro/node',
    '/Users/test/.ouro/hermes-agent/venv/bin'
  ])
...
test('managed Node dirs lead with the platform-native layout but always offer both', () => {
  const posix = hermesManagedNodePathEntries('/Users/test/.ouro', {
    platform: 'darwin',
    pathModule: path.posix
  })
  ...
  assert.deepEqual(posix, ['/Users/test/.ouro/node/bin', '/Users/test/.ouro/node'])
...
test('managed Node dirs are empty without a Ouro home', () => {
...
test('every managed Node dir outranks the inherited PATH on both platforms', () => {
  for (const [platform, pathModule, home, inherited, delimiter] of [
    ['darwin', path.posix, '/Users/test/.ouro', '/usr/local/bin:/usr/bin', ':'],
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  ...
  assert.ok(
    env.PATH.startsWith(
      '/Users/test/.ouro/node/bin:/Users/test/.ouro/node:/Users/test/.ouro/hermes-agent/venv/bin:'
    )
  )
...
test('normalizeHermesHomeRoot maps profile homes back to the global Ouro root', () => {
  assert.equal(
    normalizeHermesHomeRoot('/Users/test/.ouro/profiles/oracle', { pathModule: path.posix }),
    '/Users/test/.ouro'
  )
  assert.equal(
  ...
  assert.equal(normalizeHermesHomeRoot('/Users/test/.ouro', { pathModule: path.posix }), '/Users/test/.ouro')
})

test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  const env = buildDesktopBackendEnv({
    hermesHome: 'C:\\Users\\test\\AppData\\Local\\hermes',
    pythonPathEntries: ['C:\\repo\\ouro-agent'],
    venvRoot: 'C:\\Users\\test\\AppData\\Local\\hermes\\ouro-agent\\venv',
    currentEnv: {
