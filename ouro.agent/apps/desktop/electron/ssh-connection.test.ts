...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env OURO_DASHBOARD_SESSION_TOKEN=abc123deadbeef OURO_DESKTOP=1 ouro dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /OURO_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /OURO_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Ouro-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Ouro-Session-Token: hdr_888'), /X-Ouro-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Ouro-Session-Token: hdr_888').includes('hdr_888'))
})
