 ...
 *     `X-Ouro-Session-Token` header; WS uses `?token=`.
...
//     reuse-detected — Portal NAS #293 / ouro #37247). When the AT cookie
...
// The Nous portal (NAS) does NOT use Ouro gateway session cookies — it is a
...
// Ouro Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote Ouro installation. Preserve an explicit mapping
  // when it is a valid Ouro profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `OURO_HOME` scopes it.
 *
...
/**
 * True if any cookie in `cookies` is a ouro session ACCESS-token cookie
 ...
 * the Ouro gateway session cookies, so cloud sign-in / discovery liveness
