 ...
 * signals (venv python interpreter, venv ouro shim, bootstrap-complete
 ...
 *
 * Returns null when `command` is not a venv ouro shim, the underlying
...
export function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Ouro at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Ouro Python at ${python}`,
    command: python,
