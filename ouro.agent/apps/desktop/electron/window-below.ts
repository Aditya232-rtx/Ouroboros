// window-below.ts — which OS window sits directly underneath a Ouro window.
//
...
export function enumerationFailureNote(platform: string, env: NodeJS.ProcessEnv): string {
  ...
  if (env.HYPRLAND_INSTANCE_SIGNATURE) {
    return (
      ...
      'Check that `hyprctl clients` works from the same session Ouro is ' +
      'running in.'
    )
  }
  ...
  if (wayland) {
    return (
      ...
      'X11/Xorg session, or run Ouro under XWayland with DISPLAY set.'
    )
  }
 ...
 *
 * Walks past every window owned by our own process (all Ouro windows share
...
export async function readWindowBelow(
  ...
  if (process.platform === 'darwin' && !titlesAvailable) {
    result.note =
      ...
      'Screen Recording permission, which Ouro does not request for this.'
  }
