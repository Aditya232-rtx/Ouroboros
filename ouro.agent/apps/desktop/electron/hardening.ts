...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Ouro Desktop cannot save remote gateway tokens. ' +
        ...
        'or set OURO_DESKTOP_REMOTE_URL and OURO_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set OURO_DESKTOP_REMOTE_URL and OURO_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
