...
function createTempGitRepo() {
  ...
  try {
    ...
    git('config', 'user.name', 'Ouro Test')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/Aditya232-rtx/ouro.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/Aditya232-rtx/ouro/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/Aditya232-rtx/ouro.git',
