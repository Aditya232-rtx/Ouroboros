 ...
 * runtime, remote gateways, Ouro Cloud instances, SSH hosts) that are all
...
/**
 * Whether "Update all instances" may drive this connection. Ouro Cloud
 * instances are platform-managed — we never run `ouro update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Ouro-managed names; an empty result stores nothing.
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Ouro Cloud' : 'Remote gateway'),
