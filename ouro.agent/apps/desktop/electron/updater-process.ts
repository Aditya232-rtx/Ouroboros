 ...
 *
 * The staged Tauri `ouro-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `ouro update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `ouro update`, swaps/relaunches
 * the app, and writes .ouro-update-result.json. With the app gone before
 * the update starts, the OURO_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `ouro update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into OURO_HOME on *every* platform
 * (`ouro-setup.exe` on Windows, `ouro-setup` elsewhere — see
 ...
 * than the hand-off protocol holds the update marker, spawns `ouro update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(hermesHome, 'ouro-setup.exe')
  if (fileExists(candidate)) {
    return candidate
  }
  // Pre-rebrand installs staged `hermes-setup.exe`; keep them updatable.
  const legacyCandidate = path.join(hermesHome, 'hermes-setup.exe')
  return fileExists(legacyCandidate) ? legacyCandidate : null
}
 ...
 * "Another Ouro update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `ouro update`, which
 ...
 * the installer is written to OURO_HOME at install/repair time, making mtime
