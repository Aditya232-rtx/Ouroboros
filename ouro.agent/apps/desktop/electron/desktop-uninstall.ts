 ...
 * the three user-facing uninstall modes to the `ouro uninstall` CLI flags,
 ...
 *
 *   macOS:   …/Ouro.app/Contents/MacOS/Ouro  → …/Ouro.app
 *   Windows: …\Ouro\Ouro.exe                 → …\Ouro  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Ouro.app/Contents/MacOS/Ouro → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Ouro.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Ouro.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Ouro$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Ouro install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "OURO_HOME=${String(hermesHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
