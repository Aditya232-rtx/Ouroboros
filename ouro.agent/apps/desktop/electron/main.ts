...
const USER_DATA_OVERRIDE = process.env.OURO_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.OURO_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.OURO_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with OURO_DESKTOP_DISABLE_GPU
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[hermes] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. OURO_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
...
// it). The value arrives via OURO_DESKTOP_PASSWORD_STORE, bridged by the
// `ouro desktop` launcher from detection or `desktop.password_store` in
...
// never go through `ouro desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Ouro sessions/config to every packaged app on the machine.
...
// always-on timers. A completely idle, minimized Ouro burned ~20% CPU
...
// OURO_HOME — the user-facing root for everything Ouro-related. Mirrors
// scripts/install.ps1's $HermesHome and scripts/install.sh's $OURO_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\ouro (matches install.ps1)
//   macOS / Linux: ~/.ouro (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.ouro directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\ouro yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// OURO_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// OURO_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.ouro / %LOCALAPPDATA%\ouro (or a pre-rebrand
// %LOCALAPPDATA%\hermes).
function resolveHermesHome() {
  if (process.env.OURO_HOME) {
    return normalizeHermesHomeRoot(process.env.OURO_HOME)
  }
  ...
  if (IS_WINDOWS) {
    ...
    // at login, so a OURO_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\ouro and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('OURO_HOME')
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const modern = path.join(process.env.LOCALAPPDATA, 'ouro')
    const preRebrand = path.join(process.env.LOCALAPPDATA, 'hermes')
    const legacy = path.join(app.getPath('home'), '.ouro')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.ouro setup (no LOCALAPPDATA install yet) or an existing pre-rebrand
    // %LOCALAPPDATA%\hermes install so users don't lose state.
    if (!directoryExists(modern) && directoryExists(preRebrand)) {
      return preRebrand
    }
  ...
  return path.join(app.getPath('home'), '.ouro')
}

const OURO_HOME = resolveHermesHome()

function pathWithHermesManagedNode(...entries) {
  const managed = hermesManagedNodePathEntries(OURO_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_OURO_ROOT — the canonical mutable Ouro install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_OURO_ROOT = path.join(OURO_HOME, 'ouro-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_OURO_ROOT, 'venv')
...
//
// We deliberately put the marker INSIDE ACTIVE_OURO_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_OURO_ROOT, '.ouro-bootstrap-complete')
...
// active-profile.json records which Ouro profile the desktop launches its
// local backend as. When set, startHermes() passes `ouro --profile <name>
// dashboard …`, which deterministically pins OURO_HOME (see
// _apply_profile_override in hermes_cli/main.py) and bypasses the sticky
// ~/.ouro/active_profile file. Unset (null) preserves the legacy behavior:
...
// desktop.log lives under OURO_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by hermes_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(OURO_HOME, 'logs', 'desktop.log')
...
const BOOT_FAKE_MODE = process.env.OURO_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.OURO_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.OURO_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.OURO_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.OURO_DESKTOP_APP_NAME || 'Ouro'
...
// electron-builder derives from the build `appId` (com.ouro.agent) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.ouro.agent')
}

// Seed the native About panel with the live Ouro version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `ouro update` mid-session is reflected without an app restart;
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.OURO_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.OURO_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// respawning ouro serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Ouro backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_OURO_ROOT
  })
}
...
// The Tauri updater writes OURO_HOME/.ouro-update-in-progress for the whole
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(OURO_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Ouro will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(OURO_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Ouro update did not finish',
        `${result.message}\n\nDetails: ${path.join(OURO_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
// spawns `ouro serve`; runtimes older than serve only have `dashboard`. We
...
function backendSupportsServe(backend) {
  ...
  if (supported === null) {
    try {
      ...
      // `ouro --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, OURO_HOME, ...(backend.env || {}) },
...
function findPythonForRoot(root) {
  const override = process.env.OURO_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Ouro deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves OURO_GIT_BASH_PATH
...
// normal OURO_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// ─── Self-update (git-pull against the running backend's ouro root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_OURO_ROOT.
// OURO_DESKTOP_OURO_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.OURO_DESKTOP_OURO_ROOT && path.resolve(process.env.OURO_DESKTOP_OURO_ROOT),
    !IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHermesSourceRoot(ACTIVE_OURO_ROOT) ? ACTIVE_OURO_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_OURO_ROOT
}
...
// Windows that binary owns ALL repo mutation — running `ouro update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(OURO_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `ouro update` can write
// fresh entry points. On Windows this is the file the running backend
// `ouro.exe` holds open; on POSIX it's never mandatory-locked.
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'ouro.exe')
    : path.join(updateRoot, 'venv', 'bin', 'ouro')
}
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    throw new Error(`Could not persist ownership for the Ouro backend: ${error.message}`)
  }
}

function releaseBackendChild(child) {
  const identity = child?.ouroBackendIdentity
...
// `ouro update`). Instead we spawn the staged Ouro-Setup binary with
// --update and quit, so it can run `ouro update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `ouro update` → relaunch),
      ...
      // in: the OURO_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `ouro update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `ouro desktop`, never the Tauri installer that self-copies
      // ouro-setup.exe into OURO_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `ouro update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `ouro update` if branch detection fails.
        }
    ...
    const handoffConflict = updateHandoffConflict(OURO_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Ouro — this window will close and the updater will open. Don’t reopen Ouro yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(OURO_HOME, rememberLog)
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Ouro install open ' +
        '(a second Ouro window or a terminal running hermes?). Close it and retry.'
    ...
    // Ouro processes running from this venv.  The updater normally refuses
    ...
    // `ouro update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `ouro update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      // The `ouro update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(OURO_HOME, child.pid)
      }
      ...
      // abort with "Another Ouro update is already running (PID <itself>)" —
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Ouro will keep running — try again, or run \`ouro update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'ouro.exe' : 'ouro')
  ...
  // not just the `ouro.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvHermes) || fileExists(path.join(updateRoot, '.ouro-bootstrap-complete'))
...
// Python-level pre-update snapshot inside `ouro update`.
...
// out, runs `ouro update`, swaps/relaunches the app bundle, and writes
// .ouro-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'ouro update', percent: null })

    return { ok: true, manual: true, command: 'ouro update', hermesRoot: updateRoot }
  }
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Ouro — this window will close. Don’t reopen Ouro yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_OURO_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_OURO_ROOT) &&
    fileExists(venvPython) &&
    canImportHermesCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_OURO_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function resolveWebDist() {
  const override = process.env.OURO_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set OURO_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: ouro desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: ouro desktop --force-build`
  )
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Ouro.app/Contents/...`
...
// createActiveBackend — build a backend pointing at ACTIVE_OURO_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Ouro at ${ACTIVE_OURO_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: OURO_HOME,
      pythonPathEntries: [ACTIVE_OURO_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_OURO_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveHermesBackend(backendArgs) {
  // 1. Explicit override -- OURO_DESKTOP_OURO_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.OURO_DESKTOP_OURO_ROOT && path.resolve(process.env.OURO_DESKTOP_OURO_ROOT)

  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Ouro source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Ouro source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_OURO_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\hermes\\ouro-agent (Windows) or ~/.ouro/hermes-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Ouro runtime at ${ACTIVE_OURO_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  //    OURO_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.OURO_DESKTOP_IGNORE_EXISTING !== '1') {
    let hermesCommand = null
    const hermesOverride = process.env.OURO_DESKTOP_HERMES
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Ouro override under WSL: ${hermesOverride}`)
      }
    } else {
      hermesCommand = findOnPath('ouro') || findOnPath('hermes')
    }

    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Ouro CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      // OURO_DESKTOP_HERMES is an explicit deployment override (used by
      ...
      if (shouldTrustHermesOverride(hermesOverride) || verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        ...
        return {
          label: `existing Ouro CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Ouro CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  if (python) {
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\hermes\ouro-agent\venv.
  ...
  return {
    ...
    label: 'Ouro not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Ouro install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Ouro recovery was handed off to Ouro Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      hermesHome: OURO_HOME,
      logRoot: path.join(OURO_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Ouro install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Ouro bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(OURO_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any
  ...
  if (!isHermesSourceRoot(ACTIVE_OURO_ROOT)) {
    throw new Error(
      `Ouro install at ${ACTIVE_OURO_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Ouro' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Ouro on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Ouro venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Ouro at ${ACTIVE_OURO_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Ouro runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Ouro backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Ouro backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Ouro backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Ouro-Session-Token`` header — used by the auth-mode probe before
...
// signal so the chat composer doesn't stay stuck on "Starting Ouro...".
...
//
// Hosted Ouro gateways gate the dashboard behind an OAuth provider (e.g.
...
//     alongside the ~15-min access token (Portal NAS #293 / ouro #37247).
...
// single-shot boot() with no retry) surfaced as the "Ouro couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Ouro
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Ouro Cloud agent…' : 'Sign in to Ouro gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Ouro backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Ouro Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// so a single override flips every Ouro surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.OURO_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Ouro gateway session cookies, so this
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Ouro Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Ouro Cloud sign-in.'))
...
// Discover the hosted (Ouro Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Ouro Cloud. Open Settings → Gateway, choose Ouro Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Ouro Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Ouro Cloud session has expired. Sign in to Ouro Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Ouro Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root OURO_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.OURO_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.OURO_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Ouro Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by OURO_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Ouro Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Ouro Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (ouro #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Ouro gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Ouro gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Ouro gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.OURO_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.hermesVersion || 'ouro (version unknown)'} at ${result.hermesPath || '?'}`
  )
...
//   2. env override (OURO_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.OURO_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.OURO_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'OURO_DESKTOP_REMOTE_URL is set but OURO_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Ouro backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Ouro
...
async function testDesktopConnectionConfig(input: any = {}) {
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Ouro on the remote host before connecting with Desktop SSH.'
            }
          }
  ...
  // connect to Ouro gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  // --profile wins over the inherited OURO_HOME env (see _apply_profile_override
  ...
  rememberLog(`Starting Ouro backend for profile "${profile}" via ${backend.label}`)

  const parentStartMarker = await desktopParentStartMarker()
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(OURO_HOME, 'profiles', key))
  )
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        OURO_HOME,
        HERMES_HOME: OURO_HOME,
        ...backend.env,
        // Pin the gateway's tool/terminal cwd to the same directory we chose for
        // the child process. Inherited TERMINAL_CWD (or a stale config bridge)
        // can still point at the install dir even when spawn cwd is home.
        TERMINAL_CWD: hermesCwd,
        OURO_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        OURO_WEB_DIST: webDist,
        HERMES_WEB_DIST: webDist,
        ...(readyFile ? { OURO_DESKTOP_READY_FILE: readyFile, HERMES_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Ouro backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Ouro backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)

    if (!ready) {
      rejectStart?.(
        new Error(`Ouro backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Ouro backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Ouro backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
async function startHermes() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Ouro Desktop is already running in another window.')
  }
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Ouro backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Ouro backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Ouro backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Ouro backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.ouro/active_profile file) and
    // resolves OURO_HOME the same way `ouro -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Ouro runtime', 28)
    ...
    await advanceBootProgress('backend.spawn', `Starting Ouro backend via ${backend.label}`, 84)
    rememberLog(`Starting Ouro backend via ${backend.label}`)
    ...
    const hermesProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin OURO_HOME for the child so Python's get_hermes_home()
          // resolves to the SAME location our resolveHermesHome() picked. Without
          // this pin, Python falls back to ~/.ouro on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\hermes, which differs from C:\Users\<u>\.ouro.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets OURO_HOME via setx; the desktop
    ...
    hermesProcess.once('error', error => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Ouro backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Ouro backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Ouro backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Ouro backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Ouro backend exit (${signal || code})`)
      ...
      rememberLog(`Ouro backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Ouro backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Ouro backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Ouro backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Ouro backend to become ready', 90)
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Ouro backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Ouro backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Ouro is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Ouro app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Ouro icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Ouro in the normal
...
// window showing only the composer and its scrollback, so Ouro can be driven
...
// "Starting Ouro…". Before the renderer's backoff loop reconnects, it asks us
...
// it (`ouro --tui --resume <id>`). Not the in-app terminal pane: the point is
...
ipcMain.handle('hermes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    if (!backend.command) {
      return { ok: false, error: 'Ouro is not installed yet' }
    }
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, OURO_HOME)
      }),
      { mode: 0o700 }
    )
  ...
  return Promise.all(
    registry.connections.map(async connection => {
      try {
        ...
        // The root OURO_HOME is an agent too; enumerations that omit it
...
// Fan out `ouro update` to every eligible registered connection at once.
...
// --- Ouro Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('hermes:profile:set', async (_event, name) => {
  ...
  // new OURO_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
function terminalShellCommand() {
  // OURO_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.OURO_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Ouro' TUI into
  ...
  env.TERM_PROGRAM = 'Ouro'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a hermes/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from OURO_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.OURO_DESKTOP_TERMINAL = '1'
...
// The LOCAL Desktop runtime-plugin root: `<OURO_HOME>/desktop-plugins`,
// resolved from the main-process OURO_HOME (see resolveHermesHome) — NOT from
...
async function localPluginsRoot(dirName: string): Promise<string> {
  ...
  const base = profile && profile !== 'default' ? path.join(OURO_HOME, 'profiles', profile) : OURO_HOME
...
// The LOCAL agent-plugin root (`<OURO_HOME>/plugins`), same Electron-local
...
// Resolve the canonical Ouro version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Ouro version instead of the Electron app's own package.json version,
...
// Re-resolve the live Ouro version and push it into the native About panel
// just before showing it, so an in-place `ouro update` is reflected without
...
// the actual removal via `ouro uninstall …` so the cross-platform PATH /
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_OURO_ROOT
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        ...
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, OURO_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Ouro agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  try {
    await releaseBackendLock(ACTIVE_OURO_ROOT, 'uninstall')
  } catch (error) {
...
// hermes://mcp/install?name=NAME&config=B64 — the vendor "Add to Ouro"
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${OURO_PROTOCOL}://`)) || null
}
...
ipcMain.handle('hermes:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${OURO_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(OURO_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(OURO_PROTOCOL)
    }
  } catch (err) {
...
app.on('before-quit', event => {
  ...
  // quitting Ouro never keeps another app's chord hostage.
