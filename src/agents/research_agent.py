...
def dispatch_node(state: ResearchState):
    ...
    # Intelligence is now available via:
    #   - /api/research/findings  (REST endpoint for frontend)
    #   - ChromaDB vector memory  (queried by RED agent during scans)
    # Users can manually trigger a warloop scan from the dashboard if needed.
    print(f"[+] Intelligence for {cve_id} committed to ChromaDB and ready for RED agent consumption.")
