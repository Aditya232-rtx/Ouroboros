...
Trigger the research agent for this run — cross-references the target's inferred stack against recently disclosed CVEs, independent of the main scan. Idempotent per run; a duplicate trigger while one is already in flight is a no-op.
