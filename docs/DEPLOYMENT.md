...
| Remediation gateway | — | WhatsApp/Slack connection for the remediation agent; launched on demand, not tied to a fixed port |
...
**A new number always replaces the previous one.** If someone else's phone was previously paired, entering a different number forces a fresh QR pairing rather than silently continuing to route through the old session — this matters if more than one person tests the flow on the same machine.
