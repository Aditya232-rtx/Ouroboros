...
5. **Learn.** Every human judgment call, whether a finding marked false positive or a PR declined with a reason, is captured as structured feedback and logged against that exact signature, so future scans of the same target don't repeat the same false alarm or the same broken fix.
...
├── orchestrator/   FastAPI service that owns scan lifecycle, findings, and the remediation handoff
