---
name: penetration-testing-with-fang
description: Pentest a web app, API, codebase, repository, URL, domain, or IP with Fang — autonomous AI penetration testing that exploits and proves vulnerabilities (OWASP Top 10 and beyond — injection, XSS, SSRF, auth/access-control flaws, IDOR, business logic) instead of just flagging them. Runs self-hosted with the open-source CLI or via the managed app.fang.ai cloud, and returns validated findings with proof-of-concept exploits (Markdown, JSON, CSV, SARIF). Use when the user asks to pentest, hack, security-scan, security-audit, or find vulnerabilities in an app, API, website, or repo.
license: Apache-2.0
metadata:
  author: Aditya232-rtx
  homepage: https://docs.fang.ai
---

# Run a Fang pentest

Fang runs autonomous AI pentesting agents that dynamically exploit a target and only report findings validated with a working proof-of-concept. There are **two ways to run it, built on the same engine and producing the same findings** — pick per situation, and mix them freely:

- **Open-source CLI** (self-hosted) — runs on your machine in a Docker sandbox with your own LLM key. Free, fully local, BYO-LLM, air-gap capable. Docs: [docs.fang.ai](https://docs.fang.ai).
- **Cloud API** (managed) — runs on Fang's infrastructure via `https://app.fang.ai/api/v1`. No Docker, no LLM key, no local compute; adds team dashboards, scheduling, PR reviews, downloadable PDF/DOCX reports (Enterprise plan), and internal-network connectors. Docs: [docs.app.fang.ai](https://docs.app.fang.ai). Full workflow in the **managed-pentesting-with-fang** skill.
...
If unsure and the user has (or will create) an app.fang.ai account, prefer **Cloud** — it avoids all local-infra friction. If they want zero signup / full local control, use the **OSS CLI**.
...
2. **Fang installed** — check with `fang --version`. Install if missing:
   ```bash
   curl -sSL https://fang.ai/install | bash   # or: pipx install fang-agent
   ```
3. **LLM configured** — two environment variables:
   ```bash
   export FANG_LLM="openai/gpt-5.4"      # any LiteLLM model id (openai/..., anthropic/..., openrouter/...)
...
fang -n -t ./ --scan-mode standard --max-budget 10

# Deployed app / API (black-box)
fang -n -t https://staging.example.com --max-budget 20

# Repo + deployed app together (best coverage)
fang -n -t https://github.com/org/app -t https://staging.example.com

# Focused testing with credentials or scope hints
fang -n -t https://app.example.com \
...
| `--resume RUN_NAME` | Resume a prior run from `fang_runs/`. |
...
A `0` is not proof of full coverage: if `--max-budget`/`--max-turns` is reached before the scan completes, it wraps up early and still exits `0`. When you need assurance the scan finished, give it enough budget and check `fang_runs/<run>/run.json`: a hard budget stop leaves `status: "stopped"`, but an agent that wrapped up early on a budget *warning* still calls `finish_scan` and records `"completed"` — so also sanity-check the run's cost against `--max-budget` and the report's stated coverage before treating a clean result as full coverage.

### Reading results

Artifacts land in `fang_runs/<run-name>/`:
...
Full details, asset registration, polling, reports, PR reviews, schedules, and webhooks are in the **managed-pentesting-with-fang** skill. Minimal launch-and-poll:

```bash
export FANG_API_TOKEN="<token>"   # org-scoped bearer, from Settings → API Access at app.fang.ai
BASE=https://app.fang.ai/api/v1

# 1. Launch a scan against an already-registered domain/repo asset
scan_id=$(curl -sS "$BASE/scans" \
  -H "Authorization: Bearer $FANG_API_TOKEN" -H "Content-Type: application/json" \
...
curl -sS "$BASE/scans/$scan_id" -H "Authorization: Bearer $FANG_API_TOKEN" | jq '.status'

# 3. Read validated findings from the scan detail's `vulnerabilities[]`, or export SARIF
curl -sS "$BASE/scans/$scan_id/sarif" -H "Authorization: Bearer $FANG_API_TOKEN" -o findings.sarif
```
...
Summarize findings by severity (critical/high/medium/low/info) and include the PoC evidence. To remediate and verify fixes (via either path), use the **fix-security-vulnerabilities-with-fang** skill. To wire scanning into CI/CD, use the **ci-security-scanning-with-fang** skill.
