...
echo -e "${BLUE}🦉 Fang Build Script${NC}"
echo "================================"
...
TUI_BINARY="build/sidecar/fang-tui"
if [ "$OS_NAME" = "windows" ]; then
    TUI_BINARY="${TUI_BINARY}.exe"
fi
mkdir -p build/sidecar
(cd fang/interface/tui && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o "../../../$TUI_BINARY" ./cmd/fang-tui)

echo -e "\n${BLUE}Building binary with PyInstaller...${NC}"
uv run pyinstaller fang.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="fang-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/fang.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/fang.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    cp "dist/fang" "$RELEASE_DIR/$BINARY_NAME"
