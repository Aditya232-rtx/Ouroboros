...
class CustomBuildHook(BuildHookInterface[Any]):
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        root = Path(self.root)
        executable = "fang-tui.exe" if os.name == "nt" else "fang-tui"
        ...
        subprocess.run(  # noqa: S603 - fixed build command using the resolved Go binary
            ...
            cwd=root / "fang" / "interface" / "tui",
            env=env,
            check=True,
        )

        build_data["force_include"][str(output)] = f"fang/bin/{executable}"
        build_data["pure_python"] = False
        platform_tag = os.environ.get("FANG_WHEEL_PLATFORM_TAG")
        if not platform_tag:
