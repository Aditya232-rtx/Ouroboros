"""Sandbox backend registry — selected via FANG_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`FangDockerSandboxClient` to inject NET_ADMIN /
    ...
    Fang manages session lifetime explicitly via ``client.delete()`` so we
    trigger ``start()`` ourselves.
    """
    import docker
    from agents.sandbox.sandboxes.docker import DockerSandboxClientOptions

    from fang.runtime.docker_client import FangDockerSandboxClient

    client = FangDockerSandboxClient(docker.from_env())
    client.fang_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown FANG_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
