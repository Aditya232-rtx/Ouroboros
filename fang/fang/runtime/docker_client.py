"""FangDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
_SANDBOX_NETWORK_ENV = "FANG_DOCKER_SANDBOX_NETWORK"
...
def _apply_resource_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    mem_limit = os.environ.get("FANG_SANDBOX_MEM_LIMIT", "").strip()
    if mem_limit:
        create_kwargs["mem_limit"] = mem_limit

    shm_size = os.environ.get("FANG_SANDBOX_SHM_SIZE", "").strip()
    if shm_size:
        create_kwargs["shm_size"] = shm_size

    cpus = os.environ.get("FANG_SANDBOX_CPUS", "").strip()
    if cpus:
    ...
    pids_limit = os.environ.get("FANG_SANDBOX_PIDS_LIMIT", "").strip()
    if pids_limit:
...
def _apply_log_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    ``max-size * max-file``. Set ``FANG_SANDBOX_LOG_MAX_SIZE`` to ``0``/``off``
    to opt back out to docker's default."""
    max_size = os.environ.get("FANG_SANDBOX_LOG_MAX_SIZE", "50m").strip()
    if max_size.lower() in ("0", "off", "none", "unlimited"):
        return
    max_file = os.environ.get("FANG_SANDBOX_LOG_MAX_FILE", "3").strip() or "3"
...
def _apply_run_labels(create_kwargs: dict[str, Any]) -> None:
    run_id = os.getenv("FANG_RUN_ID")
    if not run_id:
        return
    labels = create_kwargs.setdefault("labels", {})
    if not isinstance(labels, dict):
        return
    labels["fang-run-id"] = run_id
    run_type = os.getenv("FANG_RUN_TYPE")
    if run_type:
        labels["fang-run-type"] = run_type


class FangDockerSandboxSession(DockerSandboxSession):
    sandbox_network: str = ""
...
class FangDockerSandboxClient(DockerSandboxClient):
    ...
    fang_bind_mounts: list[dict[str, Any]] | None = None
    ...
    async def _create_container(
        ...
        *,
        ...
        # Fang delta from the SDK body: drop ``entrypoint`` override and
        ...
        # Fang injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        # Fang injection: local source trees, sorted shallowest-first so a
        # nested spec lands on top of the tree it covers.
        bind_mounts = self.fang_bind_mounts or ()
        if bind_mounts:
    ...
    async def create(self, **kwargs: Any) -> SandboxSession:
        ...
        if network and isinstance(inner, DockerSandboxSession):
            inner.__class__ = FangDockerSandboxSession
            cast("FangDockerSandboxSession", inner).sandbox_network = network
        return session
