...
_SCAN_ID: ContextVar[str | None] = ContextVar("fang_scan_id", default=None)
_AGENT_ID: ContextVar[str | None] = ContextVar("fang_agent_id", default=None)
...
class _FangContextFilter(logging.Filter):
...
# drowned in their internals when FANG_DEBUG=1.
...
_HANDLER_TAG = "_fang_scan_handler"


# ``openai.agents`` is the openai-agents SDK's canonical logger root.
_TRACKED_ROOTS: tuple[str, ...] = ("fang", "openai.agents")
...
def configure_dependency_logging() -> None:
    """Quiet dependency logging/warnings that obscure Fang scan logs."""
...
def setup_scan_logging(run_dir: Path, *, debug: bool | None = None) -> Callable[[], None]:
    ...
    Args:
        run_dir: Per-scan output directory. ``{run_dir}/fang.log`` is
        ...
        debug: When ``True``, stderr handler runs at DEBUG instead of
            ERROR. ``None`` (default) reads ``FANG_DEBUG`` env: ``1`` /
    ...
    if debug is None:
        debug = (os.environ.get("FANG_DEBUG") or "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    log_path = run_dir / "fang.log"

    formatter = logging.Formatter(_FORMAT, datefmt=_DATEFMT)
    context_filter = _FangContextFilter()
