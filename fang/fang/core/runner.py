...
from fang.agents.factory import build_fang_agent, make_child_factory
from fang.agents.prompt import render_system_prompt
from fang.config import load_settings
...
from fang.core.agents import AgentCoordinator
from fang.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from fang.core.execution import (
...
from fang.core.hooks import BudgetExceededError, ReportUsageHooks, recomputed_budget_flags
from fang.core.inputs import (
    build_root_task,
    build_scope_context,
    make_model_settings,
)
from fang.core.paths import run_dir_for, runtime_state_dir
from fang.core.sessions import open_agent_session
from fang.report.state import get_global_report_state
from fang.runtime import session_manager
from fang.telemetry.logging import set_scan_id, setup_scan_logging
from fang.tools.output_store import (
    WORKSPACE_SPILL_DIR,
...
if TYPE_CHECKING:
    ...
    from fang.runtime.status import StatusSink
...
async def run_fang_scan(
    *,
    ...
    """Run or resume one Fang scan against a sandbox.
    ...
    :func:`fang.runtime.session_manager.create_or_reuse`.
    ...
    logger.info(
        "%s Fang scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set FANG_LLM env or pass model= to run_fang_scan().",
        )
    ...
    from fang.tools.notes.tools import hydrate_notes_from_disk
    from fang.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        logger.warning(
            ...
            "Resume with 'fang --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Fang scan %s failed", scan_id)
        if root_id is not None:
            with contextlib.suppress(Exception):
        ...
        logger.info("Fang scan %s done", scan_id)
        teardown_logging()
