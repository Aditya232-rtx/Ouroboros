...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `fang/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **Fang defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `fang/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `fang.core.sessions.strip_all_images_from_session`, invoked from
  `fang/core/execution.py`.
