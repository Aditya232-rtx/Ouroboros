...
- **Fang config:** `containers/Dockerfile` sets `AGENT_BROWSER_*` env
  (executable path, UA, launch args, screenshot dir).
- **Skill:** `fang/skills/tooling/agent_browser.md` — **always-loaded**
  into every agent prompt by `fang/agents/prompt.py:_resolve_skills`.
