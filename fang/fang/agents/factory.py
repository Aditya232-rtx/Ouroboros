"""Build SandboxAgents for root + child Fang runs."""
...
from fang.tools.agents_graph.tools import (
    agent_finish,
    create_agent,
...
from fang.tools.finish.tool import finish_scan
from fang.tools.load_skill.tool import load_skill
from fang.tools.notes.tools import (
    create_note,
    delete_note,
    get_note,
    list_notes,
...
from fang.tools.output_store import bound_and_store, bound_text
from fang.tools.proxy.tools import (
    list_requests,
    list_sitemap,
    repeat_request,
    scope_rules,
...
from fang.tools.reporting.tool import (
    create_dependency_report,
    create_vulnerability_report,
    get_report,
    list_reports,
)
from fang.tools.respond.tool import respond_to_user
from fang.tools.thinking.tool import think
from fang.tools.todo.tools import (
    create_todo,
    delete_todo,
    list_todos,
    mark_todo_done,
...
from fang.tools.web_search.tool import web_search
...
def _with_bounded_result(tool: FunctionTool) -> FunctionTool:
    ...
    if getattr(tool, "_fang_bounded", False):
        return tool
    ...
    tool._fang_bounded = True  # type: ignore[attr-defined]
    return tool
...
def _with_coerced_arguments(tool: FunctionTool) -> FunctionTool:
    if getattr(tool, "_fang_coerced", False):
        return tool
    invoke_tool = tool.on_invoke_tool
    ...
    tool.on_invoke_tool = invoke
    tool._fang_coerced = True  # type: ignore[attr-defined]
    return tool
...
# ``fang.runtime.backends.register_backend``: register before the first
# ``build_fang_agent`` call and every agent (root + children) gets them.
