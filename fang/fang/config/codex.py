...
from fang.utils.secret_files import write_secret_text
...
AUTH_PATH = Path.home() / ".fang" / "subscription-auth.json"
...
def _refresh_guard() -> Iterator[None]:
    """Serialize token refresh within (lock) and across (flock) Fang processes,
...
class CodexContentGuardrailError(Exception):
    ...
    def __init__(self, model: str, original: BaseException | None = None) -> None:
        ...
        super().__init__(
            ...
            f"Set FANG_LLM to a model that isn't blocked and re-run."
        )
...
def get_valid_token() -> tuple[str, str]:
    ...
    record = read_record()
    if record is None:
        raise CodexAuthError("not_authenticated", "not signed in; run: fang auth login")
...
def build_openai_client() -> AsyncOpenAI:
    ...
    return AsyncOpenAI(
        api_key="fang-codex-oauth",  # placeholder; the hook overwrites Authorization
...
def subscription_model(model_name: str | None) -> str | None:
    """The model slug behind a ``chatgpt/<model>`` FANG_LLM, or None."""
