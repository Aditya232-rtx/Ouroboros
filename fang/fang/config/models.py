...
from fang.config import codex
from fang.config.loader import load_settings
from fang.config.tool_call_ids import TurnCallIdRewriter, dedupe_input
from fang.config.tool_call_limits import TurnToolCallLimiter
...
if TYPE_CHECKING:
    ...
    from fang.config.settings import LlmSettings, ReasoningEffort, Settings
...
class _NonStreamingModel(Model):
    ...
    Fang uses only issues streamed requests, so such a gateway fails every
...
class FangProvider(MultiProvider):
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply Fang config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # Fang uses LiteLLM's success callback to capture provider-reported cost.
...
def _install_openrouter_stream_cost_capture() -> None:
    ...
    lost and Fang falls back to a cost-map estimate that is missing entirely
    ...
    from fang.report.state import streamed_openrouter_costs

    class _FangOpenRouterStreamingHandler(OpenRouterChatCompletionStreamingHandler):
    ...
    class _FangOpenrouterConfig(OpenrouterConfig):
        def get_model_response_iterator(
            self, streaming_response: Any, sync_stream: bool, json_mode: bool | None = False
        ) -> Any:
            return _FangOpenRouterStreamingHandler(
    ...
    litellm.OpenrouterConfig = _FangOpenrouterConfig  # type: ignore[misc]


OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://fang.ai",
    "X-Title": "Fang",
...
def _configure_extra_headers(llm: LlmSettings) -> None:
    ...
    regardless of the ``FANG_LLM`` prefix.
    """
...
def _register_litellm_cost_callback() -> None:
    ...
    from fang.report.state import litellm_cost_callback
