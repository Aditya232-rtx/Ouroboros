...
telemetry live here so ``fang.interface.main`` (the CLI) and
``fang.interface.tui.runtime`` (interactive setup) depend on one module
...
from fang.config import Settings, codex, load_settings
...
from fang.utils.api_spec import (
    SpecParseError,
...
async def preflight_model_connection(
    ...
    *,
    ...
    from fang.config.models import FangProvider, configure_sdk_model_defaults
    ...
    model = FangProvider().get_model(model_name)
...
def _persist_run_record(args: argparse.Namespace) -> None:
    from fang.report.writer import write_run_record
