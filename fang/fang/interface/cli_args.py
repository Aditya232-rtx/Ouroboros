"""Command-line argument parsing for the ``fang`` scan entrypoint."""
...
from fang.config import apply_config_override
from fang.config.settings import DEFAULT_MAX_TURNS
from fang.core.paths import run_dir_for, runtime_state_dir
from fang.interface.scan_setup import attach_workspace_mount, build_targets_info
from fang.interface.update_check import self_update
from fang.interface.utils import (
    check_mountable_dir,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fang Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  fang --target https://example.com

  # GitHub repository analysis
  fang --target https://github.com/user/repo
  fang --target git@github.com:user/repo.git

  # Local code analysis
  fang --target ./my-project

  # API spec test (OpenAPI/Swagger file or Postman collection export)
  fang --target ./openapi.yaml --target https://api.example.com
  fang --target ./collection.postman_collection.json

  # Postman collection pulled live by id (needs POSTMAN_API_KEY); optional environment
  fang --target postman://<collection-uuid> --target https://api.example.com
  fang --target "postman://<collection-uuid>?env=<environment-uuid>"

  # Domain penetration test
  fang --target example.com

  # IP address penetration test
  fang --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  fang --target https://github.com/user/repo --target https://example.com
  fang --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  fang --target-list ./targets.txt

  # Custom instructions (inline)
  fang --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  fang --target example.com --instruction-file ./instructions.txt
  fang --target https://app.com --instruction-file /path/to/detailed_instructions.md

  # Extra files placed in the sandbox workspace
  fang --target ./my-project --workspace-file ./wordlist.txt
  fang --target https://app.com --workspace-file ./openapi.yaml:specs/openapi.yaml
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"fang {get_version()}",
    )

    parser.add_argument(
        "--update",
        action="store_true",
        help="Update fang to the latest version and exit. Self-updates the "
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.fang/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./fang_runs/). "
...
def _load_resume_state(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    from fang.report.writer import read_run_record
