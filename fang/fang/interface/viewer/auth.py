...
the Fang relay (``FANG_APP_URL``). The browser never talks to the relay
directly, and the report password generated locally is never sent to it.

State lives in ``~/.fang/viewer-auth.json`` (0600). ``is_verified`` is a local
...
AUTH_PATH = Path.home() / ".fang" / "viewer-auth.json"
...
def feedback_submit(email: str, message: str) -> None:
    """Relay a feedback message + email to Fang. No verification is required;
