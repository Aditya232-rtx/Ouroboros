...
// TUI strips them in fang/interface/tui/renderers/shell_renderer.py). Only a
...
function cleanOutput(raw: string, command: string = ""): string {
  ...
  if (cleaned.trim()) {
    ...
    for (const line of lines) {
      ...
      // Skip [FANG_N]$ prompt lines
      if (/^\[FANG_\d+\]\$\s*/.test(line)) continue;
    ...
    // Strip trailing [FANG_N]$ lines
    while (filtered.length > 0 && /^\[FANG_\d+\]\$\s*/.test(filtered[filtered.length - 1])) {
      filtered.pop();
    }
