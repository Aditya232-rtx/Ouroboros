...
/**
 * Pure, dependency-free parsers that turn a Fang CLI local run
 * (`fang_runs/<run>/{run.json,vulnerabilities.json}`) into the app's own
...
function parseJson(text: string, label: string): unknown {
  try {
    ...
    throw new RunParseError(
      `${label} isn't valid JSON. Make sure you selected a Fang run directory.`
    );
  }
}
