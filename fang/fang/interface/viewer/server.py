...
from fang.core.paths import run_record_path
from fang.interface.viewer import auth
...
# port is appended (``fang_viewer_session_<port>``) because browsers scope
...
SESSION_COOKIE_PREFIX = "fang_viewer_session"
...
class _ViewerState:
    def __init__(
        ...
        # The fang_runs directory that holds the launched run; used to
        ...
        # standalone ``fang view`` / finished runs, so steering is unavailable.
    ...
    class ViewerHandler(BaseHTTPRequestHandler):
        ...
        def _handle_event(self) -> None:
            ...
            if event == "cta_clicked":
                from fang.telemetry import posthog
        ...
        def _handle_report_send(self) -> None:
            ...
            from fang.interface.viewer.report_pdf import build_encrypted_report
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``fang view``
    ...
    ``None`` (standalone ``fang view``), steering is reported unavailable.
    """
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="fang-viewer", daemon=True)
    thread.start()
