"""`fang view [<run>]` command: serve a run's viewer UI locally."""
...
from fang.interface.viewer.server import authorized_url, bundle_is_built, serve
from fang.interface.viewer.transcript import read_run_summary
...
def run_view(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog="fang view",
        description="Open a local web view of a Fang run (live or finished).",
    )
    parser.add_argument(
        "run",
        nargs="?",
    ...
    if not bundle_is_built():
        console.print(
            ...
            "Build it with: [cyan]cd fang/interface/viewer/frontend && npm ci && npm run build[/]"
        )
        raise SystemExit(1)
