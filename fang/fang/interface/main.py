...
from fang.config import codex, load_settings, persist_current
from fang.core.paths import run_dir_for
from fang.interface.cli_args import parse_arguments
from fang.interface.environment import (
    check_docker_installed,
    pull_docker_image,
    validate_environment,
)
from fang.interface.interactive import (
    InteractiveSetupUnavailableError,
    run_tui,
)
from fang.interface.scan_setup import (
    ModelConnectionError,
    preflight_model_connection,
    prepare_run,
    telemetry_start,
)
from fang.interface.update_check import (
    is_binary_install,
    notify_update,
...
from fang.telemetry import posthog, scarf
from fang.telemetry.logging import configure_dependency_logging


BEDROCK_MODEL_PREFIX = "bedrock/"
BEDROCK_MISSING_MODULE_ERROR = "No module named 'boto3'"
BEDROCK_EXTRA_HINT = (
    'Bedrock support is optional. Install it with: pipx install "fang-agent[bedrock]"'
)
VERTEX_MODEL_MARKER = "vertex"
VERTEX_MISSING_MODULE_ERROR = "No module named 'google"
VERTEX_EXTRA_HINT = (
    'Vertex AI support is optional. Install it with: pipx install "fang-agent[vertex]"'
)
...
def _subscription_error_hint(exc: BaseException) -> str | None:
    ...
    if "not supported when using codex with a chatgpt account" in joined:
        return (
            ...
            "Set FANG_LLM to a model your plan includes (e.g. chatgpt/gpt-5.4)."
        )
    if (
...
async def warm_up_llm(show_model_warning: bool = True) -> None:
    ...
    from fang.config.models import (
    ...
    from fang.core.inputs import make_model_settings
    ...
    try:
        ...
        if show_model_warning and raw_model and not is_recommended_or_frontier_model(raw_model):
            ...
            warn_text.append(f"'{raw_model}'", style="bold cyan")
            warn_text.append(
                " is not a recommended frontier model for Fang.\nSecurity scans work best with:\n",
                style="white",
            )
        ...
        if settings.dedupe.model:
            from fang.report.dedupe import _dedupe_extra_args

            dedupe_model = settings.dedupe.model.strip()
            raw_model = dedupe_model
            deduper = FangProvider().get_model(dedupe_model)
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    from fang.report.state import get_global_report_state
    ...
    view_text.append("    ")
    view_text.append(f"fang view {args.run_name}", style="#22c55e")
    panel_parts.extend(["\n", view_text])
    ...
    if not scan_completed:
        ...
        resume_text.append("  ")
        resume_text.append(f"fang --resume {args.run_name}", style="#22c55e")
        panel_parts.extend(["\n", resume_text])
    ...
    console.print(
        "[#60a5fa]fang.ai[/]  [dim]·[/]  "
        "[#60a5fa]docs.fang.ai[/]  [dim]·[/]  "
        "[#60a5fa]discord.gg/fang-ai[/]"
    )
    console.print()
...
def main() -> None:
    ...
    # `fang view [<run>]` is a viewer-only subcommand, dispatched before the
    # scan argument parser (which requires a target) and before any scan setup.
    if len(sys.argv) > 1 and sys.argv[1] == "view":
        from fang.interface.viewer.cli import run_view

        run_view(sys.argv[2:])
        return

    # `fang auth …` manages model-subscription sign-in and exits; it needs no
    # target, Docker, or scan setup.
    if len(sys.argv) > 1 and sys.argv[1] == "auth":
        from fang.interface.auth_cli import run_auth
    ...
    try:
        if args.non_interactive:
            from fang.interface.cli import run_cli
