...
def validate_environment() -> None:
    ...
    if codex.subscription_model(settings.llm.model):
        if not codex.is_authenticated():
            console.print(
                f"[red]FANG_LLM={settings.llm.model} uses your ChatGPT subscription, "
                "but you're not signed in.[/] Run [cyan]fang auth login chatgpt[/] first."
            )
            sys.exit(1)
    ...
    if not settings.llm.model:
        missing_required_vars.append("FANG_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "FANG_LLM":
                ...
                error_text.append("FANG_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "FANG_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("FANG_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export FANG_LLM='openai/gpt-5.4'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_BASE":
                    ...
                    error_text.append(
                        "export FANG_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )
