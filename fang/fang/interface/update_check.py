"""Update notifications and self-update for the fang CLI.
...
result in ``~/.fang``, a non-intrusive notice with the upgrade command
for the detected install method, and a ``fang --update`` self-update
...
from fang.telemetry._common import get_version


logger = logging.getLogger(__name__)

GITHUB_REPO = "Aditya232-rtx/fang"
PYPI_PACKAGE = "fang-agent"
CHECK_INTERVAL_SECONDS = 24 * 60 * 60
REQUEST_TIMEOUT_SECONDS = 5

_CACHE_PATH = Path.home() / ".fang" / "update-check.json"

_background_thread: threading.Thread | None = None


def _is_disabled() -> bool:
    return bool(os.environ.get("FANG_NO_UPDATE_CHECK")) or any(
        os.environ.get(key)
...
def get_upgrade_command(method: str | None = None) -> str:
    ...
    commands = {
        "binary": "fang --update",
        "pipx": "pipx upgrade fang-agent",
        "uv": "uv tool upgrade fang-agent",
        "pip": "pip install --upgrade fang-agent",
    }
...
def notify_update(console: Console) -> None:
    ...
    if not latest:
        return
    console.print(
        f"[#eab308]A new version of fang is available:[/] "
...
def run_package_upgrade(console: Console, method: str) -> bool:
    ...
    if result.returncode != 0:
        ...
        return False
    console.print("[#22c55e]✓ fang updated — restart the scan to use the new version[/]")
    return True
...
def prompt_update_if_available(console: Console) -> bool:
    ...
    Returns True if fang was updated (caller should re-exec / exit).
    """
    latest = get_available_update()
    ...
    choice = Prompt.ask("Update fang?", choices=["y", "n", "s"], default="n")
    console.print()
    if choice == "s":
        skip_version(latest)
        return False
...
def _download_and_replace(version: str, target: str, console: Console) -> bool:
    ...
    filename = f"fang-{version}-{target}{archive_ext}"
    url = f"https://github.com/{GITHUB_REPO}/releases/download/v{version}/{filename}"
    binary_name = f"fang-{version}-{target}" + (".exe" if is_windows else "")
...
def self_update(console: Console | None = None, version: str | None = None) -> bool:
    ...
    if not is_binary_install():
        method = get_install_method()
        console.print(
            f"[#eab308]This fang was installed via {method};[/] "
    ...
    if not latest:
        console.print("[bold red]Could not determine the latest fang version.[/]")
        return False

    current = get_version()
    if current != "unknown" and not _is_newer(latest, current):
        console.print(f"[#22c55e]fang {current} is already the latest version.[/]")
        return True
    ...
    try:
        ...
        console.print(
            ...
            "[#60a5fa]curl -sSL https://fang.ai/install | bash[/]"
        )
        return False

    _write_cache(latest_version=latest, checked_at=time.time())
    console.print(f"[#22c55e]✓ Updated fang to {latest}[/]")
    return True
