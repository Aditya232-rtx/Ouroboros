...
from fang.core.paths import runtime_state_dir
from fang.interface.tui.history import load_session_history
...
_INTERNAL_TURN_PREFIXES = (
    # fang.core.agents._message_to_session_item: everything the coordinator
    # delivers from another agent or from the system, which wraps the stall,
    # terminal and budget-extension notices in fang.core.execution too.
    "[Message from ",
    # fang.core.inputs.child_initial_input: a subagent's parent context.
    "== Inherited context from parent",
    # fang.core.execution: the no-tool-call recovery nudge, both modes.
    ...
    # fang.core.hooks: budget warnings, the only notices injected unwrapped.
    *(
        f"[{label}] {subject}"
