"""UI-independent state and command controller for interactive Fang clients."""
...
from fang.config.models import is_recommended_or_frontier_model
from fang.config.settings import DEFAULT_MAX_TURNS
from fang.interface.tui.backend.live_view import TuiLiveView
from fang.interface.tui.backend.projection import (
    MAX_TERMINAL_EVENTS,
...
from fang.interface.utils import is_subscription_run


if TYPE_CHECKING:
    import argparse

    from fang.report.state import ReportState
...
class TuiController:
    ...
    async def _start(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        model = (load_settings().llm.model or "").strip()
        if not model:
            raise ValueError("No model configured. Set FANG_LLM first.")
    ...
    async def _open_viewer(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from fang.interface.tui.backend.messages import (
                send_user_message_to_agent,
            )
            from fang.interface.viewer.server import (
                authorized_url,
                bundle_is_built,
                serve,
            )
