...
from fang.config import load_settings, persist_current
from fang.core.agents import AgentCoordinator
from fang.core.hooks import BudgetExceededError
from fang.core.runner import run_fang_scan
...
from fang.interface.tui.backend import TuiBackendServer, TuiController
from fang.interface.tui.backend.live_view import TuiLiveView
from fang.interface.tui.sidecar import (
    check_return_code,
    child_environment,
    launch_tui_process,
    package_version,
...
from fang.interface.utils import read_workspace_files
from fang.report.state import ReportState, set_global_report_state
from fang.utils.resource_paths import get_fang_resource_path
...
class GoTuiRuntime:
    ...
    async def _run_scan(self) -> None:
        image = str(load_settings().runtime.image or "fang-sandbox:latest")
        try:
            await run_fang_scan(
    ...
    def binary_command() -> list[str]:
        ...
        if (source / "go.mod").is_file() and shutil.which("go"):
            return ["go", "run", "./cmd/fang-tui"]
        packaged = get_fang_resource_path("bin", tui_executable())
        if packaged.is_file():
            return [str(packaged)]
        raise RuntimeError(
            "Bubble Tea TUI binary not found. Reinstall Fang from an official platform wheel."
        )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            env["FANG_VERSION"] = package_version()
