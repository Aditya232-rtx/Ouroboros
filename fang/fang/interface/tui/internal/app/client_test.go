...
import (
	...
	"github.com/Aditya232-rtx/fang/tui/internal/protocol"
)
...
func TestConnectFromEnvironmentAuthenticatesTCPTransport(t *testing.T) {
	...
	t.Setenv("FANG_TUI_ADDR", listener.Addr().String())
	t.Setenv("FANG_TUI_TOKEN", "one-use-token")
	t.Setenv("FANG_TUI_FD", "")
	...
	if os.Getenv("FANG_TUI_ADDR") != "" || os.Getenv("FANG_TUI_TOKEN") != "" {
...
func TestConnectFromEnvironmentRequiresCompleteTransport(t *testing.T) {
	t.Setenv("FANG_TUI_FD", "")
	t.Setenv("FANG_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("FANG_TUI_TOKEN", "")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "FANG_TUI_ADDR and FANG_TUI_TOKEN") {
		t.Fatalf("error = %v, want missing transport error", err)
	}
}

func TestConnectFromEnvironmentPrefersInheritedDescriptor(t *testing.T) {
	t.Setenv("FANG_TUI_FD", "not-a-number")
	t.Setenv("FANG_TUI_ADDR", "127.0.0.1:1")
	t.Setenv("FANG_TUI_TOKEN", "token")

	_, err := ConnectFromEnvironment()
	if err == nil || !strings.Contains(err.Error(), "invalid FANG_TUI_FD") {
