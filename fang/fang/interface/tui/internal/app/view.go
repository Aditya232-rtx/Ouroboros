...
func (m Model) viewInner() string {
	...
	if !m.ready {
		return lipgloss.Place(m.width, m.height, lipgloss.Center, lipgloss.Center, lipgloss.NewStyle().Foreground(dim).Render("Connecting to Fang…"), lipgloss.WithWhitespaceBackground(black))
	}
	main := m.mainView()
...
func (m Model) splashView() string {
	...
	welcome := lipgloss.NewStyle().Bold(true).Foreground(white).Render("Welcome to ") +
		lipgloss.NewStyle().Bold(true).Foreground(green).Render("Fang") +
	...
	url := lipgloss.NewStyle().Bold(true).Foreground(green).Render("fang.ai")
