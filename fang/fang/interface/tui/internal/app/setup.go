...
func (m Model) setupHeaderView(fit setupFit) string {
	...
	case logoCompact:
		rows = append(rows, center.Render(lipgloss.NewStyle().Bold(true).Foreground(brightGreen).Render("FANG")))
	}
	if fit.tagline {
...
// banner is the Fang wordmark: block letters with a bevelled edge.
const banner = ` ███████╗████████╗██████╗ ██╗██╗  ██╗
 ██╔════╝╚══██╔══╝██╔══██╗██║╚██╗██╔╝
 ███████╗   ██║   ██████╔╝██║ ╚███╔╝
 ╚════██║   ██║   ██╔══██╗██║ ██╔██╗
...
func (m Model) setupSummaryView(width int) string {
	...
	if model := strings.TrimSpace(m.snapshot.Model); model != "" {
		...
		chips = append(chips, render.Col(amber).Render("○ no model")+
			render.Dim().Render(" · set FANG_LLM or configure one in your config"))
	}
