...
func TestSetupLogCollapsesRepeatedAttempts(t *testing.T) {
	...
	for range 4 {
		...
		m.setupMsg("No model configured. Set FANG_LLM first.", render.Col(red))
	}
	...
	for i := range 3 {
		...
		payload, err := json.Marshal(protocol.CommandResult{
			...
			Error: &protocol.CommandError{
				Code:    "invalid_state",
				Message: "No model configured. Set FANG_LLM first.",
			},
		})
		if err != nil {
			t.Fatal(err)
...
func TestFocusedPanelsCarryTheGreenBorder(t *testing.T) {
	...
	borderColorsOf := func(focus focusMode) string {
		...
		m.snapshot.Agents = []protocol.Agent{{ID: "a0", Name: "Fang", Status: "running"}}
