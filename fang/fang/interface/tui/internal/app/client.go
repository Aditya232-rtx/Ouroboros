...
func ConnectInherited(fdValue string) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid FANG_TUI_FD: %w", err)
	}
	file := os.NewFile(uintptr(fd), "fang-tui-ipc")
	if file == nil {
		return nil, fmt.Errorf("invalid FANG_TUI_FD %d", fd)
	}
...
func ConnectFromEnvironment() (*Client, error) {
	if fd := os.Getenv("FANG_TUI_FD"); fd != "" {
		_ = os.Unsetenv("FANG_TUI_FD")
		return ConnectInherited(fd)
	}

	address := os.Getenv("FANG_TUI_ADDR")
	token := os.Getenv("FANG_TUI_TOKEN")
	_ = os.Unsetenv("FANG_TUI_ADDR")
	_ = os.Unsetenv("FANG_TUI_TOKEN")
	if address == "" || token == "" {
		return nil, fmt.Errorf("FANG_TUI_FD or FANG_TUI_ADDR and FANG_TUI_TOKEN are required")
	}
