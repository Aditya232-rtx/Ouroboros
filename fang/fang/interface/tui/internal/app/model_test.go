...
func TestSetupStartScreenFitsNarrowTerminal(t *testing.T) {
	...
	if !strings.Contains(view, topRow) && !strings.Contains(view, "FANG") {
...
func TestModalKeepsBackgroundVisible(t *testing.T) {
	...
	if !strings.Contains(view, "Fang Help") {
...
func TestBudgetPauseShowsOneWarningToastUntilResumed(t *testing.T) {
	model := New(nil)
	model.snapshot.Agents = []protocol.Agent{{ID: "root", Name: "Fang", Status: "budget_paused"}}
...
func TestLongErrorDoesNotBreakTheFrame(t *testing.T) {
	...
	bootstrap := protocol.CollectionBootstrap{
		...
		Items: []json.RawMessage{rawJSON(t, protocol.Agent{ID: "a0", Name: "Fang", Status: "running"})},
	}
	...
	if !strings.Contains(ansi.Strip(model.View()), "Fang") {
