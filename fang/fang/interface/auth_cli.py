"""`fang auth` — ChatGPT subscription sign-in (login / status / logout).

Signing in only stores OAuth tokens (``~/.fang/subscription-auth.json``); model
selection stays with ``FANG_LLM``. A ``chatgpt/<model>`` FANG_LLM runs on the
subscription.
"""
...
from fang.config import codex, load_settings
...
_USAGE = "Usage:\n  fang auth login chatgpt [--manual]\n  fang auth status\n  fang auth logout"


def run_auth(argv: list[str]) -> int:
    """Entry point for ``fang auth …``. Returns a process exit code."""
    console = Console()
    # Bare `fang auth` (no subcommand) defaults to login.
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="fang auth login", add_help=True)
    parser.add_argument(
        "provider",
        nargs="?",
...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print("[yellow]Not signed in.[/] Run [cyan]fang auth login chatgpt[/] to sign in.")
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        console.print(f"  Runs use the subscription (FANG_LLM=[bold]{settings.llm.model}[/]).")
    else:
        console.print(
            "  [yellow]Note:[/] set [cyan]FANG_LLM[/] to e.g. [cyan]chatgpt/gpt-5.4[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("FANG_LLM", style="bold white")
    ...
    text.append("fang --target https://example.com", style="bold cyan")
    console.print()
    console.print(
        Panel(
            text,
...
def _logo_img_tag() -> str:
    """Return an ``<img>`` for the Fang logo as an inline data URI, or "".
...
<body>
  <a class="topbar" href="https://fang.ai" target="_blank" rel="noopener"
     aria-label="Fang — fang.ai">
    <!--LOGO-->
    <span>Fang</span>
  </a>
  <div class="brand">Fang</div>
  <h1>You're signed in</h1>
  <main class="card">
    <div class="badge">✓</div>
    <p class="msg">Fang is connected to your ChatGPT subscription. Head back to your
      terminal — your security test runs there.</p>
    <div class="rule"></div>
    <p class="tagline">Autonomous AI hackers that <b>find and fix</b> your app's
      vulnerabilities.</p>
    <nav class="links">
      <a href="https://fang.ai" target="_blank" rel="noopener">fang.ai</a>
      <span class="dot">·</span>
      <a href="https://docs.fang.ai" target="_blank" rel="noopener">docs</a>
      <span class="dot">·</span>
      <a href="https://discord.gg/fang-ai" target="_blank" rel="noopener">community</a>
    </nav>
  </main>
