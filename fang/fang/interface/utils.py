...
from fang.utils.api_spec import detect_spec_format
...
def _resolve_repo_diff_scope(
    ...
    if _is_repo_shallow(repo_path):
        raise ValueError(
            "Fang requires full git history for diff-scope. Please set fetch-depth: 0 "
            "in your CI config."
        )
...
def write_fetched_collection(collection: dict[str, Any], collection_uid: str) -> str:
    ...
    """
    staging = Path(tempfile.gettempdir()) / "fang_api_specs" / "fetched"
...
def stage_api_specs(targets_info: list[dict[str, Any]], run_name: str) -> list[dict[str, Any]]:
    ...
    staging = Path(tempfile.gettempdir()) / "fang_api_specs" / run_name
    staging.mkdir(parents=True, exist_ok=True)
...
def clone_repository(repo_url: str, run_name: str, dest_name: str | None = None) -> str:
    ...
    temp_dir = Path(tempfile.gettempdir()) / "fang_repos" / run_name
...
def check_docker_connection() -> Any:
    try:
        ...
        error_text.append(
            "Please ensure Docker Desktop is installed and running, and try running fang again.\n",
            style="white",
        )
