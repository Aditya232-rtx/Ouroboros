...
Fang offers three scan modes to balance speed and thoroughness.

## Quick

```bash
fang --target ./app --scan-mode quick
```
...
```bash
fang --target ./app --scan-mode standard
```
...
```bash
fang --target ./app --scan-mode deep
```
