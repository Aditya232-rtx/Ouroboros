---
title: "CLI Reference"
description: "Command-line options for Fang"
---

## Basic Usage

```bash
fang (--target <target> | --target-list <path>) [options]
```
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.fang/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, Fang enables streaming success callbacks to
...
fang --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
fang --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
fang -n --target ./ --scan-mode quick

# Cap cost and per-agent turns
fang --target https://example.com --max-budget 25 --max-turns 300

# Force diff-scope against a specific base ref
fang -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
fang -t https://github.com/org/app -t https://staging.example.com

# API spec + live target (OpenAPI/Swagger file or Postman collection)
fang -t ./openapi.yaml -t https://api.example.com

# Postman collection pulled live by id (+ optional environment)
fang -t "postman://<collection-uuid>?env=<environment-uuid>"
