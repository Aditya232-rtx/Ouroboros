...
description: "Run Fang security scans on every pull request"
---

Integrate Fang into your GitHub workflow to catch vulnerabilities before they reach production.
...
| Secret | Description |
|--------|-------------|
| `FANG_LLM` | Model name (e.g., `openai/gpt-5.4`) |
...
<Note>
For pull_request workflows, Fang automatically uses changed-files diff-scope in CI/headless runs. If diff resolution fails, ensure full history is fetched (`fetch-depth: 0`) or set `--diff-base`.
</Note>
