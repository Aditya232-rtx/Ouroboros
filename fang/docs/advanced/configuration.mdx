---
title: "Configuration"
description: "Environment variables for Fang"
---

Configure Fang using environment variables or a config file.

## LLM Configuration

<ParamField path="FANG_LLM" type="string" required>
...
<ParamField path="FANG_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="FANG_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="FANG_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="FANG_DEDUPE_MODEL" type="string">
  Model used to judge whether a candidate finding duplicates an existing report.
  Falls back to `FANG_LLM` when unset.
</ParamField>
...
<ParamField path="FANG_DEDUPE_REASONING_EFFORT" type="string">
...
<ParamField path="FANG_TELEMETRY" default="1" type="string">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, Fang keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, Fang still writes complete run telemetry locally to:

```bash
fang_runs/<run_name>/events.jsonl
```

When remote vars are set, Fang dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="FANG_IMAGE" default="ghcr.io/Aditya232-rtx/fang-sandbox:1.3.0" type="string">
...
<ParamField path="FANG_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

## Sandbox Configuration

<ParamField path="FANG_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="FANG_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

Fang stores configuration in `~/.fang/cli-config.json`. You can also specify a custom config file:

```bash
fang --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "FANG_LLM": "openai/gpt-5.4",
    "LLM_API_KEY": "sk-...",
    "FANG_REASONING_EFFORT": "high"
  }
}
```
...
export FANG_SANDBOX_EXECUTION_TIMEOUT="300"
