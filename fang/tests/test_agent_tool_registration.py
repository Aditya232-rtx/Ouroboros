...
from fang.agents import factory
...
def test_registered_tools_appear_before_lifecycle_tool() -> None:
    ...
    root = factory.build_fang_agent(is_root=True)
    child = factory.build_fang_agent(is_root=False)
...
def test_per_call_extra_tools_stack_with_registry() -> None:
    ...
    agent = factory.build_fang_agent(is_root=True, extra_tools=[_tool("per_call")])
...
def test_respond_to_user_is_interactive_only() -> None:
    ...
    interactive = factory.build_fang_agent(is_root=True, interactive=True)
    autonomous = factory.build_fang_agent(is_root=True, interactive=False)
...
def test_wait_for_agents_is_available_in_both_modes() -> None:
    for interactive in (True, False):
        agent = factory.build_fang_agent(is_root=True, interactive=interactive)
