...
Models regularly invent tool names that Fang does not register (``read_file``
is a common one, borrowed from other agent frameworks). The SDK's default is to
raise ``ModelBehaviorError``, which ends the whole run: nothing in Fang retries
...
from fang.config.models import _NonStreamingModel
...
def _agent() -> Agent[Any]:
    ...
    return Agent(name="Fang", instructions="test", tools=[real_tool], model="gw-model")
