...
from fang.runtime.backends import (
    _BACKENDS,
    _BIND_MOUNT_BACKENDS,
    backend_supports_bind_mounts,
    register_backend,
)
from fang.runtime.session_manager import (
    build_bind_mounts,
...
def test_extra_file_becomes_in_memory_manifest_entry() -> None:
    entries = build_extra_file_entries(
        [{"workspace_path": "/workspace/.fang/dependency-issues.jsonl", "content": b"{}\n"}]
    )

    assert set(entries) == {".fang/dependency-issues.jsonl"}
    entry = entries[".fang/dependency-issues.jsonl"]
...
def test_extra_file_str_content_is_encoded_utf8() -> None:
    entries = build_extra_file_entries(
        [{"workspace_path": "/workspace/.fang/note.txt", "content": "héllo"}]
    )

    entry = entries[".fang/note.txt"]
    assert isinstance(entry, File)
...
def test_extra_file_beside_a_source_tree_is_kept(tmp_path: Path) -> None:
    ...
    assert set(entries) == {".fang/dependency-issues.jsonl", "repo-notes.txt"}
    assert [m["target"] for m in mounts] == [
        "/workspace/.fang/dependency-issues.jsonl",
...
def test_extra_file_becomes_read_only_bind_mount_of_staged_copy(tmp_path: Path) -> None:
    ...
    assert len(mounts) == 1
    mount = mounts[0]
    assert mount["target"] == "/workspace/.fang/dependency-issues.jsonl"
...
def test_extra_file_bind_mounts_and_entries_agree_on_the_sandbox_path(tmp_path: Path) -> None:
    extra = [{"workspace_path": "/workspace/.fang/dependency-issues.jsonl", "content": b"{}\n"}]
