...
from fang.config import codex, loader
from fang.config.loader import load_settings
from fang.config.models import FangProvider, _NonStreamingModel, _TurnGuardModel
...
def _reset_settings(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    for key in ("FANG_LLM", "LLM_DISABLE_STREAMING"):
...
def test_get_model_wraps_when_disabled(
    ...
    inner = _DummyModel()
    monkeypatch.setattr("fang.config.models.MultiProvider.get_model", lambda *_: inner)
    monkeypatch.setenv("LLM_DISABLE_STREAMING", "true")
    load_settings()

    model = FangProvider().get_model("openai/gpt-4o-mini")
...
def test_get_model_guards_subscription_model_but_keeps_it_streaming(
    ...
    model = FangProvider().get_model("gpt-5.5")
    assert isinstance(model, _TurnGuardModel)
