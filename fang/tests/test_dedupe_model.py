...
from fang.config.settings import DedupeSettings
from fang.report.dedupe import _dedupe_model_settings
...
def test_dedupe_key_sent_per_call_not_via_global_env() -> None:
    dedupe = DedupeSettings(FANG_DEDUPE_MODEL="deepseek/cheap", DEDUPE_LLM_API_KEY="dedupe-key")
...
def test_dedupe_settings_omit_api_key_when_unset() -> None:
    dedupe = DedupeSettings(FANG_DEDUPE_MODEL="deepseek/cheap")
    settings = _dedupe_model_settings(dedupe, "deepseek/cheap", 300)
...
def test_dedupe_endpoint_sent_per_call() -> None:
    dedupe = DedupeSettings(
        FANG_DEDUPE_MODEL="openai/cheap",
...
def test_dedicated_dedupe_model_uses_own_headers_not_main() -> None:
    dedupe = DedupeSettings(
        FANG_DEDUPE_MODEL="deepseek/cheap",
...
def test_dedupe_model_read_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("FANG_DEDUPE_MODEL", "deepseek/deepseek-v4-flash")
    monkeypatch.setenv("FANG_DEDUPE_REASONING_EFFORT", "low")
...
def test_config_file_loads_dedupe_model(
    ...
    for key in ("FANG_LLM", "FANG_DEDUPE_MODEL", "FANG_DEDUPE_REASONING_EFFORT"):
    ...
    path.write_text(
        json.dumps(
            {
                "env": {
                    "FANG_LLM": "openai/root",
                    "FANG_DEDUPE_MODEL": "deepseek/cheap",
                    "FANG_DEDUPE_REASONING_EFFORT": "minimal",
                }
            }
        ),
        encoding="utf-8",
