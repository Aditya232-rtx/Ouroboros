...
from fang.interface import update_check
...
def _isolated_cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("FANG_NO_UPDATE_CHECK", raising=False)
...
def test_get_available_update_disabled_by_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(update_check, "get_version", lambda: "1.0.0")
    monkeypatch.setenv("FANG_NO_UPDATE_CHECK", "1")
    assert update_check.get_available_update() is None
...
def test_get_upgrade_command_all_methods() -> None:
    assert update_check.get_upgrade_command("binary") == "fang --update"
    assert update_check.get_upgrade_command("pipx") == "pipx upgrade fang-agent"
    assert update_check.get_upgrade_command("uv") == "uv tool upgrade fang-agent"
    assert update_check.get_upgrade_command("pip") == "pip install --upgrade fang-agent"
...
def test_sha256_file(tmp_path: Path) -> None:
    path = tmp_path / "blob"
    path.write_bytes(b"fang")
    assert update_check._sha256_file(path) == hashlib.sha256(b"fang").hexdigest()
