...
def _create_release_archive(tmp_path: Path) -> Path:
    binary_name = f"fang-{RELEASE_VERSION}-{RELEASE_TARGET}"
    binary_path = tmp_path / binary_name
    _write_executable(binary_path, f"#!/bin/sh\nprintf 'fang {RELEASE_VERSION}\\n'\n")
...
while [ "$#" -gt 0 ]; do
  ...
  printf '%s\\n' "$1" >> "$FANG_TEST_CURL_LOG"
  shift
done
cp "$FANG_TEST_ARCHIVE" "$output"
""",
    )
    return mock_bin
...
def _create_installer_environment(
    ...
    environment = {
        ...
        "FANG_TEST_ARCHIVE": str(archive_path),
        "FANG_TEST_CURL_LOG": str(curl_log_path),
...
def test_installer_downloads_and_runs_linux_arm64_release(tmp_path: Path) -> None:
    ...
    expected_filename = f"fang-{RELEASE_VERSION}-{RELEASE_TARGET}.tar.gz"
    assert expected_filename in curl_log_path.read_text(encoding="utf-8")

    installed_binary = home_path / ".fang/bin/fang"
    ...
    assert installed_result.stdout.strip() == f"fang {RELEASE_VERSION}"
...
def test_installer_rejects_unsupported_architecture(tmp_path: Path) -> None:
    ...
    assert not (home_path / ".fang").exists()
