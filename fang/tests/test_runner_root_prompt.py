"""Tests for root scan prompt options in run_fang_scan.

Verify that ``root_instructions_override`` and ``extra_system_prompt_context``
flow through to the root agent's ``build_fang_agent`` call.
"""
...
import fang.tools.notes.tools as notes_tools
import fang.tools.todo.tools as todo_tools
from fang.core import runner
...
def _patch_engine_scaffold(
    ...
    """Stub out everything around build_fang_agent and stop at run_agent_loop.

    Returns a dict that will be populated with the kwargs the runner passed to
    ``build_fang_agent`` for the root agent.
    """
    ...
    def _build_fang_agent(**kwargs: Any) -> object:
        if kwargs.get("is_root") and "kwargs" not in captured:
            captured["kwargs"] = kwargs
        return object()

    monkeypatch.setattr(runner, "build_fang_agent", _build_fang_agent)
...
async def test_root_prompt_options_flow_into_root_agent(
    ...
    scope_context = {
        ...
        "authorization_source": "fang_platform_verified_targets",
        "authorized_targets": [
            {
