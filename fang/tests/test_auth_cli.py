"""Tests for the `fang auth` CLI: subcommand routing and provider naming."""
...
from fang.interface import auth_cli
...
def _tmp_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(codex, "AUTH_PATH", tmp_path / "home" / ".fang" / "subscription-auth.json")
