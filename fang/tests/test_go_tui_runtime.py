...
from fang.interface.tui import runtime as go_tui
from fang.interface.tui import sidecar
from fang.interface.tui.runtime import GoTuiRuntime
...
def test_binary_command_prefers_packaged_sidecar(
    ...
    sidecar = tmp_path / "fang-tui"
    sidecar.write_text("binary")
    monkeypatch.setattr(go_tui, "tui_source_dir", lambda: tmp_path / "tui-src")
    monkeypatch.setattr(go_tui, "get_fang_resource_path", lambda *_parts: sidecar)
    monkeypatch.setattr(
        shutil,
        "which",
...
def test_binary_command_prefers_current_source_over_packaged_sidecar(
    ...
    assert GoTuiRuntime.binary_command() == ["go", "run", "./cmd/fang-tui"]
...
def test_binary_command_reports_missing_sidecar(
    ...
    monkeypatch.setattr(go_tui, "get_fang_resource_path", lambda *_parts: tmp_path / "missing")
...
def test_binary_command_ignores_unconstrained_path_sidecar(
    ...
    monkeypatch.setattr(go_tui, "get_fang_resource_path", lambda *_parts: tmp_path / "missing")
    monkeypatch.setattr(go_tui, "tui_source_dir", lambda: tmp_path / "tui-src")
    monkeypatch.setattr(shutil, "which", lambda _name: "/untrusted/path/fang-tui")
...
def test_child_environment_excludes_credentials(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("FANG_TUI_TOKEN", "stale-transport-token")
    ...
    assert "FANG_TUI_TOKEN" not in env
...
host, port = os.environ["FANG_TUI_ADDR"].rsplit(":", 1)
with socket.create_connection((host, int(port))) as connection:
    connection.sendall(os.environ["FANG_TUI_TOKEN"].encode("ascii"))
"""
    env = os.environ.copy()
    env.pop("FANG_TUI_FD", None)
...
async def test_scan_passes_max_turns_and_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(go_tui, "run_fang_scan", run_scan)
...
async def test_agent_state_sync_uses_latest_graph_snapshot_shape() -> None:
    runtime = GoTuiRuntime(args())
    await runtime.coordinator.register("root", "Fang", parent_id=None)
    ...
    assert runtime.live_view.agents["root"]["name"] == "Fang"
