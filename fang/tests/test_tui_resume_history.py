...
from fang.interface.tui.backend.live_view import TuiLiveView as GoTuiLiveView
from fang.interface.tui.live_view import (
...
def test_internal_turn_prefixes_still_match_what_is_injected() -> None:
    ...
    Both nudges are written inline in fang.core.execution, so there is nothing to
    ...
    for nudge in nudges:
        assert any(nudge in literal for literal in injected), (
            f"the classifier expects {nudge!r}, which fang.core.execution no longer "
...
def test_user_instruction_opens_the_transcript_when_the_root_agent_appears(
    ...
    view.upsert_agent("ab12", name="Fang", parent_id=None, status="running")
