...
from fang.config import ContextSettings
from fang.llm import compaction
...
def _patch_summary(
    ...
    monkeypatch.setattr(compaction, "FangProvider", FakeProvider)
...
async def test_maybe_compact_skips_when_summary_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(compaction, "FangProvider", BoomProvider)
    session = FakeSession(_turns(12))
    before = await session.get_items()
