...
from fang.core import execution
...
from fang.core.sessions import seed_initial_input
from fang.tools.agents_graph.tools import agent_finish, stop_agent
...
async def test_reserve_stop_notifies_root_once(monkeypatch: pytest.MonkeyPatch) -> None:
    coordinator = AgentCoordinator()
    await coordinator.register("root", "fang", parent_id=None)
...
async def test_budget_stop_unblocks_parked_agent() -> None:
    ...
    coordinator = AgentCoordinator()
    await coordinator.register("parent", "fang", parent_id=None)
...
async def test_structured_provider_refusal_fails_interactive_agent(
    ...
    monkeypatch.setattr(
        "fang.core.execution.Runner.run_streamed", lambda *_args, **_kwargs: stream
    )
    coordinator = AgentCoordinator()
    await coordinator.register("root", "fang", parent_id=None)
...
async def test_crashing_noninteractive_child_settles_and_wakes_its_parent(
    ...
    monkeypatch.setattr("fang.core.execution.Runner.run_streamed", _boom)
    coordinator = AgentCoordinator()
    await coordinator.register("root", "fang", parent_id=None)
    await coordinator.register("child", "recon", parent_id="root")
