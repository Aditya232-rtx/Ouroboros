...
if [ -n "${FANG_HOST_UID:-}" ] && [ "${FANG_HOST_UID}" != "0" ] && [ "${FANG_HOST_UID}" != "$(id -u)" ]; then
  exec sudo -E -- bash -c '
    set -e
    gid="${FANG_HOST_GID:-$FANG_HOST_UID}"
    old_uid="$1"
    old_gid="$2"
    export PATH="$3"
    shift 3
    sed -i "s|^pentester:x:${old_uid}:${old_gid}:|pentester:x:${FANG_HOST_UID}:${gid}:|" /etc/passwd
    sed -i "s|^pentester:x:${old_gid}:|pentester:x:${gid}:|" /etc/group
    chown -R "${FANG_HOST_UID}:${gid}" /home/pentester /app/certs
    chown "${FANG_HOST_UID}:${gid}" /workspace
    exec setpriv --reuid "${FANG_HOST_UID}" --regid "${gid}" --init-groups "$0" "$@"
  ' "$0" "$(id -u)" "$(id -g)" "$PATH" "$@"
fi
...
# hostname (rather than an IP literal), set FANG_CAIDO_ALLOWED_DOMAINS to a
...
if [ -n "${FANG_CAIDO_ALLOWED_DOMAINS:-}" ]; then
  IFS=',' read -ra _caido_domains <<< "${FANG_CAIDO_ALLOWED_DOMAINS}"
